//+------------------------------------------------------------------+
//|                                                  HelpMe_V10.6.mq5 |
//|                 Combined EA - Ali Naderi v10.6   (EA)             |
//+------------------------------------------------------------------+
#property copyright "HelpMe v10.6 - Ali Naderi"
#property version   "10.60"
// ═══════════════════════════════════════════════════════════════════
// v10.6 CHANGELOG
// ─────────────────────────────────────────────────────────────────
// 🐛 FIX 1 (بحرانی): اشتباه در تفسیر واحد widthPips → عرض زون اشتباه
//    v10.5: widthPips=200 به عنوان پیپ استاندارد → width=0.0200 (خیلی بزرگ!)
//    v10.6: widthPips=20  → width = 20×0.0001 = 0.0020 = ۲۰۰ پوینت ✅
//    Zone 0 برای AUDCAD = [0.91050 .. 0.91250] (همان تعریف کاربر)
//    جدول: base_high اصلاح شد 0.93050 → 0.91250
//    تأثیر: در بکتست ۲۰۲۵ اکنون قیمت بالای Zone 0 صحیح می‌رود (Zone 4+)
//
// 🐛 FIX 2: خطوط Zone در حالت لایو (بدون پوزیشن / بازار بسته) رسم نمی‌شد
//    ریشه: وقتی hasPos=false، DeleteZoneLines() صدا زده می‌شد
//    اصلاح: وقتی ShowZoneLines=true، خطوط Zone حفظ و رسم می‌شوند
//           حتی بدون پوزیشن، حتی وقتی بازار بسته است
//
// 🐛 FIX 3: خطوط Zone با تغییر تایم‌فریم پاک می‌شدند و رسم نمی‌شدند
//    ریشه: CHARTEVENT_CHART_CHANGE فقط با g_zombieSupported=true رسم می‌کرد
//           که آن هم فقط وقتی پوزیشن باز بود true بود
//    اصلاح: CHARTEVENT_CHART_CHANGE بدون شرط g_zombieSupported رسم می‌کند
//           OnInit نیز خطوط را بلافاصله پس از ساخت رسم می‌کند
// ─────────────────────────────────────────────────────────────────
// v10.5 CHANGELOG
// ─────────────────────────────────────────────────────────────────
// 🐛 FIX 1 (بحرانی): Zone_GetCfg پسوند بروکر را نادیده می‌گرفت
//    AUDCAD.raw / AUDCAD_ecn → هیچ‌وقت پیدا نمی‌شد → همیشه خاکستری!
//    اصلاح: پاک‌سازی پسوند قبل از جستجو در جدول
// 🗑️ CLEANUP: handleATR_D1_200 حذف شد (dead code از v10.3)
// 🗑️ CLEANUP: zombieWidthPips از HourlySnapshot و BtLightEvent حذف شد
// 🗑️ CLEANUP: کامنت stZOMBIE اصلاح شد
// ═══════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════
...
input int    ZoneBorderYellowPips= 10;    // v10.5: فاصله از مرز برای چراغ زرد (پیپ استاندارد — همان واحد widthPips) | 10 pip = 0.0010
...
// مثال AUDCAD: base_low=0.91050 base_high=0.91250 width=20pip(=200pt)
...
struct SymbolZoneCfg
{
   ...
   int    widthPips;   // عرض هر طبقه (پوینت استاندارد) — 20 × GetPipSize(0.0001) = 0.0020 = ۲۰۰ پوینت
   bool   supported;
};
// جدول مرجع — هر کاربر می‌تواند بر اساس بک‌تست/مشاهده‌اش گسترش دهد
SymbolZoneCfg g_zoneTable[] =
{
   // ══ واحد widthPips = پوینت استاندارد (نه پیپ) ══
   // widthPips × GetPipSize(0.0001) = عرض واقعی هر طبقه به قیمت
   // AUDCAD: widthPips=20 → 20 × 0.0001 = 0.0020 = ۲۰۰ پوینت = ۲۰ پیپ استاندارد
   // Zone 0 = [base_low=0.91050 .. base_low+0.0020=0.91250)
   // base_high فقط برای tooltip است — باید = base_low + widthPips×0.0001
   // symbol   base_low  base_high  widthPips  supported
   { "AUDCAD",  0.91050,  0.91250,   20,        true  },
   // برای افزودن ارز: base_high = base_low + widthPips×0.0001
   // مثال (عرض ۲۰۰ پوینت = ۲۰ پیپ): { "AUDNZD",  1.06000,  1.06200,  20,  true  },
};
                  ...
                  const string &sparam)
{
   ...
   // v10.6 FIX: تغییر چارت یا تایم‌فریم → خطوط طبقات Zone را دوباره رسم کن
   // بدون شرط g_zombieSupported — حتی بدون پوزیشن باز هم رسم می‌شود
   if(id == CHARTEVENT_CHART_CHANGE)
   {
      if(ShowZoneLines)
      {
         ...
         if(Zone_GetCfg(_Symbol, _zc))
         {
            ...
            // از آخرین entryZone استفاده کن (0 اگر پوزیشنی نیست)
            DrawAbsoluteZoneLines(_zc.base_low, (double)_zc.widthPips * _pip,
                                  _pip, ZoneLineCount, g_zombieEntryZone);
         }
      }
