                       ...
                       double adxV, double fastMAV, double slowMAV)
{
   ...
   string tipB = StringFormat(
      ...
      "Wick Filter          : %s\n"
      "─────────────────────────────────────\n"
      "Candle Pattern (الگوی کندلی)   :  %s\n"
      "Ichimoku (ایچیموکو)            :  %s\n"
      "Price Gap (شکاف قیمتی)         :  %s\n"
      "Stop Hunt (نقدینگی)            :  %s\n"
      "Mean Return (بازگشت به میانگین):  %s\n"
      "Trend Filter (همجهتی با روند)  :  %s\n"
      "Volume (حجم هوشمند)            :  %s\n"
      "Market State (رژیم بازار)      :  %s\n"
      "RSI (واگرایی مخفی)             :  %s\n"
      "Fractal (تایید فرکتالی)        :  %s",
   ...
   if(ObjectFind(0, arrowName) >= 0)
      ObjectSetString(0, arrowName, OBJPROP_TOOLTIP, tipB);
}
      ...
      if(score >= localRequiredScore)
         {
            ...
            // لیبل زیر فلش
            CreateSignalLabel(time[shift], arrowY - arrowOffset * 0.5, sigLevel, true, score,
...
void CreateDashboard()
{
   ...
   if(ShowPresetButtons)
   {
      ...
      for(int i=0;i<5;i++)
      {
         ...
         color bc  = (currentPreset==(ENUM_TIMEFRAME_PRESET)i) ? clrNavy : clrDarkSlateGray;
   ...
   if(ShowFilterButtons)
   {
      ...
      for(int i=0;i<3;i++)
      {
         ...
         color bc  = (currentFilterMode==i) ? clrDarkGreen : clrDarkSlateGray;
   ...
   if(ShowAIButtons)
   {
      ...
      // row 1: Support/Resist | Candle Pattern | Trend Filter
      {
         ...
         for(int i=0;i<3;i++){
            ...
            CreateButton(nm,rx+i*(fAIBtnW+gap),yPos,fAIBtnW,btnH,ls[i],clrWhite,as[i]?clrDarkGreen:clrMaroon);
      ...
      // row 4: Local EMA | Global EMA | Liquid Level
      {
         ...
         CreateButton(dashboardPrefix+"MABtn_MAPLUS", rx,               yPos, fAIBtnW, btnH,
            "Local EMA",   clrWhite, maAny?clrCadetBlue:clrDarkSlateGray);
         ...
         CreateButton(dashboardPrefix+"MABtn_FEMA",   rx+fAIBtnW+gap,   yPos, fAIBtnW, btnH,
            "Global EMA",  clrWhite, g_femaActive?clrMediumSpringGreen:clrDarkSlateGray);
         ...
         CreateButton(dashboardPrefix+"LiqLevelFilterBtn", rx+2*(fAIBtnW+gap), yPos, fAIBtnW, btnH,
            "Liquid Level", clrWhite, g_liquidActive?clrGreen:clrDarkSlateGray);
   ...
   // ─── ALERTS (چراغ‌ها) ────────────────────────────────────────
   {
      CreateLabel(dashboardPrefix+"TLTitle",   labelLx, yPos, "Alerts:", clrYellow, dashFont);
      yPos += lineH;
...
void UpdateDashboard()
{
   ...
   // ── Chart Trend (جایگزین StatusLabel خارجی + InfoPosition) ────
   {
      ...
      {
         // بدون معامله — روند کلی چارت نشان داده می‌شود
   ...
   if(marketClosed)
   {
      ...
      UpdateLabel(dashboardPrefix + "SessionStatus",
                  StringFormat("Now: بازار بسته | باز می‌شود: یکشنبه %02d:00 (محلی)", localH), clrRed);
   ...
   if(!marketClosed && ObjectFind(0, dashboardPrefix + "ADXStatus") >= 0)
   {
      ...
      UpdateLabel(dashboardPrefix + "ADXStatus", "ADX: بازار بسته", clrDimGray);
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
          ...
          StringFind(sparam, "BQ_")   >= 0 || StringFind(sparam, "SQ_")   >= 0))
      {
         ...
         string _tipB      = ObjectGetString(0, _arrowName, OBJPROP_TOOLTIP);
         string _tipA      = ObjectGetString(0, _lblName,   OBJPROP_TOOLTIP);

         string _msgTitle = (_forBuy ? "Buy Signal" : "Sell Signal") + " | " + _level;
         string _msgBody;
         if(StringLen(_tipB) > 10)
            _msgBody = _tipB;   // tipB در v12.04 سطر بالایی Score را خودش دارد
         else
         {
            _msgBody =
               "─────────────────────────────────────\n"
               ...
               "ADX                      : No signal\n"
               "BB Filter              : No signal\n"
               "ATR Intensity    : No signal\n"
               "Wick Filter          : No signal\n"
               "─────────────────────────────────────\n"
               "Candle Pattern (الگوی کندلی)   :  No signal\n"
               "Ichimoku (ایچیموکو)            :  No signal\n"
               "Price Gap (شکاف قیمتی)         :  No signal\n"
               "Stop Hunt (نقدینگی)            :  No signal\n"
               "Mean Return (بازگشت به میانگین):  No signal\n"
               "Trend Filter (همجهتی با روند)  :  No signal\n"
               "Volume (حجم هوشمند)            :  No signal\n"
               "Market State (رژیم بازار)      :  No signal\n"
               "RSI (واگرایی مخفی)             :  No signal\n"
               "Fractal (تایید فرکتالی)        :  No signal\n"
               "─────────────────────────────────────\n"
               + _tipA;
         }
      ...
      if(StringFind(sparam, dashboardPrefix + "MABtn_") >= 0)
      {
         ...
         if(clicked == "MAPLUS")
         {
            ...
            color btnCol = newState ? clrCadetBlue : clrDarkSlateGray;
      ...
      if(sparam == dashboardPrefix + "LiquidBtn" || sparam == dashboardPrefix + "LiqLevelFilterBtn")
      {
         ...
         else
         {
            ...
            if(_symPos == 0)
            {
               ...
               if(ObjectFind(0, dashboardPrefix+"LiqLevelFilterBtn") >= 0)
                  ObjectSetInteger(0, dashboardPrefix+"LiqLevelFilterBtn", OBJPROP_BGCOLOR, clrRed);
            ...
            else
            {
               ...
               if(lqResult == 1)
               {
                  ...
                  g_liquidBtnState = 2;
                  if(ObjectFind(0, dashboardPrefix+"LiquidBtn") >= 0)
                     ObjectSetInteger(0, dashboardPrefix+"LiquidBtn", OBJPROP_BGCOLOR, clrGreen);
                  if(ObjectFind(0, dashboardPrefix+"LiqLevelFilterBtn") >= 0)
                     ObjectSetInteger(0, dashboardPrefix+"LiqLevelFilterBtn", OBJPROP_BGCOLOR, clrGreen);
