//+------------------------------------------------------------------+
//|                                                HelpMe_V14_12.mq5 |
//|                     Combined EA - Ali Naderi v14.12 (EA)         |
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.12 - Ali Naderi"
#property version   "14.12"
// ════════════════════════════════════════════════════════════════════
                        ...
                        double adxVal, double flowScore, bool isForBuy)
{
   ...
   if(spikeVal >= 1 && crisisVal == 0)
      return StringFormat("🟢 Run — Spike=%s | CRISIS=%s | CleanH=%dh%s"
                          ...
                          spikeName, crisisName, dirCleanH, restartNote);
...
string StatusQuery_BuildReport()
{
   ...
   if(!activeMode || hasVirtual)
   {
      ...
      // 🆕 v14.09 P1: rawPhase از آستانه‌های سمبل‌محور (هماهنگ با Calc_SpikeDetector)
      double sq_th_w, sq_th_s;
      Spike_GetThresholds(_Symbol, sq_th_w, sq_th_s);
      int rawPhaseSell = (spikeScoreSell < sq_th_w) ? 0 : (spikeScoreSell <= sq_th_s) ? 1 : 2;
      int rawPhaseBuy  = (spikeScoreBuy  < sq_th_w) ? 0 : (spikeScoreBuy  <= sq_th_s) ? 1 : 2;

      // 🆕 v14.11: Flow threshold از تابع (رفع BUG-FLOW-HARDCODE)
      double _sq_fgTh, _sq_frTh; Flow_GetThresholds(_Symbol, _sq_fgTh, _sq_frTh);
      double _sq_thW, _sq_thS;   Spike_GetThresholds(_Symbol, _sq_thW, _sq_thS);
      bool flowRedSell    = (fsSell < _sq_frTh);
      bool flowRedBuy     = (fsBuy  < _sq_frTh);
      bool flowYellowSell = (spikeScoreSell >= _sq_thS && fsSell < _sq_fgTh && fsSell >= _sq_frTh);
      bool flowYellowBuy  = (spikeScoreBuy  >= _sq_thS && fsBuy  < _sq_fgTh && fsBuy  >= _sq_frTh);
      ...
      // 🆕 v14.09 P0: score نمایشی جهت‌دار — پنالتی کوچک بر اساس Flow منفی جهت
      // Flow منفی (جریان علیه جهت) → عدد نمایشی کمی بالاتر → کاربر تفاوت می‌بیند
      // منطق Rule، Crisis، rawPhase کاملاً دست‌نخورده می‌مانند
      double flowPenaltySell = MathMax(0.0, -flowSell) * 0.04;
      double flowPenaltyBuy  = MathMax(0.0, -flowBuy)  * 0.04;
      spikeSellDisplayScore  = spikeDataReady ? baseScore * (1.0 + flowPenaltySell) : 0.0;
      spikeBuyDisplayScore   = spikeDataReady ? baseScore * (1.0 + flowPenaltyBuy)  : 0.0;
...
int GBPNZD_Replay_SpikeAtBar(int h1Shift, int &spikeBuyOut, int &spikeSellOut)
{
   ...
   // 🆕 v14.11: rawPhase از per-symbol thresholds (رفع هاردکد 1.00/1.50)
   double _gn_thW, _gn_thS; Spike_GetThresholds(_Symbol, _gn_thW, _gn_thS);
   int rawPhase = (score < _gn_thW) ? 0 : (score <= _gn_thS) ? 1 : 2;

   // قانون ترکیبی Spike+FLOW: Flow تاریخی inline محاسبه می‌شود — هر دو جهت
   // 🐛 v14.00 FIX: جدول Sister بر حسب سمبل انتخاب می‌شود (نه همیشه GBPNZD)
   ...
   {
      string ssymFlow = FlowFindSym(spArr[si].sym);   // 🆕 v14.11: نام متفاوت جلوگیری از shadow
   ...
   // 🆕 v14.11: Flow gate از Flow_GetThresholds — رفع scope bug (sym→_Symbol)
   double _gb_fgTh, _gb_frTh; Flow_GetThresholds(_Symbol, _gb_fgTh, _gb_frTh);
   double _gb_thW,  _gb_thS;  Spike_GetThresholds(_Symbol, _gb_thW,  _gb_thS);

   // ── Buy Spike ─────────────────────────────────────────────────
   {
      bool flowRedBuy     = (fsBuy < _gb_frTh);
      bool flowYellowBuy  = (fsBuy >= _gb_frTh && fsBuy < _gb_fgTh && score >= _gb_thS);
   ...
   // ── Sell Spike ────────────────────────────────────────────────
   {
      bool flowRedSell    = (fsSell < _gb_frTh);
      bool flowYellowSell = (fsSell >= _gb_frTh && fsSell < _gb_fgTh && score >= _gb_thS);
...
void Alert_CheckGBPNZDRule()
{
   ...
   if(barBoundaryReached)
   {
      ...
      // 🆕 v14.12: Spike CoolDown — جلوگیری از ریست CleanH توسط یک Spike گذرا
      // اگه inp_SpikeCooldownBars > 0، باید N کندل متوالی Spike>=Warning داشته باشیم
      // قبل از اینکه HourDirty واقعاً CleanH را ریست کند (فقط برای Spike-only-dirty)
      // Crisis-Red همیشه فوری dirty می‌کند — CoolDown فقط روی Spike-only-dirty اثر دارد
      // Full Spike (dispPhase=2: FLOW قرمز + score>th_spike) هم فوری dirty می‌کند
      bool effectiveHourDirtyBuy  = g_gbpnzdHourDirtyBuy;
      bool effectiveHourDirtySell = g_gbpnzdHourDirtySell;
      if(inp_SpikeCooldownBars > 0)
      {
         // "Spike-only-dirty": این کندل Spike>=Warning داشت ولی Crisis قرمز نبود
         bool spikeOnlyDirtyBuy  = g_gbpnzdHourDirtyBuy  && !g_gbpnzdHourRedBuy;
         bool spikeOnlyDirtySell = g_gbpnzdHourDirtySell && !g_gbpnzdHourRedSell;
         // Full Spike (dispPhase=2 = FLOW قرمز تأیید کرد) — CoolDown دور زده می‌شود
         bool fullSpikeBuy  = (g_gbnzdSpikeBuy  == 2);
         bool fullSpikeSell = (g_gbnzdSpikeSell == 2);
         // آپدیت counter: شمار کندل‌های متوالی Spike-only-dirty (نه Full Spike)
         if(spikeOnlyDirtyBuy && !fullSpikeBuy)  g_spikeCooldownBuyCounter++;
         else if(!spikeOnlyDirtyBuy)              g_spikeCooldownBuyCounter = 0;
         if(spikeOnlyDirtySell && !fullSpikeSell) g_spikeCooldownSellCounter++;
         else if(!spikeOnlyDirtySell)             g_spikeCooldownSellCounter = 0;
         // اگه counter به آستانه نرسیده و نه Full Spike → Spike-dirty موقتاً نادیده گیر
         if(!fullSpikeBuy && spikeOnlyDirtyBuy && g_spikeCooldownBuyCounter < inp_SpikeCooldownBars)
            effectiveHourDirtyBuy = g_gbpnzdHourRedBuy;   // فقط Crisis-Red باقی می‌ماند
         if(!fullSpikeSell && spikeOnlyDirtySell && g_spikeCooldownSellCounter < inp_SpikeCooldownBars)
            effectiveHourDirtySell = g_gbpnzdHourRedSell;
         if(ShowDebugLogs)
            Print("🔧 CoolDown Buy=", g_spikeCooldownBuyCounter, "/", inp_SpikeCooldownBars,
                  " effDirty=", effectiveHourDirtyBuy,
                  " | Sell=", g_spikeCooldownSellCounter, "/", inp_SpikeCooldownBars,
                  " effDirty=", effectiveHourDirtySell);
      }
      Rule_UpdateCounters(g_gbpnzdHourRedBuy,  effectiveHourDirtyBuy,  g_gbpnzdCrisisRedHBuy,  g_gbpnzdCleanHBuy);
      Rule_UpdateCounters(g_gbpnzdHourRedSell, effectiveHourDirtySell, g_gbpnzdCrisisRedHSell, g_gbpnzdCleanHSell);
      g_gbpnzdCrisisRedH = MathMax(g_gbpnzdCrisisRedHBuy, g_gbpnzdCrisisRedHSell);
      g_gbpnzdCleanH     = MathMin(g_gbpnzdCleanHBuy,     g_gbpnzdCleanHSell);
   }
