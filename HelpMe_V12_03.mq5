                       ...
                       double adxV, double fastMAV, double slowMAV)
{
   ...
   string tipA = StringFormat("Score: %d / %d", score, BaseRequiredScore + 4);

   // Part B — on the arrow object: همه فیلترها با فرمت درخواستی
   ...
   string tipB = StringFormat(
      ...
      "Wick Filter          : %s\n"
      "─────────────────────────────────────────\n"
      "Candle Pattern (Candlestick) :  %s\n"
      "Ichimoku (Ichimoku Cloud) :   %s\n"
      "Price Gap (FVG) :   %s\n"
      "Stop Hunt (Liquidity) :   %s\n"
      "Mean Return (Mean Reversion) :  %s\n"
      "Trend Filter (Trend Alignment) : %s\n"
      "Volume (Smart Volume) :  %s\n"
      "Market State (Market Regime) :  %s\n"
      "RSI (Hidden Divergence) : [off]\n"
      "Fractal (Fractal Confirm) : [off]",
      sigState, dir, score,
      (maTxt  == "(+)") ? positiveTxt : (maTxt  == "(-)") ? negativeTxt : offTxt,
      (rsiTxt == "(+)") ? positiveTxt : (rsiTxt == "(-)") ? negativeTxt : (rsiTxt == "[off]") ? offTxt : noSigTxt,
      (adxTxt == "(+)") ? positiveTxt : (adxTxt == "(-)") ? negativeTxt : offTxt,
      (bbTxt  == "(checked)") ? positiveTxt : offTxt,
      (atTxt  == "(checked)") ? positiveTxt : offTxt,
      (wkTxt  == "(checked)") ? positiveTxt : offTxt,
      (prTxt  == "(checked)") ? positiveTxt : offTxt,
      (icTxt  == "(checked)") ? positiveTxt : offTxt,
      (fvTxt  == "(checked)") ? positiveTxt : offTxt,
      (swTxt  == "(checked)") ? positiveTxt : offTxt,
      (rtTxt  == "(checked)") ? positiveTxt : offTxt,
      (mtTxt  == "(checked)") ? positiveTxt : offTxt,
      (vlTxt  == "(checked)") ? positiveTxt : offTxt,
      (rgTxt  == "(checked)") ? positiveTxt : offTxt
   );
...
void CreateDashboard()
{
   ...
   int labelMidX = lx + innerW * 2 / 5;  // right column — فاصله کافی از ستون چپ
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      int pwrOffX = lx + (int)MathRound(4*dpiScale);  // کمی فاصله از لبه راست BG
   ...
   if(ShowAIButtons)
   {
      ...
      int fAIBtnW = (int)MathRound(aBtnW * 1.20);
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
         ...
         (StringFind(sparam, "BLbl_") >= 0 || StringFind(sparam, "SLbl_") >= 0))
      {
         ...
         // نمایش مسیج باکس اگه label دارای ❓ باشه (فرمت جدید: "High ❓")
         bool _hasQuestion = (StringFind(_lblTxt, "❓") >= 0);
         if(_hasQuestion)
         {
            // تشخیص جهت و level از نام
            bool _forBuy = (StringFind(sparam, "BLbl_") >= 0);
            // حذف " ❓" از انتها برای بدست آوردن level
            string _level = _lblTxt;
            StringReplace(_level, " ❓", "");
            ...
            {
               // استفاده از tip کامل arrow object که اطلاعات کامل دارد
               _msgBody = _tipB + "\n─────────────────────────────────────────\n" + _tipA;
            }
            else
            {
               // فرمت پیش‌فرض بدون داده زنده
               _msgBody =
                  "─────────────────────────────────────────\n"
                  "MA Cross            : --\n"
                  "RSI                        : --\n"
                  ...
                  "Wick Filter          : --\n"
                  "─────────────────────────────────────────\n"
                  "Candle Pattern (Candlestick) : --\n"
                  "Ichimoku (Ichimoku Cloud) : --\n"
                  "Price Gap (FVG) : --\n"
                  "Stop Hunt (Liquidity) : --\n"
                  "Mean Return (Mean Reversion) : --\n"
                  "Trend Filter (Trend Alignment) : --\n"
                  "Volume (Smart Volume) : --\n"
