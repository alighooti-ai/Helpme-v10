//+------------------------------------------------------------------+
//|                                                 HelpMe_V11.3.mq5  |
//|                 Combined EA - Ali Naderi v11.3  (EA)              |
//+------------------------------------------------------------------+
#property copyright "HelpMe v11.3 - Ali Naderi"
...
input string Alert_TelegramToken     = "97864663566TOKEN";  // (BotFather از) توکن ربات تلگرام | Bot To
input string Alert_TelegramChatID    = "97864663566CHATID"; // شناسه چت (برای کانال با - شروع می‌شود) | Chat ID/Channel ID
...
void Alert_SendTelegram(const string message)
{
   ...
   // v11.3: اگه token پیش‌فرض یا خالی باشه → silent skip (بدون هیچ پیام)
   // اگه token واقعی (حتی اشتباه) باشه → ارسال کن، خطاش رو HTTP error نشون بده
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if(sparam == dashboardPrefix + "ResetBtn")
      {
         ...
         if(EnableAllLogs) Print("↺ FULL RESET v11.3: all buttons → default, mutexes + chart color cleared");
