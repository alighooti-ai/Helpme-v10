//+------------------------------------------------------------------+
//|                                                HelpMe_V14_24.mq5 |
//|                     Combined EA - Ali Naderi v14.24 (EA)         |
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.24 - Ali Naderi"
#property version   "14.24"
// ════════════════════════════════════════════════════════════════════
// 📋 v14.24 | 2026-07-25 — [CALIBRATION] High Alert جهت‌دار + Killer + RTM Mode
//    درخواست کاربر (۳ بخش) بر اساس یافته‌های جدید کالیبراسیون. جزئیات
//    کامل، جدول قبل/بعد، و مثال عددی در فایل MD همراه.
//
//    [🆕 HIGH-ALERT-CALIBRATION] دو ورودی جدید:
//    inp_HighAlert_ADX_Buy=40 و inp_HighAlert_Flow_Buy=-2 (جایگزین ۳۵/۷-
//    مشترک قبلی، فقط برای Buy). Sell دست‌نخورده (۳۵/۷-، هاردکد) ماند —
//    طبق درخواست صریح. تابع مشترک جدید HighAlert_Check(forBuy,adx,flow)
//    در هر ۵ نقطه‌ای که «سمبل خودی» High Alert را محاسبه می‌کنند اعمال شد:
//    TL_Update (۴ نقطه: لاگ CSV ساعتی، چراغ داشبورد HIGHALT، پوش تلگرام
//    فوری، state-machine حالت بکتست) + BuildDirectionSnapshot سمبل خودی
//    (که هم گزارش «وضعیت» و هم *تصمیم واقعی* Rule_Transition را تغذیه
//    می‌کند — یعنی این کالیبراسیون واقعاً روی زمان‌بندی STOP/CLOSE اثر
//    می‌گذارد، نه فقط روی متن نوتیفیکیشن). پردازش موازی EURGBP/AUDCAD
//    (وقتی سمبل «خارجی» هستند) و مسیر On-Demand عمداً دست‌نخورده ماندند —
//    طبق محدودهٔ درخواست کاربر؛ در MD پرچم‌گذاری شده.
//
//    [🆕 KILLER-CALIBRATION] دو ورودی جدید: inp_Killer_TrendScore_Buy=-44
//    و inp_Killer_TrendScore_Sell=11 (جایگزین ۵۰∓ ثابت/متقارن قبلی) —
//    فقط مقایسهٔ سطح Red با ۵۰ عوض شد؛ آستانهٔ Yellow (۴۰) طبق درخواست
//    صریح دست‌نخورده ماند. این محاسبه فقط در TL_Update وجود دارد (بدون
//    نسخهٔ موازی برای EURGBP/AUDCAD، چون به GetMaxStep پوزیشن واقعی
//    همین چارت وابسته است).
//
//    [🆕 RTM-MODE] ورودی جدید enum: inp_RTM_Mode (پیش‌فرض RTM_MOMENTUM).
//    MOMENTUM: دورشدن از EMA200 در جهت سیگنال = ادامهٔ روند (امتیاز/چراغ
//    مثبت، درجه‌بندی‌شده با همان آستانه‌های قدیمی)، نزدیک‌شدن = ریسک
//    برگشت. REVERSION: منطق قدیمی (دو زیرحالت With-Trend/Counter-Trend)
//    عیناً بدون کوچک‌ترین تغییر. اعمال‌شده در AnalyzeSignal (امتیازدهی
//    سیگنال، برای بک‌تست/تحلیل) و TL_Update (چراغ زندهٔ RTM که وارد rc/
//    Crisis می‌شود).
//
//    محدودهٔ تغییر: فقط پارامترهای ورودی جدید + شرط‌های مقایسه که این
//    پارامترها را می‌خوانند. هیچ ساختار، state machine، یا فرمول دیگری
//    دست نخورد. ⚠️ این تغییرات — برخلاف v14.23 که فقط متن پیام بود —
//    مستقیماً روی *تصمیم* واقعی Rule/Killer/RTM اثر می‌گذارند؛ قبل از
//    اجرای لایو حتماً کامپایل و در Strategy Tester تست شود.
// ════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════
// 📋 v14.23 | 2026-07-25 — [MSG-REDESIGN] بازطراحی کامل پیام‌ها + گیت ارز خارجی
//    درخواست کاربر (۴ بخش): (۱) گیت پیام Rule ارزهای خارجی، (۲) پیام‌های
//    پوزیشن‌آگاه واقعی (نه فقط علامتی)، (۳) سربرگ انگلیسی روی همهٔ پیام‌ها +
//    تفکیک نکات به‌جای جمله‌های ثابت مبهم، (۴) پاک‌سازی فرمت گزارش Status.
//    جزئیات کامل در فایل MD همراه (v14.23_Message_Redesign_Changelog.md).
//
//    [🆕 FOREIGN-RULE-GATE] اینپوت جدید Alert_SendForeignRuleMessages
//    (پیش‌فرض False). پیام‌های Rule (STOP/CLOSE/RESUME + یادآور + Drift
//    Guard/Infinite Lock AUDCAD) برای سمبل خودِ چارت همیشه ارسال می‌شوند؛
//    برای دو ارز «خارجی» (Parallel — همان دو ارز دیگر از سه‌گانهٔ
//    AUDCAD/GBPNZD/EURGBP) فقط اگر این اینپوت True شود. محاسبه/ردیابی
//    state آن دو ارز (برای On-Demand و غیره) بدون تغییر در پس‌زمینه ادامه
//    دارد — فقط ارسال واقعی پیام تلگرام گیت شد؛ پاسخ On-Demand هم تحت
//    تأثیر قرار نمی‌گیرد (درخواست صریح کاربر است، نه پوش خودکار).
//    تابع کمکی: Alert_ShouldSendRuleMsgFor(sym).
//
//    [🆕 SMART-CLOSE-TEXT-DIRECTIONAL] تابع جدید HasRealPositionOnSymbolDir
//    (جهت‌دار) جایگزین تک‌چک HasRealPositionsOnSymbol در همهٔ پیام‌های
//    Rule/یادآور شد. قبلاً اگر فقط یک جهت پوزیشن باز داشت، پیام جهت دیگر هم
//    فکر می‌کرد پوزیشن دارد و می‌گفت «ببند»؛ حالا هر جهت مستقل چک می‌شود و
//    بدون پوزیشن می‌گوید «معاملهٔ جدید باز نکن؛ اگه باز بود باید می‌بستیش»
//    به‌جای «الان ببند».
//
//    [🆕 HEADER-TAXONOMY] همهٔ پیام‌ها حالا با یک سربرگ انگلیسیِ ثابت شروع
//    می‌شوند تا منبع پیام همیشه روشن باشد: 🚦Rule / ⏰Rule Reminder /
//    🆘Crisis / ⚠️High Alert / 🌊Flow / 📈Trend / 📉RTM / 🏗️Structure /
//    ⚡Spike / ☠️Killer / 🧟Zone / 🔴3 Lights / 🐌Drift / 🔒Lock / ✅Rule.
//    (MktPhase هم برای یکدستی همین الگو گرفت، هرچند در فهرست اصلی کاربر
//    نبود — چون Alert_OnMktPhase پیش‌فرض خاموش است، ریسک کمی داشت.)
//
//    [🆕 FACTUAL-LIGHT-ATTRIBUTION] در پیام Crisis و پیام Rule=CLOSE روی
//    چارت اصلی (Alert_CheckGBPNZDRule/Alert_CheckAndSend)، جملهٔ ثابت قبلی
//    («Crisis قرمزه و روند+جریان‌پول کاملاً برخلاف جهتته» که همیشه عیناً
//    تکرار می‌شد) با نکات جداگانه از g_lightRTM/Trend/Struct/FLOW یا
//    snapBuy/snapSell.rtm/trend/structD1/flowLight — یعنی همان داده‌ای که
//    خودِ تصمیم را ساخت — جایگزین شد؛ فقط چراغی که واقعاً قرمز است ذکر
//    می‌شود. برای پیام‌های EURGBP/AUDCAD (که فقط شمارش rc در دسترس دارند نه
//    تفکیک هر چراغ)، از «X از ۴ چراغ قرمزن» استفاده شد — دقیق‌تر از جملهٔ
//    ثابت قبلی، هرچند کامل مثل چارت اصلی نیست (نیازمند گسترش
//    Generic_ReplayCrisisAtBar در آینده اگر تفکیک کامل لازم شد).
//
//    [🔧 STATUS-CLEANUP] "worst of the two"→"worst"؛ Hi Alert بدون پوزیشن
//    حالا جهت بدترین را نشان می‌دهد («worst: Buy»)، مثل Crisis؛ حذف
//    "(last calc: DATE)" از MktPhase.
//    [🔧 DURATION-FORMAT] SQ_FormatDuration/SQ_FormatDurationBars: وقتی
//    دقیقه دقیقاً صفر است فقط ساعت نشان داده می‌شود ("23h00m"→"23h") —
//    مرکزی، روی گزارش Status و همهٔ پیام‌های Rule/یادآور اثر گذاشت.
//    [🔧 REMINDER-GOOD-NEWS] یادآورهای Rule حالا یک نکتهٔ «خبر خوب اینکه N
//    ساعت آرامش پیاپی ثبت شده» هم دارند (پیشرفت به سمت رفعِ توقف/بستن).
//
//    محدودهٔ تغییر: فقط متن/ساخت پیام‌ها + یک اینپوت گیت جدید + دو تابع
//    کمکیِ خالص (بدون side effect روی تصمیم). هیچ آستانه، فرمول، یا منطق
//    تصمیم‌گیری Rule/Crisis/Spike/... دست نخورده.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.22 | 2026-07-22 — رفع RESUME زودهنگام + پاک‌سازی رشته‌های ورژن قدیمی
//    [🔴 BUG-RESUME-GAP] گزارش کاربر: در بک‌تست، Rule گاهی فقط ۱ ساعت Stop
//    می‌ماند و فوراً به Run برمی‌گشت، با اینکه انتظار می‌رفت حداقل ۲۴ ساعت
//    پاک پشت‌سرهم لازم باشد. تأیید شد با شبیه‌سازی روی دادهٔ واقعی: ۷ نمونه
//    پیدا شد، همه با الگوی یکسان — یک ساعت تنهای Crisis=Orange+Spike=Warning.
//    ریشه: ApplyClosedBarToRule فقط crisis==Red (نه Orange) را «دِرتی»
//    حساب می‌کرد، ولی Rule_Transition همان تابع، STOP-B را با
//    spike==1 && crisis IN {Red,Orange} می‌سنجد — یعنی همان ساعتی که STOP
//    را روشن می‌کند، می‌توانست هم‌زمان cleanH را صفر نکند (چون Orange بود
//    نه Red خالص). اگر cleanH از یک دورهٔ آرام قبلی از قبل نزدیک ۲۴ بود،
//    همان یک بلوک تنها کافی بود تا بلافاصله RESUME بزند.
//    راه‌حل: یک متغیر جدید crisisElevated (Red یا Orange) اضافه شد که فقط
//    cleanH را تغذیه می‌کند. عمداً crisisRedH/CLOSE-B دست‌نخورده ماندند —
//    آن یک آستانهٔ جدا و مهم‌تر است (تصمیم به بستن پوزیشن، نه فقط رد ورود
//    جدید) و گسترش آن به Orange تصمیم متفاوتی‌ست که باید جدا بررسی شود.
//    شبیه‌سازی پایتون تأیید کرد: بعد از فیکس، هر ۷ نمونه به‌جای ۱ ساعت،
//    مطابق شرایط واقعی (تا رسیدن به ۲۴ ساعت پاک واقعی) در Stop می‌مانند.
//    [CLEANUP] رشته‌های نسخهٔ قدیمی در پیام‌های Print راه‌اندازی و برچسب
//    عنوان داشبورد (v14.10/v14.11/v14.15 باقی‌مانده از نسخه‌های خیلی قبل)
//    به v14.22 به‌روزرسانی شدند. فقط متن لاگ/UI — هیچ منطقی تغییر نکرد.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.21 | 2026-07-22 — پچ FLOW-FREEZE + دو تغییر تأییدشدهٔ کالیبراسیون
//    [🔴 CRITICAL BUG-FLOW-FREEZE] رفع شد در FlowEvaluate: علاوه بر چک‌های
//    قبلی (EXIST + iBars(H4)>50 از v13.49)، حالا تازگی دادهٔ هر خواهر هم چک
//    می‌شود — iTime(sym, H4, 1) باید حداکثر ۷۲ ساعت (weekend-safe) از
//    TimeCurrent() عقب‌تر باشد، وگرنه رد می‌شود. ریشهٔ باگ: چک‌های قبلی فقط
//    «وجود» و «تعداد» کندل را می‌سنجیدند نه «تازگی» — یک خواهر که تاریخچهٔ
//    محلی‌اش در Strategy Tester از یک تاریخ جلوتر نرود از هر دو چک رد
//    می‌شد ولی c1/c6 را از همان دادهٔ ثابت می‌خواند، پس Flow_Score برای
//    همیشه قفل می‌شد. تأیید شده روی دادهٔ واقعی GBPNZD: فریز دقیقاً از
//    2026-07-06 08:00 (Buy=+5.00, Sell=-6.62 هم‌زمان) که علت واگرایی
//    گزارش‌شدهٔ لایو/بک‌تست در ۲۰ جولای بود، نه یک باگ معکوس‌شدن Buy/Sell.
//    محدودهٔ تغییر: فقط FlowEvaluate. هیچ پارامتری از Spike، Crisis یا
//    Cooldown دست نخورده.
//    [CALIBRATION] inp_MktPhase_GBPNZD_TrendMin: 1.15 → 1.22. چک صدک‌های
//    واقعی (۳۱۳۱ ساعت پاک، قبل از فریز FLOW) نشان داد 1.15 روی صدک ۷۷.۸ام
//    می‌نشیند نه ۹۰ام که برچسبش ادعا می‌کرد (RangeMax=0.85 تأیید شد، دقیقاً
//    صدک ۱۹.۱ام). تأییدشده توسط کاربر بعد از گزارش کالیبراسیون.
//    [CALIBRATION] Alert_OnStruct: false → true. تحلیل نشان داد D1_Status
//    مستقیم بعد از ورود جداکننده نیست، ولی BREAK-شدن آن حین معاملهٔ باز با
//    نرخ ~۱۰برابر بیشتر با رسیدن به Max Step ۴-۵ همراه است. این آلارم از
//    قبل به‌طور کامل پیاده‌سازی شده بود (خط ۸۰۸۳، همون الگوی ADX/RTM با
//    Cooldown) و فقط غیرفعال بود — کد جدیدی نوشته نشد، فقط فلگ روشن شد.
//    تأییدشده توسط کاربر بعد از گزارش کالیبراسیون.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.20.4 | 2026-07-20 — رفع باگ حیاتی «آخر هفته» + اصلاح زبان پیام‌ها
//    [🔴 CRITICAL BUG-WEEKEND-STALE-GV] گزارش دقیق کاربر بعد از ری‌استارت
//    آخر هفته: GBPNZD_GV_Load داشت با یک حلقهٔ wall-clock (نه بر مبنای
//    کندل واقعی) شکاف بین آخرین ذخیره و الان را پر می‌کرد — روی چارت M1،
//    یک آخر هفتهٔ معمولی معادل هزاران «دقیقهٔ تقویمی» بود، پس CleanH به‌غلط
//    تا سقف ~۷۲ ساعت پر می‌شد، حتی وقتی هیچ کندل واقعی‌ای در آن بازه شکل
//    نگرفته بود. نتیجه: GBPNZD Buy از STOP (Clean=5h) به RESUME نادرست
//    (Clean=72h) جهش کرد بدون اینکه واقعاً بازار آرام مانده باشد. رفع شد:
//    این حلقه حالا با iBarShift (شمارش کندل واقعی) کار می‌کند — در طول
//    بستگی کامل بازار، صفر کندل واقعی وجود دارد پس اصلاً اجرا نمی‌شود.
//    علاوه بر این، آستانهٔ «GV stale» در هر ۳ ارز از ۷۲ ساعت هاردکد به
//    input قابل‌تنظیم inp_GVStaleHours (پیش‌فرض ۱۲۰=۵ روز) تبدیل شد — چون
//    ۷۲ ساعت به‌راحتی با یک آخر هفته + کمی تأخیر اتصال مجدد رد می‌شد.
//    (EURGBP/AUDCAD این باگ خاص را نداشتند — GV آن‌ها اصلاً گپ‌فیل ندارد.)
//    [STATUS-ENGLISH-ONLY] بازخورد کاربر: پیام «وضعیت» (چه خودی، چه
//    On-Demand ارز خارجی) کاملاً به انگلیسی برگشت — این پیام‌ها چگالی
//    بالای خط‌های کوتاه دارند و هر کلمهٔ فارسی وسطشان چیدمان راست‌به‌چپ/
//    چپ‌به‌راست تلگرام را بهم می‌ریزد. بقیهٔ پیام‌ها (Rule/یادآور/آلارم
//    چراغ‌ها) طبق تأیید صریح کاربر فارسی ماندند.
//    [BUG-ATTRIBUTION-FIX] یادداشت «(N ساعت پیاپی)» در پیام CLOSE هر سه
//    ارز قبلاً همیشه زیر خط فروش می‌چسبید، حتی وقتی خرید بود که قرمز شده
//    بود. حالا به خط همون جهتی می‌چسبد که واقعاً level=2 گرفته.
//    [BUG-CLARITY-FIX] «(N ساعته)» در پیام‌های یادآور مبهم بود (معلوم نبود
//    یعنی چقدر تو توقف بوده یا چیز دیگه). الان می‌گه «N ساعت آرامش پیاپی
//    تا الان» — یعنی پیشرفت به‌سمت رفع توقف، نه مدت توقف. همچنین از
//    SQ_FormatDurationBars استفاده شد (قبلاً روی چارت غیر-H1 عدد را
//    هزاران نشان می‌داد، نه ساعت واقعی).
// ════════════════════════════════════════════════════════════════════
// 📋 v14.20.3 | 2026-07-19 — بازبینی رفتاری + کالیبراسیون سبک‌ها + بازطراحی
//    کامل متن پیام‌ها
//    [BUGFIX] Rule_ReminderIsDue: اگه transition درست قبل از مرز یک اسلات
//    اتفاق می‌افتاد، یادآور می‌تونست چند ثانیه بعد از پیام گذار دوباره فایر
//    بشه. حالا حداقل نصف بازهٔ تکرار هم باید از آخرین رویداد گذشته باشه.
//    [CALIBRATION] با کد و داده چک شد: Spike (GBPNZD/EURGBP/AUDCAD) از قبل
//    درست کالیبره بود (AUDCAD's کامنت داخلی stale بود، مقدار input واقعی
//    از قبل درست بود — کامنت اصلاح شد). Crisis آستانه‌های هر ۳ ارز input و
//    درست‌اند. Flow/RTM/Struct/Zombie بدون دادهٔ جفت‌ارزهای خواهر قابل
//    کالیبراسیون کامل نیستند — تغییری روی اونا داده نشد.
//    [MSG-REDESIGN] بازخورد کاربر: بازطراحی کامل تقریباً همهٔ پیام‌های
//    تلگرام — بدون نام سمبل تکراری (با/بدون پسوند)، هر خط با کلمهٔ فارسی
//    شروع می‌شه (حتی جلوی اسم چراغ‌های انگلیسی مثل Crisis/Flow)، بدون
//    rc=/CrisisRedH= خام، بدون breakdown تایم‌فریمی Spike، MktPhase/Trend/
//    Spike بدون عدد خام (فقط رنگ+کلمه مثل بقیه)، Buy/Sell→خرید/فروش،
//    لحن ساده و خودمونی. شامل: وضعیت (خودی+On-Demand)، پیام‌های گذار و
//    یادآور Rule (هر ۳ ارز)، ۸ آلارم چراغ، تأیید Spike، آلارم MktPhase،
//    Drift Guard، Infinite Lock.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.20.2 | 2026-07-18 — گزارش تست لایو دوم: رفع ریشه‌ای «دوتا H1»،
//    یادآورهای اسلات‌بندی‌شده، و کالیبراسیون کامل MktPhase
//    منبع: بازخورد لایو کاربر (اسکرین‌شات پیام‌های واقعی تلگرام) + دادهٔ
//    کندل M1 واقعی هر سه ارز (ژانویه–ژوئیهٔ ۲۰۲۶، ۶.۵ ماه، ارسالی کاربر).
//
//    [EXTRA-CHARTS-ROOT-CAUSE] علت واقعی «دوتا H1»: Alert_SendTelegramPhoto
//    (تابع قدیمی v13.10، در TG_Outbox_Process صدا زده می‌شود) از قبل عکسِ
//    تایم‌فریم *جاری* چارت را می‌فرستاد؛ نسخهٔ v14.20.1 یک ورودی «جاری» دیگر
//    هم به Alert_SendExtraCharts اضافه کرده بود → همان چارت دوبار. حالا
//    Alert_SendExtraCharts دیگر تایم‌فریم جاری را تکرار نمی‌کند — فقط
//    M30→M15→M5→M1 (با رد شدن از هرکدام که تصادفاً با تایم‌فریم جاری یکی
//    باشد). ترتیب نهایی: [جاری، از Alert_SendTelegramPhoto] → M30 → M15 → M5 → M1.
//    [REMINDER-ALIGNED] علت «ساعت‌های نامنظم» (۲۲:۵۲ به‌جای ۱۱:۳۰/۱۲:۳۰):
//    طرح قبلی reminder را نسبت به «آخرین ارسال واقعی» می‌سنجید — هر وقفهٔ
//    اجرای EA (مثلاً برای نصب آپدیت) لنگر زمان‌بندی را به لحظهٔ ری‌استارت
//    می‌انداخت و برای همیشه از آن لحظه پیروی می‌کرد. حالا Rule_ReminderIsDue
//    زمان را به اسلات‌های ثابت repMin‌دقیقه‌ای تقسیم می‌کند — یادآورها همیشه
//    روی مرزهای ثابت می‌افتند، صرف‌نظر از این‌که EA کِی ری‌استارت شده.
//    EURGBP_CheckReminderForDirection/AUDCAD_CheckReminderForDirection هم
//    به همین تابع مشترک وصل شدند (قبلاً منطق تکراری داشتند).
//    [MKTPHASE-CALIBRATION+ER] آستانه‌های Range/Trend دیگر هاردکد (0.85/1.30
//    برای هر سه ارز) نیستند — inp_MktPhase_{GBPNZD,EURGBP,AUDCAD}_{RangeMax,
//    TrendMin} با صدک ۲۰ام/۹۰ام توزیع تاریخی هر ارز مقداردهی شدند (جزئیات
//    در کامنت بالای Calc_MktPhase). علاوه بر این، یک لایهٔ تأیید اختیاری
//    جدید (inp_MktPhase_UseEfficiencyFilter + HM_DailyEfficiencyRatio) اضافه
//    شد: بک‌تست نشان داد نسبت ADR بالا به‌تنهایی روزهای شلاقی/بی‌جهت را از
//    روند واقعی تشخیص نمی‌دهد؛ حالا Trend فقط با تأیید Efficiency Ratio ثبت
//    می‌شود. MktPhase_GetThresholds/MktPhase_Classify/HM_DailyEfficiencyRatio
//    هم در Calc_MktPhase (چارت خودی) هم در HM_QuickMktPhaseForSymbol
//    (On-Demand ارز خارجی) به‌طور یکسان استفاده می‌شوند.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.20 | 2026-07-17 — پچ جامع بر اساس تحلیل پنج‌گانهٔ v14.19
//    منبع: تحلیل خط‌به‌خط پنج مدل مستقل روی HelpMe_V14_19.mq5 (۱۸٬۰۶۲ خط).
//    همهٔ ایرادات اجماعی/تک‌منبعی رفع شدند + ۳ خواستهٔ شخصی کاربر اضافه شد.
//    معماری اصلی (Snapshot، Rule، Cooldown، Outbox) دست‌نخورده ماند —
//    همهٔ تغییرات نقطه‌ای و محدود به همان بخش‌ها هستند.
//
//    [SECURITY-WARNING] توکن تلگرام هاردکد (خط ~۱۶۴۹) — طبق درخواست
//    کاربر مقدارش تغییر نکرد (خودش بعداً از BotFather عوض می‌کند)؛ فقط
//    یک کامنت هشدار دائمی بالای آن خط اضافه شد. اجماع کامل ۵/۵ تحلیل.
//    [SUFFIX-FIX] بلوک‌های موازی EURGBP/AUDCAD قبلاً رشتهٔ خام "EURGBP"/
//    "AUDCAD" را مستقیم به handleها، Generic_ReplayCrisisAtBar/SpikeAtBar،
//    و HasRealPositionsOnSymbol می‌دادند — بدون resolve پسوند بروکر
//    (برخلاف Sister/Flow که از FlowFindSym استفاده می‌کردند). تابع جدید
//    ResolveBrokerSymbolForParallel (دقیقاً الگوی FlowFindSym) یک‌بار در
//    OnInit صدا زده می‌شود و نتیجه در g_eurgbpBrokerSymbol/
//    g_audcadBrokerSymbol کش می‌شود؛ همه‌جا جایگزین رشتهٔ خام شد.
//    [COOLDOWN-FIX] موتور «چارت خودی» (GBPNZD_InitReplay،
//    Alert_CheckGBPNZDRule) و LogHourlySnapshot همیشه inp_SpikeCooldownBars
//    هاردکد می‌گرفتند، حتی وقتی _Symbol واقعاً AUDCAD بود (سناریوی رسمی
//    «EA روی چارت AUDCAD»). تابع جدید GetCooldownBarsForSymbol(sym) در
//    هر ۶ نقطهٔ فراخوانی ApplyClosedBarToRule (+۳ نمایش داشبورد/دیباگ
//    برای هماهنگی) جایگزین شد. بلوک موازی AUDCAD از قبل درست بود — دست‌نخورده.
//    [ONDEMAND-RELIABILITY-FIX] تحویل نهایی گزارش On-Demand
//    (OnDemand_RunAndSend) از Alert_SendTelegram خام (بدون retry) به
//    Alert_Send (تلاش فوری + fallback به TG_Outbox با backoff) تغییر کرد.
//    [COMBINED-REMINDER] بهبود کاربر: اگر یادآور تکراری Buy و Sell
//    هم‌زمان لازم شود، به‌جای دو پیام جدا در یک چرخهٔ OnTimer، یک پیام
//    ترکیبی واحد فرستاده می‌شود (Rule_CheckReminders بازنویسی شد؛ حالت
//    تک‌جهته دقیقاً همان رفتار قبلی v14.19 را حفظ می‌کند).
//    [EXTRA-CHARTS] بهبود کاربر: Alert_SendExtraCharts حالا تایم‌فریم جاریِ
//    چارتی که EA رویش اجراست را همیشه اول می‌فرستد، بعد M30 → M15 → M5 → M1
//    (D1 کلاً حذف شد). 🔧 اصلاح v14.20.1 بعد از تست لایو: طراحی اولیه
//    (append در انتها + D1) وقتی EA روی H1 بود باعث ترتیب/تکرار گیج‌کننده
//    می‌شد؛ حالا تایم‌فریم جاری همیشه در جایگاه اول است و اگر با یکی از
//    M30/M15/M5/M1 یکی باشد، در جایگاه خودش دوباره فرستاده نمی‌شود.
//    [ONDEMAND-MKTPHASE] بهبود کاربر: پیام‌های Buy/Sell درخواست On-Demand
//    حالا یک خط "MktPhase: Range/Neutral/Trend! (ratio=X.XXX)" دارند —
//    با تابع جدید HM_QuickMktPhaseForSymbol که مستقیماً روی D1 خودِ سمبل
//    خارجی محاسبه می‌کند (نه g_lightMktPhase که مال چارت خودی است).
// ════════════════════════════════════════════════════════════════════
// 📋 v14.19 | 2026-07-15 — کالیبراسیون AUDCAD (قطعهٔ سوم/آخر پازل)
//    اجماع نسبی ۷ تحلیل مستقل روی ۶ ماه دادهٔ واقعی AUDCAD. برخلاف
//    EURGBP (v14.16)، AUDCAD در منطقهٔ مرده نیست — FLOW به‌اندازهٔ کافی
//    Red می‌شود (~۱۷٪) و هر ۱۳ اسپایک تأییدشده بالای ۱.۵۰ هستند — پس
//    Warn/Sig عمداً جدا نگه داشته شدند. جزئیات کامل استدلال/اعداد در
//    v14.19_Changes.md.
//
//    [AUDCAD-SPIKE] Warn 1.00→1.45 (≈P75)، Sig 1.50→1.85 (≈P95) —
//    همون متدولوژی صدکی GBPNZD (v14.11: Sig≈P95)، نه حدس. هر دو داخل
//    بازهٔ اجماع (Warn 1.20-1.60 / Sig 1.50-2.10) هستند.
//    [AUDCAD-WEIGHTS] wM15 0.15→0.12، wM30 0.15→0.13 (نویز کمتر) |
//    wH4 0.25→0.28، wD1 0.15→0.17 (فرسایش چندروزه، مثل Short آوریل) |
//    wH1 بدون تغییر. مجموع = 1.00.
//    [AUDCAD-FLOW] بدون تغییر (Green=3.0 / Red=-5.0) — اجماع ۶/۷.
//    [AUDCAD-CRISIS-INPUT] ۲۰ آستانهٔ Crisis از هاردکد به Input —
//    بدون تغییر هیچ عددی (اجماع ۷/۷: min ADX واقعی در Red = 38.1).
//    [AUDCAD-COOLDOWN-INDEPENDENT] Cooldown مستقل تازه (نه
//    inp_SpikeCooldownBars مشترک GBPNZD/EURGBP) = 3، نه اجماع اکثریت 2
//    — SpikeScore میانگین بالاتر (1.20 vs 1.15 GBPNZD) یعنی baseline
//    نویزی‌تر. تصمیم مرزی؛ برگرداندن به 2 فقط تغییر یک عدد است.
//    [AUDCAD-ZOMBIE-RECAL] عرض Zone 200→100 پیپ — تخمین نسبتی (نه
//    replay واقعی)؛ نیاز به صحت‌سنجی با لاگ واقعی بعد از استقرار.
//    [AUDCAD-DRIFT-GUARD] + [AUDCAD-INFINITE-LOCK] دو ویژگی تازه، فقط
//    AUDCAD (پیشنهاد کاربر). محدودیت شناخته‌شده: فقط در حالت Parallel
//    کار می‌کنند (چارت≠AUDCAD) — دلیل کامل در v14.19_Changes.md بخش ۶.
//
//    معماری Stateful مستقل کامل برای AUDCAD (g_audcad*، GV، Replay،
//    یادآور) — دقیقاً همان الگوی EURGBP در v14.17، شامل
//    g_audcadParallelActive (معادل g_eurgbpParallelActive از v14.18)
//    از همون ابتدا. صفر تغییر در GBPNZD یا EURGBP — تأیید شده با diff
//    کامل خط‌به‌خط قبل از تحویل.
// ════════════════════════════════════════════════════════════════════
...
// 🆕 v14.25 [SIGNAL-CALIBRATION]: GBPNZD طبق کالیبراسیون بخش ۸ گزارش، ۸۵.۶٪
// زمان در Ranging طبقه‌بندی می‌شود — قفل کامل سیگنال روی این ارز عملاً
// سیستم فلش‌ها را بی‌اثر می‌کرد. برای GBPNZD این قفل غیرفعال شد؛ برای
// EURGBP/AUDCAD دست‌نخورده (true) ماند، طبق درخواست صریح.
bool BlockSignalsInRanging_ForSymbol()
{
   if(StringFind(_Symbol, "GBPNZD") >= 0) return false;
   return BlockSignalsInRanging;
}
...
// 🐛 v14.20.4 [BUG-WEEKEND-STALE-GV]: قبلاً این عدد هاردکد 72 ساعت بود.
// مشکل: این عدد با «wall-clock» (زمان تقویمی، نه ساعت بازار) مقایسه
// می‌شود. یک آخر هفتهٔ معمولی فارکس (جمعه بسته تا یکشنبه باز) خودش حدود
// ۴۸-۵۰ ساعت است؛ اگه EA/VPS/ترمینال حتی چند ساعت بیشتر از حد معمول
// (مثلاً به‌خاطر ری‌استارت) در طول همان آخر هفته پایین بماند، این مجموع
// به‌راحتی از ۷۲ ساعت رد می‌شود — با اینکه در واقع «هیچ اتفاقی» در بازار
// نیفتاده (بازار فقط بسته بوده). نتیجه: GV به‌غلط stale تشخیص داده
// می‌شد و به‌جای ادامهٔ state واقعی، یک Replay کامل از کندل‌های بسته
// اجرا می‌شد — و چون Replay محدودیت شناخته‌شده‌ای دارد (نمی‌تواند
// رویدادهای فقط-تیکیِ داخل یک کندلِ از قبل بسته‌شده را ببیند)، ممکن است
// یک STOP واقعی و به‌درستی‌تشخیص‌داده‌شده را نادیده بگیرد و CleanH را به‌طور
// نادرست جهش بدهد (دقیقاً چیزی که کاربر بعد از یک ری‌استارت آخر هفته دید).
// راه‌حل: این آستانه حالا input است و پیش‌فرضش ۱۲۰ ساعت (۵ روز) — با
// راحتی یک آخر هفتهٔ معمولی + یک تعطیلی طولانی‌تر (مثلاً یک دوشنبهٔ
// تعطیل) + چند ساعت بافر اتصال مجدد را پوشش می‌دهد، و فقط اگه EA واقعاً
// نزدیک به یک هفته کامل خاموش بوده باشد Replay کامل اجرا می‌شود.
input int      inp_GVStaleHours = 120;  // 🐛 حداکثر ساعتِ اعتبار GV قبل از Replay کامل (پیش‌فرض ۱۲۰=۵ روز، پوشش‌دهندهٔ آخر هفته+تعطیلی)
...
// 🆕 v14.23 [MSG-REDESIGN] نسخهٔ جهت‌دار بالا: HasRealPositionsOnSymbol با هر دو
// جهت (Buy یا Sell) true برمی‌گرداند — یعنی وقتی فقط یک Sell باز داریم، پیام
// جهت Buy همچنان فکر می‌کند «پوزیشن واقعی هست» و می‌گوید «هنوز نبستی؟». این
// تابع دقیقاً همان فیلتر را می‌زند + نوع پوزیشن (POSITION_TYPE) را هم چک
// می‌کند — طبق درخواست کاربر: «اگه نداشتیم روی اون ارز و با اون حالت Buy یا
// Sell» باید بگه معامله باز نکن، نه فقط بر اساس وجودِ هر پوزیشنی روی سمبل.
bool HasRealPositionOnSymbolDir(string sym, bool forBuy)
{
   ENUM_POSITION_TYPE wantType = forBuy ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(!HM_PositionBelongsToSymbol(PositionGetSymbol(i), sym)) continue;
...
// 🔴 v14.20 SECURITY WARNING: این توکن هاردکد است و در ریپوی گیت‌هاب (در صورت عمومی بودن)
// قابل مشاهده برای هرکسی است. این پچ عمداً مقدار را تغییر نمی‌دهد (کاربر خودش بعداً از طریق
// @BotFather توکن را revoke/رجنریت می‌کند)، اما این کامنت به‌عنوان یادآوری دائمی باقی می‌ماند:
// این توکن را به‌محض ممکن بودن باطل کنید و یک توکن جدید جایگزین این رشته کنید،
// یا بهتر، آن را از سورس خارج کرده و به‌صورت input یا فایل خارج از Git نگه دارید.
...
input bool   Alert_OnStruct          = true;    // 🆕 v14.21: فعال شد — تحلیل کالیبراسیون نشون داد D1_Status=BREAK حین معاملهٔ باز ~۱۰برابر بیشتر با رسیدن به Step۴-۵ همراهه؛ این آلارم (از قبل پیاده‌سازی‌شده، فقط خاموش بود) همون هشدار میان‌معامله‌ست | Alert on Daily Struct (was already built, just off)
...
// 🆕 v14.23 [FOREIGN-RULE-GATE]: پیش‌فرض False طبق درخواست کاربر — پیام‌های
// Rule (STOP/CLOSE/RESUME + یادآور + Drift Guard/Infinite Lock) فقط برای
// سمبل خودِ چارت ارسال می‌شوند. سمبل «خودی» همیشه صرف‌نظر از این تنظیم
// پیام می‌فرستد؛ این فقط دو ارز «خارجی» (Parallel — دو ارز دیگر از
// سه‌گانهٔ AUDCAD/GBPNZD/EURGBP) را کنترل می‌کند. محاسبه/ردیابی state آن دو
// ارز بدون تغییر ادامه می‌یابد — فقط ارسال واقعی پیام تلگرام گیت می‌شود
// (پاسخ On-Demand تحت تأثیر قرار نمی‌گیرد، چون آن یک درخواست صریح کاربره).
input bool   Alert_SendForeignRuleMessages = false;  // 🌐 ارسال پیام Rule برای ۲ ارز خارجی هم | Send Rule alerts for the other 2 (foreign) symbols too
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.24: کالیبراسیون سراسری (هر سه ارز) — بر اساس یافته‌های کاربر
// ════════════════════════════════════════════════════════════════════
// این تنظیمات (بر خلاف کالیبراسیون GBPNZD/AUDCAD زیر که per-symbol هستند)
// سراسری‌اند — دقیقاً چون مقادیر قبلی‌شان هم به‌صورت ثابت/هاردکد و مشترک
// بین هر سه ارز بودند (نه per-symbol). محدودهٔ اعمال: طبق درخواست صریح
// کاربر فقط پردازش «سمبل خودی» (چارتی که EA رویش اجراست) — یعنی
// TL_Update, BuildDirectionSnapshot(سمبل خودی), AnalyzeSignal. پردازش
// موازی EURGBP/AUDCAD وقتی سمبل «خارجی» هستند، عمداً دست‌نخورده ماند (به
// همان آستانه‌های قدیمی) — جزئیات کامل در فایل MD همراه.
enum ENUM_RTM_MODE
{
   RTM_MOMENTUM,   // 🆕 پیش‌فرض: دورشدن از EMA200 = ادامهٔ روند (امتیاز/چراغ مثبت)، نزدیک‌شدن = ریسک برگشت
   RTM_REVERSION   // رفتار قدیمی: دو زیرحالت With-Trend/Counter-Trend مثل قبل، بدون تغییر
};
input group "════════ 🆕 v14.24 کالیبراسیون سراسری (High Alert / Killer / RTM) ════════"
// --- High Alert (جهت‌دار) ---
// قبلاً آستانه (ADX≥35 AND Flow<-7.0) برای Buy و Sell یکسان بود. حالا Buy
// آستانهٔ خودش را دارد؛ Sell دست‌نخورده (۳۵ و ۷-، هاردکد) باقی ماند — طبق
// درخواست صریح کاربر.
input double inp_HighAlert_ADX_Buy  = 40.0;  // ⚠️ High Alert: آستانهٔ ADX برای Buy (جایگزین ۳۵ مشترک قبلی) | Buy-specific ADX threshold
input double inp_HighAlert_Flow_Buy = -2.0;  // ⚠️ High Alert: آستانهٔ Flow برای Buy (جایگزین ۷- مشترک قبلی) | Buy-specific Flow threshold
// --- Killer (جهت‌دار) ---
// قبلاً آستانهٔ TrendScore خطر (چراغ قرمز Killer) ±۵۰ ثابت/متقارن بود.
// طبق یافته‌های کالیبراسیون جدید کاربر، این عدد برای هر جهت فرق می‌کند.
input double inp_Killer_TrendScore_Buy  = -44.0; // ☠️ Killer: TrendScore خام ≤ این مقدار = خطر برای Buy (قبلاً ۵۰-) | Buy danger threshold
input double inp_Killer_TrendScore_Sell =  11.0; // ☠️ Killer: TrendScore خام ≥ این مقدار = خطر برای Sell (قبلاً ۵۰+) | Sell danger threshold
input double inp_Killer_TrendScore_Yellow_Sell = 10.0; // 🆕 v14.25: آستانهٔ Yellow جهت‌دار برای Sell (بود: مشترک ۴۰ با Buy) | Sell-specific Yellow threshold
// --- RTM Mode ---
input ENUM_RTM_MODE inp_RTM_Mode = RTM_MOMENTUM; // 📉 حالت RTM — MOMENTUM(پیش‌فرض): دورشدن=ادامهٔ روند | REVERSION: رفتار قدیمی
input bool inp_RTM_ExcludeFromScore = true; // 🆕 v14.25: امتیاز RTM از مجموع سیگنال کنار گذاشته شود (حالت Momentum تازه و تست‌نشده) | Exclude RTM from signal score for now
...
input group "════════ ⚖️ کالیبراسیون AUDCAD ════════"
// 🆕 v14.19: کالیبره شد — دیگر Placeholder نیست (اجماع نسبی ۷ تحلیل مستقل
// روی ۶ ماه دادهٔ واقعی AUDCAD). روش Warn/Sig: همون متدولوژی صدکی که برای
// GBPNZD در v14.11 استفاده شد (Sig≈P95 از توزیع SpikeScore) — نه یک عدد
// حدسی. Warn/Sig عمداً از هم جدا نگه داشته شدند (برخلاف EURGBP v14.16):
// FLOW برای AUDCAD به‌اندازهٔ کافی Red می‌شود (~۱۷٪ ساعت‌ها) و طبق بریف
// کاربر هر ۱۳ اسپایک تأییدشده بالای ۱.۵۰ هستند — پس منطقهٔ مرده اینجا
// مصداق ندارد. جزئیات کامل استدلال در v14.19_Changes.md بخش ۱.
input double inp_AUDCAD_SpikeWarn   = 1.45;  // 🆕 v14.19: از Placeholder 1.00 (≈P75=1.43 از توزیع) | AUDCAD Warning threshold
input double inp_AUDCAD_SpikeSig    = 1.85;  // 🆕 v14.19: از Placeholder 1.50 (≈P95=1.87 از توزیع) | AUDCAD Spike threshold
input double inp_AUDCAD_wM15        = 0.12;  // ⬇️ v14.19: کاهش جزئی نویز (قبل: 0.15) | AUDCAD M15 weight
input double inp_AUDCAD_wM30        = 0.13;  // ⬇️ v14.19: کاهش جزئی نویز (قبل: 0.15) | AUDCAD M30 weight
input double inp_AUDCAD_wH1         = 0.30;  // بدون تغییر — وزن اصلی | AUDCAD H1 weight
input double inp_AUDCAD_wH4         = 0.28;  // ⬆️ v14.19: افزایش جزئی برای فرسایش چندروزه (قبل: 0.25) | AUDCAD H4 weight
input double inp_AUDCAD_wD1         = 0.17;  // ⬆️ v14.19: افزایش جزئی برای فرسایش چندروزه (قبل: 0.15) | AUDCAD D1 weight
input double inp_AUDCAD_FlowGreen   =  3.0;  // بدون تغییر — اجماع ۶/۷ | AUDCAD Flow Green threshold
input double inp_AUDCAD_FlowRed     = -5.0;  // بدون تغییر — اجماع ۶/۷ | AUDCAD Flow Red threshold
// --- 🆕 v14.19 [AUDCAD-CRISIS-INPUT]: از هاردکد به Input — بدون تغییر عدد
// (همون کاری که GBPNZD در v14.11 گرفت). شاخهٔ AUDCAD در Crisis_GetThresholds
// اکنون این ۲۰ ورودی را می‌خواند؛ شاخه‌های GBPNZD و EURGBP در همان تابع
// byte-for-byte دست‌نخورده ماندند. ---
input double inp_AUDCAD_RedA_ADX    = 38.0;  // Crisis RedA: ADX | RedA ADX
input double inp_AUDCAD_RedA_Flow   =  6.0;  // Crisis RedA: Flow | RedA Flow
input double inp_AUDCAD_RedB_ADX    = 40.0;  // Crisis RedB: ADX | RedB ADX
input double inp_AUDCAD_RedB_Flow   =  8.5;  // Crisis RedB: Flow | RedB Flow
input double inp_AUDCAD_OrA_ADX     = 32.0;  // Crisis OrA: ADX | OrA ADX
input double inp_AUDCAD_OrA_Flow    =  5.0;  // Crisis OrA: Flow | OrA Flow
input double inp_AUDCAD_OrB_ADX     = 28.0;  // Crisis OrB: ADX | OrB ADX
input double inp_AUDCAD_OrB_Ts      = 35.0;  // Crisis OrB: TrendScore | OrB TrendScore
input double inp_AUDCAD_OrC_ADX     = 32.0;  // Crisis OrC: ADX | OrC ADX
input double inp_AUDCAD_OrC_Flow    =  7.5;  // Crisis OrC: Flow | OrC Flow
input double inp_AUDCAD_OrC_Ts      = 25.0;  // Crisis OrC: TrendScore | OrC TrendScore
input double inp_AUDCAD_OrD_ADX     = 34.0;  // Crisis OrD: ADX | OrD ADX
input double inp_AUDCAD_OrD_Ts      = 35.0;  // Crisis OrD: TrendScore | OrD TrendScore
input double inp_AUDCAD_Y2_ADX      = 22.0;  // Crisis Y2: ADX | Y2 ADX
input double inp_AUDCAD_Y2_Flow     =  3.0;  // Crisis Y2: Flow | Y2 Flow
input double inp_AUDCAD_Y3_Flow     =  7.0;  // Crisis Y3: Flow | Y3 Flow
input double inp_AUDCAD_Y3_Ts       = 30.0;  // Crisis Y3: TrendScore | Y3 TrendScore
input double inp_AUDCAD_Y4_ADX      = 25.0;  // Crisis Y4: ADX | Y4 ADX
input double inp_AUDCAD_Y4_Ts       = 45.0;  // Crisis Y4: TrendScore | Y4 TrendScore
input double inp_AUDCAD_Y5_ADX      = 35.0;  // Crisis Y5: ADX | Y5 ADX
input double inp_AUDCAD_Y5_Flow     =  7.5;  // Crisis Y5: Flow | Y5 Flow
// --- 🆕 v14.19 [AUDCAD-COOLDOWN-INDEPENDENT]: Cooldown مستقل، جدا از
// inp_SpikeCooldownBars مشترک GBPNZD/EURGBP. تصمیم: 3 (نه اجماع اکثریت 2) —
// میانگین SpikeScore بالاتر (1.20 در برابر 1.15 GBPNZD) یعنی خط پایهٔ
// نویزی‌تر، پس یک کندل تأیید اضافه منطقی‌تر است. این یک تصمیم مرزی است؛
// اگه لاگ‌های واقعی نشون بدن Resume با 3 خیلی کند شده، برگردوندنش به 2
// فقط عوض کردن همین عدد است — بدون کامپایل مجدد. جزئیات در v14.19_Changes.md بخش ۲.
input int    inp_AUDCAD_CooldownBars = 3;    // 🆕 AUDCAD-only cooldown bars (مستقل از GBPNZD/EURGBP)
// --- 🆕 v14.19: دو بهبود جدید، فقط AUDCAD (پیشنهاد بخش ۴.۱.۶ بریف کاربر) ---
input int    inp_AUDCAD_DriftGuardDays   = 5;   // 🆕 [AUDCAD-DRIFT-GUARD] پوزیشن بازتر از N روز + هنوز در Zone بدتر از ورود → هشدار جدا (طراحی‌شده برای گرفتن فرسایش کند مثل بحران آوریل ۶۶‑روزه، قبل از اینکه ۶۶ روز طول بکشد)
input int    inp_AUDCAD_InfiniteLockDays = 6;   // 🆕 [AUDCAD-INFINITE-LOCK] STOP/CLOSE پیوسته بیش از N روز → یک پیام تشدیدشدهٔ یک‌بار (طبق فلسفهٔ طراحی بخش ۶.۳: ۵-۷ روز = Infinite Stop Lock)
...
// 🆕 v14.20.2 [MKTPHASE-CALIBRATION]: قبلاً 0.85/1.30 برای هر سه ارز هاردکد
// بود. کالیبراسیون روی دادهٔ واقعی ۶.۵ ماههٔ هر ارز (کندل M1، ژانویه–ژوئیهٔ
// ۲۰۲۶، ارسالی کاربر) نشان داد این مقادیر فقط برای GBPNZD معنادار بودند —
// با همان آستانه‌ها، EURGBP ~۵۰٪ روزها و AUDCAD ~۵۰٪ روزها «Range» می‌شدند
// و «Trend» برای هیچ‌کدام تقریباً هرگز فعال نمی‌شد (بیشینهٔ نسبت مشاهده‌شده
// در کل بازه: GBPNZD=1.32، EURGBP=1.28، AUDCAD=1.01 — یعنی AUDCAD حتی به
// مرز قدیمی Trend=1.30 نزدیک هم نمی‌شد). مقادیر زیر از صدک ۲۰ام (مرز Range)
// و ۹۰ام (مرز Trend) توزیع تاریخیِ هر ارز گرفته شده‌اند.
input group "════════ ⚖️ کالیبراسیون MktPhase ════════"
input double inp_MktPhase_GBPNZD_RangeMax = 0.85;  // 🆕 صدک ۲۰ام GBPNZD (بدون تغییر نسبت به قبل) | GBPNZD Range boundary
input double inp_MktPhase_GBPNZD_TrendMin = 1.22;  // 🆕 v14.21: اصلاح شد — 1.15 واقعاً روی صدک ۷۷.۸ام می‌نشست نه ۹۰ام (چک مجدد با دادهٔ Dec31-Jul20)؛ 1.22 صدک ۹۰ام واقعی است | GBPNZD Trend boundary (corrected to true 90th percentile)
input double inp_MktPhase_EURGBP_RangeMax = 0.72;  // 🆕 صدک ۲۰ام EURGBP (کاهش از 0.85 — توزیع EURGBP فشرده‌تر است) | EURGBP Range boundary
input double inp_MktPhase_EURGBP_TrendMin = 1.15;  // 🆕 صدک ۹۰ام EURGBP | EURGBP Trend boundary
input double inp_MktPhase_AUDCAD_RangeMax = 0.75;  // 🆕 صدک ۲۰ام AUDCAD | AUDCAD Range boundary
input double inp_MktPhase_AUDCAD_TrendMin = 0.95;  // 🆕 صدک ۹۰ام AUDCAD (کاهش زیاد از 1.30 — AUDCAD حتی به 1.0 هم به‌ندرت می‌رسد) | AUDCAD Trend boundary
// 🆕 v14.20.2 [MKTPHASE-ER] بهبود پیشنهادی کاربر («خلاق‌تر شو، حتی برای
// GBPNZD»): بک‌تست روی همان دادهٔ ۶.۵ ماهه نشان داد نسبت ADR (بالا) به‌تنهایی
// روزهای «رنج پرنوسان/شلاقی» (whipsaw) را از روزهای واقعاً «روند‌دار» تشخیص
// نمی‌دهد — برای GBPNZD، از ۲۳ روزی که نسبت‌شان بالا بود، ۱۱ تا واقعاً
// شلاقی بودند (Efficiency Ratio پایین) و فقط ۳ تا جهت‌دار/تمیز. افزودن تأیید
// Efficiency Ratio (پایین‌تر = حرکت شلاقی/بی‌جهت، نزدیک ۱ = حرکت تمیز و
// یک‌طرفه) باعث می‌شود «Trend!» فقط وقتی فعال شود که هم دامنه بالا رفته
// باشد هم حرکت واقعاً جهت‌دار باشد. این یک لایهٔ اختیاری و آزمایشی است —
// اگر ترجیح می‌دهید دقیقاً مثل قبل (فقط بر پایه نسبت ADR) کار کند، خاموشش کنید.
input bool   inp_MktPhase_UseEfficiencyFilter   = true;  // 🆕 تأیید Trend با Efficiency Ratio (تجربی)
input double inp_MktPhase_MinEfficiencyForTrend = 0.20;  // 🆕 حداقل ER برای تأیید Trend (۰=بدون‌جهت، ۱=کاملاً یک‌طرفه)
...
SymbolZoneCfg g_zoneTable[] =
{
   ...
   // 🆕 v14.19 [AUDCAD-ZOMBIE-RECAL]: width 200→100 pip. علت: 200 پیپ برای
   // GBPNZD/EURGBP کالیبره شده بود، اما موج معمول AUDCAD طبق بریف کاربر تنها
   // ۳۵-۶۰ پیپ است (GBPNZD ≈ ۱۴۰ پیپ) — با نسبت موج‌به‌عرض تقریبی GBPNZD
   // (400/140 ≈ 2.9x) روی همون بازه اعمال شد → ~100-170 پیپ؛ روی سمت
   // محافظه‌کارتر (100) گرد شد. این یک تخمین مبتنی بر نسبت است، نه بازپخش
   // واقعی تیک — لطفاً بعد از استقرار با لاگ واقعی صحت‌سنجی کنید (v14.19_Changes.md بخش ۵).
   // base_high صرفاً مستندسازی است؛ Zone_ComputeFromPrice فقط از base_low+widthPips استفاده می‌کند.
   // AUDCAD: Zone 0 = [0.90000 .. 0.91000) | width=100pip
   { "AUDCAD",  0.90000,  0.91000,   100,       true  },
...
// 🆕 v14.20 [SUFFIX-FIX]: نام واقعی سمبل EURGBP در بروکر — ممکن است پسوند
// داشته باشد (مثلاً "EURGBP.a")؛ در OnInit یک‌بار با ResolveBrokerSymbolForParallel
// محاسبه و اینجا cache می‌شود. از این پس هر جای کد که قبلاً رشتهٔ خام "EURGBP"
// برای دسترسی به قیمت/handle/پوزیشن استفاده می‌شد، باید این متغیر را بخواند.
string   g_eurgbpBrokerSymbol = "EURGBP";
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.19: معماری Stateful مستقل AUDCAD — کاملاً موازی با بلوک‌های
// GBPNZD و EURGBP بالا (که هیچ‌کدام تغییر نکردند). دقیقاً همان الگوی
// EURGBP در v14.17 — همان لیست متغیرها، فقط با پیشوند audcad — به‌علاوهٔ
// ۶ متغیر جدید برای دو ویژگی تازه که فقط مخصوص AUDCAD هستند: Drift Guard
// و Infinite Lock (پیشنهاد بخش ۴.۱.۶ بریف کاربر). جزئیات در v14.19_Changes.md.
// ════════════════════════════════════════════════════════════════════
int      g_audcadLevelBuy           = 0;    // 0=GREEN | 1=STOP | 2=CLOSE
int      g_audcadLevelSell          = 0;
int      g_audcadStopReasonBuy      = -1;
int      g_audcadStopReasonSell     = -1;
int      g_audcadCleanHBuy          = 0;
int      g_audcadCleanHSell         = 0;
int      g_audcadCrisisRedHBuy      = 0;
int      g_audcadCrisisRedHSell     = 0;
datetime g_audcadLastH              = 0;
bool     g_audcadHourRedBuy         = false;
bool     g_audcadHourDirtyBuy       = false;
bool     g_audcadHourRedSell        = false;
bool     g_audcadHourDirtySell      = false;
bool     g_audcadReplayDone         = false;
int      g_audcadReplayRetry        = 0;
bool     g_audcadNeedsLiveCheck     = false;
int      g_audcadPrevCrisisBuy      = -1;
int      g_audcadPrevSpikeBuy       = -1;
int      g_audcadPrevCrisisSell     = -1;
int      g_audcadPrevSpikeSell      = -1;
int      g_audcadCrisisBuy          = -1;
int      g_audcadCrisisSell         = -1;
int      g_audcadSpikeBuy           = -1;
int      g_audcadSpikeSell          = -1;
int      g_audcadSpikeStreakBuy     = 0;
int      g_audcadSpikeStreakSell    = 0;
datetime g_audcadCleanSinceBuy       = 0;
datetime g_audcadCleanSinceSell      = 0;
datetime g_audcadRedSinceBuy         = 0;
datetime g_audcadRedSinceSell        = 0;
datetime g_audcadLastDirtyTimeBuy    = 0;
datetime g_audcadLastDirtyTimeSell   = 0;
int      g_audcadLastDirtyReasonBuy  = 0;
int      g_audcadLastDirtyReasonSell = 0;
datetime g_audcadLastReminderBuy     = 0;
datetime g_audcadLastReminderSell    = 0;
int      g_audcadLastReminderLevelBuy  = 0;
int      g_audcadLastReminderLevelSell = 0;
// 🆕 v14.19: handle های اندیکاتور اختصاصی و دائمیِ AUDCAD (H1/H4/D1) —
// همان 🔒 DESIGN LOCK مستند‌شده برای GBPNZD/EURGBP: عمداً از handle های
// TL یا on-demand استفاده نشد؛ این چهار handle مستقل و همیشه روی AUDCAD‌اند.
int      g_audcadHandleEMA200_H1 = INVALID_HANDLE;
int      g_audcadHandleATR_H1    = INVALID_HANDLE;
int      g_audcadHandleADX_H4    = INVALID_HANDLE;
int      g_audcadHandleATR_D1    = INVALID_HANDLE;
// 🆕 v14.20 [SUFFIX-FIX]: معادل g_eurgbpBrokerSymbol برای AUDCAD — نام واقعی
// سمبل AUDCAD در بروکر (با پسوند احتمالی)، یک‌بار در OnInit resolve و cache می‌شود.
string   g_audcadBrokerSymbol = "AUDCAD";
// 🆕 v14.19: true مگر چارت خودش AUDCAD باشد — یک‌بار در OnInit محاسبه
// می‌شود (دقیقاً معادل g_eurgbpParallelActive برای EURGBP در v14.18).
bool     g_audcadParallelActive = true;
// 🆕 [AUDCAD-DRIFT-GUARD] + [AUDCAD-INFINITE-LOCK] v14.19 — دو ویژگی
// تازه، فقط AUDCAD. levelSince = از چه زمانی این جهت پیوسته >0 است
// (STOP/CLOSE)؛ *Fired = آیا هشدار این اپیزود قبلاً ارسال شده (جلوگیری
// از اسپم؛ با بازگشت سطح به GREEN خودکار ریست می‌شود).
datetime g_audcadLevelSinceBuy          = 0;
datetime g_audcadLevelSinceSell         = 0;
bool     g_audcadInfiniteLockFiredBuy   = false;
bool     g_audcadInfiniteLockFiredSell  = false;
bool     g_audcadDriftGuardFiredBuy     = false;
bool     g_audcadDriftGuardFiredSell    = false;
...
#define HM_AUDCAD_SCHEMA_VERSION  1419.0
...
// 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: کاربر صریحاً خواست پیام «وضعیت» کاملاً
// انگلیسی بماند — چون این پیام چگالی بالایی از خط‌های کوتاه دارد، هر
// کلمهٔ فارسی وسطش جهت راست‌به‌چپ/چپ‌به‌راست تلگرام را بهم می‌ریزد. بقیهٔ
// پیام‌ها (Rule/یادآور/آلارم چراغ‌ها) طبق نظر کاربر مشکلی ندارند و فارسی ماندند.
...
// ════════════════════════════════════════════════════════════════════
// 🔧 v14.20.3 [MSG-REDESIGN]: قبلاً عدد خام score نشون داده می‌شد
// ("🟢 Normal score=0.84") که برای کاربر عادی معنایی نداشت — مثل بقیهٔ
// چراغ‌ها فقط رنگ + یک کلمه.
...
// 🔧 v14.23 [MSG-REDESIGN] بازخورد کاربر: "11h00m" با دقیقهٔ صفر شلوغ و
// بی‌فایده به‌نظر می‌رسید — وقتی دقیقه دقیقاً صفر است، فقط ساعت نشان داده
// می‌شود ("11h")؛ در غیر این صورت رفتار قبلی (h+m) دست‌نخورده می‌ماند.
string SQ_FormatDuration(datetime sinceTime)
{
   ...
   if(h > 0) return (m > 0) ? StringFormat("%dh%02dm", h, m) : StringFormat("%dh", h);
...
// ════════════════════════════════════════════════════════════════════
// 🔧 v14.23 [MSG-REDESIGN]: همان سادگی SQ_FormatDuration — دقیقهٔ صفر نشان
// داده نمی‌شود ("23h00m" → "23h"). این تابع در همهٔ خطوط Rule/یادآور و
// گزارش «وضعیت» استفاده می‌شود، پس این یک تغییر مرکزی همه‌جا را ساده می‌کند.
...
// 🔧 v14.20.3 [MSG-REDESIGN]: قبلاً "| LastDirty: HH:MM Crisis Red | Spike
// Cooldown: n/N" می‌داد — جزئیات داخلی که برای کاربر عادی قابل‌فهم نبود.
// حالا فقط اگه واقعاً تو دورهٔ «خنک‌شدن Spike» باشیم، یک جملهٔ ساده اضافه می‌شود.
string SQ_DirtyCooldownSuffix(bool isForBuy)
{
   int streak = isForBuy ? g_gbpnzdSpikeStreakBuy : g_gbpnzdSpikeStreakSell;
   int cdBars = GetCooldownBarsForSymbol(_Symbol);  // 🆕 v14.20 [COOLDOWN-FIX]
   if(cdBars > 0 && streak > 0 && streak < cdBars)
      return StringFormat(" (cooldown %d/%d)", streak, cdBars);
   return "";
}
...
{
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: پیام وضعیت طبق نظر کاربر باید کاملاً
   // یک‌زبانه (انگلیسی) بماند — همون توضیح ساده‌شده، فقط بدون کلمهٔ فارسی.
   string crisisName = (crisisVal == 2) ? "Red" :
                       (crisisVal == 3) ? "Orange" :
                       (crisisVal == 1) ? "Yellow" :
                       (crisisVal == 0) ? "Green"  : "N/A";

   string restartNote = (!g_gbpnzdReplayDone) ? " — still rebuilding history" : "";
   ...
   {
      if(dirCleanH >= 24)
         return StringFormat("🟢 Back to normal — %s calm in a row", cleanDurStr);
      return StringFormat("🔴 Better to close — %s calm so far%s%s", cleanDurStr, restartNote, dcSuffix);
   }
   if(dirLevel == 1)
   {
      if(dirCleanH >= 24)
         return StringFormat("🟢 Back to normal — %s calm in a row", cleanDurStr);
      return StringFormat("⛔ Don't open new — %s calm so far%s%s", cleanDurStr, restartNote, dcSuffix);
   }
   ...
   if(trigCloseA)
      return "🔴 Better to close — Crisis is Red and trend+flow are fully against you";
   if(trigCloseB)
      return "🔴 Better to close — Crisis has stayed Red for 6h straight";
   if(trigStopA)
      return StringFormat("⛔ Don't open new — sudden strong move + Crisis %s%s", crisisName, restartNote);
   if(trigStopB)
      return StringFormat("⛔ Don't open new — moderately strong move + Crisis %s%s", crisisName, restartNote);

   // Run — نشون بده چرا ایمنه
   if(dirCleanH >= 24)
      return StringFormat("🟢 All normal — %s calm in a row", cleanDurStr);

   if(spikeVal >= 1 && crisisVal == 0)
      return StringFormat("🟢 All normal — volatility is up but Crisis is Green, so no trigger yet%s", dcSuffix);

   return StringFormat("🟢 All normal — %s calm in a row%s%s", cleanDurStr, restartNote, dcSuffix);
}
...
string StatusQuery_BuildReport()
{
   ...
   if(!IsRuleSymbol(sym))
   {
      return "📊 Status " + CleanSymbol(sym) + "\n"
           + "─────────────────────\n"
           + "This symbol is not supported\n"
           + "Open the chart on GBPNZD, EURGBP, or AUDCAD.";
   }
   ...
   // ── Trend ───────────────────────────────────────────────────────
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: انگلیسی، بدون عدد خام Score.
   string trendStr;
   if(!activeMode)
      trendStr = "⚫ --";
   else
   {
      ...
      else
      {
         ...
         trendStr = aligned ? "🟢 With your trade" : "🔴 Against your trade";
      }
   }
   ...
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: "X of 4 lights red" به‌جای RC خام.
   ...
   if(activeMode && g_crisisState >= 0)
      crisisStr = SQ_Light(g_crisisState == 3 ? 3 :
                           g_crisisState == 2 ? 2 :
                           g_crisisState == 1 ? 1 : 0)
                  + StringFormat("  (%d/4 lights red)", crisisRC);
   else if(ruleActiveNoPos && sqSnapBuy.valid && sqSnapSell.valid)
   {
      ...
      // 🔧 v14.23 [MSG-REDESIGN]: "worst of the two" → "worst" (کوتاه‌تر، طبق بازخورد کاربر)
      crisisStr = SQ_Light(worstCrisis) + StringFormat("  (%d/4 lights red, worst: %s — no position)", worstRC, worstDir);
   }
   else
      crisisStr = "⚫  --";
   ...
   string hiAlertStr;
   if(highAlert)
      hiAlertStr = "🔴 Trend + flow fully against you";
   else if(highAlertNoPos)
   {
      // 🔧 v14.23 [MSG-REDESIGN] بازخورد کاربر: بدون پوزیشن، مثل Crisis
      // بدترین جهت را نشان بده (به‌جای فقط یک قرمزِ بدون جهت) — یکدست‌تر.
      bool buyHA  = sqSnapBuy.valid  && sqSnapBuy.highAlert;
      bool sellHA = sqSnapSell.valid && sqSnapSell.highAlert;
      string haDir = (buyHA && sellHA) ? "Buy & Sell" : (buyHA ? "Buy" : "Sell");
      hiAlertStr = StringFormat("🔴 Active  (worst: %s — no position)", haDir);
   }
   ...
   // ── Mkt Phase ──────────────────────────────────────────────────
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: بدون عدد خام ratio، رنگ+کلمه انگلیسی.
   string mktPhaseStr;
   if(g_lightMktPhase == -1)
      mktPhaseStr = "⚫ --";
   else
      // 🔧 v14.23 [MSG-REDESIGN] بازخورد کاربر: "(last calc: DATE)" اطلاعات
      // کم‌ارزشی برای پیام روزمره بود — حذف شد، رنگ+کلمه به‌تنهایی کافی است.
      mktPhaseStr = (g_lightMktPhase==0) ? "🟢 Range" : (g_lightMktPhase==1) ? "🟡 Neutral" : "🔴 Trend";
   ...
   // ── ADX ─────────────────────────────────────────────────────────
   // 🔧 v14.20.3 [MSG-REDESIGN]: "35.2 ER=1.15" برای کاربر عادی معنایی
   // نداشت — حالا فقط رنگ + کلمه، مثل بقیهٔ چراغ‌ها.
   string adxStr = activeMode ? SQ_Light(g_lightTrend) : "⚫ --";

   // ── Flow ────────────────────────────────────────────────────────
   string flowStr = activeMode ? SQ_Light(g_lightFLOW) : "⚫ --";
   ...
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: کاربر صریحاً خواست این پیام کاملاً
   // یک‌زبانه (انگلیسی) بماند تا چیدمان راست‌به‌چپ/چپ‌به‌راست تلگرام بهم
   // نریزد — سمبل بدون پسوند بروکر همچنان حفظ شد (آن مشکلی نداشت).
   string msg = "";
   msg += "📊 Status " + CleanSymbol(sym) + "\n";
   msg += "────────────────────────\n";
   msg += "Trade: "   + tradeIcon    + "\n";
   ...
   msg += "────────────────────────\n";
   msg += "Sell Rule: " + spikeRuleSellStr + "\n";
   msg += "Buy Rule: "  + spikeRuleBuyStr  + "\n";
   msg += "────────────────────────\n";
   msg += "ADX: "   + adxStr  + "\n";
   msg += "Flow: "  + flowStr + "\n";
   msg += "Daily: " + dailyStr + "\n";
...
int OnInit()
{
   ...
   // 🆕 v14.19: معادل بالا برای AUDCAD — فقط وقتی چارت خودش AUDCAD نیست.
   g_audcadParallelActive = (CleanSymbol(_Symbol) != "AUDCAD");
   // 🆕 v14.20 [SUFFIX-FIX]: نام واقعی بروکر برای هر دو بلوک موازی را همین یک‌بار
   // resolve و cache کن — فقط وقتی واقعاً لازم می‌شود (بلوک موازی فعال است).
   // اگر چارت خودش EURGBP/AUDCAD باشد، آن بلوک موازی خاموش است و نیازی به resolve نیست.
   if(g_eurgbpParallelActive) g_eurgbpBrokerSymbol = ResolveBrokerSymbolForParallel("EURGBP");
   if(g_audcadParallelActive) g_audcadBrokerSymbol = ResolveBrokerSymbolForParallel("AUDCAD");
   ...
   if(EnableAllLogs) Print("🚀 HelpMe v14.24 - Initializing...");
   ...
   if(EnableAllLogs)
   {
      Print("✅ HelpMe v14.24 initialized! [CALIBRATION: HighAlert-Buy + Killer-TrendScore + RTM-Mode]");
      Print("🤖 AI Features:");
   ...
   {
      // 🆕 v14.20 [SUFFIX-FIX]: از g_eurgbpBrokerSymbol (resolve‌شده چند خط بالاتر)
      // به‌جای رشتهٔ خام "EURGBP" — اگر بروکر پسوند بخواهد، این دیگر INVALID_HANDLE نمی‌دهد.
      g_eurgbpHandleEMA200_H1 = iMA  (g_eurgbpBrokerSymbol, PERIOD_H1, 200, 0, MODE_EMA, PRICE_CLOSE);
      g_eurgbpHandleATR_H1    = iATR (g_eurgbpBrokerSymbol, PERIOD_H1, ATRPeriod);
      g_eurgbpHandleADX_H4    = iADX (g_eurgbpBrokerSymbol, PERIOD_H4, ADXPeriod);
      g_eurgbpHandleATR_D1    = iATR (g_eurgbpBrokerSymbol, PERIOD_D1, ATRPeriod);
      if(g_eurgbpHandleEMA200_H1 == INVALID_HANDLE || g_eurgbpHandleATR_H1 == INVALID_HANDLE ||
         ...
         Print("⚠️ v14.17: EURGBP dedicated handles failed (symbol used: '" + g_eurgbpBrokerSymbol + "') — EURGBP_InitReplay/Alert_CheckEURGBPRule will retry via OnTimer");
      else
         Print("✅ v14.17: EURGBP dedicated handles OK (H1/H4/D1) — symbol='" + g_eurgbpBrokerSymbol + "'");
   }
   else
      Print("ℹ️ v14.18: چارت خودش EURGBP است — بلوک موازی EURGBP غیرفعال ماند (موتور عمومی مدیریت می‌کند)");

   // 🆕 v14.19: handle های اختصاصی و دائمیِ AUDCAD — دقیقاً معادل بلوک
   // EURGBP بالا (v14.17). فقط وقتی چارت خودش AUDCAD نباشد.
   if(g_audcadParallelActive)
   {
      // 🆕 v14.20 [SUFFIX-FIX]: از g_audcadBrokerSymbol resolve‌شده استفاده می‌شود.
      g_audcadHandleEMA200_H1 = iMA  (g_audcadBrokerSymbol, PERIOD_H1, 200, 0, MODE_EMA, PRICE_CLOSE);
      g_audcadHandleATR_H1    = iATR (g_audcadBrokerSymbol, PERIOD_H1, ATRPeriod);
      g_audcadHandleADX_H4    = iADX (g_audcadBrokerSymbol, PERIOD_H4, ADXPeriod);
      g_audcadHandleATR_D1    = iATR (g_audcadBrokerSymbol, PERIOD_D1, ATRPeriod);
      if(g_audcadHandleEMA200_H1 == INVALID_HANDLE || g_audcadHandleATR_H1 == INVALID_HANDLE ||
         g_audcadHandleADX_H4    == INVALID_HANDLE || g_audcadHandleATR_D1 == INVALID_HANDLE)
         Print("⚠️ v14.19: AUDCAD dedicated handles failed (symbol used: '" + g_audcadBrokerSymbol + "') — AUDCAD_InitReplay/Alert_CheckAUDCADRule will retry via OnTimer");
      else
         Print("✅ v14.19: AUDCAD dedicated handles OK (H1/H4/D1) — symbol='" + g_audcadBrokerSymbol + "'");
   }
   else
      Print("ℹ️ v14.19: چارت خودش AUDCAD است — بلوک موازی AUDCAD غیرفعال ماند (موتور عمومی مدیریت می‌کند)");
   ...
   {
      double closeBufE[];
      CopyClose(g_eurgbpBrokerSymbol, PERIOD_H1, 0, 300, closeBufE);
      CopyClose(g_eurgbpBrokerSymbol, PERIOD_H4, 0, 300, closeBufE);
      CopyClose(g_eurgbpBrokerSymbol, PERIOD_D1, 0, 300, closeBufE);
   }
   // 🆕 v14.19: همون تحریک دانلود، برای AUDCAD هم — فقط اگه بلوک موازی فعال باشه
   if(g_audcadParallelActive)
   {
      double closeBufA[];
      CopyClose(g_audcadBrokerSymbol, PERIOD_H1, 0, 300, closeBufA);
      CopyClose(g_audcadBrokerSymbol, PERIOD_H4, 0, 300, closeBufA);
      CopyClose(g_audcadBrokerSymbol, PERIOD_D1, 0, 300, closeBufA);
   }
   ...
   Print("📊 HelpMe v14.24 (Calibrated for GBPNZD/EURGBP/AUDCAD) — Chart TF: ",
   ...
   if(PERIOD_CURRENT >= PERIOD_D1)
      Print("⚠️ HelpMe v14.24: Chart TF=", EnumToString(PERIOD_CURRENT),
   ...
   // 🆕 v14.19: همان کار برای AUDCAD — کاملاً مستقل، هیچ اثری روی بلوک‌های
   // GBPNZD/EURGBP بالا ندارد.
   if(g_audcadParallelActive && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol)
      && !(bool)MQLInfoInteger(MQL_TESTER))
   {
      AUDCAD_InitReplay();
      g_audcadNeedsLiveCheck = true;
   }

   // 🆕 v14.11: تأیید Inputs بعد از شروع — نتیجه در Experts Log قابل مشاهده است
   v14_11_SanityCheck();
   Print("▶️ HelpMe v14.24 | OnInit OK | Symbol=", _Symbol, " | TF=", _Period,
...
void OnTimer()
{
   ...
   // 🆕 v14.19: همان الگو برای AUDCAD + دو چک تازهٔ مخصوص AUDCAD (Drift
   // Guard و Infinite Lock — بخش ۴.۱.۶ بریف کاربر). هر ۳۰ ثانیه کافیه
   // چون این‌ها در مقیاس روز/ساعت کار می‌کنند، نه کندل.
   if(g_audcadParallelActive)
   {
      AUDCAD_CheckReminders();
      AUDCAD_CheckDriftGuard();
      AUDCAD_CheckInfiniteLock();
   }
   ...
   // 🆕 v14.19: همان retry مکانیزم برای AUDCAD — کاملاً مستقل از بلوک‌های بالا
   if(g_audcadParallelActive && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol)
      && !(bool)MQLInfoInteger(MQL_TESTER)
      && !g_audcadReplayDone
      && g_audcadReplayRetry < 20)
   {
      g_audcadReplayRetry++;
      AUDCAD_InitReplay();
      if(g_audcadReplayDone)
         Print("✅ v14.19 AUDCAD_InitReplay succeeded on retry #", g_audcadReplayRetry);
   }
...
void OnDeinit(const int reason)
{
   ...
   // 🆕 v14.19: همان کار برای AUDCAD
   if(g_audcadHandleEMA200_H1 != INVALID_HANDLE) { IndicatorRelease(g_audcadHandleEMA200_H1); g_audcadHandleEMA200_H1 = INVALID_HANDLE; }
   if(g_audcadHandleATR_H1    != INVALID_HANDLE) { IndicatorRelease(g_audcadHandleATR_H1);    g_audcadHandleATR_H1    = INVALID_HANDLE; }
   if(g_audcadHandleADX_H4    != INVALID_HANDLE) { IndicatorRelease(g_audcadHandleADX_H4);    g_audcadHandleADX_H4    = INVALID_HANDLE; }
   if(g_audcadHandleATR_D1    != INVALID_HANDLE) { IndicatorRelease(g_audcadHandleATR_D1);    g_audcadHandleATR_D1    = INVALID_HANDLE; }
...
void OnTick()
{
   ...
   // 🆕 v14.19: چک Rule مستقل AUDCAD — همان الگوی EURGBP بالا، مستقل کامل.
   if(g_isNewChartBar && g_audcadParallelActive) Alert_CheckAUDCADRule();
   if(isNewBar)
   {
                   ...
                   const long &tick_volume[])
{
   ...
   for(int shift = start; shift >= 1; shift--)
   {
      ...
      if(localEnableMarketRegime && BlockSignalsInRanging_ForSymbol())
      {
                  ...
                  int &filterContrib[])
{
   ...
   if(localEnableMTF)
   {
      if(RequireH1Confirmation)
      {
         ...
         // 🆕 v14.25 [SIGNAL-CALIBRATION]: وزن ×۳ — طبق بخش ۸ گزارش، تنها
         // فیلتری با edge قوی و پایدار در هر دو جهت (Buy+۵.۵٪, Sell+۴.۵٪).
         score += 9;  // Bonus for H1 confirmation (بود: +3)
         filterContrib[FILT_MTF] = 1;
      }
      ...
      if(RequireH4Confirmation)
      {
         ...
         { score += 6; filterContrib[FILT_MTF] = 1; }  // بود: +2
   ...
   if(UseWickFilter)
   {
      ...
      // 🆕 v14.25 [SIGNAL-CALIBRATION]: امتیاز صفر شد — بخش ۸ گزارش نشون داد
      // این فیلتر لیفت منفی داره (-۶.۷٪ Buy). filterContrib برای تشخیص/
      // تولتیپ نگه داشته شده، فقط دیگه روی score اثر نمی‌ذاره.
      if(bodyRatio >= 0.8)
      { score += 0; filterContrib[FILT_WICK] = 1; }  // بود: +2
      else if(bodyRatio >= localMinBodyToTotalRatio)
      { score += 0; filterContrib[FILT_WICK] = 1; }  // بود: +1
      else
   ...
   if(localEnableLiqSwp)
   {
      ...
      // امتیازدهی LiqSwp
      // 🆕 v14.25 [SIGNAL-CALIBRATION]: وزن ×۱.۵ روی مقادیر v7.0 — بخش ۸
      // گزارش (بعد از تصحیح باگ تست اولیه) نشون داد این فیلتر لیفت +۱۳
      // تا +۱۵٪ داره، از بهترین‌های کل سیستم.
      if(swpAge >= 0)
      {
         // v14.25: +14, +9, +5  (v7.0 بود: +9, +6, +3 | قبل از اون: +3,+2,+1)
         int swpScore = (swpAge <= 2) ? 14 : (swpAge <= 5) ? 9 : 5;
         score += swpScore;
         filterContrib[FILT_LIQSWP] = 1;
         if(ShowScoreInfo)
            Print("💧 LiqSwp: sweep ", swpAge, " bars ago → +", swpScore, " [v14.25:×1.5]");
      }
      else
      {
   ...
   // ─────────────────────────────────────────────────────────────────
   // 🆕 v14.25 [SIGNAL-CALIBRATION]: امتیاز قبل از RTM ذخیره می‌شود تا در
   // صورت فعال بودن inp_RTM_ExcludeFromScore، بعد از این بلوک بازگردانده شود
   // — طبق درخواست کاربر، چون در حالت Momentum هنوز تست نشده، فعلاً خنثی
   // بماند بدون حذف منطق (برای تست بعدی حاضر می‌ماند).
   int _scoreBeforeRTM = score;
   if(localEnableRTM && handleRTM_EMA != INVALID_HANDLE)
   {
      if(shift < ArraySize(g_rtmEMA) && g_rtmEMA[shift] > 0.0 && atrv > 0.0)
      {
         ...
         // 🆕 v14.24 [RTM-MODE]: حالت MOMENTUM (پیش‌فرض جدید) دقیقاً برعکسِ
         // REVERSION (رفتار قدیمی) امتیاز می‌دهد — دورشدن از EMA200 در جهت
         // سیگنال = ادامهٔ روند (امتیاز مثبت درجه‌بندی‌شده، همان آستانه‌های
         // قدیمی ۲/۳/۳.۵/۴ ATR)، و نزدیک‌ماندن/طرف مقابل = ریسک برگشت (امتیاز
         // منفی کوچک). حالت REVERSION عیناً همان منطق قدیمی (بدون کوچک‌ترین
         // تغییر) را اجرا می‌کند.
         if(inp_RTM_Mode == RTM_MOMENTUM)
         {
            if(forBuy)
            {
               if(above)  // 🆕 MOMENTUM: قیمت بالای EMA200 → ادامهٔ روند صعودی برای BUY (مطلوب)
               {
                  if(distATR >= 4.0)       { score += 3; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: BUY ", DoubleToString(distATR,1), "x ATR above mean +3 [Momentum]"); }
                  else if(distATR >= 3.5)  { score += 3; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: BUY 3.5x ATR above +3 [Momentum]"); }
                  else if(distATR >= 3.0)  { score += 2; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: BUY 3x ATR above +2"); }
                  else if(distATR >= 2.0)  { score += 1; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: BUY 2x ATR above +1"); }
                  else                       filterContrib[FILT_RTM] = 0;
               }
               else  // قیمت زیر/نزدیک EMA200 → ریسک برگشت برای یک BUY (نامطلوب)
               {
                  if(distATR >= 2.0) { score -= 1; filterContrib[FILT_RTM] = -1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: BUY below mean -1 (reversion risk)"); }
                  else                 filterContrib[FILT_RTM] = 0;
               }
            }
            else  // SELL
            {
               if(!above)  // 🆕 MOMENTUM: قیمت زیر EMA200 → ادامهٔ روند نزولی برای SELL (مطلوب)
               {
                  if(distATR >= 4.0)       { score += 3; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: SELL ", DoubleToString(distATR,1), "x ATR below mean +3 [Momentum]"); }
                  else if(distATR >= 3.5)  { score += 3; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: SELL 3.5x ATR below +3 [Momentum]"); }
                  else if(distATR >= 3.0)  { score += 2; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: SELL 3x ATR below +2"); }
                  else if(distATR >= 2.0)  { score += 1; filterContrib[FILT_RTM] = 1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: SELL 2x ATR below +1"); }
                  else                       filterContrib[FILT_RTM] = 0;
               }
               else  // قیمت بالای/نزدیک EMA200 → ریسک برگشت برای یک SELL (نامطلوب)
               {
                  if(distATR >= 2.0) { score -= 1; filterContrib[FILT_RTM] = -1; if(ShowScoreInfo) Print("🔄 RTM[MOM]: SELL above mean -1 (reversion risk)"); }
                  else                 filterContrib[FILT_RTM] = 0;
               }
            }
         }
         else   // RTM_REVERSION — رفتار قدیمی، عیناً بدون تغییر
         {
            if(forBuy)
            {
   ...
   // 🆕 v14.25 [SIGNAL-CALIBRATION]: طبق درخواست کاربر، امتیاز RTM از
   // مجموع signal-score کنار گذاشته می‌شود (فقط filterContrib برای
   // تشخیص/تولتیپ باقی می‌ماند) — چون حالت Momentum تازه معرفی شده و
   // هنوز آزمایش مستقل نشده. با inp_RTM_ExcludeFromScore=false دوباره
   // فعال می‌شود، بدون نیاز به دست‌زدن به منطق بالا.
   if(inp_RTM_ExcludeFromScore) score = _scoreBeforeRTM;
   // ─────────────────────────────────────────────────────────────────
...
// 🆕 v14.20 [SUFFIX-FIX]: resolve نام واقعی سمبل بروکر برای بلوک‌های موازی
// EURGBP/AUDCAD — دقیقاً همان الگوی اثبات‌شدهٔ FlowFindSym بالا (BID زنده →
// EXIST+کافی‌بودن دیتا → پسوند چارت جاری → fallback به نام خام)، اما این بار
// برای *خودِ* سمبل اصلیِ هر بلوک موازی (نه یک جفت خواهر). مشکلی که رفع می‌کند:
// قبل از v14.20، هندل‌های اختصاصی EURGBP/AUDCAD و همهٔ فراخوانی‌های
// Generic_ReplayCrisisAtBar/SpikeAtBar/HasRealPositionsOnSymbol برای این دو
// ارز، رشتهٔ خام "EURGBP"/"AUDCAD" را مستقیم می‌فرستادند؛ اگر بروکر برای این
// دو سمبل پسوند لازم داشت (مثلاً "EURGBP.a")، این فراخوانی‌ها بی‌صدا داده
// نامعتبر/صفر برمی‌گرداندند. این تابع فقط یک‌بار در OnInit صدا زده می‌شود و
// نتیجه در g_eurgbpBrokerSymbol/g_audcadBrokerSymbol کش می‌شود.
string ResolveBrokerSymbolForParallel(string cleanSym)
{
   // ۱) بدون پسوند — BID زنده
   if(SymbolInfoDouble(cleanSym, SYMBOL_BID) > 0.0) return cleanSym;

   // ۲) بدون پسوند ولی BID موقتاً صفر (مثلاً در بک‌تست) — EXIST + دیتای H1 کافی
   if(SymbolInfoInteger(cleanSym, SYMBOL_EXIST) && iBars(cleanSym, PERIOD_H1) > 50) return cleanSym;

   // ۳) پسوند چارت جاری را امتحان کن
   if(StringLen(_Symbol) > 6)
   {
      string sfx       = StringSubstr(_Symbol, 6);
      string candidate = cleanSym + sfx;
      if(SymbolInfoDouble(candidate, SYMBOL_BID) > 0.0) return candidate;
      if(SymbolInfoInteger(candidate, SYMBOL_EXIST) && iBars(candidate, PERIOD_H1) > 50) return candidate;
   }

   // ۴) fallback: نام خام برگردان (رفتار قبلی v14.19 — حداقل رفتار قبلی حفظ می‌شود،
   // هرچند در این حالت هندل‌سازی احتمالاً با INVALID_HANDLE مواجه می‌شود و
   // ReplayRetry در OnTimer طبق منطق قبلی چند بار دوباره تلاش می‌کند)
   return cleanSym;
}
...
double FlowEvaluate(SisterEntry &mat[], int sz, bool forBuy)
{
   ...
   for(int i = 0; i < sz; i++)
   {
      ...
      // 🆕 [PATCH FLOW-FREEZE] چک تازگی دیتا (recency) — علت اصلی فریز بلندمدت
      // مشکل: v13.49 فقط EXIST + iBars(H4)>50 را به‌عنوان جایگزین BID زنده
      // پذیرفت. این دو چک فقط «وجود» و «تعداد» کندل‌های H4 را می‌سنجند، نه
      // «تازگی» آن‌ها. اگر تاریخچهٔ H4 محلی یک خواهر در Strategy Tester از یک
      // تاریخ مشخص جلوتر نرود (ولی صدها کندل قدیمی‌تر داشته باشد)، هر دو چک
      // true می‌مانند و c1/c6 هر دو از همان پنجرهٔ ثابت/قدیمی خوانده می‌شوند —
      // نتیجه: Flow_Score برای بقیهٔ بک‌تست روی یک عدد ثابت قفل می‌شود، دقیقاً
      // مثل چیزی که در بک‌تست GBPNZD (فریز از 2026.07.06 08:00 تا انتهای دیتا،
      // Buy=+5.00 و Sell=-6.62 برای هر دو جهت هم‌زمان) دیده شد.
      // راه‌حل: آخرین کندل بستهٔ H4 این خواهر باید حداکثر ۷۲ ساعت قدیمی‌تر از
      // زمان جاری (شبیه‌سازی‌شده در Tester، واقعی در لایو) باشد. آستانهٔ ۷۲
      // ساعت عمداً سخاوتمندانه انتخاب شده تا گپ عادی آخر هفتهٔ فارکس (~۴۸
      // ساعت) هرگز false-positive ندهد، ولی فریز چندروزه/چندهفته‌ای را در
      // حداکثر ۳ روز (نه ۲ هفته) تشخیص بدهد.
      datetime sisterLastH4Time = iTime(sym, PERIOD_H4, 1);
      if(sisterLastH4Time <= 0 || TimeCurrent() - sisterLastH4Time > 72 * 3600)
      {
         if(ShowDebugLogs)
            Print("⚠️ [PATCH FLOW-FREEZE] FlowEvaluate: sister '", mat[i].sym, "' → resolve='", sym,
                  "' آخرین کندل H4=", TimeToString(sisterLastH4Time),
                  " (بیش از ۷۲ ساعت قدیمی) — داده بی‌اعتبار، سهم صفر");
         continue;
      }
...
void Alert_SendTelegramPhoto(const string caption)
{
   ...
   Alert_SendChartPhoto(0, capFull, "hm_chart_" + tf + ".png");  // 🔧 v14.20.2: نام فایل هم دیگر دینامیک است، نه همیشه h1
}

// ─── V13.10: ارسال عکس چارت‌های اضافی (M5, M30, D1) ────────────
// فقط برای StatusQuery — بدون متن وضعیت (فقط عکس)
// 🆕 v14.20 بهبود #6: تبدیل ENUM_TIMEFRAMES به برچسب کوتاه ("PERIOD_M5" → "M5")
string HM_ShortTFName(ENUM_TIMEFRAMES tf)
{
...
{
   // 🔧 v14.20.2 (رفع ریشه‌ای «دوتا H1» — کاربر بعد از تست دوم گزارش داد):
   // علت واقعی این بود که Alert_SendTelegramPhoto (تابع قدیمی v13.10، دقیقاً
   // همین چند خط بالاتر در TG_Outbox_Process صدا زده می‌شود) از قبل عکسِ
   // تایم‌فریمِ *جاریِ* چارت را می‌فرستد (با chartId=0 = چارت جاری). نسخهٔ
   // قبلی این تابع هم یک ورودی «تایم‌فریم جاری» اضافه می‌کرد → همان چارت
   // دوبار (یک‌بار از Alert_SendTelegramPhoto، یک‌بار از اینجا) فرستاده
   // می‌شد. الان این تابع دیگر تایم‌فریم جاری را دوباره اضافه نمی‌کند —
   // فقط M30 → M15 → M5 → M1 را می‌فرستد؛ ترتیب نهایی که کاربر می‌بیند:
   // [جاری از Alert_SendTelegramPhoto] → M30 → M15 → M5 → M1.
   ENUM_TIMEFRAMES curTf = (ENUM_TIMEFRAMES)Period();

   ENUM_TIMEFRAMES tfs[4]   = { PERIOD_M30, PERIOD_M15, PERIOD_M5, PERIOD_M1 };
   string          tfNames[4] = { "M30", "M15", "M5", "M1" };

   for(int i = 0; i < 4; i++)
   {
      if(tfs[i] == curTf) continue;   // اگه تایم‌فریم جاری خودش M30/M15/M5/M1 باشه، دوباره نفرست (همون یکی که Alert_SendTelegramPhoto فرستاد کافیه)
...
{
   // 🔧 v14.23 [MSG-REDESIGN] بازخورد کاربر: هر پیام حالا با یک سربرگ
   // انگلیسی مشخص (🚦Rule / 🆘Crisis / 🌊Flow / ...) شروع می‌شود تا همیشه
   // معلوم باشد پیام از کدام سیستم آمده (قبلاً وقتی چند چراغ با هم در یک
   // خط ذکر می‌شدند، معلوم نبود منبع واقعی کدام است). سمبل+جهت+زمان هم به
   // یک خط ثابت اضافه شد (قبلاً بدون زمان بود).
   string dirFa    = forBuy ? "خرید" : "فروش";
   string timeStrL = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);
   string head     = CleanSymbol(_Symbol) + " (" + dirFa + ") | " + timeStrL + "\n";
   ...
   if(Alert_OnCrisis || Alert_OnCrisisOrange)
   {
      ...
      {
         string rangFa = (g_crisisState == 2) ? "قرمز" : "نارنجی";
         string rangEm = (g_crisisState == 2) ? "🔴" : "🟠";
         // 🆕 v14.23 [MSG-REDESIGN]: به‌جای یک جملهٔ کلی («روند و جریان‌پول
         // برخلاف معامله‌ته») که همیشه یکسان بود صرف‌نظر از این‌که واقعاً
         // کدام چراغ قرمز شده، حالا از g_lightRTM/Trend/Struct/FLOW —
         // همان چراغ‌هایی که همین لحظه برای همین جهت محاسبه شدند — هر
         // چراغِ واقعاً قرمز جداگانه و فقط اگر واقعاً قرمز باشد ذکر می‌شود.
         string crisisNotes = "";
         if(g_lightRTM    == 2) crisisNotes += "نکته: قیمت داره از میانگین برخلافت دور می‌شه (RTM)\n";
         if(g_lightTrend  == 2) crisisNotes += "نکته: روند خلاف جهت توئه (Trend)\n";
         if(g_lightStruct == 2) crisisNotes += "نکته: ساختار روزانه برخلافت شکسته (Structure)\n";
         if(g_lightFLOW   == 2) crisisNotes += "نکته: جریان پول برخلاف توئه (Flow)\n";
         // 🆕 v14.23: مسیرهای Orange-D/fallback کم‌اعتماد می‌توانند Crisis را
         // فقط از روی TrendScore+ADX (مستقل از این ۴ چراغ) روشن کنند — یعنی
         // ممکن است هیچ‌کدام از ۴ چراغ بالا واقعاً قرمز نباشند. برای این‌که
         // پیام هرگز جای خالیِ عجیب نداشته باشد ولی همچنان صادق بماند (چیزی
         // که واقعاً قرمز نیست را «قرمز» نگوید)، در این حالت نادر یک نکتهٔ
         // عمومی و درست جایگزین می‌شود.
         if(StringLen(crisisNotes) == 0)
            crisisNotes = "نکته: ترکیب روند و ADX به‌تنهایی این هشدار رو روشن کرده (Trend)\n";
         string msg = "🆘 Crisis :\n"
            + head
            + "مجموعه چراغ‌ها " + rangEm + " " + rangFa + " — بازار داره برخلافت می‌ره\n"
            + "━━━━━━━━━━━━━━━━\n"
            + crisisNotes
            + "نکته: اگه Rule هنوز نگفته ببند، خودت دستی چک کن";
         Alert_Send(msg);
      }
   }
   ...
   if(Alert_OnHighAlert)
   {
      ...
      {
         string msg = "⚠️ High Alert :\n"
            + head
            + "هشدار شدید روشن شد 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: یعنی داریم به Crisis نزدیک می‌شیم\n"
            + "نکته: اگه Crisis هم قرمز شد بیشتر به معامله سر بزن";
         Alert_Send(msg);
      }
   ...
   if(Alert_OnZombie)
   {
      ...
      if(g_lightZOMBIE == 2 && Alert_CooldownOK(g_lastAlertTime_ZOMBIE, isNewEdge))
      {
         Alert_Send("🧟 Zone :\n"
            + head
            + "قیمت دو طبقه از ورودت دور شده 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: اگه Crisis هم قرمزه خیلی خطرناکه");
      }
   ...
   if(Alert_OnFlow)
   {
      ...
      if(g_lightFLOW == 2 && Alert_CooldownOK(g_lastAlertTime_FLOW, isNewEdge))
      {
         Alert_Send("🌊 Flow :\n"
            + head
            + "ارزهای هم‌گروهت هم دارن برخلاف معامله‌ت حرکت می‌کنن 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: اگه Crisis هم قرمزه، منتظر برگشت نباش");
      }
   ...
   if(Alert_OnADX)
   {
      ...
      if(g_lightTrend == 2 && Alert_CooldownOK(g_lastAlertTime_Trend, isNewEdge))
      {
         Alert_Send("📈 Trend :\n"
            + head
            + "روند قوی برخلاف معامله‌ت شکل گرفته 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: به‌تنهایی کافی نیست، ولی Crisis رو هم چک کن");
      }
   ...
   if(Alert_OnRTM)
   {
      ...
      if(g_lightRTM == 2 && Alert_CooldownOK(g_lastAlertTime_RTM, isNewEdge))
      {
         Alert_Send("📉 RTM :\n"
            + head
            + "قیمت داره از EMA200 برخلاف معامله‌ت فاصله می‌گیره 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: تنها ملاک نیست ولی با Crisis ترکیبش خطرناکه");
      }
   ...
   if(Alert_OnStruct)
   {
      ...
      {
         Alert_Send("🏗️ Structure :\n"
            + head
            + "سقف یا کف روزانه شکسته شده 🔴\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: برگشت سخت‌تر می‌شه، Crisis رو جدی بگیر");
      }
      g_prevAlertStruct = g_lightStruct;
   }

   // ─── 3 LIGHTS: 3 چراغ از 4 همزمان قرمز ───────────────────────
   // چراغ‌های مشمول: Trend, RTM, Structure, Flow (همان 4 چراغ RC)
   if(Alert_OnThreeLights)
   {
      ...
      if(now3Red && Alert_CooldownOK(g_lastAlertTime_3Lights, isNewEdge))
      {
         ...
         if(g_lightTrend  == 2) which += "Trend ";
         if(g_lightStruct == 2) which += "Structure ";
         if(g_lightFLOW   == 2) which += "Flow ";
         Alert_Send("🔴 3 Lights :\n"
            + head
            + StringFormat("همزمان %d از ۴ چراغ قرمز شدن\n", redCount4)
            + "قرمز: " + which + "\n"
            + "━━━━━━━━━━━━━━━━\n"
            + "نکته: منتظر نمون — همین الان Crisis رو چک کن");
      }
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.24 [HIGH-ALERT-CALIBRATION]: تابع مشترک برای چک شرط High Alert —
// طبق درخواست کاربر فقط Buy آستانهٔ اختصاصی گرفت (از Inputهای جدید
// inp_HighAlert_ADX_Buy/Flow_Buy)؛ Sell دست‌نخورده روی مقدار قبلی (ADX≥35
// AND Flow<-7.0، هاردکد) ماند. این تابع در همهٔ نقاطی که «سمبل خودی»
// (چارت جاری) شرط High Alert را چک می‌کنند (TL_Update و
// BuildDirectionSnapshot) به‌کار می‌رود — تا این کالیبراسیون هم روی
// دیتای زندهٔ نمایش/تلگرام و هم روی تصمیم واقعیِ Rule (که از
// BuildDirectionSnapshot تغذیه می‌شود) به‌طور یکدست اعمال شود، نه فقط در
// محل نمایش. پردازش موازی EURGBP/AUDCAD وقتی سمبل «خارجی» هستند عمداً
// این تابع را صدا نمی‌زند (طبق محدودهٔ درخواست کاربر) — جزئیات در MD.
bool HighAlert_Check(bool forBuy, double adxVal, double flowVal)
{
   if(forBuy) return (adxVal >= inp_HighAlert_ADX_Buy) && (flowVal < inp_HighAlert_Flow_Buy);
   return (adxVal >= 35.0) && (flowVal < -7.0);   // Sell: بدون تغییر، طبق درخواست صریح کاربر
}
   ...
   string symOverride = "")   // 🆕 v14.00: برای FOREIGN on-demand — سمبلی غیر از _Symbol
{
   ...
   else if(StringFind(checkSym, "AUDCAD") >= 0)
   {
      ...
      // 🆕 [AUDCAD-CRISIS-INPUT] v14.19: از هاردکد به Input — طبق اجماع ۷/۷
      // (min ADX واقعی در Red = 38.1) این آستانه‌ها همگی صحیح بودند؛ فقط به
      // Input تبدیل شدند تا بدون کامپایل مجدد قابل‌تنظیم باشند (مثل GBPNZD
      // در v14.11). هیچ عددی تغییر نکرد.
      th_RedA_ADX = inp_AUDCAD_RedA_ADX;  th_RedA_Flow = inp_AUDCAD_RedA_Flow;  // 38.0 / 6.0
      th_RedB_ADX = inp_AUDCAD_RedB_ADX;  th_RedB_Flow = inp_AUDCAD_RedB_Flow;  // 40.0 / 8.5 — بدون تغییر
      th_OrA_ADX  = inp_AUDCAD_OrA_ADX;   th_OrA_Flow  = inp_AUDCAD_OrA_Flow;   // 32.0 / 5.0
      th_OrB_ADX  = inp_AUDCAD_OrB_ADX;   th_OrB_Ts    = inp_AUDCAD_OrB_Ts;     // 28.0 / 35.0
      th_OrC_ADX  = inp_AUDCAD_OrC_ADX;   th_OrC_Flow  = inp_AUDCAD_OrC_Flow;   // 32.0 / 7.5 — بدون تغییر
      th_OrC_Ts   = inp_AUDCAD_OrC_Ts;                                            // 25.0
      th_OrD_ADX  = inp_AUDCAD_OrD_ADX;   th_OrD_Ts    = inp_AUDCAD_OrD_Ts;     // 34.0 / 35.0
      th_Y2_ADX   = inp_AUDCAD_Y2_ADX;    th_Y2_Flow   = inp_AUDCAD_Y2_Flow;    // 22.0 / 3.0
      th_Y3_Flow  = inp_AUDCAD_Y3_Flow;   th_Y3_Ts     = inp_AUDCAD_Y3_Ts;      // 7.0 / 30.0 — بدون تغییر
      th_Y4_ADX   = inp_AUDCAD_Y4_ADX;    th_Y4_Ts     = inp_AUDCAD_Y4_Ts;      // 25.0 / 45.0 — بدون تغییر
      th_Y5_ADX   = inp_AUDCAD_Y5_ADX;    th_Y5_Flow   = inp_AUDCAD_Y5_Flow;    // 35.0 / 7.5 — بدون تغییر
   }
...
//
// 🔧 v14.20.2: مرزهای زیر دیگر هاردکد نیستند — از MktPhase_GetThresholds
// (per-symbol input، پایین‌تر در همین فایل) خوانده می‌شوند. مقادیر پیش‌فرض:
//   GBPNZD: Range<0.85 / Trend>1.15   EURGBP: Range<0.72 / Trend>1.15   AUDCAD: Range<0.75 / Trend>0.95
// (علاوه بر این، اگر inp_MktPhase_UseEfficiencyFilter فعال باشد، Trend فقط
// با تأیید Efficiency Ratio ثبت می‌شود — نگاه کنید MktPhase_Classify پایین‌تر.)
// مرزهای زیر صرفاً برای مرجع تاریخی GBPNZD نگه داشته شده‌اند:
...
// 🆕 نکتهٔ مهم برای تصمیم‌گیری v14.20.2: کالیبراسیون جدید (پایین‌تر در همین
// فایل) بر پایهٔ کندل M1 واقعی ژانویه–ژوئیهٔ ۲۰۲۶ (۶.۵ ماه، دورهٔ نسبتاً آرام)
// انجام شد و برای GBPNZD مرز Trend را از 1.30 به 1.15 کاهش داد (چون در همین
// بازه، حداکثر نسبت مشاهده‌شده فقط 1.32 بود و 1.30 عملاً هرگز رد نمی‌شد).
// اما آستانهٔ قدیمی 1.30 روی رویداد واقعی آوریل ۲۰۲۵ (MA10~1.73) هم درست کار
// کرده بود. اگر ترجیح می‌دهید محافظه‌کارتر بمانید، inp_MktPhase_GBPNZD_TrendMin
// را به 1.30 برگردانید — چون حالا input است، بدون کامپایل مجدد قابل تغییره.
// ════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.20.2 [MKTPHASE-CALIBRATION]: انتخاب آستانه‌های Range/Trend برحسب
// ارز — دقیقاً همان الگوی Crisis_GetThresholds بالا (branch بر CleanSymbol).
// ════════════════════════════════════════════════════════════════════
void MktPhase_GetThresholds(string sym, double &rangeMax, double &trendMin)
{
   string c = CleanSymbol(sym);
   if(c == "EURGBP")
   {
      rangeMax = inp_MktPhase_EURGBP_RangeMax;
      trendMin = inp_MktPhase_EURGBP_TrendMin;
   }
   else if(c == "AUDCAD")
   {
      rangeMax = inp_MktPhase_AUDCAD_RangeMax;
      trendMin = inp_MktPhase_AUDCAD_TrendMin;
   }
   else // GBPNZD و هر سمبل دیگر (پیش‌فرض)
   {
      rangeMax = inp_MktPhase_GBPNZD_RangeMax;
      trendMin = inp_MktPhase_GBPNZD_TrendMin;
   }
}

// 🆕 v14.20.2 [MKTPHASE-ER]: نسخهٔ روزانه (D1)، پارامتری‌شدهٔ Efficiency Ratio
// برای یک سمبل دلخواه. توجه: این با CalculateEfficiencyRatio موجود (خط
// ~14894) فرق دارد — آن تابع همیشه روی _Symbol+PERIOD_CURRENT کار می‌کند
// (برای فیلتر ورود Xmoon/Regime، granularity ریز)؛ این یکی مخصوص تحلیل
// روزانهٔ MktPhase است و می‌تواند برای هر سمبلی (از‌جمله ارز خارجیِ
// On-Demand) صدا زده شود. خروجی: 0=کاملاً شلاقی/بی‌جهت … 1=کاملاً یک‌طرفه.
// اگر داده کافی نباشد -1 برمی‌گرداند.
double HM_DailyEfficiencyRatio(string sym, int lookbackDays)
{
   double closes[];
   ArraySetAsSeries(closes, true);
   int need = lookbackDays + 1;
   int got  = CopyClose(sym, PERIOD_D1, 1, need, closes);
   if(got < need) return -1.0;

   double netChange   = MathAbs(closes[0] - closes[need-1]);
   double totalChange = 0.0;
   for(int i = 0; i < need - 1; i++)
      totalChange += MathAbs(closes[i] - closes[i+1]);

   if(totalChange <= 0.0) return 0.0;
   return netChange / totalChange;
}

// 🆕 v14.20.2: طبقه‌بندی مشترک فاز — هم مسیر بازیابی از کش (GV) و هم مسیر
// محاسبهٔ تازه در Calc_MktPhase، و هم HM_QuickMktPhaseForSymbol (On-Demand)
// از همین یک تابع استفاده می‌کنند تا منطق هیچ‌جا واگرا نشود.
// erAvailable=false یعنی ER محاسبه نشده/معتبر نیست (مثلاً inp_MktPhase_UseEfficiencyFilter
// خاموش است یا دادهٔ D1 کافی برای ER نبود) — در این حالت مثل قبل فقط از ratio استفاده می‌شود.
int MktPhase_Classify(double ratio, double er, bool erAvailable,
                       double rangeMax, double trendMin, bool useER, double erMin,
                       string &labelOut, bool &erDowngradedOut)
{
   erDowngradedOut = false;
   if(ratio < rangeMax) { labelOut = "Range"; return 0; }
   if(ratio > trendMin)
   {
      if(!useER || !erAvailable || er >= erMin) { labelOut = "Trend!"; return 2; }
      erDowngradedOut = true;   // نسبت ADR بالا بود ولی حرکت شلاقی/بی‌جهت بود (ER کم) — به Neutral افت می‌کند
      labelOut = "Neutral";
      return 1;
   }
   labelOut = "Neutral";
...
void Calc_MktPhase()
{
   ...
   if(firstRun)
   {
      ...
      string gvErKey  = "HelpMe_MktPhaseER_"  + _Symbol;   // 🆕 v14.20.2
      ...
      double cachedEr  = GlobalVariableCheck(gvErKey)  ? GlobalVariableGet(gvErKey)  : -1.0;

      if(cachedDay > 0 && cachedVal > 0 && (datetime)cachedDay == todayD1)
      {
         g_mktPhaseRatio = cachedVal;

         // 🆕 v14.20.2 [MKTPHASE-CALIBRATION+ER]: آستانه‌های اختصاصی ارز + تأیید ER کش‌شده
         double rangeMax, trendMin;
         MktPhase_GetThresholds(_Symbol, rangeMax, trendMin);
         string lbl; bool erDowngraded;
         bool erAvail = (cachedEr >= 0.0);
         int ph = MktPhase_Classify(cachedVal, cachedEr, erAvail, rangeMax, trendMin,
                                     inp_MktPhase_UseEfficiencyFilter, inp_MktPhase_MinEfficiencyForTrend,
                                     lbl, erDowngraded);
         g_lightMktPhase = ph;
         g_mktPhasePrev  = ph;
         ...
         string distLine2;
         if(ph == 0)
            distLine2 = StringFormat("تا Neutral: %.3f مانده (مرز: %.2f)", rangeMax - cachedVal, rangeMax);
         else if(ph == 1)
            distLine2 = erDowngraded
               ? StringFormat("نسبت از مرز Trend (%.2f) گذشت ولی ER=%.2f<%.2f بود (شلاقی/بی‌جهت) — Neutral ماند", trendMin, cachedEr, inp_MktPhase_MinEfficiencyForTrend)
               : StringFormat("تا Range: %.3f | تا Trend: %.3f", cachedVal - rangeMax, trendMin - cachedVal);
         else
            distLine2 = StringFormat("%.3f بالاتر از مرز Trend (%.2f) | ER=%.2f ✅ تأیید شد", cachedVal - trendMin, trendMin, cachedEr);

         string erLine2 = inp_MktPhase_UseEfficiencyFilter
            ? StringFormat("Efficiency Ratio: %s\n", erAvail ? DoubleToString(cachedEr,2) : "--")
            : "";
         string tip2 = StringFormat(
            ...
            "%s%s\n"
            "─────────────────────\n"
            "< %.2f  🟢 Range  : رنج — سیستم عالی\n"
            "%.2f-%.2f 🟡 Neutral: گذار — احتیاط\n"
            "> %.2f  🔴 Trend  : روند — خطر%s\n"
            "─────────────────────\n"
            "آخرین تغییر: %s",
            _Symbol, cachedVal, erLine2, distLine2, rangeMax, rangeMax, trendMin, trendMin,
            inp_MktPhase_UseEfficiencyFilter ? StringFormat(" (+ER≥%.2f)", inp_MktPhase_MinEfficiencyForTrend) : "",
            lcStr2);
   ...
   // 🆕 v14.20.2 [MKTPHASE-ER]: محاسبهٔ Efficiency Ratio (اختیاری، طبق inp_MktPhase_UseEfficiencyFilter)
   double er = -1.0;
   bool   erAvail = false;
   if(inp_MktPhase_UseEfficiencyFilter)
   {
      er = HM_DailyEfficiencyRatio(_Symbol, MA_WIN);
      erAvail = (er >= 0.0);
   }
   ...
   GlobalVariableSet("HelpMe_MktPhaseER_"  + _Symbol, erAvail ? er : -1.0);   // 🆕 v14.20.2

   // تعیین فاز — 🆕 v14.20.2: آستانه‌های اختصاصی ارز + تأیید ER (به‌جای 0.85/1.30 هاردکد)
   double rangeMax, trendMin;
   MktPhase_GetThresholds(_Symbol, rangeMax, trendMin);
   int    newPhase;
   string phaseLabel;
   bool   erDowngraded;
   newPhase = MktPhase_Classify(ma10, er, erAvail, rangeMax, trendMin,
                                 inp_MktPhase_UseEfficiencyFilter, inp_MktPhase_MinEfficiencyForTrend,
                                 phaseLabel, erDowngraded);
   ...
   if(Alert_OnMktPhase && phaseChanged && newPhase >= 1)
   {
      ...
      {
         string aPrev  = (g_mktPhasePrev == 0) ? "رنج" :
                         (g_mktPhasePrev == 1) ? "خنثی" : "روند";
         // 🔧 v14.23 [MSG-REDESIGN]: سربرگ انگلیسی برای یکدستی با بقیهٔ پیام‌ها
         // اضافه شد (این چراغ در فهرست اصلی کاربر نبود، ولی برای هم‌ترازی با
         // ۱۰ چراغ دیگر همون الگو اعمال شد — قابل بازگشت اگه لازم نبود).
         string mpHdr = "🌍 MktPhase :\n" + CleanSymbol(_Symbol) + " | " + TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES) + "\n";
         string aMsg;
         if(newPhase == 2)
            aMsg = mpHdr
                 + "فاز بازار روند شد 🔴\n"
                 + "━━━━━━━━━━━━━━━━\n"
                 + "نکته: فاز قبلی " + aPrev + " بود\n"
                 + "نکته: بازار از حالت رنج خارج شده، Xmoon تو ریسک بیشتریه — Rule رو چک کن";
         else
            aMsg = mpHdr
                 + "فاز بازار خنثی شد 🟡\n"
                 + "━━━━━━━━━━━━━━━━\n"
                 + "نکته: فاز قبلی " + aPrev + " بود\n"
                 + "نکته: بازار داره از رنج خارج میشه، مراقب باش";
         Alert_Send(aMsg);
   ...
   // 🆕 v14.20.2: مرزها دیگر هاردکد نیستند، از آستانهٔ اختصاصی ارز خوانده می‌شوند
   MqlDateTime lc;
   ...
   string distLine;
   if(newPhase == 0)
      distLine = StringFormat("تا Neutral: %.3f مانده (مرز: %.2f)", rangeMax - ma10, rangeMax);
   else if(newPhase == 1)
      distLine = erDowngraded
         ? StringFormat("نسبت از مرز Trend (%.2f) گذشت ولی ER=%.2f<%.2f بود (شلاقی/بی‌جهت) — Neutral ماند", trendMin, er, inp_MktPhase_MinEfficiencyForTrend)
         : StringFormat("تا Range: %.3f | تا Trend: %.3f", ma10 - rangeMax, trendMin - ma10);
   else
      distLine = StringFormat("%.3f بالاتر از مرز Trend (%.2f) | ER=%.2f ✅ تأیید شد", ma10 - trendMin, trendMin, er);

   string erLine = inp_MktPhase_UseEfficiencyFilter
      ? StringFormat("Efficiency Ratio: %s\n", erAvail ? DoubleToString(er,2) : "--")
      : "";
   string tip = StringFormat(
      ...
      "%s%s\n"
      "━━━━━━━━━━\n"
      "< %.2f  🟢 رنج   : سیستم Xmoon عالیه\n"
      "%.2f-%.2f 🟡 خنثی  : احتیاط\n"
      "> %.2f  🔴 ترند  : خطر — Xmoon در ریسکه%s\n"
      "━━━━━━━━━━\n"
      "آخرین تغییر: %s",
      _Symbol, ma10, erLine, distLine, rangeMax, rangeMax, trendMin, trendMin,
      inp_MktPhase_UseEfficiencyFilter ? StringFormat(" (+ER≥%.2f)", inp_MktPhase_MinEfficiencyForTrend) : "",
      lcStr);
...
// 🆕 v14.20 بهبود #7 کاربر: تخمین سریع و بدون-side-effect فاز بازار (MktPhase)
// برای یک سمبل دلخواه (کاربرد اصلی: ارز خارجیِ On-Demand که چارت رویش نیست).
// عمداً از همان فرمول Calc_MktPhase بالا (BASELINE=60, MA_WIN=10, D1) استفاده
// می‌کند، اما بدون کش GlobalVariable، بدون آلارم تلگرام و بدون دست‌زدن به
// g_lightMktPhase/g_mktPhasePrev — چون آن globalها مال چارت خودی (_Symbol)
// هستند و اینجا داریم دربارهٔ یک سمبل *دیگر* حرف می‌زنیم؛ استفاده از آن globalها
// برای سمبل خارجی همان دستهٔ باگی می‌بود که در Fix #2 (بلوک موازی) رفع شد.
// 🔧 v14.20.2: حالا از MktPhase_GetThresholds (اختصاصی هر ارز) و در صورت فعال
// بودن inp_MktPhase_UseEfficiencyFilter، از HM_DailyEfficiencyRatio هم برای
// تأیید Trend استفاده می‌کند — دقیقاً همان منطق Calc_MktPhase، برای سمبل خارجی.
// phaseOut: 0=Range، 1=Neutral، 2=Trend؛ -1 یعنی دادهٔ D1 کافی نبود (>= 72 روز لازم).
void HM_QuickMktPhaseForSymbol(string sym, int &phaseOut, double &ratioOut)
{
   phaseOut = -1;
   ratioOut = 0.0;
   ...
   int chH = CopyHigh(sym, PERIOD_D1, 1, NEED, arrH);
   int chL = CopyLow (sym, PERIOD_D1, 1, NEED, arrL);
   if(chH < NEED || chL < NEED) return;   // دادهٔ کافی نیست — phaseOut همون -1 می‌ماند
   ...
   double ma10 = 0.0;
   for(int i = 0; i < MA_WIN; i++)
      ma10 += (arrH[i] - arrL[i]) / baseline;
   ma10 /= MA_WIN;

   ratioOut = ma10;

   double rangeMax, trendMin;
   MktPhase_GetThresholds(sym, rangeMax, trendMin);

   double er = -1.0; bool erAvail = false;
   if(inp_MktPhase_UseEfficiencyFilter)
   {
      er = HM_DailyEfficiencyRatio(sym, MA_WIN);
      erAvail = (er >= 0.0);
   }
   string lblTmp; bool erDownTmp;
   phaseOut = MktPhase_Classify(ma10, er, erAvail, rangeMax, trendMin,
                                 inp_MktPhase_UseEfficiencyFilter, inp_MktPhase_MinEfficiencyForTrend,
                                 lblTmp, erDownTmp);
}

// خط آماده برای چسباندن داخل پیام On-Demand — 🔧 v14.20.3 [MSG-REDESIGN]:
// قبلاً عدد خام ratio نشون داده می‌شد ("MktPhase: Range (ratio=0.812)") که
// برای کاربر عادی معنایی نداشت. حالا مثل بقیه چراغ‌ها (Crisis/Spike/Rule)
// فقط دایرهٔ رنگی + یک کلمهٔ ساده.
string HM_MktPhaseLineForMsg(string sym)
{
   int phase; double ratio;
   HM_QuickMktPhaseForSymbol(sym, phase, ratio);
   if(phase < 0) return "MktPhase: ⚪ N/A (low data)";
   string txt = (phase==0) ? "🟢 Range" : (phase==1) ? "🟡 Neutral" : "🔴 Trend";
   return "MktPhase: " + txt;
}
...
void Spike_GetThresholds(string sym, double &th_warn, double &th_spike)
{
   ...
   {
      // 🔧 v14.20.2: این کامنت قبلاً می‌گفت «placeholder، کالیبراسیون بعداً» —
      // اما مقدار پیش‌فرض input (خط ۱۸۱۴-۱۸۱۵) از v14.19 به بعد واقعاً
      // 1.45/1.85 است (کالیبره‌شده، نه placeholder). آن کامنت را اصلاح کردیم
      // تا خواننده/تحلیل بعدی با کامنت قدیمی گمراه نشود.
      th_warn  = inp_AUDCAD_SpikeWarn;   // v14.19: از Placeholder 1.00 → 1.45 (P75 توزیع واقعی)
      th_spike = inp_AUDCAD_SpikeSig;    // v14.19: از Placeholder 1.50 → 1.85 (P95 توزیع واقعی)
   }
   else
   {
...
void Calc_SpikeDetector()
{
   ...
   if(Alert_OnSpike && dispPhase == 2)
   {
      ...
      if(Alert_CooldownOK(g_lastAlertTime_SPIKE, isEdge))
      {
         ...
         // 🔧 v14.20.3 [MSG-REDESIGN] بازخورد کاربر: قبلاً Score + ۵ عدد
         // تایم‌فریم جدا نشون داده می‌شد که برای کاربر عادی بی‌معنا بود، و
         // "حرکت انفجاری + Flow قرمز" هم توضیح نمی‌داد یعنی چی. حالا مفهوم
         // به زبان ساده گفته می‌شه: قیمت ناگهانی حرکت کرده و ارزهای
         // هم‌گروه هم تأییدش می‌کنن (پس صرفاً نویز نیست).
         // 🔧 v14.23 [MSG-REDESIGN]: سربرگ انگلیسی + زمان اضافه شد
         string aMsg =
            "⚡ Spike :\n"
            + CleanSymbol(_Symbol) + " | " + TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES) + "\n"
            "قیمت خیلی سریع‌تر از حد معمول حرکت کرده، و ارزهای هم‌گروه هم دارن برخلاف معامله‌ت هم‌جهت حرکت می‌کنن — یعنی این فقط یه نوسان تصادفی نیست 🔴\n"
            "━━━━━━━━━━━━━━━━\n"
            "نکته: پوزیشن باز رو زیر نظر بگیر\n"
            "نکته: هشدار توقف/بستن Rule فقط وقتی Crisis حداقل زرد بشه فعال میشه";
         Alert_Send(aMsg);
   ...
   string cooldownNote = "";
   int dispCdBars = GetCooldownBarsForSymbol(_Symbol);  // 🆕 v14.20 [COOLDOWN-FIX]
   if(dispCdBars > 0)
   {
      bool buyCooling  = (g_gbpnzdSpikeStreakBuy  > 0 && g_gbpnzdSpikeStreakBuy  < dispCdBars);
      bool sellCooling = (g_gbpnzdSpikeStreakSell > 0 && g_gbpnzdSpikeStreakSell < dispCdBars);
      if(buyCooling || sellCooling)
      {
         string cdBuyTxt  = buyCooling  ? StringFormat("Buy %d/%d",  g_gbpnzdSpikeStreakBuy,  dispCdBars) : "";
         string cdSellTxt = sellCooling ? StringFormat("Sell %d/%d", g_gbpnzdSpikeStreakSell, dispCdBars) : "";
   ...
   if(Alert_OnSpikeFlowRed)
   {
      ...
      {
         // 🔧 v14.23 [MSG-REDESIGN]: سربرگ + زمان اضافه شد؛ توضیح می‌ده هم
         // Spike هم Flow با هم قرمزن (نه فقط یکی) — طبق درخواست شفافیت منبع.
         string sfMsg = StringFormat(
            "⚡ Spike :\n"
            "%s | %s\n"
            "حرکت سریع + سونامی پول — هر دو هم‌زمان قرمز 🔴\n"
            "━━━━━━━━━━━━━━━━\n"
            "نکته: Score حرکت %.2f (Spike)\n"
            "نکته: جریان پول %.1f برخلافت (Flow)\n"
            "نکته: معاملهٔ خلاف جهت رو ببند",
            CleanSymbol(_Symbol), TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES),
            score, g_lastFlowScore);
         Alert_Send(sfMsg);
      }
...
void BuildDirectionSnapshot(string symbol, bool forBuy, ENUM_TIMEFRAMES tf, int shift, HMDirSnapshot &out)
{
   ...
   if(isOwnChart)
   {
      ...
      // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: از تابع مشترک HighAlert_Check —
      // این خط هم گزارش وضعیت (sqSnapBuy/sqSnapSell) و هم تصمیم واقعیِ
      // Rule_Transition (که از همین Snapshot تغذیه می‌شود) را تغذیه می‌کند؛
      // پس اینجا اصلاح‌شدن یعنی کالیبراسیون جدید واقعاً روی تصمیم اثر می‌گذارد،
      // نه فقط روی متن پیام.
      out.highAlert = HighAlert_Check(forBuy, adxVal, flowScore);
...
// 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: On-Demand هم مثل «وضعیت» چگالی بالای
// خط‌های کوتاه دارد — همون مشکل RTL/LTR. کاملاً انگلیسی نگه داشته شد.
...
string OD_RuleLevelText(int level)
{
   ...
   if(level == 1) return "⛔ Don't open new";
   if(level == 2) return "🔴 Better to close";
   return "❔ Unknown";
}
...
void OnDemand_RunAndSend()
{
   ...
   // 🔧 v14.20.4 [STATUS-ENGLISH-ONLY]: کاربر صریحاً خواست این پیام (مثل
   // «وضعیت») کاملاً یک‌زبانه (انگلیسی) بماند — چگالی بالای خط‌های کوتاه
   // باعث بهم‌ریختگی راست‌به‌چپ/چپ‌به‌راست می‌شود. سمبل تمیز (بدون پسوند) و
   // بقیهٔ سادگی‌های v14.20.3 (بدون rc=/ratio خام) حفظ شدند، فقط زبان عوض شد.
   string odMktPhaseLine = HM_MktPhaseLineForMsg(broker);

   string odWarnLine = "Note: EA isn't running directly on this pair — this is a snapshot based on the last 72h, may lag a few minutes behind reality.\n";
   string odLowConfLine = odLowConfidence ? "Note: data for this pair wasn't fully synced — result is approximate.\n" : "";
   string odMisalignLine = odTimeMisaligned ? "Note: this pair's latest candle isn't aligned with the main pair — data may be a few minutes old.\n" : "";

   string hdr = "📟 Status " + clean + "\n"
              + odWarnLine
              + odLowConfLine
              + odMisalignLine
              + odMktPhaseLine + "\n"
   ...
   string buyCrisisLine = (levelBuy > 0 || crisisRedHBuy > 0)
      ? StringFormat("Crisis streak: %d hours", crisisRedHBuy)
      : StringFormat("Calm streak: %d hours", cleanHBuy);
   string msgBuy = hdr
      + "If you had a Buy position:\n"
      + "Crisis: " + OD_LightText(lastCrBuy) + "   Spike: " + OD_LightText(lastSpBuy) + "\n"
      + buyCrisisLine + "\n"
      + "Rule: " + OD_RuleLevelText(levelBuy) + "\n"
      + "─────────────────────\n";
   if(buyCnt > 0)
      msgBuy += "Open Buy trades: " + IntegerToString(buyCnt) + " | Profit: " + DoubleToString(buyProfit, 2)
              + " | Since: " + TimeToString(buyOldest, TIME_DATE|TIME_MINUTES);
   else
      msgBuy += "No open Buy trades.";

   string sellCrisisLine = (levelSell > 0 || crisisRedHSell > 0)
      ? StringFormat("Crisis streak: %d hours", crisisRedHSell)
      : StringFormat("Calm streak: %d hours", cleanHSell);
   string msgSell = hdr
      + "If you had a Sell position:\n"
      + "Crisis: " + OD_LightText(lastCrSell) + "   Spike: " + OD_LightText(lastSpSell) + "\n"
      + sellCrisisLine + "\n"
      + "Rule: " + OD_RuleLevelText(levelSell) + "\n"
      + "─────────────────────\n";
   if(sellCnt > 0)
      msgSell += "Open Sell trades: " + IntegerToString(sellCnt) + " | Profit: " + DoubleToString(sellProfit, 2)
               + " | Since: " + TimeToString(sellOldest, TIME_DATE|TIME_MINUTES);
   else
      msgSell += "No open Sell trades.";

   // 🆕 v14.20 [ONDEMAND-RELIABILITY-FIX]: قبلاً Alert_SendTelegram خام بود که
   // هیچ retry/outbox نداشت — اگر همین یک WebRequest شکست می‌خورد، کل گزارش
   // On-Demand (که ساختنش چند ثانیه طول کشیده) بی‌صدا گم می‌شد. Alert_Send اول
   // فوری تلاش می‌کند و فقط روی شکست، به TG_Outbox با retry+backoff می‌رود.
   Alert_Send(msgBuy);
   Alert_Send(msgSell);
...
bool GBPNZD_GV_Load()
{
   ...
   // 🐛 v14.20.4 [BUG-WEEKEND-STALE-GV]: قبلاً هاردکد 72 ساعت بود — یک
   // آخر هفتهٔ معمولی + کمی تأخیر اتصال مجدد به‌راحتی از آن رد می‌شد.
   // حالا از inp_GVStaleHours (پیش‌فرض 120 ساعت) استفاده می‌شود.
   if(TimeCurrent() - savedAt > inp_GVStaleHours * 3600) return false;
   ...
   // 🐛 v14.20.4 [BUG-WEEKEND-STALE-GV] — گزارش دقیق کاربر بعد از ری‌استارت
   // آخر هفته: قبلاً این حلقه با گام تقویمی (wall-clock: h += gapStep تا
   // h<=nowBar) از LastH تا الان پیش می‌رفت — یعنی هر واحد زمانیِ تقویمی
   // بین آخرین ذخیره و الان را، حتی اگه در آن بازه بازار کاملاً بسته بوده
   // (کل آخر هفته)، به‌عنوان یک کندلِ واقعی با همون وضعیت لحظهٔ ذخیره
   // (تمیز/کثیف) حساب می‌کرد. روی چارت M1 (تنظیم معمول این اکسپرت طبق
   // مستندات)، یک آخر هفتهٔ معمولی (~۴۸-۵۸ ساعت) معادل چند هزار «دقیقهٔ
   // تقویمی» است — یعنی CleanH تا سقف maxFill (معادل ۷۲ ساعت) پر می‌شد، حتی
   // با اینکه هیچ کندل واقعی‌ای در آن بازه شکل نگرفته بود. نتیجهٔ دقیقی که
   // کاربر دید: بعد از یک ری‌استارت در آخر هفته، Buy از STOP با Clean=5h به
   // RESUME با Clean=72h جهش کرد — کاملاً غلط، چون در واقع فقط چند ساعت
   // واقعی بازار باز بوده.
   //
   // رفع: این حلقه حالا بر مبنای شیفتِ کندلِ *واقعی* (iBarShift) کار
   // می‌کند — فقط به تعداد کندل‌هایی که واقعاً بین LastH و الان بسته
   // شده‌اند تکرار می‌شود، نه به تعداد واحدهای زمانی تقویمی. در طول یک
   // بستگیِ کامل بازار (آخر هفته/تعطیلی)، هیچ کندل واقعی‌ای بین LastH و
   // الان وجود ندارد، پس این حلقه اصلاً اجرا نمی‌شود (barsToFill=0) —
   // دقیقاً همان رفتاری که از یک Replay مبتنی‌بر-کندل انتظار می‌رود، و
   // دقیقاً همان چیزی که ApplyClosedBarToRule/GBPNZD_InitReplay (که با
   // shift کار می‌کنند نه wall-clock) از قبل به‌درستی انجام می‌دادند —
   // این گپ فقط در همین یک تابع (GV_Load) وجود داشت.
   {
      int gapStep = (g_chartPeriodSeconds > 0) ? g_chartPeriodSeconds : 3600;
      ENUM_TIMEFRAMES gapTf = (ENUM_TIMEFRAMES)Period();
      // آخرین کندل بسته در TF چارت — همان مرزی که Alert_CheckGBPNZDRule چک می‌کند
      datetime nowBar = iTime(_Symbol, gapTf, 1);
      if(nowBar == 0) nowBar = (datetime)(((long)TimeCurrent()) / gapStep * gapStep);

      // حداکثر ۷۲ ساعت معادل (نه ۷۲ کندل ثابت) — همون سقف ایمنی قبلی،
      // برای مواردی که LastH خیلی قدیمی‌تر از تاریخچهٔ موجود باشد
      int maxFillCap = (int)MathMin(72 * 3600 / gapStep, 5184); // سقف مطلق ۵۱۸۴ (M1×72h)
      int barsToFill = 0;

      if(g_gbpnzdLastH > 0 && g_gbpnzdLastH < nowBar)
      {
         // 🆕 v14.20.4: چند کندل *واقعی* بین LastH و نوار جاری بسته شده؟
         int shiftOfLastH = iBarShift(_Symbol, gapTf, g_gbpnzdLastH, false);
         if(shiftOfLastH > 1)
            barsToFill = (int)MathMin(shiftOfLastH - 1, maxFillCap);
         else if(shiftOfLastH < 0)
            barsToFill = maxFillCap;  // LastH خارج از تاریخچهٔ موجود بود — با احتیاط سقف قبلی
         // اگه shiftOfLastH <= 1، یعنی از LastH تا الان اصلاً کندل واقعی
         // جدیدی بسته نشده (مثلاً کل این فاصله آخر هفته بوده) → barsToFill=0
      }

      datetime h = g_gbpnzdLastH;
      for(int fillI = 0; fillI < barsToFill; fillI++)
      {
         h += gapStep;
      ...
      Print(StringFormat(
         "✅ v14.20.4 GV_Load gap-fill (کندل واقعی=%d، نه wall-clock): Buy CleanH=%d RedH=%d | Sell CleanH=%d RedH=%d",
         barsToFill, g_gbpnzdCleanHBuy, g_gbpnzdCrisisRedHBuy,
...
// 🆕 v14.23 [FOREIGN-RULE-GATE]: آیا پیام Rule (STOP/CLOSE/RESUME + یادآور +
// Drift Guard/Infinite Lock) برای این سمبل واقعاً باید ارسال (Alert_Send)
// شود؟ سمبل «خودی» (همان چارتی که EA رویش اجراست) همیشه true است — این
// تنظیم فقط دو ارز «خارجی» را کنترل می‌کند. توجه: این تابع فقط ارسال پیام
// را گیت می‌کند؛ محاسبه/ذخیرهٔ state آن دو ارز (برای On-Demand و غیره)
// همچنان بدون تغییر در پس‌زمینه ادامه دارد.
bool Alert_ShouldSendRuleMsgFor(string sym)
{
   if(CleanSymbol(sym) == CleanSymbol(_Symbol)) return true;
   return Alert_SendForeignRuleMessages;
}
...
void v14_11_SanityCheck()
{
   ...
   if(MathAbs(warn - inp_GBPNZD_SpikeWarn) > 0.001 || MathAbs(spike - inp_GBPNZD_SpikeSig) > 0.001)
      Print("⚠️ v14.21 SANITY FAIL: Spike_GetThresholds GBPNZD mismatch warn=", warn, " spike=", spike);
   else
      Print("✅ v14.21 SANITY OK: Spike GBPNZD warn=", warn, " spike=", spike);

   double gTh, rTh;
   Flow_GetThresholds("GBPNZD", gTh, rTh);
   if(MathAbs(gTh - inp_GBPNZD_FlowGreen) > 0.001 || MathAbs(rTh - inp_GBPNZD_FlowRed) > 0.001)
      Print("⚠️ v14.21 SANITY FAIL: Flow_GetThresholds GBPNZD mismatch green=", gTh, " red=", rTh);
   else
      Print("✅ v14.21 SANITY OK: Flow GBPNZD green=", gTh, " red=", rTh);

   double wM15, wM30, wH1, wH4, wD1;
   Rule_SpikeWeightsForSymbol("GBPNZD", wM15, wM30, wH1, wH4, wD1);
   double wSum = wM15 + wM30 + wH1 + wH4 + wD1;
   if(MathAbs(wSum - 1.0) > 0.005)
      Print("⚠️ v14.21 SANITY FAIL: SpikeWeights GBPNZD sum=", wSum, " (expected 1.0)");
   else
      Print("✅ v14.21 SANITY OK: SpikeWeights GBPNZD H1=", wH1, " H4=", wH4, " sum=", wSum);
}
...
// 🆕 v14.20 [COOLDOWN-FIX]: انتخاب پارامتر Cooldown درست برحسب سمبل.
// مشکلی که رفع می‌کند: پیش از این، موتور «چارت خودی» (GBPNZD_InitReplay،
// Alert_CheckGBPNZDRule) و LogHourlySnapshot (بک‌تست) همیشه inp_SpikeCooldownBars
// را هاردکد پاس می‌دادند — حتی وقتی _Symbol واقعاً AUDCAD بود (یکی از سه
// سناریوی رسمی مستندشده: «EA روی چارت AUDCAD»). بلوک موازی AUDCAD
// (AUDCAD_InitReplay/Alert_CheckAUDCADRule) از قبل درست بود چون مستقیماً
// inp_AUDCAD_CooldownBars را صدا می‌زد — این تابع همان انتخاب را در یک
// نقطهٔ واحد متمرکز می‌کند تا هر ۴ محل فراخوانیِ ApplyClosedBarToRule
// (لایو GBPNZD، Replay GBPNZD، بک‌تست، و به‌صورت ضمنی بلوک‌های موازی) هم‌ارز شوند.
int GetCooldownBarsForSymbol(string sym)
{
   if(CleanSymbol(sym) == "AUDCAD") return inp_AUDCAD_CooldownBars;
   return inp_SpikeCooldownBars;
}
                           ...
                           int cooldownBars, datetime nowTime, HMRuleState &state)
{
   ...
   bool crisisElevated    = (snap.crisis >= 2);   // 🆕 [PATCH RESUME-GAP] قرمز یا نارنجی — فقط برای cleanH
   ...
   bool hourRed       = crisisRed || extraRed;   // بدون تغییر — فقط crisisRedH/CLOSE-B را تغذیه می‌کند

   // 🆕 [PATCH RESUME-GAP] چرا: Rule_Transition پایین همین تابع STOP-B را با
   // spike==1 && crisis IN {Red,Orange} می‌سنجد — یعنی یک ساعت تنهای
   // Orange+Warning کافی است تا STOP فعال شود. ولی تا همین‌جا effectiveDirty
   // فقط با crisis==Red (نه Orange) و با نسخهٔ فیلترشدهٔ-با-Cooldown اسپایک
   // محاسبه می‌شد — یعنی همان ساعتی که STOP را روشن می‌کند می‌توانست هم‌زمان
   // «تمیز» حساب شود و cleanH را صفر نکند. اگر cleanH از قبل نزدیک ۲۴ بود
   // (دوره‌ای آرام قبل از این بلوک تنها)، همان یک ساعت STOP کافی بود تا
   // بلافاصله RESUME بزند — دقیقاً همان الگوی «Stop یک ساعته بعد سبز» که
   // علی در بک‌تست دید و با انتظار درستِ «حداقل ۲۴ ساعت» مغایرت داشت.
   // راه‌حل: cleanH حالا با crisisElevated (Red یا Orange) صفر می‌شود، نه
   // فقط Red. crisisRedH/CLOSE-B عمداً دست‌نخورده ماندند (پایین را ببین).
   bool hourElevatedForClean = crisisElevated || extraRed;
   bool effectiveDirty = hourElevatedForClean || spikeDirty || extraDirty;

   // ── DirtyReason (Patch 6/7) ───────────────────────────────────────
   if(effectiveDirty)
   {
      state.lastDirtyTime   = nowTime;
      state.lastDirtyReason = (hourElevatedForClean && spikeDirty) ? HM_DIRTY_REASON_BOTH :
                               hourElevatedForClean                  ? HM_DIRTY_REASON_CRISIS :
   ...
   // توجه: پارامتر اول عمداً همچنان hourRed خام (فقط Red) است — crisisRedH
   // فقط برای CLOSE-B است و طبق تصمیم، این پچ آستانهٔ CLOSE را عوض نمی‌کند.
...
void GBPNZD_InitReplay()
{
   ...
   for(int sh = _replayBarsToScan; sh >= 1; sh--)
   {
      ...
      // 🆕 v14.20 [COOLDOWN-FIX]: قبلاً همیشه inp_SpikeCooldownBars بود، حتی اگر
      // این تابع (که generic روی _Symbol کار می‌کند) روی چارت AUDCAD اجرا می‌شد.
      ApplyClosedBarToRule(snapReplayBuy, false, false, GetCooldownBarsForSymbol(_Symbol), replayEvalTime, stReplayBuy);
      ...
      ApplyClosedBarToRule(snapReplaySell, false, false, GetCooldownBarsForSymbol(_Symbol), replayEvalTime, stReplaySell);
...
void Alert_CheckGBPNZDRule()
{
   ...
   if(barBoundaryReached)
   {
      ...
      if(snapBuy.valid)
      {
         ...
         // 🆕 v14.20 [COOLDOWN-FIX]: GetCooldownBarsForSymbol(_Symbol) به‌جای inp_SpikeCooldownBars
         // هاردکد — این تابع generic روی _Symbol کار می‌کند و اگر EA مستقیم روی
         // چارت AUDCAD اجرا شود، حالا به‌درستی inp_AUDCAD_CooldownBars می‌گیرد.
         ApplyClosedBarToRule(snapBuy, g_gbpnzdHourDirtyBuy, g_gbpnzdHourRedBuy,
                               GetCooldownBarsForSymbol(_Symbol), snapBuy.evalBarTime, stBuy);
      ...
      if(snapSell.valid)
      {
         ...
         ApplyClosedBarToRule(snapSell, g_gbpnzdHourDirtySell, g_gbpnzdHourRedSell,
                               GetCooldownBarsForSymbol(_Symbol), snapSell.evalBarTime, stSell);
      ...
      if(ShowDebugLogs)
         Print("🔧 v14.13 Rule Buy: streak=", g_gbpnzdSpikeStreakBuy, "/", GetCooldownBarsForSymbol(_Symbol),
               " level=", g_gbpnzdLevelBuy, " cleanH=", g_gbpnzdCleanHBuy,
               " | Sell: streak=", g_gbpnzdSpikeStreakSell, "/", GetCooldownBarsForSymbol(_Symbol),
   ...
   // 🔧 v14.20.3 [MSG-REDESIGN]: قبلاً "Buy: CRISIS=🔴 Red | Spike=🔴 Spike"
   // بود — کلمهٔ رنگ دوبار تکرار می‌شد. حالا فقط رنگ، بدون تکرار اسم.
   string timeStr   = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);
   string crBuyTxt  = (crBuyDisp==2)?"🔴":(crBuyDisp==3)?"🟠":(crBuyDisp==1)?"🟡":(crBuyDisp==0)?"🟢":"⚪";
   string crSellTxt = (crSellDisp==2)?"🔴":(crSellDisp==3)?"🟠":(crSellDisp==1)?"🟡":(crSellDisp==0)?"🟢":"⚪";
   string spBuyTxt  = (spBuyDisp==2)?"🔴":(spBuyDisp==1)?"🟠":(spBuyDisp==0)?"🟢":"⚪";
   string spSellTxt = (spSellDisp==2)?"🔴":(spSellDisp==1)?"🟠":(spSellDisp==0)?"🟢":"⚪";
   string detailLine= "خرید:  Crisis " + crBuyTxt  + "   Spike " + spBuyTxt  + "\n"
                    + "فروش: Crisis " + crSellTxt + "   Spike " + spSellTxt;
   // 🔧 v14.20.3: عدد خام ADX/Flow حذف شد — فقط یادداشت اگه داده ناقص بود
   string adxFlowLine = adxDataOk ? "" : "نکته: دادهٔ این کندل ناقص بود (ADX/Flow نامشخص)";
   ...
   if(newLevelBuy > oldLevelBuy || newLevelSell > oldLevelSell)
   {
      ...
      // 🆕 v14.23 [MSG-REDESIGN] بازخورد کاربر: به‌جای یک چک کلی «آیا اصلاً
      // پوزیشنی روی این سمبل هست» (که اگه فقط Sell باز بود، پیام Buy رو هم
      // گمراه می‌کرد)، حالا هر جهت پوزیشن خودش را جداگانه چک می‌کند.
      bool noPosBuyDir  = !HasRealPositionOnSymbolDir(_Symbol, true);
      bool noPosSellDir = !HasRealPositionOnSymbolDir(_Symbol, false);
      ...
      if(newLevel == 1)
      {
         ...
         if(newLevelBuy == 1 && oldLevelBuy < 1)
            buyReasonTxt  = (newReasonBuy  == 0) ? "حرکت شدید + Crisis غیر سبز" : "حرکت نسبتاً شدید + Crisis نارنجی/قرمز";
         if(newLevelSell == 1 && oldLevelSell < 1)
            sellReasonTxt = (newReasonSell == 0) ? "حرکت شدید + Crisis غیر سبز" : "حرکت نسبتاً شدید + Crisis نارنجی/قرمز";
         string reasonLine = "";
         if(StringLen(buyReasonTxt)  > 0) reasonLine += "خرید: " + buyReasonTxt  + "\n";
         if(StringLen(sellReasonTxt) > 0) reasonLine += "فروش: " + sellReasonTxt + "\n";

         // 🆕 v14.23: نکتهٔ «پوزیشن نداری» حالا جهت‌دار است، نه یک هشدار کلی.
         bool buyIsNewStop  = (newLevelBuy  == 1 && oldLevelBuy  < 1);
         bool sellIsNewStop = (newLevelSell == 1 && oldLevelSell < 1);
         string noPosNote = "";
         if(buyIsNewStop && sellIsNewStop && noPosBuyDir && noPosSellDir)
            noPosNote = "نکته: پوزیشن بازی نداری (نه خرید، نه فروش) — فقط معاملهٔ جدید باز نکن";
         else if(buyIsNewStop && noPosBuyDir)
            noPosNote = "نکته: پوزیشن خریدی نداری — فقط معاملهٔ خرید جدید باز نکن";
         else if(sellIsNewStop && noPosSellDir)
            noPosNote = "نکته: پوزیشن فروشی نداری — فقط معاملهٔ فروش جدید باز نکن";

         msg = "🚦Rule :\n"
             + "⛔ ریسک بالاست، پوزیشن جدید نگیر\n"
             + "━━━━━━━━━━━━━━━━\n"
             + CleanSymbol(_Symbol) + " | " + timeStr + "\n"
             + detailLine + "\n"
             + (StringLen(adxFlowLine) > 0 ? adxFlowLine + "\n" : "")
             + "━━━━━━━━━━━━━━━━\n"
             + reasonLine
             + "نکته: معاملهٔ جدید باز نکن\n"
             + "نکته: معاملات فعلی رو مدیریت کن، گوش‌به‌زنگ باش\n"
             + "نکته: اگه Crisis قرمز شد و هشدار شدید هم روشن شد، یا با ضرر ببند یا هج کن";
         if(StringLen(noPosNote) > 0)
            msg += "\n" + noPosNote;
      }
      else if(newLevel == 2)
      {
         string closeNoteBuy  = (newLevelBuy  == 2 && newReasonBuy  != 10) ? StringFormat("  (%d ساعت پیاپی)", g_gbpnzdCrisisRedHBuy)  : "";
         string closeNoteSell = (newLevelSell == 2 && newReasonSell != 10) ? StringFormat("  (%d ساعت پیاپی)", g_gbpnzdCrisisRedHSell) : "";

         // 🆕 v14.23 [MSG-REDESIGN] بازخورد کاربر: به‌جای یک جملهٔ ثابت
         // («Crisis قرمزه و روند+جریان‌پول کاملاً برخلاف جهتته») که همیشه
         // عیناً تکرار می‌شد صرف‌نظر از این‌که واقعاً کدام چراغ باعثش شده،
         // حالا از snapBuy/snapSell — همان Snapshotی که خودِ تصمیم Rule را
         // ساخت — هر چراغِ واقعاً قرمز (RTM/Trend/Structure/Flow) جداگانه
         // و فقط اگر واقعاً قرمز باشد ذکر می‌شود.
         bool buyIsNewClose  = (newLevelBuy  == 2);
         bool sellIsNewClose = (newLevelSell == 2);
         bool bothClosing    = buyIsNewClose && sellIsNewClose;
         string lightNotes = "";
         if(buyIsNewClose && snapBuy.valid)
         {
            string pfx = bothClosing ? "خرید — " : "";
            if(snapBuy.rtm==2)       lightNotes += "نکته: " + pfx + "قیمت داره از میانگین برخلافت دور می‌شه (RTM)\n";
            if(snapBuy.trend==2)     lightNotes += "نکته: " + pfx + "روند خلاف جهتته (Trend)\n";
            if(snapBuy.structD1==2)  lightNotes += "نکته: " + pfx + "ساختار روزانه برخلافت شکسته (Structure)\n";
            if(snapBuy.flowLight==2) lightNotes += "نکته: " + pfx + "جریان پول برخلافته (Flow)\n";
         }
         if(sellIsNewClose && snapSell.valid)
         {
            string pfx = bothClosing ? "فروش — " : "";
            if(snapSell.rtm==2)       lightNotes += "نکته: " + pfx + "قیمت داره از میانگین برخلافت دور می‌شه (RTM)\n";
            if(snapSell.trend==2)     lightNotes += "نکته: " + pfx + "روند خلاف جهتته (Trend)\n";
            if(snapSell.structD1==2)  lightNotes += "نکته: " + pfx + "ساختار روزانه برخلافت شکسته (Structure)\n";
            if(snapSell.flowLight==2) lightNotes += "نکته: " + pfx + "جریان پول برخلافته (Flow)\n";
         }

         // 🆕 v14.23: نکتهٔ پایانی هر جهت هم حالا پوزیشن‌آگاه است — دقیقاً
         // طبق درخواست کاربر: بدون پوزیشن، «باز نکن که اگه بود باید می‌بستی»؛
         // با پوزیشن، همون دستور اصلی «ببند».
         string closeNotes = "";
         if(buyIsNewClose)
            closeNotes += noPosBuyDir
               ? "نکته: خرید — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: خرید — پوزیشن رو با هر ضرری ببند\n";
         if(sellIsNewClose)
            closeNotes += noPosSellDir
               ? "نکته: فروش — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: فروش — پوزیشن رو با هر ضرری ببند\n";

         msg = "🚦Rule :\n"
             + "🔴 شرایط داغونه، بهتره الان ببندی\n"
             + "━━━━━━━━━━━━━━━━\n"
             + CleanSymbol(_Symbol) + " | " + timeStr + "\n"
             + "خرید:  Crisis " + crBuyTxt  + "   Spike " + spBuyTxt  + closeNoteBuy  + "\n"
             + "فروش: Crisis " + crSellTxt + "   Spike " + spSellTxt + closeNoteSell + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + lightNotes
             + closeNotes
             + "نکته: Xmoon رو فعلاً خاموش کن";
      }
   }
   else if(newLevelBuy == 0 && newLevelSell == 0
           && (oldLevelBuy > 0 || oldLevelSell > 0))
   {
      string cleanLine = StringFormat("%d ساعت پیاپی بدون خطر گذشت (خرید=%d | فروش=%d)",
         MathMin(g_gbpnzdCleanHBuy, g_gbpnzdCleanHSell), g_gbpnzdCleanHBuy, g_gbpnzdCleanHSell);
      msg = "✅ Rule :\n"
          + "✅ اوضاع آروم شد — می‌تونی برگردی\n"
          + "━━━━━━━━━━━━━━━━\n"
          + CleanSymbol(_Symbol) + " | " + timeStr + "\n"
          + "Crisis 🟢   Spike 🟢   (خرید و فروش)\n"
          + cleanLine + "\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "نکته: می‌تونی دوباره با Xmoon معامله کنی، سیگنال‌ها رو روشن کن";
   }
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.20 [COMBINED-REMINDER] بهبود #5 کاربر: تشخیص خالص «آیا این جهت الان
// به یادآور نیاز دارد» — بدون send و بدون تغییر state. توسط
// Rule_CheckReminderForDirection، EURGBP_CheckReminderForDirection،
// AUDCAD_CheckReminderForDirection (هر سه ارز، از v14.20.2 به بعد یکی شدند)
// و Rule_CheckReminders (برای تصمیم ترکیب/تفکیک) استفاده می‌شود.
//
// 🔧 v14.20.2 [ALIGNED-REMINDER] — گزارش کاربر: در v14.19 یادآورها دقیقاً
// راس ساعت‌های مشخص (مثلاً ۱۱:۳۰، ۱۲:۳۰، ۱۳:۳۰) می‌آمدند، اما بعد از
// آپدیت به v14.20 روی ساعت‌های نامنظم (۲۲:۵۲، ۲۳:۵۲) افتاد و «آشفته»
// به‌نظر رسید. علت ریشه‌ای: طرح قبلی «حداقل repMin دقیقه از آخرین
// یادآوریِ واقعی گذشته باشد» بود — یعنی لنگر زمان‌بندی هربار روی همان
// لحظه‌ای می‌ایستد که آخرین یادآوری واقعاً ارسال شده. اگر EA به هر دلیلی
// (آپدیت، ری‌استارت MT5/VPS، قطعی) مدتی متوقف بماند، اولین بررسی بعد از
// روشن‌شدن مجدد بلافاصله «دیر شده» تشخیص داده و یک یادآور فوری در همان
// لحظهٔ نامنظم می‌فرستد — و از آن به بعد، همهٔ یادآورهای بعدی هم از همان
// لنگرِ جدید و نامنظم پیروی می‌کنند (چون هربار «آخرین ارسال واقعی» به‌روز
// می‌شود). راه‌حل: به‌جای «فاصله از آخرین ارسال»، از «اسلات ثابت زمانی»
// استفاده می‌شود — زمان به بازه‌های repMin دقیقه‌ای (از epoch) تقسیم
// می‌شود و یادآور فقط وقتی لازم است که وارد یک اسلاتِ جدید نسبت به
// آخرین ارسال شده باشیم. این باعث می‌شود مرزهای یادآوری برای همیشه ثابت
// و از قبل قابل‌پیش‌بینی بمانند (مثلاً همیشه رأس هر ساعت اگر repMin=60)،
// صرف‌نظر از اینکه EA کِی ری‌استارت شده — دقیقاً همان رفتار «راس ساعت»ی
// که کاربر در v14.19 می‌دید، اما این‌بار برای همیشه پایدار و غیرقابل‌جابه‌جایی.
bool Rule_ReminderIsDue(int level, datetime lastReminderTime, datetime nowR)
{
   ...
   int repSec = repMin * 60;

   if(lastReminderTime == 0) return true;   // هیچ لنگری نیست — اولین بار مجاز است

   // 🔧 v14.20.2 [ALIGNED-REMINDER edge-case]: اگه transition درست چند ثانیه/
   // دقیقه قبل از مرز یک اسلات اتفاق افتاده باشه (مثلاً ۱۴:۵۹:۵۰ با اسلاتِ
   // یک‌ساعته)، صرفِ «اسلات‌بندی» به‌تنهایی می‌تونه باعث بشه یادآور فقط چند
   // ثانیه بعد از پیام تغییر سطح دوباره فایر بشه — یعنی همون کلاس «۲ پیام
   // نزدیک به هم» که این‌کار داشت رفعش می‌کرد. حداقل نصف بازهٔ تکرار باید
   // از آخرین رویداد (چه transition چه یادآور قبلی) گذشته باشه. این باعث
   // می‌شه فقط همون *اولین* یادآور بعد از یک transition نزدیک به مرز، کمی
   // دیرتر از مرز «تمیز» بیفته — یادآورهای بعدی خودشون را با مرز اصلی هماهنگ می‌کنند.
   if((nowR - lastReminderTime) < (repSec / 2)) return false;

   long slotNow  = ((long)nowR) / repSec;
   long slotLast = ((long)lastReminderTime) / repSec;
   if(slotNow <= slotLast) return false;
   return true;
}

// خط جزئیات یک جهت — در پیام تکی و پیام ترکیبی هر دو استفاده می‌شود.
string Rule_ReminderDirLine(bool forBuy, int level, int cleanH)
{
   // 🔧 v14.23 [MSG-REDESIGN] بازخورد کاربر: مدت‌زمان از این خط جدا و به یک
   // نکتهٔ مجزا با قاب «خبر خوب» منتقل شد (پیشرفت به سمت رفعِ توقف/بستن) —
   // این خط حالا فقط جهت+حکم را نشان می‌دهد، کوتاه‌تر و قابل‌اسکن‌تر.
   // (پارامتر cleanH دیگر این‌جا استفاده نمی‌شود؛ فراخوان‌کننده‌ها خودشان
   // با همان SQ_FormatDurationBars نکتهٔ جدا می‌سازند — امضای تابع برای
   // سازگاری با محل‌های فراخوانی موجود دست‌نخورده ماند.)
   string dirLbl = forBuy ? "خرید" : "فروش";
   return dirLbl + ": " + ((level==2)?"🔴 بستن بهتره":"⛔ توقف ورود");
}
                                     ...
                                     datetime nowR)
{
   ...
   // 🆕 v14.20: محاسبه از Rule_ReminderIsDue مشترک می‌آید (بدون تغییر منطق).
   if(!Rule_ReminderIsDue(level, lastReminderTime, nowR)) return;

   lastReminderTime  = nowR;
   lastReminderLevel = level;

   // 🆕 v14.23 [MSG-REDESIGN] بازخورد کاربر: پوزیشن این جهت به‌طور مستقل چک
   // می‌شود (نه کل سمبل) — اگه فقط پوزیشن جهت مخالف باز باشه، این یادآور
   // اشتباهاً فکر نمی‌کنه پوزیشنِ همین جهت هم وجود داره.
   bool noRealPositionsDir = !HasRealPositionOnSymbolDir(_Symbol, forBuy);
   string durStr = SQ_FormatDurationBars(cleanH, g_chartPeriodSeconds);

   // 🔧 v14.23: سربرگ استاندارد «⏰ Rule Reminder :» — این پیام واقعاً از
   // Rule می‌آید نه Crisis (Crisis فقط یکی از ورودی‌های تصمیم Rule است)،
   // پس عمداً هرگز کلمهٔ Crisis داخل این یادآور نوشته نمی‌شود.
   string headline = (level == 2) ? "یادآوری می‌کنم وضعیت هنوز بده" : "یادآوری می‌کنم ریسک هنوز بالاست";
   string tailMsg = (level == 2)
      ? (noRealPositionsDir ? "بازار هنوز خیلی پرخطره پس معاملهٔ جدید باز نکن" : "هنوز نبستی؟ الان ببند")
      : (noRealPositionsDir ? "بازار همچنان پرخطره پس هنوز معاملهٔ جدید باز نکن" : "هنوز معاملهٔ جدید باز نکن");
   string remMsg = "⏰ Rule Reminder :\n"
      + headline + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + CleanSymbol(_Symbol) + " | " + TimeToString(nowR, TIME_DATE|TIME_MINUTES) + "\n"
      + Rule_ReminderDirLine(forBuy, level, cleanH) + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + "نکته: بیشتر حواست به چارت و تغییر وضعیت Rule باشه\n"
      + "نکته: " + tailMsg + "\n"
      + "نکته: خبر خوب اینکه " + durStr + " آرامش پیاپی تا الان ثبت شده";
   Alert_Send(remMsg);
}
...
void Rule_CheckReminders()
{
   ...
   // 🆕 v14.20 [COMBINED-REMINDER] بهبود #5 کاربر: اگر هر دو جهت هم‌زمان به
   // یادآور نیاز دارند، به‌جای دو پیام جدا در یک چرخهٔ OnTimer (که می‌تواند
   // کاربر را سردرگم کند — سؤال ۱۳ تحلیل)، یک پیام ترکیبی واحد فرستاده می‌شود.
   bool buyDue  = Rule_ReminderIsDue(g_gbpnzdLevelBuy,  g_gbpnzdLastReminderBuy,  nowR);
   bool sellDue = Rule_ReminderIsDue(g_gbpnzdLevelSell, g_gbpnzdLastReminderSell, nowR);

   if(buyDue && sellDue)
   {
      g_gbpnzdLastReminderBuy       = nowR;
      g_gbpnzdLastReminderLevelBuy  = g_gbpnzdLevelBuy;
      g_gbpnzdLastReminderSell      = nowR;
      g_gbpnzdLastReminderLevelSell = g_gbpnzdLevelSell;

      // 🆕 v14.23 [MSG-REDESIGN]: پوزیشن هر جهت جداگانه چک می‌شود؛ هدر
      // استاندارد «⏰ Rule Reminder :» + نکتهٔ «خبر خوب» برای هر جهت.
      bool noPosBuyDir  = !HasRealPositionOnSymbolDir(_Symbol, true);
      bool noPosSellDir = !HasRealPositionOnSymbolDir(_Symbol, false);
      bool anyClose  = (g_gbpnzdLevelBuy == 2 || g_gbpnzdLevelSell == 2);
      string headline = anyClose ? "یادآوری می‌کنم وضعیت هنوز بده (هر دو جهت)" : "یادآوری می‌کنم ریسک هنوز بالاست (هر دو جهت)";

      string posNotes = "";
      posNotes += noPosBuyDir
         ? "نکته: خرید — پوزیشن نداری، پس معاملهٔ جدید باز نکن\n"
         : ((g_gbpnzdLevelBuy==2) ? "نکته: خرید — هنوز نبستی؟ الان ببند\n" : "نکته: خرید — هنوز معاملهٔ جدید باز نکن\n");
      posNotes += noPosSellDir
         ? "نکته: فروش — پوزیشن نداری، پس معاملهٔ جدید باز نکن\n"
         : ((g_gbpnzdLevelSell==2) ? "نکته: فروش — هنوز نبستی؟ الان ببند\n" : "نکته: فروش — هنوز معاملهٔ جدید باز نکن\n");

      string remMsg = "⏰ Rule Reminder :\n"
         + headline + "\n"
         + "━━━━━━━━━━━━━━━━\n"
         + CleanSymbol(_Symbol) + " | " + TimeToString(nowR, TIME_DATE|TIME_MINUTES) + "\n"
         + Rule_ReminderDirLine(true,  g_gbpnzdLevelBuy,  g_gbpnzdCleanHBuy)  + "\n"
         + Rule_ReminderDirLine(false, g_gbpnzdLevelSell, g_gbpnzdCleanHSell) + "\n"
         + "━━━━━━━━━━━━━━━━\n"
         + "نکته: بیشتر حواست به چارت و تغییر وضعیت Rule باشه\n"
         + posNotes
         + StringFormat("نکته: خبر خوب اینکه خرید %s و فروش %s آرامش پیاپی تا الان ثبت شده",
              SQ_FormatDurationBars(g_gbpnzdCleanHBuy, g_chartPeriodSeconds),
              SQ_FormatDurationBars(g_gbpnzdCleanHSell, g_chartPeriodSeconds));
      Alert_Send(remMsg);
      return;
   }

   // فقط یکی (یا هیچ‌کدام) الان نیاز دارد — دقیقاً رفتار قبلی v14.19: هرکدام
   // به‌طور مستقل بررسی/ارسال می‌شود (و اگر سطحش صفر شده، ریست هم می‌شود).
...
bool EURGBP_GV_Load()
{
   ...
   datetime savedAt = (datetime)GlobalVariableGet(pfx+"Saved");
   // 🐛 v14.20.4 [BUG-WEEKEND-STALE-GV]: مثل GBPNZD — inp_GVStaleHours به‌جای 72 هاردکد
   if(TimeCurrent() - savedAt > inp_GVStaleHours * 3600) return false;   // stale
...
void EURGBP_InitReplay()
{
   ...
   for(int sh = _replayBarsToScan; sh >= 1; sh--)
   {
      ...
      // 🆕 v14.20 [SUFFIX-FIX]: g_eurgbpBrokerSymbol به‌جای رشتهٔ خام "EURGBP" —
      // توابع Crisis_GetThresholds/Flow_GetThresholds/Sister_GetTable که داخل
      // Generic_ReplayCrisisAtBar صدا زده می‌شوند خودشان CleanSymbol() انجام
      // می‌دهند، پس این تغییر classification را نمی‌شکند؛ فقط دسترسی به قیمت را درست می‌کند.
      int crBuy  = Generic_ReplayCrisisAtBar(g_eurgbpBrokerSymbol, g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              sh, true,  flowBuyOut,  adxBuyOut,  rcBuyOut);
      int crSell = Generic_ReplayCrisisAtBar(g_eurgbpBrokerSymbol, g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              sh, false, flowSellOut, adxSellOut, rcSellOut);

      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar(g_eurgbpBrokerSymbol, sh, spikeBuyOut, spikeSellOut);

      bool haBuy  = (adxBuyOut  >= 35.0) && (flowBuyOut  < -7.0);
      bool haSell = (adxSellOut >= 35.0) && (flowSellOut < -7.0);

      datetime replayEvalTime = iTime(g_eurgbpBrokerSymbol, PERIOD_H1, sh + 1);
   ...
   g_eurgbpLastH = iTime(g_eurgbpBrokerSymbol, PERIOD_CURRENT, 1);
                                       ...
                                       datetime nowR)
{
   ...
   // 🔧 v14.20.2: از Rule_ReminderIsDue مشترک (اسلات‌بندی‌شده) استفاده می‌شود —
   // به‌جای منطق تکراریِ قبلی، تا EURGBP هم از رفع باگ «ساعت‌های نامنظم» بهره ببرد.
   if(!Rule_ReminderIsDue(level, lastReminderTime, nowR)) return;

   lastReminderTime  = nowR;
   lastReminderLevel = level;

   // 🆕 v14.23 [FOREIGN-RULE-GATE]: تایمر/state بالا همیشه به‌روزرسانی
   // می‌شود (تا وقتی این تنظیم بعداً روشن شد، رفتار زمان‌بندی درست بمونه)؛
   // فقط ارسال واقعیِ پیام طبق اینپوت جدید Alert_SendForeignRuleMessages
   // گیت می‌شود — چون EURGBP این‌جا همیشه سمبل «خارجی» نسبت به چارت است.
   if(!Alert_ShouldSendRuleMsgFor(g_eurgbpBrokerSymbol)) return;

   // 🆕 v14.23 [MSG-REDESIGN]: پوزیشن این جهت جداگانه چک می‌شود (نه کل سمبل)
   bool noRealPositionsDir = !HasRealPositionOnSymbolDir(g_eurgbpBrokerSymbol, forBuy);
   string dirLbl  = forBuy ? "خرید" : "فروش";
   string durStrE = SQ_FormatDurationBars(cleanH, g_chartPeriodSeconds);

   // 🔧 v14.23: سربرگ استاندارد «⏰ Rule Reminder :» — هرگز کلمهٔ Crisis
   // داخل این یادآور نمی‌آید (این پیام از Rule می‌آید، نه از Crisis).
   string headline = (level == 2) ? "یادآوری می‌کنم وضعیت هنوز بده" : "یادآوری می‌کنم ریسک هنوز بالاست";
   string tailMsg = (level == 2)
      ? (noRealPositionsDir ? "بازار هنوز خیلی پرخطره پس معاملهٔ جدید باز نکن" : "هنوز نبستی؟ الان ببند")
      : (noRealPositionsDir ? "بازار همچنان پرخطره پس هنوز معاملهٔ جدید باز نکن" : "هنوز معاملهٔ جدید باز نکن");
   string remMsg = "⏰ Rule Reminder :\n"
      ...
      + dirLbl + ": " + ((level==2)?"🔴 بستن بهتره":"⛔ توقف ورود") + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + "نکته: بیشتر حواست به چارت و تغییر وضعیت Rule باشه\n"
      + "نکته: " + tailMsg + "\n"
      + "نکته: خبر خوب اینکه " + durStrE + " آرامش پیاپی تا الان ثبت شده";
   Alert_Send(remMsg);
}
...
void Alert_CheckEURGBPRule()
{
   ...
   // 🆕 v14.20 [SUFFIX-FIX]: g_eurgbpBrokerSymbol — نام خام "EURGBP" روی بروکرهای
   // پسونددار همیشه SYMBOL_TIME=0 برمی‌گرداند و باعث می‌شد eurgbpMarketStale
   // همیشه false بماند و بلوک به‌درستی تشخیص «داده تازه» ندهد.
   datetime lastQuoteTimeE = (datetime)SymbolInfoInteger(g_eurgbpBrokerSymbol, SYMBOL_TIME);
   ...
   if(barBoundaryReachedE)
   {
      double flowBuyOut = 0.0, adxBuyOut = 0.0;  int rcBuyOut = 0;
      double flowSellOut= 0.0, adxSellOut= 0.0;  int rcSellOut= 0;
      // 🆕 v14.20 [SUFFIX-FIX]: g_eurgbpBrokerSymbol به‌جای رشتهٔ خام
      int crBuy  = Generic_ReplayCrisisAtBar(g_eurgbpBrokerSymbol, g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
      ...
      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar(g_eurgbpBrokerSymbol, 1, spikeBuyOut, spikeSellOut);

      datetime evalTimeE = iTime(g_eurgbpBrokerSymbol, PERIOD_H1, 2);   // آخرین کندل کاملاً بسته (shift=1 در Generic_Replay* یعنی +1 نسبت به shift جاری؛ سازگار با iTime(sh+1) در InitReplay)
      ...
      snapBuy.rc = rcBuyOut;   // 🆕 v14.23 [MSG-REDESIGN]: قبلاً محاسبه می‌شد ولی ذخیره نمی‌شد؛ برای نکتهٔ «X از ۴ چراغ قرمز» لازم است
      ...
      snapSell.rc = rcSellOut;   // 🆕 v14.23 همان دلیل بالا
      haveSnap = true;
   ...
   // 🆕 v14.23 [MSG-REDESIGN]: چک جهت‌دار جایگزین چک کلی قبلی شد — اگه فقط
   // پوزیشن یک جهت باز باشه، پیام جهت دیگه اشتباه نگه.
   bool noPosBuyDirE  = !HasRealPositionOnSymbolDir(g_eurgbpBrokerSymbol, true);
   bool noPosSellDirE = !HasRealPositionOnSymbolDir(g_eurgbpBrokerSymbol, false);
   ...
   // 🔧 v14.20.3 [MSG-REDESIGN]: مثل GBPNZD — بدون تکرار رنگ در Crisis=/Spike=،
   // بدون تکرار زیاد اسم ارز، Buy/Sell به فارسی.
   string crBuyTxt  = (crBuyDisp==2)?"🔴":(crBuyDisp==3)?"🟠":(crBuyDisp==1)?"🟡":(crBuyDisp==0)?"🟢":"⚪";
   string crSellTxt = (crSellDisp==2)?"🔴":(crSellDisp==3)?"🟠":(crSellDisp==1)?"🟡":(crSellDisp==0)?"🟢":"⚪";
   string spBuyTxt  = (spBuyDisp==2)?"🔴":(spBuyDisp==1)?"🟠":(spBuyDisp==0)?"🟢":"⚪";
   string spSellTxt = (spSellDisp==2)?"🔴":(spSellDisp==1)?"🟠":(spSellDisp==0)?"🟢":"⚪";
   string detailLineE = "خرید:  Crisis " + crBuyTxt  + "   Spike " + spBuyTxt  + "\n"
                       + "فروش: Crisis " + crSellTxt + "   Spike " + spSellTxt;
   ...
   if(newLevelBuy > oldLevelBuy || newLevelSell > oldLevelSell)
   {
      ...
      if(newLevel == 1)
      {
         bool buyIsNewStopE  = (newLevelBuy  == 1 && oldLevelBuy  < 1);
         bool sellIsNewStopE = (newLevelSell == 1 && oldLevelSell < 1);
         string noPosNoteE = "";
         if(buyIsNewStopE && sellIsNewStopE && noPosBuyDirE && noPosSellDirE)
            noPosNoteE = "\nنکته: پوزیشن بازی نداری (نه خرید، نه فروش) — فقط معاملهٔ جدید باز نکن";
         else if(buyIsNewStopE && noPosBuyDirE)
            noPosNoteE = "\nنکته: پوزیشن خریدی نداری — فقط معاملهٔ خرید جدید باز نکن";
         else if(sellIsNewStopE && noPosSellDirE)
            noPosNoteE = "\nنکته: پوزیشن فروشی نداری — فقط معاملهٔ فروش جدید باز نکن";
         ...
         msg = "🚦Rule :\n"
             ...
             + detailLineE + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + "نکته: معاملهٔ جدید باز نکن، معاملات فعلی رو مدیریت کن"
             + noPosNoteE;
      }
      else if(newLevel == 2)
      {
         string closeNoteBuyE  = (newLevelBuy  == 2) ? StringFormat("  (%d ساعت پیاپی)", g_eurgbpCrisisRedHBuy)  : "";
         string closeNoteSellE = (newLevelSell == 2) ? StringFormat("  (%d ساعت پیاپی)", g_eurgbpCrisisRedHSell) : "";

         // 🆕 v14.23 [MSG-REDESIGN]: به‌جای جملهٔ ثابت قبلی، از rc واقعی
         // (تعداد چراغ‌های قرمز از ۴ — RTM/Trend/Structure/Flow) که همین
         // الان محاسبه شد استفاده می‌شود. توجه: مسیر EURGBP/AUDCAD (بر خلاف
         // چارت اصلی) فقط شمارشِ چراغ‌های قرمز را در دسترس دارد، نه این‌که
         // دقیقاً کدام‌یک — برای تفکیک کامل RTM/Trend/Structure/Flow روی
         // این سمبل، باید Generic_ReplayCrisisAtBar هم گسترش داده شود.
         bool buyIsNewCloseE  = (newLevelBuy  == 2);
         bool sellIsNewCloseE = (newLevelSell == 2);
         string rcNotesE = "";
         if(buyIsNewCloseE && haveSnap)
            rcNotesE += StringFormat("نکته: خرید — %d از ۴ چراغ قرمزن (Crisis)\n", snapBuy.rc);
         if(sellIsNewCloseE && haveSnap)
            rcNotesE += StringFormat("نکته: فروش — %d از ۴ چراغ قرمزن (Crisis)\n", snapSell.rc);

         string closeNotesE = "";
         if(buyIsNewCloseE)
            closeNotesE += noPosBuyDirE
               ? "نکته: خرید — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: خرید — پوزیشن رو با هر ضرری ببند\n";
         if(sellIsNewCloseE)
            closeNotesE += noPosSellDirE
               ? "نکته: فروش — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: فروش — پوزیشن رو با هر ضرری ببند\n";
         ...
         msg = "🚦Rule :\n"
             ...
             + "━━━━━━━━━━━━━━━━\n"
             + "EURGBP | " + timeStrE + "\n"
             + "خرید:  Crisis " + crBuyTxt  + "   Spike " + spBuyTxt  + closeNoteBuyE  + "\n"
             + "فروش: Crisis " + crSellTxt + "   Spike " + spSellTxt + closeNoteSellE + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + rcNotesE
             + closeNotesE;
      }
   ...
   {
      msg = "✅ Rule :\n"
          + "✅ اوضاع EURGBP آروم شد — می‌تونی برگردی\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "EURGBP | " + timeStrE + "\n"
          + "Crisis 🟢   Spike 🟢   (خرید و فروش)\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "نکته: می‌تونی دوباره معامله کنی";
   }

   // 🆕 v14.23 [FOREIGN-RULE-GATE]: state (زیر) همیشه به‌روز می‌شود؛ فقط
   // ارسال واقعی پیام طبق اینپوت جدید گیت می‌شود — چون EURGBP این‌جا همیشه
   // «سمبل خارجی» نسبت به چارت جاری است (این تابع فقط وقتی چارت≠EURGBP
   // فعال است، طبق g_eurgbpParallelActive در OnTimer/OnTick اجرا می‌شود).
   if(StringLen(msg) > 0 && Alert_ShouldSendRuleMsgFor(g_eurgbpBrokerSymbol))
      Alert_Send(msg);
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.19: معماری Stateful مستقل AUDCAD — کاملاً موازی با بلوک EURGBP
// بالا (v14.17) که خودش موازی با GBPNZD اصلی بود. هیچ‌کدام از بلوک‌های
// GBPNZD/EURGBP در این فایل تغییر نکردند. تنها تفاوت عمدی نسبت به الگوی
// EURGBP: cooldown مستقل (inp_AUDCAD_CooldownBars، نه inp_SpikeCooldownBars
// مشترک) — پیاده‌سازی واقعی «Cooldown per-symbol» طبق بریف کاربر بخش
// ۴.۱ (تصمیم: 3). جزئیات کامل در v14.19_Changes.md بخش ۱-۲.
// ════════════════════════════════════════════════════════════════════
void AUDCAD_HMRuleState_LoadBuy(HMRuleState &st)
{
   st.level             = g_audcadLevelBuy;
   st.stopReason        = g_audcadStopReasonBuy;
   st.crisisRedH        = g_audcadCrisisRedHBuy;
   st.cleanH            = g_audcadCleanHBuy;
   st.cleanSince        = g_audcadCleanSinceBuy;
   st.redSince          = g_audcadRedSinceBuy;
   st.spikeStreak       = g_audcadSpikeStreakBuy;
   st.lastReminderTime  = g_audcadLastReminderBuy;
   st.lastDirtyTime     = g_audcadLastDirtyTimeBuy;
   st.lastDirtyReason   = g_audcadLastDirtyReasonBuy;
}
void AUDCAD_HMRuleState_SaveBuy(const HMRuleState &st)
{
   g_audcadLevelBuy            = st.level;
   g_audcadStopReasonBuy       = st.stopReason;
   g_audcadCrisisRedHBuy       = st.crisisRedH;
   g_audcadCleanHBuy           = st.cleanH;
   g_audcadCleanSinceBuy       = st.cleanSince;
   g_audcadRedSinceBuy         = st.redSince;
   g_audcadSpikeStreakBuy      = st.spikeStreak;
   g_audcadLastReminderBuy     = st.lastReminderTime;
   g_audcadLastDirtyTimeBuy    = st.lastDirtyTime;
   g_audcadLastDirtyReasonBuy  = st.lastDirtyReason;
}
void AUDCAD_HMRuleState_LoadSell(HMRuleState &st)
{
   st.level             = g_audcadLevelSell;
   st.stopReason        = g_audcadStopReasonSell;
   st.crisisRedH        = g_audcadCrisisRedHSell;
   st.cleanH            = g_audcadCleanHSell;
   st.cleanSince        = g_audcadCleanSinceSell;
   st.redSince          = g_audcadRedSinceSell;
   st.spikeStreak       = g_audcadSpikeStreakSell;
   st.lastReminderTime  = g_audcadLastReminderSell;
   st.lastDirtyTime     = g_audcadLastDirtyTimeSell;
   st.lastDirtyReason   = g_audcadLastDirtyReasonSell;
}
void AUDCAD_HMRuleState_SaveSell(const HMRuleState &st)
{
   g_audcadLevelSell           = st.level;
   g_audcadStopReasonSell      = st.stopReason;
   g_audcadCrisisRedHSell      = st.crisisRedH;
   g_audcadCleanHSell          = st.cleanH;
   g_audcadCleanSinceSell      = st.cleanSince;
   g_audcadRedSinceSell        = st.redSince;
   g_audcadSpikeStreakSell     = st.spikeStreak;
   g_audcadLastReminderSell    = st.lastReminderTime;
   g_audcadLastDirtyTimeSell   = st.lastDirtyTime;
   g_audcadLastDirtyReasonSell = st.lastDirtyReason;
}

// پیشوند GV: Rule_GVPrefix("AUDCAD") == "HM_AUDCAD_" (تابع generic موجود؛
// صفر خط تغییر در خود Rule_GVPrefix).
void AUDCAD_GV_Save()
{
   string pfx = Rule_GVPrefix("AUDCAD");
   GlobalVariableSet(pfx+"Ver",        HM_AUDCAD_SCHEMA_VERSION);
   GlobalVariableSet(pfx+"PerSec",     (double)g_chartPeriodSeconds);
   GlobalVariableSet(pfx+"LvBuy",      (double)g_audcadLevelBuy);
   GlobalVariableSet(pfx+"LvSell",     (double)g_audcadLevelSell);
   GlobalVariableSet(pfx+"ClHBuy",     (double)g_audcadCleanHBuy);
   GlobalVariableSet(pfx+"ClHSell",    (double)g_audcadCleanHSell);
   GlobalVariableSet(pfx+"RdHBuy",     (double)g_audcadCrisisRedHBuy);
   GlobalVariableSet(pfx+"RdHSell",    (double)g_audcadCrisisRedHSell);
   GlobalVariableSet(pfx+"LastH",      (double)g_audcadLastH);
   GlobalVariableSet(pfx+"SRBuy",      (double)g_audcadStopReasonBuy);
   GlobalVariableSet(pfx+"SRSell",     (double)g_audcadStopReasonSell);
   GlobalVariableSet(pfx+"PrCrBuy",    (double)g_audcadPrevCrisisBuy);
   GlobalVariableSet(pfx+"PrCrSell",   (double)g_audcadPrevCrisisSell);
   GlobalVariableSet(pfx+"PrSpBuy",    (double)g_audcadPrevSpikeBuy);
   GlobalVariableSet(pfx+"PrSpSell",   (double)g_audcadPrevSpikeSell);
   GlobalVariableSet(pfx+"CrBuy",      (double)g_audcadCrisisBuy);
   GlobalVariableSet(pfx+"CrSell",     (double)g_audcadCrisisSell);
   GlobalVariableSet(pfx+"SpBuy",      (double)g_audcadSpikeBuy);
   GlobalVariableSet(pfx+"SpSell",     (double)g_audcadSpikeSell);
   GlobalVariableSet(pfx+"HRedBuy",    (double)(g_audcadHourRedBuy    ? 1 : 0));
   GlobalVariableSet(pfx+"HDirtyBuy",  (double)(g_audcadHourDirtyBuy  ? 1 : 0));
   GlobalVariableSet(pfx+"HRedSell",   (double)(g_audcadHourRedSell   ? 1 : 0));
   GlobalVariableSet(pfx+"HDirtySell", (double)(g_audcadHourDirtySell ? 1 : 0));
   GlobalVariableSet(pfx+"SpkStrBuy",  (double)g_audcadSpikeStreakBuy);
   GlobalVariableSet(pfx+"SpkStrSell", (double)g_audcadSpikeStreakSell);
   GlobalVariableSet(pfx+"ClSinceBuy", (double)g_audcadCleanSinceBuy);
   GlobalVariableSet(pfx+"ClSinceSell",(double)g_audcadCleanSinceSell);
   GlobalVariableSet(pfx+"RdSinceBuy", (double)g_audcadRedSinceBuy);
   GlobalVariableSet(pfx+"RdSinceSell",(double)g_audcadRedSinceSell);
   GlobalVariableSet(pfx+"DrtTBuy",    (double)g_audcadLastDirtyTimeBuy);
   GlobalVariableSet(pfx+"DrtTSell",   (double)g_audcadLastDirtyTimeSell);
   GlobalVariableSet(pfx+"DrtRBuy",    (double)g_audcadLastDirtyReasonBuy);
   GlobalVariableSet(pfx+"DrtRSell",   (double)g_audcadLastDirtyReasonSell);
   GlobalVariableSet(pfx+"RemTBuy",    (double)g_audcadLastReminderBuy);
   GlobalVariableSet(pfx+"RemTSell",   (double)g_audcadLastReminderSell);
   GlobalVariableSet(pfx+"RemLBuy",    (double)g_audcadLastReminderLevelBuy);
   GlobalVariableSet(pfx+"RemLSell",   (double)g_audcadLastReminderLevelSell);
   // 🆕 v14.19: پشتوانهٔ GV برای Infinite Lock (Drift Guard عمداً persist
   // نمی‌شود — پرچم per-episode است؛ بعد از ری‌استارت EA دوباره از صفر
   // ارزیابی می‌شود که بی‌خطره: فقط ممکنه هشدار کمی دیرتر بیاد، نه دیرتر
   // از حد لازم و نه هرگز).
   GlobalVariableSet(pfx+"LvSinceBuy", (double)g_audcadLevelSinceBuy);
   GlobalVariableSet(pfx+"LvSinceSell",(double)g_audcadLevelSinceSell);
   GlobalVariableSet(pfx+"Saved",      (double)TimeCurrent());
}

// بازگشت: true = لود موفق | false = GV موجود نبود یا stale بود — همان
// قرارداد EURGBP_GV_Load، با schema مستقل HM_AUDCAD_SCHEMA_VERSION.
bool AUDCAD_GV_Load()
{
   string pfx = Rule_GVPrefix("AUDCAD");
   if(!GlobalVariableCheck(pfx+"Saved")) return false;
   ...
   double savedVer = GlobalVariableGet(pfx+"Ver");
   if(savedVer < HM_AUDCAD_SCHEMA_VERSION)
   {
      Print("ℹ️ v14.19 AUDCAD_GV_Load: نسخهٔ GV قدیمی (", savedVer, " < ", HM_AUDCAD_SCHEMA_VERSION,
            ") — نادیده گرفته شد، Replay کامل اجرا می‌شود");
      return false;
   }
   if(GlobalVariableCheck(pfx+"PerSec"))
   {
      ...
      if(savedPerSec != g_chartPeriodSeconds)
      {
         Print("ℹ️ v14.19 AUDCAD_GV_Load: periodSeconds تغییر کرده — نادیده گرفته شد، Replay کامل اجرا می‌شود");
         return false;
      }
   }
   ...
   g_audcadLevelBuy        = (int)GlobalVariableGet(pfx+"LvBuy");
   g_audcadLevelSell       = (int)GlobalVariableGet(pfx+"LvSell");
   g_audcadCleanHBuy       = (int)GlobalVariableGet(pfx+"ClHBuy");
   g_audcadCleanHSell      = (int)GlobalVariableGet(pfx+"ClHSell");
   g_audcadCrisisRedHBuy   = (int)GlobalVariableGet(pfx+"RdHBuy");
   g_audcadCrisisRedHSell  = (int)GlobalVariableGet(pfx+"RdHSell");
   g_audcadLastH           = (datetime)GlobalVariableGet(pfx+"LastH");
   g_audcadStopReasonBuy   = (int)GlobalVariableGet(pfx+"SRBuy");
   g_audcadStopReasonSell  = (int)GlobalVariableGet(pfx+"SRSell");
   g_audcadCrisisBuy       = GlobalVariableCheck(pfx+"CrBuy")  ? (int)GlobalVariableGet(pfx+"CrBuy")  : -1;
   g_audcadCrisisSell      = GlobalVariableCheck(pfx+"CrSell") ? (int)GlobalVariableGet(pfx+"CrSell") : -1;
   g_audcadSpikeBuy        = GlobalVariableCheck(pfx+"SpBuy")  ? (int)GlobalVariableGet(pfx+"SpBuy")  : -1;
   g_audcadSpikeSell       = GlobalVariableCheck(pfx+"SpSell") ? (int)GlobalVariableGet(pfx+"SpSell") : -1;
   g_audcadHourRedBuy      = GlobalVariableCheck(pfx+"HRedBuy")   ? (GlobalVariableGet(pfx+"HRedBuy")   != 0.0) : (g_audcadCrisisBuy  == 2);
   g_audcadHourDirtyBuy    = GlobalVariableCheck(pfx+"HDirtyBuy") ? (GlobalVariableGet(pfx+"HDirtyBuy") != 0.0) : (g_audcadCrisisBuy  == 2 || g_audcadSpikeBuy  >= 1);
   g_audcadHourRedSell     = GlobalVariableCheck(pfx+"HRedSell")  ? (GlobalVariableGet(pfx+"HRedSell")  != 0.0) : (g_audcadCrisisSell == 2);
   g_audcadHourDirtySell   = GlobalVariableCheck(pfx+"HDirtySell")? (GlobalVariableGet(pfx+"HDirtySell") != 0.0): (g_audcadCrisisSell == 2 || g_audcadSpikeSell >= 1);
   g_audcadSpikeStreakBuy      = GlobalVariableCheck(pfx+"SpkStrBuy")   ? (int)GlobalVariableGet(pfx+"SpkStrBuy")    : 0;
   g_audcadSpikeStreakSell     = GlobalVariableCheck(pfx+"SpkStrSell")  ? (int)GlobalVariableGet(pfx+"SpkStrSell")   : 0;
   g_audcadCleanSinceBuy       = GlobalVariableCheck(pfx+"ClSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"ClSinceBuy") : 0;
   g_audcadCleanSinceSell      = GlobalVariableCheck(pfx+"ClSinceSell") ? (datetime)GlobalVariableGet(pfx+"ClSinceSell"): 0;
   g_audcadRedSinceBuy         = GlobalVariableCheck(pfx+"RdSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"RdSinceBuy") : 0;
   g_audcadRedSinceSell        = GlobalVariableCheck(pfx+"RdSinceSell") ? (datetime)GlobalVariableGet(pfx+"RdSinceSell"): 0;
   g_audcadLastDirtyTimeBuy    = GlobalVariableCheck(pfx+"DrtTBuy")     ? (datetime)GlobalVariableGet(pfx+"DrtTBuy")    : 0;
   g_audcadLastDirtyTimeSell   = GlobalVariableCheck(pfx+"DrtTSell")    ? (datetime)GlobalVariableGet(pfx+"DrtTSell")   : 0;
   g_audcadLastDirtyReasonBuy  = GlobalVariableCheck(pfx+"DrtRBuy")     ? (int)GlobalVariableGet(pfx+"DrtRBuy")      : 0;
   g_audcadLastDirtyReasonSell = GlobalVariableCheck(pfx+"DrtRSell")    ? (int)GlobalVariableGet(pfx+"DrtRSell")     : 0;
   g_audcadLastReminderBuy      = GlobalVariableCheck(pfx+"RemTBuy")  ? (datetime)GlobalVariableGet(pfx+"RemTBuy")  : 0;
   g_audcadLastReminderSell     = GlobalVariableCheck(pfx+"RemTSell") ? (datetime)GlobalVariableGet(pfx+"RemTSell") : 0;
   g_audcadLastReminderLevelBuy = GlobalVariableCheck(pfx+"RemLBuy")  ? (int)GlobalVariableGet(pfx+"RemLBuy")      : 0;
   g_audcadLastReminderLevelSell= GlobalVariableCheck(pfx+"RemLSell") ? (int)GlobalVariableGet(pfx+"RemLSell")     : 0;
   g_audcadLevelSinceBuy        = GlobalVariableCheck(pfx+"LvSinceBuy") ? (datetime)GlobalVariableGet(pfx+"LvSinceBuy") : (g_audcadLevelBuy  > 0 ? TimeCurrent() : 0);
   g_audcadLevelSinceSell       = GlobalVariableCheck(pfx+"LvSinceSell")? (datetime)GlobalVariableGet(pfx+"LvSinceSell"): (g_audcadLevelSell > 0 ? TimeCurrent() : 0);
   return true;
}

// AUDCAD_InitReplay — معادل EURGBP_InitReplay، با یک تفاوت عمدی: از
// inp_AUDCAD_CooldownBars مستقل استفاده می‌کند (نه inp_SpikeCooldownBars
// مشترک GBPNZD/EURGBP).
void AUDCAD_InitReplay()
{
   if(!IsRuleSymbol(_Symbol)) return;
   if((bool)MQLInfoInteger(MQL_TESTER)) return;

   if(g_audcadHandleEMA200_H1 == INVALID_HANDLE ||
      g_audcadHandleATR_H1    == INVALID_HANDLE ||
      g_audcadHandleADX_H4    == INVALID_HANDLE ||
      g_audcadHandleATR_D1    == INVALID_HANDLE) return;

   if(BarsCalculated(g_audcadHandleEMA200_H1) < 290) return;
   if(BarsCalculated(g_audcadHandleADX_H4)    < 25)  return;

   bool gvLoaded = AUDCAD_GV_Load();
   if(gvLoaded)
   {
      g_audcadReplayDone = true;
      Print(StringFormat(
         "✅ v14.19 AUDCAD state loaded from GlobalVariable (versioned) — Buy=%s CleanH=%d | Sell=%s CleanH=%d",
         g_audcadLevelBuy  == 0 ? "GREEN" : g_audcadLevelBuy  == 1 ? "STOP" : "CLOSE", g_audcadCleanHBuy,
         g_audcadLevelSell == 0 ? "GREEN" : g_audcadLevelSell == 1 ? "STOP" : "CLOSE", g_audcadCleanHSell));
      return;
   }

   g_audcadCrisisRedHBuy = 0; g_audcadCrisisRedHSell = 0;
   g_audcadCleanHBuy     = 0; g_audcadCleanHSell     = 0;
   g_audcadLevelBuy      = 0; g_audcadLevelSell      = 0;
   g_audcadStopReasonBuy = -1; g_audcadStopReasonSell = -1;

   int _replayPeriodSec  = (g_chartPeriodSeconds > 0) ? g_chartPeriodSeconds : 3600;
   int _replayBarsToScan = (int)MathMax(1, 259200 / _replayPeriodSec);   // ۷۲ ساعت — همان پنجرهٔ GBPNZD/EURGBP
   ...
   for(int sh = _replayBarsToScan; sh >= 1; sh--)
   {
      double flowBuyOut = 0.0, adxBuyOut = 0.0;  int rcBuyOut = 0;
      double flowSellOut= 0.0, adxSellOut= 0.0;  int rcSellOut= 0;
      // 🆕 v14.20 [SUFFIX-FIX]: g_audcadBrokerSymbol به‌جای رشتهٔ خام "AUDCAD"
      int crBuy  = Generic_ReplayCrisisAtBar(g_audcadBrokerSymbol, g_audcadHandleEMA200_H1, g_audcadHandleATR_H1,
                                              g_audcadHandleADX_H4, g_audcadHandleATR_D1,
                                              sh, true,  flowBuyOut,  adxBuyOut,  rcBuyOut);
      int crSell = Generic_ReplayCrisisAtBar(g_audcadBrokerSymbol, g_audcadHandleEMA200_H1, g_audcadHandleATR_H1,
                                              g_audcadHandleADX_H4, g_audcadHandleATR_D1,
                                              sh, false, flowSellOut, adxSellOut, rcSellOut);

      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar(g_audcadBrokerSymbol, sh, spikeBuyOut, spikeSellOut);

      bool haBuy  = (adxBuyOut  >= 35.0) && (flowBuyOut  < -7.0);
      bool haSell = (adxSellOut >= 35.0) && (flowSellOut < -7.0);

      datetime replayEvalTime = iTime(g_audcadBrokerSymbol, PERIOD_H1, sh + 1);
      ...
      snapReplayBuy.adx = adxBuyOut; snapReplayBuy.flow = flowBuyOut;
      HMRuleState stReplayBuy;
      AUDCAD_HMRuleState_LoadBuy(stReplayBuy);
      ApplyClosedBarToRule(snapReplayBuy, false, false, inp_AUDCAD_CooldownBars, replayEvalTime, stReplayBuy);
      AUDCAD_HMRuleState_SaveBuy(stReplayBuy);
      ...
      snapReplaySell.adx = adxSellOut; snapReplaySell.flow = flowSellOut;
      HMRuleState stReplaySell;
      AUDCAD_HMRuleState_LoadSell(stReplaySell);
      ApplyClosedBarToRule(snapReplaySell, false, false, inp_AUDCAD_CooldownBars, replayEvalTime, stReplaySell);
      AUDCAD_HMRuleState_SaveSell(stReplaySell);
      ...
      g_audcadCrisisBuy = crBuy; g_audcadCrisisSell = crSell;
      g_audcadSpikeBuy  = spikeBuyOut; g_audcadSpikeSell = spikeSellOut;
   }

   g_audcadHourRedBuy    = rHourRedBuy;
   g_audcadHourDirtyBuy  = rHourDirtyBuy;
   g_audcadHourRedSell   = rHourRedSell;
   g_audcadHourDirtySell = rHourDirtySell;

   g_audcadLastH = iTime(g_audcadBrokerSymbol, PERIOD_CURRENT, 1);  // 🆕 v14.20 [SUFFIX-FIX]

   g_audcadPrevCrisisBuy  = g_audcadCrisisBuy;
   g_audcadPrevSpikeBuy   = g_audcadSpikeBuy;
   g_audcadPrevCrisisSell = g_audcadCrisisSell;
   g_audcadPrevSpikeSell  = g_audcadSpikeSell;

   // 🆕 v14.19: مقداردهی اولیهٔ Infinite Lock بعد از Replay کامل — اگه
   // سطحی همچنان >0 باشه، از همین لحظه شمارش شروع می‌شه (محافظه‌کارانه:
   // ممکنه هشدار کمی دیرتر بیاد، هرگز زودتر از واقعیت نه).
   if(g_audcadLevelBuy  > 0 && g_audcadLevelSinceBuy  == 0) g_audcadLevelSinceBuy  = TimeCurrent();
   if(g_audcadLevelSell > 0 && g_audcadLevelSinceSell == 0) g_audcadLevelSinceSell = TimeCurrent();

   g_audcadReplayDone = true;
   AUDCAD_GV_Save();

   Print(StringFormat(
      "✅ v14.19 AUDCAD_InitReplay done (full Replay) — Buy=%s CleanH=%d | Sell=%s CleanH=%d",
      g_audcadLevelBuy  == 0 ? "GREEN" : g_audcadLevelBuy  == 1 ? "STOP" : "CLOSE", g_audcadCleanHBuy,
      g_audcadLevelSell == 0 ? "GREEN" : g_audcadLevelSell == 1 ? "STOP" : "CLOSE", g_audcadCleanHSell));
}

void AUDCAD_CheckReminderForDirection(bool forBuy, int level, int cleanH,
                                       datetime &lastReminderTime, int &lastReminderLevel,
                                       datetime nowR)
{
   if(level <= 0)
   ...
   // 🔧 v14.20.2: از Rule_ReminderIsDue مشترک (اسلات‌بندی‌شده) استفاده می‌شود —
   // به‌جای منطق تکراریِ قبلی، تا AUDCAD هم از رفع باگ «ساعت‌های نامنظم» بهره ببرد.
   if(!Rule_ReminderIsDue(level, lastReminderTime, nowR)) return;
   ...
   // 🆕 v14.23 [FOREIGN-RULE-GATE]: تایمر/state بالا همیشه به‌روزرسانی
   // می‌شود؛ فقط ارسال واقعیِ پیام طبق اینپوت جدید گیت می‌شود — چون AUDCAD
   // این‌جا همیشه سمبل «خارجی» نسبت به چارت جاری است.
   if(!Alert_ShouldSendRuleMsgFor(g_audcadBrokerSymbol)) return;

   // 🆕 v14.23 [MSG-REDESIGN]: پوزیشن این جهت جداگانه چک می‌شود (نه کل سمبل)
   bool noRealPositionsDir = !HasRealPositionOnSymbolDir(g_audcadBrokerSymbol, forBuy);
   string dirLbl  = forBuy ? "خرید" : "فروش";
   string durStrA = SQ_FormatDurationBars(cleanH, g_chartPeriodSeconds);

   // 🔧 v14.23: سربرگ استاندارد «⏰ Rule Reminder :» — هرگز کلمهٔ Crisis
   // داخل این یادآور نمی‌آید.
   string headline = (level == 2) ? "یادآوری می‌کنم وضعیت هنوز بده" : "یادآوری می‌کنم ریسک هنوز بالاست";
   string tailMsg = (level == 2)
      ? (noRealPositionsDir ? "بازار هنوز خیلی پرخطره پس معاملهٔ جدید باز نکن" : "هنوز نبستی؟ الان ببند")
      : (noRealPositionsDir ? "بازار همچنان پرخطره پس هنوز معاملهٔ جدید باز نکن" : "هنوز معاملهٔ جدید باز نکن");
   string remMsg = "⏰ Rule Reminder :\n"
      + headline + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + "AUDCAD | " + TimeToString(nowR, TIME_DATE|TIME_MINUTES) + "\n"
      + dirLbl + ": " + ((level==2)?"🔴 بستن بهتره":"⛔ توقف ورود") + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + "نکته: بیشتر حواست به چارت و تغییر وضعیت Rule باشه\n"
      + "نکته: " + tailMsg + "\n"
      + "نکته: خبر خوب اینکه " + durStrA + " آرامش پیاپی تا الان ثبت شده";
   Alert_Send(remMsg);
}

void AUDCAD_CheckReminders()
{
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!Alert_OnGBPNZDRule)                return;   // سوییچ مشترک — طبق همون تصمیم مستند EURGBP
   if(!Alert_GBPNZDRule_Repeat)            return;
   if(!IsRuleSymbol(_Symbol))              return;

   datetime nowR = TimeCurrent();
   AUDCAD_CheckReminderForDirection(true,  g_audcadLevelBuy,  g_audcadCleanHBuy,
                                     g_audcadLastReminderBuy, g_audcadLastReminderLevelBuy, nowR);
   AUDCAD_CheckReminderForDirection(false, g_audcadLevelSell, g_audcadCleanHSell,
                                     g_audcadLastReminderSell, g_audcadLastReminderLevelSell, nowR);
}

// Alert_CheckAUDCADRule — معادل Alert_CheckEURGBPRule برای AUDCAD.
void Alert_CheckAUDCADRule()
{
   if(!Alert_OnGBPNZDRule)                return;   // سوییچ مشترک
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!IsRuleSymbol(_Symbol))             return;

   datetime lastQuoteTimeA = (datetime)SymbolInfoInteger(g_audcadBrokerSymbol, SYMBOL_TIME);  // 🆕 v14.20 [SUFFIX-FIX]
   int  _weekendStaleThresholdA = (int)MathMax(2 * 3600, 3 * g_chartPeriodSeconds);
   bool audcadMarketStale = (lastQuoteTimeA > 0) && ((TimeCurrent() - lastQuoteTimeA) > _weekendStaleThresholdA);

   bool barBoundaryReachedA = (g_audcadLastH > 0) &&
                              (g_lastBarCloseTime > g_audcadLastH) &&
                              !audcadMarketStale;

   int oldLevelBuy  = g_audcadLevelBuy;
   int oldLevelSell = g_audcadLevelSell;

   HMDirSnapshot snapBuy, snapSell;
   bool haveSnap = false;

   if(barBoundaryReachedA)
   {
      double flowBuyOut = 0.0, adxBuyOut = 0.0;  int rcBuyOut = 0;
      double flowSellOut= 0.0, adxSellOut= 0.0;  int rcSellOut= 0;
      // 🆕 v14.20 [SUFFIX-FIX]: g_audcadBrokerSymbol به‌جای رشتهٔ خام
      int crBuy  = Generic_ReplayCrisisAtBar(g_audcadBrokerSymbol, g_audcadHandleEMA200_H1, g_audcadHandleATR_H1,
                                              g_audcadHandleADX_H4, g_audcadHandleATR_D1,
                                              1, true,  flowBuyOut,  adxBuyOut,  rcBuyOut);
      int crSell = Generic_ReplayCrisisAtBar(g_audcadBrokerSymbol, g_audcadHandleEMA200_H1, g_audcadHandleATR_H1,
                                              ...
                                              1, false, flowSellOut, adxSellOut, rcSellOut);
      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar(g_audcadBrokerSymbol, 1, spikeBuyOut, spikeSellOut);

      datetime evalTimeA = iTime(g_audcadBrokerSymbol, PERIOD_H1, 2);

      snapBuy.valid = true;  snapBuy.crisis = crBuy;  snapBuy.spike = spikeBuyOut;
      snapBuy.highAlert  = (adxBuyOut  >= 35.0) && (flowBuyOut  < -7.0);
      snapBuy.evalBarTime = evalTimeA; snapBuy.adx = adxBuyOut; snapBuy.flow = flowBuyOut;
      snapBuy.rc = rcBuyOut;   // 🆕 v14.23 [MSG-REDESIGN]: برای نکتهٔ «X از ۴ چراغ قرمز»

      snapSell.valid = true; snapSell.crisis = crSell; snapSell.spike = spikeSellOut;
      snapSell.highAlert = (adxSellOut >= 35.0) && (flowSellOut < -7.0);
      snapSell.evalBarTime = evalTimeA; snapSell.adx = adxSellOut; snapSell.flow = flowSellOut;
      snapSell.rc = rcSellOut;   // 🆕 v14.23 همان دلیل بالا
      haveSnap = true;

      HMRuleState stBuy;
      AUDCAD_HMRuleState_LoadBuy(stBuy);
      ApplyClosedBarToRule(snapBuy, false, false, inp_AUDCAD_CooldownBars, snapBuy.evalBarTime, stBuy);
      AUDCAD_HMRuleState_SaveBuy(stBuy);

      HMRuleState stSell;
      AUDCAD_HMRuleState_LoadSell(stSell);
      ApplyClosedBarToRule(snapSell, false, false, inp_AUDCAD_CooldownBars, snapSell.evalBarTime, stSell);
      AUDCAD_HMRuleState_SaveSell(stSell);

      g_audcadCrisisBuy = crBuy; g_audcadCrisisSell = crSell;
      g_audcadSpikeBuy  = spikeBuyOut; g_audcadSpikeSell = spikeSellOut;
      g_audcadLastH = g_lastBarCloseTime;
      AUDCAD_GV_Save();
   }

   if(g_audcadLevelBuy == oldLevelBuy && g_audcadLevelSell == oldLevelSell) return;

   int newLevelBuy   = g_audcadLevelBuy;
   int newLevelSell  = g_audcadLevelSell;

   // 🆕 [AUDCAD-INFINITE-LOCK] v14.19: ثبت/ریست زمان شروع قفل هر جهت —
   // مصرف‌کننده: AUDCAD_CheckInfiniteLock (فراخوانی‌شده از OnTimer).
   if(oldLevelBuy == 0 && newLevelBuy > 0)   g_audcadLevelSinceBuy  = TimeCurrent();
   if(newLevelBuy == 0)                      g_audcadLevelSinceBuy  = 0;
   if(oldLevelSell == 0 && newLevelSell > 0) g_audcadLevelSinceSell = TimeCurrent();
   if(newLevelSell == 0)                     g_audcadLevelSinceSell = 0;

   // 🆕 v14.23 [MSG-REDESIGN]: چک جهت‌دار جایگزین چک کلی قبلی شد — اگه فقط
   // پوزیشن یک جهت باز باشه، پیام جهت دیگه اشتباه نگه.
   bool noPosBuyDirA  = !HasRealPositionOnSymbolDir(g_audcadBrokerSymbol, true);
   bool noPosSellDirA = !HasRealPositionOnSymbolDir(g_audcadBrokerSymbol, false);
   string timeStrA = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);
   ...
   string spBuyTxt  = (spBuyDisp==2)?"🔴":(spBuyDisp==1)?"🟠":(spBuyDisp==0)?"🟢":"⚪";
   string spSellTxt = (spSellDisp==2)?"🔴":(spSellDisp==1)?"🟠":(spSellDisp==0)?"🟢":"⚪";
   string detailLineA = "خرید:  Crisis " + crBuyTxt  + "   Spike " + spBuyTxt  + "\n"
                       + "فروش: Crisis " + crSellTxt + "   Spike " + spSellTxt;
   ...
   if(newLevelBuy > oldLevelBuy || newLevelSell > oldLevelSell)
   {
      ...
      if(newLevel == 1)
      {
         bool buyIsNewStopA  = (newLevelBuy  == 1 && oldLevelBuy  < 1);
         bool sellIsNewStopA = (newLevelSell == 1 && oldLevelSell < 1);
         string noPosNoteA = "";
         if(buyIsNewStopA && sellIsNewStopA && noPosBuyDirA && noPosSellDirA)
            noPosNoteA = "\nنکته: پوزیشن بازی نداری (نه خرید، نه فروش) — فقط معاملهٔ جدید باز نکن";
         else if(buyIsNewStopA && noPosBuyDirA)
            noPosNoteA = "\nنکته: پوزیشن خریدی نداری — فقط معاملهٔ خرید جدید باز نکن";
         else if(sellIsNewStopA && noPosSellDirA)
            noPosNoteA = "\nنکته: پوزیشن فروشی نداری — فقط معاملهٔ فروش جدید باز نکن";
      ...
      {
         string closeNoteBuy  = (newLevelBuy  == 2) ? StringFormat("  (%d ساعت پیاپی)", g_audcadCrisisRedHBuy)  : "";
         string closeNoteSell = (newLevelSell == 2) ? StringFormat("  (%d ساعت پیاپی)", g_audcadCrisisRedHSell) : "";

         // 🆕 v14.23 [MSG-REDESIGN]: rc واقعی (چند از ۴ چراغ قرمزن) به‌جای
         // جملهٔ ثابت قبلی — همون منطق EURGBP، محدودیت مشابه: فقط شمارش در
         // دسترس است، نه این‌که دقیقاً کدام یک از RTM/Trend/Structure/Flow.
         bool buyIsNewCloseA  = (newLevelBuy  == 2);
         bool sellIsNewCloseA = (newLevelSell == 2);
         string rcNotesA = "";
         if(buyIsNewCloseA && haveSnap)
            rcNotesA += StringFormat("نکته: خرید — %d از ۴ چراغ قرمزن (Crisis)\n", snapBuy.rc);
         if(sellIsNewCloseA && haveSnap)
            rcNotesA += StringFormat("نکته: فروش — %d از ۴ چراغ قرمزن (Crisis)\n", snapSell.rc);

         string closeNotesA = "";
         if(buyIsNewCloseA)
            closeNotesA += noPosBuyDirA
               ? "نکته: خرید — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: خرید — پوزیشن رو با هر ضرری ببند\n";
         if(sellIsNewCloseA)
            closeNotesA += noPosSellDirA
               ? "نکته: فروش — پوزیشن باز نداری؛ اگه باز بود الان باید می‌بستیش، فعلاً معاملهٔ جدید باز نکن\n"
               : "نکته: فروش — پوزیشن رو با هر ضرری ببند\n";
   ...
   {
      msg = "✅ Rule :\n"
          + "✅ اوضاع AUDCAD آروم شد — می‌تونی برگردی\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "AUDCAD | " + timeStrA + "\n"
          + "Crisis 🟢   Spike 🟢   (خرید و فروش)\n"
          + "━━━━━━━━━━━━━━━━\n"
   ...
   // 🆕 v14.23 [FOREIGN-RULE-GATE]: state (پایین) همیشه به‌روز می‌شود؛ فقط
   // ارسال واقعی پیام طبق اینپوت جدید گیت می‌شود.
   if(StringLen(msg) > 0 && Alert_ShouldSendRuleMsgFor(g_audcadBrokerSymbol))
      Alert_Send(msg);

   if(newLevelBuy > oldLevelBuy && newLevelBuy > 0)
   { g_audcadLastReminderBuy = TimeCurrent(); g_audcadLastReminderLevelBuy = newLevelBuy; }
   if(newLevelSell > oldLevelSell && newLevelSell > 0)
   { g_audcadLastReminderSell = TimeCurrent(); g_audcadLastReminderLevelSell = newLevelSell; }
   if(newLevelBuy == 0)  { g_audcadLastReminderBuy  = 0; g_audcadLastReminderLevelBuy  = 0; }
   if(newLevelSell == 0) { g_audcadLastReminderSell = 0; g_audcadLastReminderLevelSell = 0; }
}

// ════════════════════════════════════════════════════════════════════
// 🆕 [AUDCAD-DRIFT-GUARD] v14.19 — فقط AUDCAD (پیشنهاد بخش ۴.۱.۶ بریف
// کاربر). چراغ‌های موجود (Crisis/Spike/Flow) روی رفتار لحظه‌ای/ساعتی
// کالیبره‌اند، نه روی «این پوزیشن چقدر باز مونده» — دقیقاً شکافی که
// گذاشت Short آوریل ۲۰۲۶ (−۱۱,۰۵۵ سنت) ۶۶ روز بدون هشدار جداگانه ادامه
// پیدا کنه. این تابع: اگه پوزیشن یک جهت بیش از N روز باز باشه
// (inp_AUDCAD_DriftGuardDays) و همون جهت هنوز STOP/CLOSE باشه، یک
// هشدار تشدیدشدهٔ جداگانه (یک‌بار در هر اپیزود) می‌فرستد.
//
// عمداً ساده: به‌جای اختراع معیار «بدتر از ورود» تازه/تست‌نشده روی Zone،
// از سیگنال Rule موجود (level>0) استفاده می‌کند — از قبل کالیبره‌شده.
// محدودیت شناخته‌شده: فقط در حالت Parallel کار می‌کند (چارت≠AUDCAD)؛
// چون به g_audcad* وابسته است. جزئیات در v14.19_Changes.md بخش ۶.
// ════════════════════════════════════════════════════════════════════
datetime AUDCAD_OldestPositionOpenTime(bool forBuy)
{
   datetime oldest = 0;
   ENUM_POSITION_TYPE wantType = forBuy ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(!HM_PositionBelongsToSymbol(PositionGetSymbol(i), g_audcadBrokerSymbol)) continue;  // 🆕 v14.20 [SUFFIX-FIX]
      if((ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE) != wantType) continue;
...
void AUDCAD_CheckDriftGuardForDirection(bool forBuy, int level, bool &guardFired)
{
   if(level <= 0) { guardFired = false; return; }   // GREEN → ریست پرچم اپیزود
   if(guardFired) return;                             // این اپیزود قبلاً هشدار داده شد

   datetime oldest = AUDCAD_OldestPositionOpenTime(forBuy);
   if(oldest == 0) return;   // پوزیشن واقعی باز نیست — چیزی برای Drift نیست

   double daysOpen = (double)(TimeCurrent() - oldest) / 86400.0;
   if(daysOpen < (double)inp_AUDCAD_DriftGuardDays) return;

   guardFired = true;
   // 🆕 v14.23 [FOREIGN-RULE-GATE]: این هشدار فقط وقتی معنا دارد که AUDCAD
   // سمبل «خارجی» (Parallel) باشد (این تابع فقط زیر g_audcadParallelActive
   // صدا زده می‌شود) — پس همیشه طبق اینپوت جدید گیت می‌شود. guardFired
   // بالا در هر صورت true می‌شود تا این اپیزود دوباره هشدار ندهد، حتی اگه
   // ارسال واقعی الان خاموش باشد.
   if(!Alert_ShouldSendRuleMsgFor(g_audcadBrokerSymbol)) return;
   string dirLbl = forBuy ? "خرید" : "فروش";
   string msg = StringFormat(
      "🐌 Drift :\n"
      "فرسایش کند — پوزیشن AUDCAD خیلی وقته بازه\n"
      "AUDCAD | %s\n"
      "━━━━━━━━━━━━━━━━\n"
      "%s: %d روزه بازه و همچنان %s\n"
      "━━━━━━━━━━━━━━━━\n"
      "نکته: شبیه اتفاق فروش آوریل ۲۰۲۶ (۶۶ روز، ۱۱هزار سنت ضرر) —\n"
      "قبل از طولانی‌تر شدن، دستی بازبینی کن",
      TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES),
      dirLbl, (int)daysOpen, (level == 2 ? "🔴 در وضعیت بستن" : "⛔ در وضعیت توقف"));
   Alert_Send(msg);
}

void AUDCAD_CheckDriftGuard()
{
   if((bool)MQLInfoInteger(MQL_TESTER)) return;
   if(inp_AUDCAD_DriftGuardDays <= 0)   return;   // 0 = غیرفعال
   AUDCAD_CheckDriftGuardForDirection(true,  g_audcadLevelBuy,  g_audcadDriftGuardFiredBuy);
   AUDCAD_CheckDriftGuardForDirection(false, g_audcadLevelSell, g_audcadDriftGuardFiredSell);
}

// ════════════════════════════════════════════════════════════════════
// 🆕 [AUDCAD-INFINITE-LOCK] v14.19 — فقط AUDCAD (بخش ۴.۱.۶ بریف کاربر +
// فلسفهٔ طراحی بخش ۶.۳: «Stop نباید دائمی باشد؛ Resume بیش از ۵-۷ روز =
// Infinite Stop Lock»). g_audcadLevelSinceBuy/Sell (ثبت‌شده در
// Alert_CheckAUDCADRule) از چه زمانی این جهت پیوسته >0 است را نگه
// می‌دارد. اگه از N روز (inp_AUDCAD_InfiniteLockDays) بگذره، یک هشدار
// تشدیدشدهٔ یک‌بار (نه تکراری مثل یادآور معمولی) می‌فرستد.
// ════════════════════════════════════════════════════════════════════
void AUDCAD_CheckInfiniteLockForDirection(bool forBuy, datetime levelSince, bool &lockFired, int level)
{
   if(level <= 0 || levelSince == 0) { lockFired = false; return; }
   if(lockFired) return;

   double daysLocked = (double)(TimeCurrent() - levelSince) / 86400.0;
   if(daysLocked < (double)inp_AUDCAD_InfiniteLockDays) return;

   lockFired = true;
   // 🆕 v14.23 [FOREIGN-RULE-GATE]: مثل Drift Guard — این تابع فقط زیر
   // g_audcadParallelActive صدا زده می‌شود، پس همیشه AUDCAD این‌جا «خارجی»
   // است. lockFired بالا در هر صورت true می‌شود تا دوباره هشدار ندهد.
   if(!Alert_ShouldSendRuleMsgFor(g_audcadBrokerSymbol)) return;
   string dirLbl = forBuy ? "خرید" : "فروش";
   string msg = StringFormat(
      "🔒 Lock :\n"
      "قفل طولانی‌مدت روی AUDCAD\n"
      "AUDCAD | %s\n"
      "━━━━━━━━━━━━━━━━\n"
      "%s: %d روز پیاپی توقف/بستن — بدون برگشت به عادی\n"
      "━━━━━━━━━━━━━━━━\n"
      "نکته: این وضعیت رو دستی بررسی کن — شاید کالیبراسیون یا شرایط بازار عوض شده",
      TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES),
      dirLbl, (int)daysLocked);
   Alert_Send(msg);
}

void AUDCAD_CheckInfiniteLock()
{
   if((bool)MQLInfoInteger(MQL_TESTER))  return;
   if(inp_AUDCAD_InfiniteLockDays <= 0)  return;   // 0 = غیرفعال
   AUDCAD_CheckInfiniteLockForDirection(true,  g_audcadLevelSinceBuy,  g_audcadInfiniteLockFiredBuy,  g_audcadLevelBuy);
   AUDCAD_CheckInfiniteLockForDirection(false, g_audcadLevelSinceSell, g_audcadInfiniteLockFiredSell, g_audcadLevelSell);
}
...
void TL_Update()
{
      ...
      && BarsCalculated(handleATR_H1_TL) > 20)
   {
      ...
      if(ok)
      {
         ...
         // 🆕 v14.24 [RTM-MODE]: در حالت MOMENTUM (پیش‌فرض جدید)، صرف‌نظر از
         // این‌که هندسیاً With-Trend هستیم یا Counter-Trend، همیشه از منطق
         // «دورشدن از EMA200 = ادامهٔ روند (سبز)، نزدیک‌شدن = ریسک برگشت
         // (قرمز)» استفاده می‌شود — دقیقاً همان منطقِ قبلیِ شاخهٔ With-Trend
         // (همان آستانه‌های ۲.۰/۰.۱۵ و ۰.۸/۰.۳۵-)، ولی حالا برای هر دو حالت
         // هندسی اعمال می‌شود. در حالت REVERSION، منطق قدیمی دوشاخه‌ای
         // (With-Trend/Counter-Trend) عیناً بدون کوچک‌ترین تغییر اجرا می‌شود.
         if(inp_RTM_Mode == RTM_MOMENTUM)
         {
            rtmTip = StringFormat(
               "حالت: MOMENTUM %s (ادامهٔ روند)\n"
               "فاصله: %.1f × ATR  %s\n"
               "──────────\n"
               "سبز: فاصله رو به رشد (ادامه)\n"
               "زرد: فاصله ثابت / کم\n"
               "قرمز: فاصله کم‌میشه = ریسک برگشت",
               modeTag, d0, dirArrow);

            if(d0 >= 2.0 && dDel > 0.15)
               newRTM = 0;           // 🟢 سبز: ادامه دارد، فاصله رو به رشد
   ...
   else
   {
      ...
      // 🆕 v14.25 [ZONE-ASYMMETRY] (اختیاری، طبق درخواست کاربر): طبق بخش ۶
      // گزارش کالیبراسیون، Zone مطلق (نه delta از ورود) اثر جهت‌دار دارد —
      // Zone=-1 مطلق در ۳۷٪ ساعات خطرناک Buy دیده شد (در برابر ۴٪ ساعات امن).
      // این یک بررسی *اضافه* روی منطق delta بالاست، نه جایگزین آن. برای
      // Sell، چون داده مستقیم روی Zone=+2 نداشتم، این را با قیاس هندسی
      // (سمت مقابل بازه) اعمال کردم — اگر تجربه چیز دیگری نشون داد، راحت
      // قابل برگشت یا تنظیم است (فقط همین ۴ خط).
      if(forBuy  && zoneNumber == -1) newZOMBIE = 2;  // Buy: پایین‌ترین Zone → قرمز مستقیم
      if(!forBuy && zoneNumber ==  2) newZOMBIE = 2;  // Sell: بالاترین Zone → قرمز مستقیم (قیاسی)
   ...
   // ══════════════════════════════════════════════════════════════
   {
      if(hasPos)
      {
         ...
         // 🆕 v14.24 [KILLER-CALIBRATION]: قبلاً آستانهٔ Red برای هر دو جهت
         // ثابت (۵۰) بود که طبق یافته‌های کالیبراسیون جدید کاربر زیادی
         // سخت‌گیرانه بود. حالا هر جهت آستانهٔ خودش را دارد:
         //   Buy : TrendScore خام ≤ inp_Killer_TrendScore_Buy  (پیش‌فرض -44)
         //   Sell: TrendScore خام ≥ inp_Killer_TrendScore_Sell (پیش‌فرض +11)
         // چون _kTs از قبل با علامت‌عوض‌شده برای Buy محاسبه می‌شود (بالا:
         // forBuy ? -g_csvScore : g_csvScore)، آستانهٔ معادل برای Buy همان
         // قرینهٔ ورودی (-inp...Buy) است؛ برای Sell چون علامت عوض نشده،
         // آستانه مستقیماً همان ورودی است. توجه: آستانهٔ Yellow (۴۰) طبق
         // درخواست صریح کاربر دست‌نخورده ماند — فقط خط مقایسهٔ Red با ۵۰ عوض شد.
         double _kTsThresholdRed = forBuy ? -inp_Killer_TrendScore_Buy : inp_Killer_TrendScore_Sell;
         bool _kRed    = _kTrend && (_kDelta >= 1) && (_kTs >= _kTsThresholdRed) && (_kStep >= 4);
         // 🆕 v14.25 [KILLER-CALIBRATION]: آستانهٔ Yellow حالا جهت‌دار است.
         // Buy طبق درخواست صریح دست‌نخورده روی ۴۰ ماند؛ Sell از ورودی
         // جدید inp_Killer_TrendScore_Yellow_Sell (پیش‌فرض ۱۰) استفاده می‌کند.
         double _kTsThresholdYellow = forBuy ? 40.0 : inp_Killer_TrendScore_Yellow_Sell;
         bool _kYellow = _kTrend && (_kDelta >= 1) && (_kTs >= _kTsThresholdYellow) && (_kStep >= 3) && !_kRed;
         ...
         if(!(bool)MQLInfoInteger(MQL_TESTER) && Alert_OnKiller)
         {
            ...
            // 🔧 v14.23 [MSG-REDESIGN]: پیشوند قدیمی "HelpMe [SYMBOL DIR]" با
            // سربرگ استاندارد جدید "☠️ Killer :" جایگزین شد؛ Buy/Sell هم مثل
            // بقیهٔ پیام‌ها به فارسی نوشته می‌شود، و زمان اضافه شد.
            string _kDirFa = forBuy ? "خرید" : "فروش";
            string _kTime  = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);
            if(_kState == 2 && Alert_CooldownOK(g_lastAlertTime_KILLER, _kEdge))
            {
               Alert_Send(StringFormat(
                  "☠️ Killer :\n"
                  "%s (%s) | %s\n"
                  "پله %d از ۵ — Zone ضرر %d طبقه، روند هم برخلافته 🔴\n"
                  "━━━━━━━━━━━━━━━━\n"
                  "نکته: Score روند برخلافت %+.0f (Trend)\n"
                  "نکته: الان وقت خروجه یا هج کن",
                  CleanSymbol(_Symbol), _kDirFa, _kTime, _kStep, _kDelta, g_csvScore));
            }
            if(_kState == 1 && g_prevAlertKILLER < 1)
            {
               Alert_Send(StringFormat(
                  "☠️ Killer :\n"
                  "%s (%s) | %s\n"
                  "پله %d — Zone ضرر %d طبقه — اگه پله ۴ بشه قرمز میشه 🟠\n"
                  "━━━━━━━━━━━━━━━━\n"
                  "نکته: Score روند %+.0f (Trend)\n"
                  "نکته: چشم باز باش",
                  CleanSymbol(_Symbol), _kDirFa, _kTime, _kStep, _kDelta, g_csvScore));
            }
   ...
   if((bool)MQLInfoInteger(MQL_TESTER))
   {
      ...
      // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
      // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: از HighAlert_Check مشترک — تا
      // لاگ CSV ساعتی هم کالیبراسیون جدید را منعکس کند (برای تحلیل‌های بعدی کاربر)
      bool _btHighAlert = HighAlert_Check(forBuy, bt_adxVal, bt_flowScore);
      LogHourlySnapshot(
         newRTM, newTrend, newStruct, newFLOW, newZOMBIE,
   ...
   // ══════════════════════════════════════════════════════════════
   {
      // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: قبلاً هر دو شاخهٔ forBuy/!forBuy
      // دقیقاً همان شرط هاردکد یکسان را چک می‌کردند (یعنی این if عملاً
      // بی‌اثر بود). حالا واقعاً از تابع مشترک جهت‌دار استفاده می‌شود.
      bool highAlert = HighAlert_Check(forBuy, g_lastAdxVal, g_lastFlowScore);
      ...
      if(highAlert)
      {
         ...
         string alertTxt = (s_alertTick % 2 == 0)
            ? "⚠️ HIGH ALERT ⚠️ "
            : "!! HIGH ALERT !! ";
         // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: تولتیپ حالا آستانهٔ واقعیِ
         // همین جهت را نشان می‌دهد (Buy جدید نامتقارن، Sell قدیمی)
         double _haAdxTh  = forBuy ? inp_HighAlert_ADX_Buy  : 35.0;
         double _haFlowTh = forBuy ? inp_HighAlert_Flow_Buy : -7.0;
         TL_SetLight("HIGHALT", 2, alertTxt,
            StringFormat("High Alert\n"
               "ADX: %.0f (باید زیر %.0f باشه) | Flow: %.1f (آستانه %.1f)\n"
               "━━━━━━━━━━\n"
               "یعنی Crisis داره نزدیک میشه\n"
               "اگه Crisis قرمز شد — آماده خروج باش",
               g_lastAdxVal, _haAdxTh, g_lastFlowScore, _haFlowTh));
      }
      else
      {
         double _haAdxTh  = forBuy ? inp_HighAlert_ADX_Buy  : 35.0;
         TL_SetLight("HIGHALT", 0, "● ALERT: OK    ",
            StringFormat("High Alert: همه چیز عادیه\nADX زیر %.0f یا Flow در محدوده مجاز", _haAdxTh));
      }
   }
   ...
   {
      // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
      // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: از HighAlert_Check مشترک — این
      // مقدار مستقیماً پیام تلگرام «⚠️ High Alert :» را تحریک می‌کند.
      bool _highAlertNow = HighAlert_Check(forBuy, g_lastAdxVal, g_lastFlowScore);
   ...
   string direction, bool highAlertActive = false, int stKILLER = -1)
{
   ...
   if(IsRuleSymbol(_Symbol))
   {
      ...
      if(canUpdate)
      {
         ...
         // 🔧 v14.24 [HIGH-ALERT-CALIBRATION]: از HighAlert_Check مشترک — تا
         // بکتست هم دقیقاً همان کالیبراسیون جدیدی را ببیند که لایو می‌بیند
         // (وگرنه بکتستِ کاربر برای اعتبارسنجی این تغییر گمراه‌کننده می‌شد).
         bool _haBuy  = HighAlert_Check(true,  g_gbnzdAdxBuy,  g_gbnzdFlowBuy);
         bool _haSell = HighAlert_Check(false, g_gbnzdAdxSell, g_gbnzdFlowSell);
         ...
         // 🆕 v14.20 [COOLDOWN-FIX]: GetCooldownBarsForSymbol(_Symbol) — قبلاً همیشه
         // inp_SpikeCooldownBars بود، حتی وقتی Symbol تست AUDCAD بود؛ حالا کامنت بالا
         // («بکتست دقیقاً همان رفتار لایو را می‌بیند») واقعاً برای هر سه ارز درست است.
         ApplyClosedBarToRule(snapBtBuy, false, false, GetCooldownBarsForSymbol(_Symbol), btEvalTime, stBtBuy);
         ...
         ApplyClosedBarToRule(snapBtSell, false, false, GetCooldownBarsForSymbol(_Symbol), btEvalTime, stBtSell);
...
void CreateDashboard()
{
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      CreateLabel(dashboardPrefix+"TitleLbl", tLblX, yPos+(int)(btnH*0.15),
                  "HelpMe v14.24", clrNavy, dashFontSmall);
