...
input int      DashboardScalePercent = 80;  // 📏 اندازه کلی داشبورد (٪) — 50..300 | Dashboard Scale %
                       ...
                       double adxV, double fastMAV, double slowMAV)
{
   ...
   // Part A — on the label object (most important — always visible on hover)
   string tipA = StringFormat(
      "%s signal | Level: %s | Score: %d\n"
      "─ Core filters ─\n"
      "MA Cross     : %s\n"
      "RSI (%.0f)    : %s\n"
      "ADX (%.1f)    : %s\n"
...
{
   if(id == "RTM")    return "Mean Rev";
   if(id == "TREND")  return "Trend";
   if(id == "STRUCT") return "Daily";
   if(id == "FLOW")   return "Flow";
   if(id == "ZOMBIE") return "Zone";
   if(id == "HIGHALT")return "High Alert";
...
void CreateDashboard()
{
   ...
   // ─── BG — بدون bgShift؛ دقیقاً از xStart ──────────────────────
   ...
   ObjectSetInteger(0, bgNm, OBJPROP_XDISTANCE,   xStart);
   ObjectSetInteger(0, bgNm, OBJPROP_YDISTANCE,   yStart - (int)MathRound(4*dpiScale));
   ObjectSetInteger(0, bgNm, OBJPROP_XSIZE,       panelW + padX);
   ...
   // returns the starting X for the first button
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      CreateButton(dashboardPrefix+"ResetBtn",
                   lx, yPos, titleBtnW, btnH, "R", clrWhite, clrDarkSlateGray);
      ObjectSetString(0,dashboardPrefix+"ResetBtn",OBJPROP_TOOLTIP,
         "Reset — all filters & presets back to default");
      // عنوان وسط
      int tLblX = lx + innerW/2 - (int)MathRound(28*dpiScale);
      ...
      CreateButton(dashboardPrefix+"PowerOffBtn",
                   closeX, yPos, titleBtnW, btnH, "X", clrWhite, clrRed);
   ...
   if(ShowPresetButtons)
   {
      ...
      string lbls[] = {"1m",   "5m",   "15m",  "30m",  "1h"};
      string tips[] = {
         "Signal gap: 10 bars (best for M1 chart)\nفاصله سیگنال ۱۰ کندل",
         "Signal gap: 12 bars (best for M5 chart)\nفاصله سیگنال ۱۲ کندل",
         "Signal gap: 15 bars — recommended\nفاصله سیگنال ۱۵ کندل — پیشنهادی",
         "Signal gap: 20 bars (best for M30 chart)\nفاصله سیگنال ۲۰ کندل",
         "Signal gap: 25 bars (best for H1 chart)\nفاصله سیگنال ۲۵ کندل"
      };
   ...
   if(ShowFilterButtons)
   {
      ...
      string lbls[] = {"More",     "Balanced", "Precise"};
      string tips[] = {
         "More signals — fewer filters (good for scalping)\nسیگنال بیشتر — فیلتر کمتر",
         "Recommended — balanced quality & quantity\nپیشنهادی — تعادل کیفیت و کمیت",
         "Fewer, higher-quality signals (good for swing)\nسیگنال کمتر — کیفیت بالاتر"
      };
   ...
   if(ShowAIButtons)
   {
      CreateLabel(dashboardPrefix+"AITitle", lx, yPos, "-- Filters --", clrCyan, dashFont);
      yPos += lineH;

      // row 1: Regime | MTF | Candle Pattern | S/R
      {
         string ns[] = {"Regime",      "MTF",          "Price",          "SR"};
         string ls[] = {"Market Mood", "Multi-TF",     "Candle Pattern", "S/R Levels"};
         bool   as[] = {localEnableMarketRegime, localEnableMTF, localEnablePriceAction, localEnableSR};
         string ts[] = {
            "Market Regime detection (Trend/Range/Volatile)\nتشخیص رژیم بازار",
            "Multi-timeframe confirmation (H1/H4)\nتایید چند تایم‌فریم",
            "Candle pattern filter (Pin Bar, Engulfing)\nفیلتر الگوی کندل",
            "Show Support/Resistance levels on chart\nنمایش سطوح حمایت/مقاومت"
         };
         int rx = ROW_X(ai4W);
         for(int i=0;i<4;i++){
      ...
      // row 2: Smart Volume | Ichimoku | Price Gap | Liquidity
      {
         string ns[] = {"Vol",         "Ichi",       "FVG",          "LiqSwp"};
         string ls[] = {"Smart Vol",   "Ichimoku",   "Price Gap",    "Liquidity"};
         bool   as[] = {localEnableSmartVol, localEnableIchimoku, localEnableFVG, localEnableLiqSwp};
         string ts[] = {
            "Smart Volume filter (removes low-volume signals)\nفیلتر حجم هوشمند",
            "Ichimoku Cloud filter (position confirmation)\nفیلتر ابر ایچیموکو",
      ...
      // row 3: Mean Revert | MA+ (all TF MAs) | EMA+
      {
         ...
         CreateButton(dashboardPrefix+"AIBtn_RTM",   rx,              yPos, aBtnW, btnH,
            "Mean Rev",  clrWhite, localEnableRTM?clrDarkGreen:clrMaroon);
         ObjectSetString(0,dashboardPrefix+"AIBtn_RTM",OBJPROP_TOOLTIP,
            "Mean Reversion (EMA200 distance) — key for Xmoon\nبازگشت به میانگین — مهم‌ترین فیلتر");
         CreateButton(dashboardPrefix+"MABtn_MAPLUS",rx+aBtnW+gap,   yPos, aBtnW, btnH,
            "MA Overlay",clrWhite, maAny?clrCadetBlue:clrDarkSlateGray);
         ObjectSetString(0,dashboardPrefix+"MABtn_MAPLUS",OBJPROP_TOOLTIP,
            "Show MA200 on M15/M30/H1/H4/D1 — click to toggle all\nنمایش MA200 روی تایم‌فریم‌های مختلف");
         CreateButton(dashboardPrefix+"MABtn_FEMA",  rx+2*(aBtnW+gap),yPos,aBtnW, btnH,
            "EMA Lines", clrWhite, g_femaActive?clrMediumSpringGreen:clrDarkSlateGray);
         ObjectSetString(0,dashboardPrefix+"MABtn_FEMA",OBJPROP_TOOLTIP,
            "Show EMA 25/50/100/200 on current chart\nنمایش EMA 25/50/100/200 روی چارت فعلی");
         yPos += btnH + secGap;
      }
   }
   ...
   // ─── MARKET INFO ─────────────────────────────────────────────
   {
      CreateLabel(dashboardPrefix+"InfoTitle",    lx, yPos, "-- Market Info --", clrWhite, dashFont);
      yPos += lineH;
      // Chart trend (جایگزین StatusLabel خارجی)
      CreateLabel(dashboardPrefix+"InfoTrend",    lx, yPos, "Chart Trend: ...",  clrGray,  dashFont);
      yPos += lineH;
      // P/L
      CreateLabel(dashboardPrefix+"InfoPL",       lx, yPos, "",                  clrGray,  dashFont);
      yPos += lineH;
      // Session — با tooltip
      CreateLabel(dashboardPrefix+"SessionStatus",lx, yPos, "Now: ...",           clrGray,  dashFont);
      ObjectSetString(0,dashboardPrefix+"SessionStatus",OBJPROP_TOOLTIP,
         "Active trading session\nTokyo:  00:00-09:00 GMT\nLondon: 08:00-17:00 GMT\nNew York: 13:00-22:00 GMT\nLondon+NY overlap: highest liquidity");
      yPos += lineH;
      // ADX
      CreateLabel(dashboardPrefix+"ADXStatus",    lx, yPos, "ADX: ...",           clrGray,  dashFont);
      yPos += lineH;
      // Spread
      CreateLabel(dashboardPrefix+"SpreadStatus", lx, yPos, "Spread: ...",        clrGray,  dashFont);
      ObjectSetString(0,dashboardPrefix+"SpreadStatus",OBJPROP_TOOLTIP,
         "Live spread:\n0-3 pt  → Green: excellent\n3-12 pt → Gold: acceptable\n>12 pt  → Red: high spread, caution\nاسپرد لحظه‌ای");
      yPos += lineH + secGap;
   }

   // ─── ALERT (چراغ‌ها) ─────────────────────────────────────────
   {
      CreateLabel(dashboardPrefix+"TLTitle",   lx, yPos, "-- Alerts --",          clrYellow, dashFont);
      yPos += lineH;
      int col2X = lx + innerW/2;
      // row 1
      CreateLabel(dashboardPrefix+"TL_RTM",    lx,    yPos, "● Mean Rev: --",    clrDimGray, dashFont);
      CreateLabel(dashboardPrefix+"TL_TREND",  col2X, yPos, "● Trend: --",       clrDimGray, dashFont);
      yPos += lineH;
      // row 2
      CreateLabel(dashboardPrefix+"TL_STRUCT", lx,    yPos, "● Daily: --",       clrDimGray, dashFont);
      CreateLabel(dashboardPrefix+"TL_FLOW",   col2X, yPos, "● Flow: --",        clrDimGray, dashFont);
      yPos += lineH;
      // row 3
      CreateLabel(dashboardPrefix+"TL_ZOMBIE", lx,    yPos, "● Zone: --",        clrDimGray, dashFont);
      CreateLabel(dashboardPrefix+"TL_HIGHALT",col2X, yPos, "● High Alert: --",  clrDimGray, dashFont);
      yPos += lineH;
      // row 4 full width: Crisis
      CreateLabel(dashboardPrefix+"TL_CRISIS", lx,    yPos, "● Crisis: --",      clrDimGray, dashFont);
      yPos += lineH + secGap;
   }
   ...
   // ─── LIQUID LEVEL BUTTON ─────────────────────────────────────
   {
      ...
      int lqX = lx + (innerW - lqW) / 2;
      CreateButton(dashboardPrefix+"LiquidBtn",   lqX, yPos, lqW, lqBtnH,
                   "Liquid Level", clrBlack, clrGold);
      CreateLabel(dashboardPrefix+"LiquidStatus", lqX, yPos+lqBtnH+2,
                   "Inactive — click to activate",  clrDimGray, dashFontSmall);
   }

   // ─── BUY / ALL / SELL — DPI-safe, panel-anchored ─────────────
   {
...
void UpdateDashboard()
{
   ...
   // ── Chart Trend (جایگزین StatusLabel خارجی + InfoPosition) ────
   {
      ...
      if(buy_cnt == 0 && sell_cnt == 0)
      {
         ...
         trendLine  = "Chart Trend: No Position | " + trendDir
                      + " (" + (g_csvScore >= 0 ? "+" : "") + IntegerToString((int)g_csvScore) + ")";
         trendColor = clrDimGray;
      }
      else if(isHedge)
      {
         trendLine  = "Chart Trend: Hedge Active (" + IntegerToString(buy_cnt) + "B / " + IntegerToString(sell_cnt) + "S)";
         trendColor = clrDimGray;
         plLine     = "P/L: " + (pl >= 0 ? "+" : "") + DoubleToString(pl, 2) + " $";
      ...
      else
      {
         ...
         trendLine  = "Chart Trend: " + dirTxt + " | " + trendDir
                      + " (" + (g_csvScore >= 0 ? "+" : "") + IntegerToString((int)g_csvScore) + ")";
