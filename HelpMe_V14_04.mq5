//+------------------------------------------------------------------+
//|                                                HelpMe_V14_04.mq5 |
//|                     Combined EA - Ali Naderi v14.04 (EA)         |
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.04 - Ali Naderi"
...
void OnDemand_RunAndSend()
{
   ...
   // پنجره ۷۲ ساعت (259200 ثانیه) با granularity تایم‌فریم چارت
   // MathMax(72,...): حداقل ۷۲ کندل تضمین — برای تایم‌فریم‌های خیلی بزرگ
...
{
   // زمان شروع کندل جاری (کندل صفر = باز)
   datetime currentBarOpen = iTime(_Symbol, PERIOD_CURRENT, 0);
   // اگه کندل جاری جدیدتر از آخرین محاسبه است → یک کندل کامل بسته شده
   if(currentBarOpen > g_lastBarCloseTime)
   {
      g_lastBarCloseTime = currentBarOpen;
      return true;
   }
   return false;
}
