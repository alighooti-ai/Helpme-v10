//+------------------------------------------------------------------+
//|                                                HelpMe_V14_07.mq5 |
//|                     Combined EA - Ali Naderi v14.07 (EA)         |
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.07 - Ali Naderi"
#property version   "14.07"
// ════════════════════════════════════════════════════════════════════
...
//
// تغییرات اعمال‌نشده (با دلیل):
// ❌ P0 (Alert فوری وسط کندل): طبق مستندات v14.06 BUG-M1، این عمدی است.
//    Rule_Transition هر تیک واکنش می‌دهد ولی counter ها فقط سر کندل آپدیت
//    می‌شوند — این طراحی برای STOP فوری (intra-candle Spike) ضروری است.
//    اضافه کردن input جدید بدون بکتست خطرناک است.
// ❌ P1 (Scale آستانه‌های Rule با TF): آستانه‌های 2/6/24 کندل از بکتست H1
//    کالیبره شده‌اند. تبدیل به ساعت و scale با TF بدون بکتست مجدد خطرناک است.
//    این تغییر معماری بزرگ است و باید جداگانه بکتست شود.
// ❌ P3 (TF-aware GV keys): این تغییر state قدیمی کاربران فعلی را از بین می‌برد.
//    تصمیم migration باید با تیم هماهنگ شود.
// ❌ P4 (InitReplay H1-only برای TF < H1): کد فعلی از PERIOD_CURRENT handle
//    استفاده می‌کند که H1/H4/D1 ثابت است. تغییر loop TF ممکن است counter scale
//    را بشکند. محتاطانه نگه داشتیم.
// ════════════════════════════════════════════════════════════════════
