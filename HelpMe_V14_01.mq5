...
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.01 - Ali Naderi"
...
void OnDemand_RunAndSend()
{
   ...
   for(int h1Shift = 72; h1Shift >= 1; h1Shift--)
   {
      double flowBuy = 0.0, adxBuy = 0.0; int rcBuy = 0;
      double flowSell = 0.0, adxSell = 0.0; int rcSell = 0;
      int crBuy  = Generic_ReplayCrisisAtBar(broker, g_odHandleEMA200, g_odHandleATR_H1, g_odHandleADX_H4, g_odHandleATR_D1, h1Shift, true,  flowBuy,  adxBuy,  rcBuy);
      int crSell = Generic_ReplayCrisisAtBar(broker, g_odHandleEMA200, g_odHandleATR_H1, g_odHandleADX_H4, g_odHandleATR_D1, h1Shift, false, flowSell, adxSell, rcSell);

      int spBuyDum = -1, spSellDum = -1;
      Generic_ReplaySpikeAtBar(broker, h1Shift, spBuyDum, spSellDum);
      // 🩹 مثل GBPNZD_InitReplay اصلی: داده‌ی ناقص = خنثی (0)، نه continue/skip —
      // continue کردن باعث می‌شد CleanH برای آن ساعت اصلاً آپدیت نشود و شمارنده
      // با رفتار واقعی GBPNZD_InitReplay فرق کند (رگرسیون پچ قبلی).
   ...
   string hdr = "📟 وضعیت لحظه‌ای " + clean + " (" + broker + ")\n"
              + "⚠️ اکسپرت روی این سمبل اجرا نیست — محاسبه یک‌باره بر پایه ۷۲ کندل H1 بسته اخیر.\n"
              // 🩹 v14.01: هشدار قبلی («فقط STOP قابل تشخیصه») حذف شد — با فیکس
              // Rule_Transition، STOP/CLOSE/RESUME هر سه از روی کندل‌های بسته
              // قابل تشخیص‌اند. تنها محدودیت واقعی باقی‌مانده accumulator ساعت
              // *نیمه‌کاره‌ی در حال شکل‌گیری* است (فقط با EA زنده روی همان چارت).
              + "⚠️ بدون accumulator ساعت جاری (نیمه‌کاره) — ممکن است چند دقیقه با لایو خودِ سمبل تأخیر داشته باشد.\n"
              + "─────────────────────\n";
...
string HM_HourStateFile(string sym)
{
   ...
   if(c == "GBPNZD") return "HelpMe_GBNZD_HourStateSnapshot.dat";
   return "HelpMe_" + c + "_HourStateSnapshot.dat";
}
...
int Sister_GetTable(string sym, SisterEntry &outArr[])
{
   ...
   ArrayCopy(outArr, SISTER_AUDCAD);   // پیش‌فرض/پایه
