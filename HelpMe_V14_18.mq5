//+------------------------------------------------------------------+
//|                                                HelpMe_V14_18.mq5 |
//|                     Combined EA - Ali Naderi v14.18 (EA)         |
//+------------------------------------------------------------------+
#property copyright "HelpMe v14.18 - Ali Naderi"
#property version   "14.18"
// ════════════════════════════════════════════════════════════════════
// 📋 v14.18 | 2026-07-14 — سازگاری واقعی با هر سه چارت (تک‌سیمبلی)
//    کاربر تأیید کرد هرگز اکسپرت را هم‌زمان روی دو یا چند چارت اجرا
//    نمی‌کند، ولی می‌خواهد وقتی EA مستقیماً روی چارت GBPNZD یا EURGBP یا
//    AUDCAD به‌تنهایی باز شود، همان چارت به‌درستی به‌عنوان سیمبل اصلی کار
//    کند. بررسی کد نشان داد موتور اصلی Rule/Replay/GV/Dashboard از v14.00
//    از قبل کاملاً بر مبنای _Symbol عمومی بود (نه هاردکد GBPNZD) — فقط
//    یک گیت واقعی این تعمیم را نگرفته بود (زیر). جزئیات کامل در
//    v14.18_Changes.md.
//
// 🐛 [PORTABILITY-GATE-FIX] رفع گیت هاردکد در StatusQuery_BuildReport
//    این تابع فقط GBPNZD را قبول می‌کرد؛ شرط از StringFind(sym,"GBPNZD")
//    به IsRuleSymbol(sym) (تابع عمومی موجود) تغییر کرد + متن پیام خطا
//    اصلاح شد. این تنها گیت واقعی بود که با جست‌وجوی کامل فایل پیدا شد؛
//    UpdateDashboard، CreateDashboard، OnInit، مسیر تلگرام status/on-demand
//    همگی صفر ارجاع هاردکد به GBPNZD داشتند (بررسی مستقیم، نه فرض).
//
// 🆕 [EURGBP-PARALLEL-GUARD] جلوگیری از پردازش دوگانه/تداخل
//    بلوک Stateful مستقل EURGBP (v14.17) اکنون فقط وقتی چارت خودش
//    EURGBP نباشد فعال می‌شود (پرچم g_eurgbpParallelActive، یک‌بار در
//    OnInit محاسبه می‌شود). دلیل: اگر همین فایل مستقیم روی چارت EURGBP
//    باز شود، موتور عمومی Alert_CheckGBPNZDRule (از طریق _Symbol) خودش
//    به‌درستی EURGBP را مدیریت می‌کند؛ بدون این guard، بلوک موازی v14.17
//    هم‌زمان و روی همان GV key ها ("HM_EURGBP_") کار می‌کرد — پردازش
//    تکراری + ریسک رقابت در نوشتن GV. GBPNZD/AUDCAD تحت تأثیر قرار
//    نمی‌گیرند (این guard فقط این‌که «بلوک EURGBP جدید کی روشن شود» را
//    کنترل می‌کند، نه هیچ منطق GBPNZD).
//
// ⚪ عمداً انجام‌نشده — تغییر نام GBPNZD_* / g_gbpnzd* / Alert_CheckGBPNZDRule
//    کاربر اجازه داد در صورت لزوم اسم GBPNZD برداشته شود. تصمیم گرفتم این
//    کار را نکنم چون کاملاً آرایشی است (صفر تأثیر رفتاری — این توابع از
//    قبل با _Symbol کار می‌کنند) در حالی که rename کامل (ده‌ها ارجاع در
//    فایلی با ۱۷هزار خط، بدون کامپایلر برای تأیید) ریسک واقعی و غیرضروری
//    اضافه می‌کند. به‌جای آن، کامنت توضیحی روشن بالای هر تابع اضافه شد که
//    دقیقاً همان هدف «کسی گمراه نشود» را با ریسک عملاً صفر تأمین می‌کند.
//    توضیح کامل و گزینهٔ انجامش در آینده در v14.18_Changes.md.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.17 | 2026-07-13 — معماری Stateful مستقل EURGBP (Rule/Cooldown/GV)
//    تأیید کاربر روی گزارش v14.16: Crisis ADX برای EURGBP اعمال شد
//    (فقط شاخهٔ EURGBP در Crisis_GetThresholds — GBPNZD دست‌نخورده) +
//    ساخت کامل معماری Stateful مستقل برای EURGBP، موازی با GBPNZD.
//    GBPNZD صفر خط تغییر کرد (تأیید‌شده با diff کامل). جزئیات کامل،
//    از جمله ۲ تصحیح فنی مهم نسبت به بریف کاربر و ۲ مورد که عمداً از
//    این فاز کنار گذاشته شدند، در v14.17_Changes.md.
//
// 🆕 [EURGBP-CRISIS-ADX] اعمال Crisis ADX تأییدشده (فقط شاخهٔ EURGBP)
//    در Crisis_GetThresholds(): RedA 20→30, OrA 17→28, OrB 16→26,
//    OrC 23→30, OrD 20→32, Y2 14→24, Y4 17→23, Y5 29→33 (RedB=35
//    بدون تغییر). شاخهٔ GBPNZD در همان تابع byte-for-byte دست‌نخورده.
//
// 🆕 [EURGBP-STATEFUL-ENGINE] معماری Rule/Cooldown/GV مستقل برای EURGBP
//    ۱۹ متغیر state جدید (g_eurgbp*) + ۴ handle اندیکاتور اختصاصی H1/H4/D1
//    + ۷ تابع جدید (EURGBP_GV_Save/Load، EURGBP_HMRuleState_*Buy/Sell،
//    EURGBP_InitReplay، EURGBP_CheckReminderForDirection/Reminders،
//    Alert_CheckEURGBPRule) — همگی کاملاً جدید، هیچ‌کدام جایگزین یا
//    ویرایش هیچ تابع GBPNZD نشدند. موتور مرکزی (ApplyClosedBarToRule،
//    Rule_UpdateCounters، Rule_Transition، BuildDirectionSnapshot،
//    Crisis/Spike/Flow_GetThresholds) که همه از قبل کاملاً generic
//    بودند — بدون هیچ تغییری برای EURGBP هم به‌کار گرفته شدند.
//
// ⚠️ تصحیح فنی مهم #۱ — BuildDirectionSnapshot برای EURGBP از مسیر
//    مستقیم استفاده نشد. بررسی مستقیم کد نشان داد شاخهٔ FOREIGN این
//    تابع فقط وقتی کار می‌کند که on-demand handleهای موقت هم‌زمان
//    دقیقاً روی EURGBP گرم شده باشند — یعنی برای یک موتور Rule پیوسته
//    غیرقابل‌اعتماد است (رجوع به v14.17_Changes.md بخش ۲). به‌جای آن،
//    از Generic_ReplayCrisisAtBar/Generic_ReplaySpikeAtBar (که از قبل
//    کاملاً generic هستند) با handleهای اختصاصی و دائمی EURGBP استفاده
//    شد — دقیقاً همان الگویی که خود GBPNZD_InitReplay برای GBPNZD به کار
//    می‌برد. BuildDirectionSnapshot خودش صفر خط تغییر کرد.
//
// ⚠️ تصحیح فنی مهم #۲ — StatusQuery_BuildReport و UpdateDashboard در
//    این فاز ویرایش نشدند. بررسی کد نشان داد StatusQuery_BuildReport
//    هر سیمبل غیر از GBPNZD را همان ابتدا رد می‌کند (نه یک مسیر
//    FOREIGN on-demand قابل‌جایگزینی همان‌طور که در بریف فرض شده بود)
//    و UpdateDashboard یک پنل تک-سیمبلی مبتنی بر آبجکت چارت است.
//    محافظت واقعی EURGBP (هشدار زنده Telegram روی تغییر STOP/CLOSE/
//    RESUME از طریق Alert_CheckEURGBPRule→Alert_Send) به این دو تابع
//    وابسته نیست و کامل کار می‌کند. جزئیات و پیشنهاد فاز بعد در
//    v14.17_Changes.md.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.16 | 2026-07-13 — کالیبراسیون EURGBP: خروج از Placeholder
//    اجماع ۱۳ تحلیل مستقل روی ۶ ماه دادهٔ واقعی EURGBP. فقط Input
//    Variables مربوط به EURGBP تغییر کردند — هیچ تابعی، هیچ پارامتر
//    GBPNZD/AUDCAD، و هیچ بخشی از معماری Snapshot/Rule/Cooldown دست
//    نخورده است. جزئیات کامل + دو مورد بررسی‌شده‌ولی‌اعمال‌نشده (نیازمند
//    تأیید کاربر) در v14.16_Changes.md.
//
// 🆕 [EURGBP-SPIKE-CALIBRATION] خروج SpikeWarn/SpikeSig از Placeholder
//    inp_EURGBP_SpikeWarn و inp_EURGBP_SpikeSig هر دو از ۱.۲۰ (همان
//    Placeholder مشترک از v14.07) به ۱.۶۰ افزایش یافتند. برخلاف GBPNZD
//    (که Warn/Sig جدا هستند: 1.50/1.90)، این دو آستانه برای EURGBP عمداً
//    برابر نگه داشته شدند: Calc_SpikeDetector فقط زمانی dispPhase را
//    تأیید می‌کند که rawPhase>=1 AND FLOW=Red باشد (با یک استثنای جزئی
//    برای rawPhase==2 + Flow زرد). چون EURGBP (هر دو طرف اروپایی) بندرت
//    FLOW=Red می‌شود، اگر Warn/Sig از هم جدا می‌بودند هر امتیازی که در
//    باند بین آن دو می‌افتاد بی‌صدا Normal ثبت می‌شد — یک «منطقهٔ مرده».
//    برابر نگه‌داشتن این دو آستانه عملاً این باند و این منطقهٔ مرده را
//    حذف می‌کند (تأیید شده مستقیماً از منطق rawPhase در Calc_SpikeDetector).
//
// ⚪ بدون تغییر — تأیید شد که مقادیر فعلی از قبل با تحلیل هم‌خوانی دارند:
//    Flow (FlowGreen=3.0 / FlowRed=-5.0) و Spike Weights (wM15=0.05 /
//    wM30=0.08 / wH1=0.32 / wH4=0.35 / wD1=0.20) برای EURGBP.
//
// ⚠️ بررسی شد، تغییری اعمال نشد — Crisis ADX/Flow/TrendScore (EURGBP)
//    این آستانه‌ها input variable نیستند؛ هاردکد داخل Crisis_GetThresholds()
//    هستند — همان تابعی که آستانه‌های GBPNZD را هم در شاخهٔ دیگری از
//    بدنهٔ خودش محاسبه می‌کند. طبق قانون «دست نزدن به هر تابعی که Crisis
//    را برای GBPNZD محاسبه می‌کند»، این تابع در v14.16 اصلاً لمس نشد.
//    برای اعمال این بخش از کالیبراسیون، تأیید صریح کاربر لازم است.
//
// ⚠️ بررسی شد، تغییری اعمال نشد — inp_SpikeCooldownBars
//    این یک Input مشترک (نه per-symbol) است که از قبل روی ۲ تنظیم شده
//    (نه ۰) و در حال حاضر فقط به GBPNZD وصل است (g_gbpnzdSpikeStreakBuy/
//    Sell + فراخوانی‌های ApplyClosedBarToRule). مسیر Cooldown مستقلی
//    برای EURGBP در کد فعلی وجود ندارد؛ تغییری اعمال نشد.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.15 | 2026-07-12 — ۴ بهبود کم‌ریسک آمادگی تولید (بعد از v14.14)
//    این نسخه ۴ بهبود مستقل و کم‌ریسک را اضافه می‌کند که در بررسی نهایی
//    v14.14 شناسایی شدند. هیچ‌کدام منطق اصلی تصمیم‌گیری Rule، معماری
//    BuildDirectionSnapshot یا آستانه‌های کالیبره‌شده Crisis/Spike را
//    تغییر نمی‌دهند — فقط استحکام و تجربه کاربری را بهتر می‌کنند.
//
// 🆕 P1 [SMART-CLOSE-TEXT] هوشمندسازی متن STOP/CLOSE بدون پوزیشن واقعی
//    Alert_CheckGBPNZDRule و Rule_CheckReminderForDirection قبلاً بدون
//    توجه به وجود پوزیشن واقعی دستور «ببند» می‌دادند — گمراه‌کننده وقتی
//    کاربر خودش پوزیشن را دستی بسته ولی Rule هنوز STOP/CLOSE است (چون
//    HelpMe خودش هیچ معامله‌ای باز/بسته نمی‌کند). تابع کمکی جدید
//    HasRealPositionsOnSymbol (همان فیلتر HM_PositionBelongsToSymbol)
//    اضافه شد؛ بدون پوزیشن واقعی روی نماد، پیام گذار STOP یک خط هشدار
//    اضافه می‌گیرد، بدنه پیام گذار CLOSE و متن یادآور دوره‌ای CLOSE از
//    «ببند» به هشدار «معامله جدید باز نکن» تغییر می‌کند.
//
// 🆕 P2 [OD-TIME-ALIGN] بررسی هم‌ترازی زمانی در OnDemand (نماد خارجی)
//    OnDemand_RunAndSend حالا زمان آخرین کندل بستهٔ نماد خارجی را با
//    نماد اصلی مقایسه می‌کند (iTime shift=1، تلرانس یک کندل چارت جاری).
//    اگه اختلاف بیش از یک کندل باشه (بروکر هنوز نماد خارجی را sync
//    نکرده)، هشدار «هم‌تراز نیست» به سربرگ گزارش On-Demand اضافه می‌شود.
//    فقط نمایشی — روی محاسبات Rule اصلی اثر ندارد.
//
// 🆕 P3 [LAST-UPDATE-LABEL] نمایش زمان آخرین به‌روزرسانی روی داشبورد
//    لیبل جدید LastUpdateLabel (زیر بخش Direction) اضافه شد — زمان
//    آخرین کندل بسته‌شده (g_lastBarCloseTime، همان مرجعی که
//    Chart_IsNewBarClosed تنظیم می‌کند) را نشان می‌دهد تا کاربر سریع
//    بفهمد EA زنده است یا داده‌ها منجمد شده‌اند. صرفاً نمایشی.
//
// 🆕 P4 [FORCE-RESET-RULE] دکمه ریست اجباری ماشین Rule
//    دکمه جدید ForceResetRuleBtn (زیر بخش Direction، متن OrangeRed روی
//    زمینه DarkSlateGray برای احتیاط بصری) اضافه شد. برخلاف ResetBtn
//    «R» که فقط تنظیمات نمایشی (Preset/FilterMode/Direction) را ریست
//    می‌کند، این دکمه همهٔ کلیدهای GlobalVariable حافظهٔ Rule GBPNZD
//    (هر دو جهت — دقیقاً همان ۳۸ کلیدی که GBPNZD_GV_Save می‌نویسد) و
//    globalهای RAM متناظر را پاک می‌کند — شامل فیلدهای پشتیبان
//    HMRuleState (spikeStreak/cleanSince/redSince/lastDirty*) که در غیر
//    این صورت اولین کندل Replay را با streak/since کهنه seed می‌کردند
//    (HMRuleState_LoadBuy/LoadSell این‌ها را از همین globalهای مسطح
//    hydrate می‌کنند) — سپس GBPNZD_InitReplay را برای بازسازی کامل از
//    ۷۲ ساعت گذشته صدا می‌زند؛ دقیقاً همان مسیر Replay که در ری‌استارت
//    EA اجرا می‌شود، بدون هیچ معامله‌ای.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.14 | 2026-07-12 — بازبینی مستقل ۴ ایراد گزارش‌شده پس از v14.13
//    این نسخه نتیجهٔ ردیابی خط‌به‌خط کد واقعی (نه فقط الگوهای رایج باگ
//    در اکسپرت‌های MQL5) در برابر ۴ ایرادی است که بعد از انتشار v14.13
//    مطرح شد. نتیجه: فقط ۱ مورد واقعی بود (آن‌هم صرفاً نمایشی، بدون اثر
//    روی تصمیم Rule) و ۳ مورد در کد فعلی رخ نمی‌دهند. جزئیات کامل با
//    ارجاع دقیق به خط کد در HelpMe_V14_14_Review.md.
//
// 🟡 P2 [CLEANH-DISPLAY-GAP] متن «Clean=» متورم بعد از آخر هفته/تعطیلات — رفع شد
//    SQ_SpikeRuleText قبلاً cleanDurStr را با SQ_FormatDuration(cleanSince)
//    می‌ساخت — تفاضل TimeCurrent() که در شکاف آخر هفته (TimeCurrent متوقف
//    نمی‌شود ولی کندل جدیدی هم ساخته نمی‌شود) عددی متورم/گمراه‌کننده
//    نشان می‌داد (مثلاً «Clean=52h30m» به‌جای مدت واقعی). توجه: خودِ تصمیم
//    Rule (Rule_Transition) از dirCleanH خام کندلی می‌آید که اصلاً به این
//    مشکل دچار نبود — این صرفاً یک اصلاح نمایشی بود، نه تغییر رفتار Rule.
//    حالا cleanDurStr از SQ_FormatDurationBars(dirCleanH, g_chartPeriodSeconds)
//    ساخته می‌شود — همان شمارندهٔ ایمن دربرابر تعطیلات که خودِ Rule هم
//    از آن استفاده می‌کند؛ متن و تصمیم همیشه ۱۰۰٪ هم‌راستا می‌مانند.
//
// ⚪ بررسی و رد شد — ادعای رقابت OnTimer/WebRequest در صف تلگرام
//    مستندات رسمی MQL5 تضمین می‌کند رویداد Timer هرگز هم‌زمان/تودرتو
//    اجرا نمی‌شود: اگه EA با رویداد قبلی مشغول باشد، رویداد Timer جدید
//    اصلاً به صف اضافه نمی‌شود (نه اینکه رویداد بعدی روی قبلی سوار شود).
//    صف Outbox پچ ۵ (idempotency + آفست پایدار در GV + retry/backoff) از
//    قبل مشکل واقعی (گم‌شدن پاسخ وضعیت) را درست حل کرده. تغییری اعمال نشد.
//
// ⚪ بررسی و رد شد — ادعای «یادآور Rule با داده کهنه در OnTimer»
//    Rule_CheckReminders دقیقاً باید همان g_gbpnzdLevelBuy/Sell قطعی را
//    گزارش کند که ماشین‌حالت کندل‌محور (by design) تولید کرده — نه اینکه
//    برای هر یادآوری یک Snapshot تازه از کندل در حال شکل‌گیری بسازد (که
//    برخلاف قفل طراحی shift=1 در سراسر v14.13 است و خودش رگرسیون تازه‌ای
//    می‌ساخت). این «کهنگی» عمداً و درست است. تغییری اعمال نشد.
//
// ⚪ بررسی و رد شد — ادعای «طول کلید GlobalVariable > 63 کاراکتر»
//    Rule_GVPrefix از CleanSymbol (سقف ۶ کاراکتر) یا پیشوند ثابت
//    "HM_GBNZD_" استفاده می‌کند؛ طولانی‌ترین کلید کامل فعلی (پیشوند+فیلد)
//    حدود ۲۰-۲۱ کاراکتر است — با فاصلهٔ زیاد زیر سقف واقعی ۶۳ کاراکتری
//    مستندشدهٔ MQL5. مستقل از suffix/prefix بروکر روی نماد. تغییری اعمال نشد.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.13 | 2026-07-11 — Single Source of Truth: BuildDirectionSnapshot
//    اجماع ۶ تحلیل مستقل (اولیه + ۵ AI) روی لاگ‌های واقعی ۱۰ جولای ۲۰۲۶.
//    ریشهٔ مشترک همهٔ باگ‌های زیر: نبود یک منبع واحد محاسبه برای
//    Crisis/Spike/Flow/ADX هر جهت — به‌جایش TL_Update بسته به وضعیت
//    پوزیشن (هج / بدون‌پوزیشن / تک‌جهته) مسیرهای جداگانه و ناهم‌ارز داشت.
//    جزئیات کامل هر پچ در v14.13_Changes.md — این‌جا فقط خلاصه.
//
// 🔴 P0 [BUG-STALE-INPUT] قفل شدن CleanH — رفع شد
//    Alert_CheckGBPNZDRule حالا در بستن هر کندل، به‌جای تکیه بر
//    g_gbnzdCrisisBuy/Sell و g_gbnzdSpikeBuy/Sell (که بسته به شاخهٔ
//    TL_Update ممکن بود برای یک جهت به‌روزرسانی نشوند)، مستقیماً
//    BuildDirectionSnapshot را برای هر دو جهت صدا می‌زند و نتیجه را
//    (به‌صورت OR با accumulator تیک‌محور موجود) وارد ApplyClosedBarToRule
//    می‌کند. توابع GBPNZD_Replay_CrisisAtBar/SpikeAtBar اکنون به‌جای
//    محاسبهٔ مستقل، به BuildDirectionSnapshot تفویض می‌کنند (رفتار
//    قدیمی‌شان برای همهٔ فراخوان‌های موجود دقیقاً حفظ شده — نگاه کن به
//    نگاشت shift در v14.13_Changes.md).
//
// 🔴 P0 [BUG-COOLDOWN-BT] Cooldown فقط در live — رفع شد
//    ApplyClosedBarToRule منطق Cooldown را یک‌بار پیاده‌سازی می‌کند و
//    اکنون در هر سه مسیر (Alert_CheckGBPNZDRule زنده، GBPNZD_InitReplay،
//    LogHourlySnapshot بکتست) به‌طور یکسان اجرا می‌شود. spikeStreak در
//    HMRuleState ذخیره و در GV پایدار می‌شود (نسخه‌بندی‌شده — پچ ۹).
//
// 🔴 P0 [BUG-SPIKEONLY-DIRTY] تشخیص اشتباه spikeOnlyDirty — رفع شد
//    spikeOnlyDirty اکنون دقیقاً یعنی «Spike≥Warning AND Crisis≠Red»
//    (نه هر HourDirty دیگری). Full Spike همیشه فوری dirty می‌کند؛
//    Crisis Red همیشه فوری dirty می‌کند و هرگز مشمول Cooldown نیست.
//
// 🔴 P0 [BUG-TG-OFFSET] مدیریت offset تلگرام — رفع شد
//    آفست تلگرام اکنون در GlobalVariable پایدار می‌شود (نه فقط RAM) —
//    زنده می‌ماند حتی اگه EA وسط پردازش کرش/ری‌استارت کند. ارسال پاسخ
//    از یک outbox صف‌بندی‌شده با retry+backoff انجام می‌شود؛ دیگر یک
//    شکست WebRequest باعث گم‌شدن دائمی «وضعیت» نمی‌شود. کلید idempotency
//    (update_id+chat_id+command) از ارسال دوباره جلوگیری می‌کند.
//
// 🔴 P0 [BUG-TELEMETRY-EVALID] مقادیر ناسازگار (ADX=0 وسط CRISIS=Red) — رفع شد
//    هر پیامی که چند فیلد از منابع مختلف می‌گیرد یک evalBarTime واحد
//    (از همان Snapshot) دارد؛ اگه فیلدی از evalId متفاوت بیاد N/A چاپ می‌شود.
//
// 🟠 P1 [BUG-REMINDER-TRANSITION] یادآور فقط در کندل گذار — رفع شد
//    یادآور از Alert_CheckGBPNZDRule (کندل‌محور) به Rule_CheckReminders()
//    منتقل شد که از OnTimer (هر ثانیه) صدا زده می‌شود — کاملاً مستقل از
//    بسته‌شدن کندل. هر جهت/سطح یک lastReminderTime مستقل در HMRuleState دارد.
//
// 🟠 P1 [MISSING-DIRTY-REASON] نبود DirtyReason — رفع شد
//    StatusQuery اکنون «LastDirty: HH:MM <علت>» و وضعیت Cooldown را نشان می‌دهد.
//
// 🟠 P1 [BUG-CLEANH-UNIT] واحد CleanH برای TF غیر H1 — رفع شد
//    HMRuleState.cleanSince (datetime واقعی) جایگزین شمارش خام کندل شد؛
//    نمایش بر حسب ساعت/دقیقهٔ واقعی؛ آستانه‌های Rule هم بر حسب ثانیه.
//
// 🟠 P1 [MISSING-SCHEMA-VERSION] کلیدهای GV بدون نسخه/TF — رفع شد
//    GBPNZD_GV_Save/Load اکنون periodSeconds و یک schemaVersion واقعی
//    (1413) ذخیره و چک می‌کنند؛ عدم تطابق → Replay کامل، نه state خراب.
//
// 🟠 P1 [BUG-CRISIS-HIDDEN-ALL] عدم نمایش Crisis در حالت All بدون پوزیشن — رفع شد
//    StatusQuery/UpdateDashboard وقتی Rule فعاله (Level>0) ولی پوزیشنی
//    باز نیست، Crisis واقعی هر جهت را (نه ⚫ --) نشان می‌دهند.
//
// 🟠 P1 [FOREIGN-INSUFFICIENT-DATA] Foreign OnDemand با دادهٔ ناقص — رفع شد
//    قبل از ارسال گزارش، iBars/SERIES_SYNCHRONIZED چک می‌شود؛ داده ناکافی
//    → «⚠️ Low Confidence» به پیام اضافه می‌شود.
//
// 🟡 P2 [WEEKEND-GAP] بررسی آخر هفته و شکاف داده — تحکیم شد
//    مکانیزم موجود (gbpnzdMarketStale) حفظ و در ApplyClosedBarToRule
//    متمرکز شد؛ اولین کندل دوشنبه از طریق BuildDirectionSnapshot درست ارزیابی می‌شود.
//
// 🟡 P2 [MKTPHASE-STALE-LOOK] برچسب زمان آخرین محاسبه MktPhase — اضافه شد
//
// همهٔ قابلیت‌های v14.12 (از جمله FIX-SPIKE-LABEL و SpikeCooldown) دست‌نخورده حفظ شدند.
// ════════════════════════════════════════════════════════════════════
// 📋 v14.12 | 2026-07-10 — سه پچ پس از تحلیل اجماع ۷ مدل
//
// 🔴 P0 [FIX-SPIKE-LABEL] ناسازگاری برچسب Spike با Score — رفع شد
//    ریشه: Calc_SpikeDetector وقتی rawPhase=2 (score > th_spike) ولی FLOW قرمز نبود
//    → dispPhase=0 (Normal) می‌داد، در حالی که StatusQuery_BuildReport همان score را
//    Warning نشون می‌داد (flowYellowSell=true). این تناقض باعث می‌شد:
//    - تلگرام: «⚡ Spike تأیید شد» (score=2.59 > th_spike=1.90)
//    - Status بعدی: «Spike=Warning» ← ناهمخوان با Calc اصلی که g_lightSpike=0 بود
//    راه‌حل: rawPhase==2 (score > th_spike) + FLOW تأیید نشده → dispPhase=1 (Warning)
//    قبل: else if(rawPhase>=1 && !flowConfirms) → dispPhase=0 (Normal)
//    بعد: rawPhase==2 → dispPhase=1 (Warning "Score≥Spike|بدون تأیید FLOW")
//         rawPhase==1 → dispPhase=0 (Normal بدون تغییر)
//    تأثیر: g_lightSpike=1 وقتی score>th_spike بدون FLOW → HourDirty=true → CleanH درست
//
// 🟠 P1 [SPIKE-COOLDOWN] پیشنهاد GLM 5.2 Max — پیاده‌سازی شد
//    پارامتر جدید: inp_SpikeCooldownBars (پیش‌فرض 0=غیرفعال)
//    وقتی 2 یا 3: باید N کندل متوالی با Spike≥Warning داشته باشیم
//    قبل از اینکه CleanH ریست شود (فقط برای Spike-only-dirty — نه Crisis-Red)
//    Full Spike (dispPhase=2: FLOW قرمز) همیشه فوری dirty می‌کند — CoolDown دور می‌زند
//    تخمین تأثیر: E[Resume] از ~2.7 روز به ~1.8 روز با CoolDown=2
//    Stop بیهوده از ~70% به ~40% کاهش
//    متغیرهای جدید: g_spikeCooldownBuyCounter / g_spikeCooldownSellCounter
//
// 🟠 P1 [FIX-RULE-CLARITY] رفع تناقض ظاهری «Spike تأیید شد» + «Rule=Run»
//    ریشه: وقتی Spike=2 (تأیید شد با FLOW قرمز) ولی Crisis=Green، Rule=Run است
//    این درست است (طراحی عمدی: _stopA نیاز به crisis>=1 دارد)
//    اما کاربر می‌دید: Alert «ورود ممنوع» + Rule «Run» → تناقض ظاهری
//    دو راه‌حل ترکیبی:
//    1. پیام Spike alert اصلاح شد: دیگر «ورود جدید ممنوع» نیست
//       بلکه «پوزیشن باز رو زیر نظر بگیر + Rule STOP/CLOSE فقط با Crisis≥Yellow»
//    2. SQ_SpikeRuleText: وقتی Spike≥Warning + Crisis=Green → یادداشت توضیحی اضافه
//       «ℹ️ Rule فعال نمی‌شود: CRISIS سبز — Stop نیاز به Crisis≥Yellow دارد»
//
// ════════════════════════════════════════════════════════════════════
// 📋 v14.11 | 2026-07-09 — کالیبراسیون GBPNZD + رفع Infinite Stop Lock + BUG-FLOW-HARDCODE
//
// 🔴 P0 [Infinite Stop Lock]: آستانه Warning برای GBPNZD از 1.10 به 1.50 رفت.
//    dirty rate: 19.3% → 8.7% (BUY)  |  E[Resume]: 37 روز → 3.8 روز
//    آستانه Spike از 1.60 به 1.90 (P95 = 1.91 در 6 ماه بکتست)
//    وزن‌ها: M15↓0.25→0.20 | M30↓0.20→0.15 | H1↑0.30→0.35 | H4↑0.15→0.20
//
// 🔴 P0 [BUG-FLOW-HARDCODE]: آستانه Flow در TL_Update / FlowEvaluate / LogHourlySnapshot
//    و تمام شاخه‌های hedge/backtest از -4.0 هاردکد به Flow_GetThresholds() تبدیل شد.
//    (GBPNZD: green=2.5, red=-5.0 | AUDCAD/EURGBP: مقادیر قبلی حفظ شدند)
//
// 🟠 P1: FlowRed GBPNZD -4.0 → -5.0 | FlowGreen +2.0 → +2.5
//    Flow Red از 24.6% به ~12.7% از ساعت‌ها کاهش یافت
//
// 🟡 P2: Crisis GBPNZD — OrA_Flow 5.5→6.0 | Y2_Flow 4.0→5.0 (هماهنگ با FlowRed)
//
// 🟡 P2: تابع جدید Flow_GetThresholds(sym, greenTh, redTh) اضافه شد
//
// 🟡 P2: تمام آستانه‌های GBPNZD/AUDCAD/EURGBP به Input Variables منتقل شدند
//    (بدون کامپایل مجدد قابل تنظیم در MT5)
//
// 🔵 P3: rawPhase در شاخه‌های hedge/backtest از هاردکد 1.00/1.50
//    به Spike_GetThresholds(_Symbol) تبدیل شد
//
// ════════════════════════════════════════════════════════════════════
// 📋 v14.09 — کالیبراسیون کامل EURGBP/AUDCAD + رفع باگ نمایشی SpikeScore
// 📋 v14.10 — رفع BUG-HA1: علامت HighAlert برای جهت Sell در ۷ محل اصلاح شد
//             قبل: haSell = ADX>=35 AND flowSell > +7.0  (اشتباه: Sell در سود → Alert)
//             بعد: haSell = ADX>=35 AND flowSell < -7.0  (صحیح: Sell در ضرر → Alert)
//             تأثیر: Close-A برای جهت Sell در هر سه جفت‌ارز فعال شد
//
// تغییرات این نسخه:
// 🔴 P0: رفع باگ نمایشی SpikeScore یکسان در حالت No-Position
//    در StatusQuery_BuildReport: پنالتی کوچک Flow-محور به displayScore
//    فقط عدد نمایشی تلگرام جهت‌دار می‌شود — منطق Rule/Crisis دست‌نخورده.
// 🟠 P1: کالیبراسیون کامل EURGBP (Crisis + Spike + Threshold helper)
//    Crisis آستانه‌ها به‌شدت کاهش یافت (ADX 26→20, Flow 5.5→4.0 برای Red-A)
//    Spike weights به H4/D1 سنگین‌تر شد (wM15=0.05, wH4=0.35, wD1=0.20)
//    تابع کمکی Spike_GetThresholds ساخته شد (GBPNZD/EURGBP/AUDCAD جداگانه)
// 🟡 P2: کالیبراسیون AUDCAD (Crisis) — شاخه جداگانه در Crisis_GetThresholds
//    Red بالاتر (38/6 vs 32/7) تا هشدار کاذب نباشد
//    Yellow/Orange کمی پایین‌تر برای روندهای کند
// 🟢 P3: تنظیم جزئی Spike GBPNZD (Warning=1.10, Spike=1.60) — کاهش نویز
// 🔵 P4: هدر CSV بکتست — GBPNZD_Rule → sym_Rule (همه ارزها درست)
//         + OnInit log به‌روزرسانی
// 🔵 BONUS: Calc_SpikeDetector از Rule_SpikeWeightsForSymbol استفاده می‌کند
//           (single source of truth برای وزن‌ها در لایو + بکتست + Replay)
//
// ════════════════════════════════════════════════════════════════════
...
// 🆕 v14.15 P1 [SMART-CLOSE-TEXT]: آیا واقعاً پوزیشن باز (هر جهتی) روی این
// نماد وجود دارد؟ همان فیلتر HM_PositionBelongsToSymbol (با احترام به
// InpFilterByMagic) — برای جلوگیری از پیام‌های گمراه‌کنندهٔ «ببند» وقتی
// HelpMe (که خودش هیچ معامله‌ای باز/بسته نمی‌کند) پوزیشنی برای بستن نمی‌بیند.
bool HasRealPositionsOnSymbol(string sym)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(HM_PositionBelongsToSymbol(PositionGetSymbol(i), sym))
         return true;
   return false;
}
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.11: Calibration Inputs — آستانه‌های قابل تنظیم بدون کامپایل
// ════════════════════════════════════════════════════════════════════
input group "════════ ⚖️ کالیبراسیون GBPNZD ════════"
// --- Spike ---
input double inp_GBPNZD_SpikeWarn   = 1.50;  // ⬆️ آستانه Warning (v14.10: 1.10) | GBPNZD Warning threshold
input double inp_GBPNZD_SpikeSig    = 1.90;  // ⬆️ آستانه Spike (v14.10: 1.60) | GBPNZD Spike threshold
// 🆕 v14.12: Spike Cooldown — جلوگیری از ریست CleanH توسط یک اسپایک گذرا
// 0 = غیرفعال (رفتار v14.11)
// 2 = ۲ کندل H1 متوالی با Spike≥Warning لازم است تا CleanH ریست شود (پیشنهاد GLM 5.2)
// 3 = ۳ کندل — محافظه‌کارتر
// Full Spike (dispPhase=2: FLOW قرمز + score>th_spike) همیشه فوری dirty می‌کند
input int    inp_SpikeCooldownBars  = 2;     // 🆕 Spike Cooldown: کندل‌های متوالی (0=غیرفعال, 2 یا 3 برای فعال)
// --- Spike Weights ---
input double inp_GBPNZD_wM15        = 0.20;  // ⬇️ وزن M15 (v14.10: 0.25) | M15 weight
input double inp_GBPNZD_wM30        = 0.15;  // ⬇️ وزن M30 (v14.10: 0.20) | M30 weight
input double inp_GBPNZD_wH1         = 0.35;  // ⬆️ وزن H1 (v14.10: 0.30) | H1 weight
input double inp_GBPNZD_wH4         = 0.20;  // ⬆️ وزن H4 (v14.10: 0.15) | H4 weight
input double inp_GBPNZD_wD1         = 0.10;  // وزن D1 (بدون تغییر) | D1 weight
// --- Flow ---
input double inp_GBPNZD_FlowGreen   =  2.50; // ⬆️ آستانه سبز Flow (v14.10: 2.0) | Flow Green threshold
input double inp_GBPNZD_FlowRed     = -5.00; // ⬆️ آستانه قرمز Flow (v14.10: -4.0) | Flow Red threshold ← KEY CHANGE
// --- Crisis ---
input double inp_GBPNZD_RedA_ADX    = 28.0;  // Crisis RedA: ADX | RedA ADX
input double inp_GBPNZD_RedA_Flow   =  6.0;  // Crisis RedA: Flow | RedA Flow
input double inp_GBPNZD_RedB_ADX    = 35.0;  // Crisis RedB: ADX | RedB ADX
input double inp_GBPNZD_RedB_Flow   =  7.5;  // Crisis RedB: Flow | RedB Flow
input double inp_GBPNZD_OrA_ADX     = 26.0;  // Crisis OrA: ADX | OrA ADX
input double inp_GBPNZD_OrA_Flow    =  6.0;  // ⬆️ Crisis OrA: Flow (v14.10: 5.5) | OrA Flow
input double inp_GBPNZD_OrB_ADX     = 24.0;  // Crisis OrB: ADX | OrB ADX
input double inp_GBPNZD_OrB_Ts      = 40.0;  // Crisis OrB: TrendScore | OrB TrendScore
input double inp_GBPNZD_OrC_ADX     = 28.0;  // Crisis OrC: ADX | OrC ADX
input double inp_GBPNZD_OrC_Flow    =  6.5;  // Crisis OrC: Flow | OrC Flow
input double inp_GBPNZD_OrC_Ts      = 25.0;  // Crisis OrC: TrendScore | OrC TrendScore
input double inp_GBPNZD_OrD_ADX     = 30.0;  // Crisis OrD: ADX | OrD ADX
input double inp_GBPNZD_OrD_Ts      = 40.0;  // Crisis OrD: TrendScore | OrD TrendScore
input double inp_GBPNZD_Y2_ADX      = 24.0;  // Crisis Y2: ADX | Y2 ADX
input double inp_GBPNZD_Y2_Flow     =  5.0;  // ⬆️ Crisis Y2: Flow (v14.10: 4.0) | Y2 Flow
input double inp_GBPNZD_Y3_Flow     =  6.0;  // Crisis Y3: Flow | Y3 Flow
input double inp_GBPNZD_Y3_Ts       = 30.0;  // Crisis Y3: TrendScore | Y3 TrendScore
input double inp_GBPNZD_Y4_ADX      = 21.0;  // Crisis Y4: ADX | Y4 ADX
input double inp_GBPNZD_Y4_Ts       = 45.0;  // Crisis Y4: TrendScore | Y4 TrendScore
input double inp_GBPNZD_Y5_ADX      = 31.0;  // Crisis Y5: ADX | Y5 ADX
input double inp_GBPNZD_Y5_Flow     =  6.5;  // Crisis Y5: Flow | Y5 Flow

input group "════════ ⚖️ کالیبراسیون AUDCAD (Placeholder) ════════"
input double inp_AUDCAD_SpikeWarn   = 1.00;  // AUDCAD Warning threshold
input double inp_AUDCAD_SpikeSig    = 1.50;  // AUDCAD Spike threshold
input double inp_AUDCAD_wM15        = 0.15;  // AUDCAD M15 weight
input double inp_AUDCAD_wM30        = 0.15;  // AUDCAD M30 weight
input double inp_AUDCAD_wH1         = 0.30;  // AUDCAD H1 weight
input double inp_AUDCAD_wH4         = 0.25;  // AUDCAD H4 weight
input double inp_AUDCAD_wD1         = 0.15;  // AUDCAD D1 weight
input double inp_AUDCAD_FlowGreen   =  3.0;  // AUDCAD Flow Green threshold
input double inp_AUDCAD_FlowRed     = -5.0;  // AUDCAD Flow Red threshold

input group "════════ ⚖️ کالیبراسیون EURGBP ════════"
input double inp_EURGBP_SpikeWarn   = 1.60;  // 🆕 v14.16: کالیبره از Placeholder 1.20 (اجماع ۱۳ تحلیل) | EURGBP Warning threshold
input double inp_EURGBP_SpikeSig    = 1.60;  // 🆕 v14.16: برابر با Warn — حذف منطقهٔ مرده (Dead Zone) | EURGBP Spike threshold
input double inp_EURGBP_wM15        = 0.05;  // EURGBP M15 weight
input double inp_EURGBP_wM30        = 0.08;  // EURGBP M30 weight
input double inp_EURGBP_wH1         = 0.32;  // EURGBP H1 weight
input double inp_EURGBP_wH4         = 0.35;  // EURGBP H4 weight
input double inp_EURGBP_wD1         = 0.20;  // EURGBP D1 weight
input double inp_EURGBP_FlowGreen   =  3.0;  // EURGBP Flow Green threshold
input double inp_EURGBP_FlowRed     = -5.0;  // EURGBP Flow Red threshold
...
datetime g_mktPhaseLastCalcTime   = 0;   // 🆕 v14.13 Patch 11: آخرین باری که واقعاً محاسبه شد (نه فقط نمایش)
...
// 🆕 v14.12: Spike CoolDown — شمارنده کندل‌های متوالی با Spike≥Warning (برای هر جهت)
// وقتی inp_SpikeCooldownBars > 0، این counter ها تعیین می‌کنند چه وقت HourDirty اعمال شود
int      g_spikeCooldownBuyCounter  = 0;   // تعداد کندل‌های متوالی با Spike≥Warning برای Buy
int      g_spikeCooldownSellCounter = 0;   // تعداد کندل‌های متوالی با Spike≥Warning برای Sell
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13: پشتوانهٔ globals برای HMRuleState هر جهت (Patch 2/4/6/7/8/9)
// این‌ها «منبع ذخیره‌سازی» HMRuleState هستند — HMRuleState_Load/Save*
// در هر فراخوانی ApplyClosedBarToRule این‌ها را hydrate/flush می‌کنند.
// g_spikeCooldownBuyCounter/SellCounter و g_gbpnzdLastReminderH قدیمی
// دیگر در منطق فعال استفاده نمی‌شوند (جایگزین شده‌اند) اما برای صفر-ریسک
// حذف نشده‌اند.
// ════════════════════════════════════════════════════════════════════
int      g_gbpnzdSpikeStreakBuy      = 0;   // کندل‌های متوالی Spike≥1 (Buy)
int      g_gbpnzdSpikeStreakSell     = 0;   // کندل‌های متوالی Spike≥1 (Sell)
datetime g_gbpnzdCleanSinceBuy       = 0;   // 🆕 Patch 8: شروع streak تمیزِ Buy (0=تمیز نیست)
datetime g_gbpnzdCleanSinceSell      = 0;
datetime g_gbpnzdRedSinceBuy         = 0;   // شروع streak Crisis=Red متوالی Buy (0=قرمز نیست)
datetime g_gbpnzdRedSinceSell        = 0;
datetime g_gbpnzdLastDirtyTimeBuy    = 0;   // 🆕 Patch 6/7: آخرین لحظهٔ dirty شدن Buy
datetime g_gbpnzdLastDirtyTimeSell   = 0;
int      g_gbpnzdLastDirtyReasonBuy  = 0;   // 0=none 1=CrisisRed 2=Spike 3=هردو
int      g_gbpnzdLastDirtyReasonSell = 0;
datetime g_gbpnzdLastReminderBuy     = 0;   // 🆕 Patch 4: یادآور مستقل هر جهت (جایگزین g_gbpnzdLastReminderH مشترک)
datetime g_gbpnzdLastReminderSell    = 0;
int      g_gbpnzdLastReminderLevelBuy  = 0; // سطحی که آخرین یادآوری برایش ارسال شد (برای تشخیص تغییر سطح)
int      g_gbpnzdLastReminderLevelSell = 0;

// ════════════════════════════════════════════════════════════════════
// 🆕 v14.17: معماری Stateful مستقل EURGBP — کاملاً موازی با بلوک GBPNZD
// بالا، بدون هیچ تغییری در آن. این‌ها «منبع ذخیره‌سازی» HMRuleState برای
// EURGBP هستند؛ EURGBP_HMRuleState_Load/SaveBuy/Sell (تابع جدید، نه
// ویرایش‌شده) این‌ها را hydrate/flush می‌کنند — دقیقاً همان الگوی
// HMRuleState_Load/SaveBuy/Sell برای GBPNZD.
// لیست کامل‌تر از چیزی است که در بریف آمده: بعد از خواندن دقیق کد GBPNZD،
// چند متغیر پشتیبان (StopReason، Prev*، cache Crisis/Spike، Hour
// accumulator‌ها، Replay retry/NeedsLiveCheck) هم لازم بودند تا الگو کامل
// و صحیح باشد — همه در v14.17_Changes.md بخش ۱ مستند شده‌اند.
// ════════════════════════════════════════════════════════════════════
int      g_eurgbpLevelBuy           = 0;    // 0=GREEN | 1=STOP | 2=CLOSE
int      g_eurgbpLevelSell          = 0;
int      g_eurgbpStopReasonBuy      = -1;   // پشتوانهٔ HMRuleState.stopReason (مثل GBPNZD)
int      g_eurgbpStopReasonSell     = -1;
int      g_eurgbpCleanHBuy          = 0;
int      g_eurgbpCleanHSell         = 0;
int      g_eurgbpCrisisRedHBuy      = 0;
int      g_eurgbpCrisisRedHSell     = 0;
datetime g_eurgbpLastH              = 0;    // آخرین ساعت پردازش‌شده برای counter
bool     g_eurgbpHourRedBuy         = false; // این ساعت: Crisis=Red دیده شد؟
bool     g_eurgbpHourDirtyBuy       = false; // این ساعت: Red یا Spike≥Warning دیده شد؟
bool     g_eurgbpHourRedSell        = false;
bool     g_eurgbpHourDirtySell      = false;
bool     g_eurgbpReplayDone         = false; // true = EURGBP_InitReplay اجرا شده و Rule قابل اعتماده
int      g_eurgbpReplayRetry        = 0;     // تعداد تلاش‌های retry در OnTimer (max 20، مثل GBPNZD)
bool     g_eurgbpNeedsLiveCheck     = false; // بعد از Replay، یک‌بار با مقادیر live چک بشه
int      g_eurgbpPrevCrisisBuy      = -1;    // snapshot ساعت قبل — برای counter ساعتی (مثل GBPNZD)
int      g_eurgbpPrevSpikeBuy       = -1;
int      g_eurgbpPrevCrisisSell     = -1;
int      g_eurgbpPrevSpikeSell      = -1;
int      g_eurgbpCrisisBuy          = -1;    // آخرین محاسبهٔ Crisis برای forBuy=true (cache نمایشی)
int      g_eurgbpCrisisSell         = -1;
int      g_eurgbpSpikeBuy           = -1;
int      g_eurgbpSpikeSell          = -1;
int      g_eurgbpSpikeStreakBuy     = 0;     // کندل‌های متوالی Spike≥1 (Buy)
int      g_eurgbpSpikeStreakSell    = 0;
datetime g_eurgbpCleanSinceBuy       = 0;    // شروع streak تمیزِ Buy (0=تمیز نیست)
datetime g_eurgbpCleanSinceSell      = 0;
datetime g_eurgbpRedSinceBuy         = 0;    // شروع streak Crisis=Red متوالی Buy (0=قرمز نیست)
datetime g_eurgbpRedSinceSell        = 0;
datetime g_eurgbpLastDirtyTimeBuy    = 0;    // آخرین لحظهٔ dirty شدن Buy
datetime g_eurgbpLastDirtyTimeSell   = 0;
int      g_eurgbpLastDirtyReasonBuy  = 0;    // 0=none 1=CrisisRed 2=Spike 3=هردو
int      g_eurgbpLastDirtyReasonSell = 0;
datetime g_eurgbpLastReminderBuy     = 0;    // یادآور مستقل هر جهت
datetime g_eurgbpLastReminderSell    = 0;
int      g_eurgbpLastReminderLevelBuy  = 0;  // سطحی که آخرین یادآوری برایش ارسال شد
int      g_eurgbpLastReminderLevelSell = 0;
// 🆕 v14.17: handle های اندیکاتور اختصاصی و دائمیِ EURGBP (H1/H4/D1 — طبق
// همان 🔒 DESIGN LOCK که برای GBPNZD در GBPNZD_InitReplay مستند شده).
// عمداً از handleEMA200_H1_TL و همتاهایش (که مخصوص GBPNZD/_Symbol هستند)
// یا از handle موقت on-demand (g_odHandle*، که فقط وقتی سیمبل on-demand
// دقیقاً EURGBP باشد معتبر است) استفاده نشد — این چهار handle مستقل،
// دائمی، و همیشه روی EURGBP هستند تا موتور Rule پیوستهٔ EURGBP هیچ‌وقت
// به وضعیت لحظه‌ایِ سیستم on-demand وابسته نباشد.
int      g_eurgbpHandleEMA200_H1 = INVALID_HANDLE;
int      g_eurgbpHandleATR_H1    = INVALID_HANDLE;
int      g_eurgbpHandleADX_H4    = INVALID_HANDLE;
int      g_eurgbpHandleATR_D1    = INVALID_HANDLE;
// 🆕 v14.18: true مگر چارت خودش EURGBP باشد — یک‌بار در OnInit محاسبه
// می‌شود. وقتی چارت خودش EURGBP است، موتور عمومی Alert_CheckGBPNZDRule
// (از طریق _Symbol) خودش EURGBP را مدیریت می‌کند و بلوک موازی این نسخه
// باید خاموش بماند تا پردازش دوگانه/رقابت روی GV پیش نیاید.
bool     g_eurgbpParallelActive = true;
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 1: HMDirSnapshot — Single Source of Truth
//
// یک عکس کامل از وضعیت یک جهت (Buy یا Sell) روی یک سمبل، در یک لحظه/کندل
// مشخص. BuildDirectionSnapshot() تنها تابعی است که این ساختار را پر
// می‌کند — هیچ تابع دیگری (Calc_SpikeDetector، UpdateCrisisLight،
// StatusQuery_BuildReport، TL_Update، GBPNZD_Replay_*) دیگر مستقل
// RTM/TREND/STRUCT/FLOW/Spike/Crisis را از صفر محاسبه نمی‌کند — یا این
// تابع را صدا می‌زنند یا (برای سازگاری صددرصد با فراخوان‌های قدیمی)
// یک wrapper نازک روی آن هستند.
//
// evalBarTime: مهر زمانی کندل بسته‌ای که این Snapshot برایش ساخته شده —
// همهٔ فیلدهای این ساختار از همین یک evalId می‌آیند (رفع BUG-TELEMETRY-EVALID:
// دیگر ADX از یک لحظه و Flow از لحظه‌ای دیگر نمی‌آید).
// ════════════════════════════════════════════════════════════════════
struct HMDirSnapshot
{
   bool     valid;         // false = داده کافی نبود (handle گرم نشده و...)
   string   symbol;
   bool     forBuy;
   int      shift;         // شیفت درخواست‌شده (1=آخرین کندل بسته، طبق قرارداد استاندارد MQL)
   datetime evalBarTime;   // زمان بسته‌شدن کندلی که این مقادیر را نمایندگی می‌کند

   int      rtm;           // 0=سبز 1=زرد 2=قرمز
   int      trend;         // 0=سبز 1=زرد 2=قرمز
   int      structD1;      // 0=سبز 1=زرد 2=قرمز  (نام structD1 برای پرهیز از کلیدواژه struct)
   int      flowLight;     // 0=سبز 1=زرد 2=قرمز  (نسخهٔ چراغی FLOW برای این جهت)
   int      rc;            // تعداد چراغ‌های قرمز از ۴تای بالا (RTM/TREND/STRUCT/FLOW)
   int      crisis;        // 0=Green 1=Yellow 2=Red 3=Orange  -1=N/A

   double   adx;           // ADX(H4) در evalBarTime
   double   flow;          // امتیاز خام Flow جهت‌دار (نه چراغ)
   double   trendScore;    // g_csvScore معادل در این کندل (برای tsAgainst)
   bool     highAlert;     // ADX≥35 AND Flow جهت‌دار < -7.0

   int      rawSpikePhase; // 0/1/2 — فاز خام بر اساس Score، قبل از دروازهٔ FLOW
   int      spike;         // 0/1/2 — dispPhase نهایی (تأییدشده/نیمه‌تأییدشده)، -1=داده ناکافی
   double   spikeScore;    // امتیاز خام وزن‌دار ۵ تایم‌فریمی (مستقل از جهت)
};

// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 2/4/6/7/8: HMRuleState — وضعیت ماشین‌حالت Rule یک جهت
//
// این ساختار state ماندگار (بین کندل‌ها) یک جهت (Buy یا Sell) را نگه
// می‌دارد. برای صفر-ریسک با کد موجود، متغیرهای全global مسطح فعلی
// (g_gbpnzdLevelBuy، g_gbpnzdCleanHBuy، ...) به‌عنوان "منبع ذخیره‌سازی
// بیرونی" باقی می‌مانند؛ در هر فراخوانی ApplyClosedBarToRule یک
// HMRuleState از روی آن‌ها پر می‌شود (hydrate)، منطق واحد روی آن اجرا
// می‌شود، و نتیجه دوباره در همان globals نوشته می‌شود (flush). به این
// ترتیب ده‌ها نقطهٔ دیگر در فایل (StatusQuery، Dashboard، GV) که این
// globals را می‌خوانند بدون تغییر درست کار می‌کنند.
// ════════════════════════════════════════════════════════════════════
struct HMRuleState
{
   int      level;             // 0=RUN 1=STOP 2=CLOSE
   int      stopReason;        // 0=StopA 1=StopB 10=CloseA 11=CloseB -1=none
   int      crisisRedH;        // شمارش کندل (نمایش/سازگاری قدیمی)
   int      cleanH;            // شمارش کندل (نمایش/سازگاری قدیمی)
   datetime cleanSince;        // 🆕 Patch 8: از چه زمانی متوالیاً تمیز است (0=تمیز نیست/نامشخص)
   datetime redSince;          // از چه زمانی متوالیاً Crisis=Red است (0=قرمز نیست)
   int      spikeStreak;       // 🆕 Patch 2: کندل‌های متوالی spike-only-dirty (برای Cooldown)
   datetime lastReminderTime;  // 🆕 Patch 4: آخرین یادآوری STOP/CLOSE ارسال‌شده (مستقل از کندل)
   datetime lastDirtyTime;     // 🆕 Patch 6/7: آخرین لحظه‌ای که این جهت dirty شد
   int      lastDirtyReason;   // 0=هیچ(تمیز) 1=CrisisRed 2=Spike 3=هردو
};
#define HM_DIRTY_REASON_NONE    0
#define HM_DIRTY_REASON_CRISIS  1
#define HM_DIRTY_REASON_SPIKE   2
#define HM_DIRTY_REASON_BOTH    3
// 🆕 v14.13 Patch 9: نسخهٔ واقعی اسکیمای GV — با هر تغییر ساختاری در
// چیزی که GBPNZD_GV_Save/Load ذخیره می‌کنند باید افزایش یابد.
#define HM_SCHEMA_VERSION  1413.0
// 🆕 v14.17: نسخهٔ مستقل اسکیمای GV برای EURGBP (طبق درخواست کاربر: 1417.0).
// عمداً جدا از HM_SCHEMA_VERSION نگه داشته شد — آن ثابت متعلق به GBPNZD
// است و نباید برای EURGBP هم استفاده شود (تغییر آینده در اسکیمای یکی
// نباید دیگری را بی‌دلیل نامعتبر کند).
#define HM_EURGBP_SCHEMA_VERSION  1417.0
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 8: مدت‌زمان واقعی (نه شمارش کندل خام) — پشتیبانی همهٔ TF
string SQ_FormatDuration(datetime sinceTime)
{
   if(sinceTime <= 0) return "0m";
   int elapsed = (int)(TimeCurrent() - sinceTime);
   if(elapsed < 0) elapsed = 0;
   int h = elapsed / 3600;
   int m = (elapsed % 3600) / 60;
   if(h > 0) return StringFormat("%dh%02dm", h, m);
   return StringFormat("%dm", m);
}

// ════════════════════════════════════════════════════════════════════
// 🩹 v14.14 [CLEANH-DISPLAY-GAP]: نسخهٔ ایمن دربرابر شکاف آخر هفته/تعطیلات.
// SQ_FormatDuration بالا از TimeCurrent()-sinceTime (wall-clock) محاسبه
// می‌کند — روی خودِ ساعت‌های تعطیلی بازار متوقف نمی‌شود، پس اگه streak
// «تمیز» از قبل از بستن بازار جمعه شروع شده باشد، بعد از بازگشایی دوشنبه
// عددی به‌شدت متورم نشان می‌دهد (شکاف تعطیلی هم جزو مدت حساب می‌شود).
// این تابع به‌جایش از تعداد کندل واقعاً بسته‌شده (dirCleanH/dirRedH —
// همان شمارنده‌ای که Rule_UpdateCounters/Rule_Transition برای *تصمیم*
// استفاده می‌کنند و در تعطیلات اصلاً افزایش نمی‌یابد چون بروکر کندلی
// نمی‌سازد) ضربدر طول کندل چارت محاسبه می‌کند — همیشه با آستانهٔ واقعی
// تصمیم Rule ۱۰۰٪ هم‌راستا و مستقل از هر تعطیلی/شکاف داده.
// ════════════════════════════════════════════════════════════════════
string SQ_FormatDurationBars(int bars, int periodSeconds)
{
   if(bars <= 0) return "0m";
   int perSec = (periodSeconds > 0) ? periodSeconds : 3600;   // fallback H1 اگه هنوز ست نشده
   long elapsed = (long)bars * (long)perSec;
...
// 🆕 v14.13 Patch 6: متن DirtyReason
string SQ_DirtyReasonText(int reason)
{
   if(reason == HM_DIRTY_REASON_CRISIS) return "Crisis Red";
   if(reason == HM_DIRTY_REASON_SPIKE)  return "Spike";
   if(reason == HM_DIRTY_REASON_BOTH)   return "Crisis Red + Spike";
   return "";
}

// 🆕 v14.13 Patch 6/8: خط اضافهٔ DirtyReason + Cooldown برای هر جهت —
// "| LastDirty: HH:MM <علت> | Spike Cooldown: n/N" (فقط اگه واقعاً مرتبط باشد)
string SQ_DirtyCooldownSuffix(bool isForBuy)
{
   datetime lastDirtyTime = isForBuy ? g_gbpnzdLastDirtyTimeBuy   : g_gbpnzdLastDirtyTimeSell;
   int      lastReason    = isForBuy ? g_gbpnzdLastDirtyReasonBuy : g_gbpnzdLastDirtyReasonSell;
   int      streak         = isForBuy ? g_gbpnzdSpikeStreakBuy     : g_gbpnzdSpikeStreakSell;

   string suffix = "";
   if(lastDirtyTime > 0 && lastReason != HM_DIRTY_REASON_NONE)
      suffix += StringFormat(" | LastDirty: %s %s", TimeToString(lastDirtyTime, TIME_MINUTES), SQ_DirtyReasonText(lastReason));
   if(inp_SpikeCooldownBars > 0 && streak > 0 && streak < inp_SpikeCooldownBars)
      suffix += StringFormat(" | Spike Cooldown: %d/%d", streak, inp_SpikeCooldownBars);
   return suffix;
}
...
//
//   🆕 v14.13 Patch 6/8: هر خروجی این تابع اکنون شامل مدت‌زمان واقعیِ
//   CleanH (نه فقط شمارش کندل خام — درست در همهٔ TF ها) و در صورت وجود،
//   DirtyReason و وضعیت Cooldown است.
// ════════════════════════════════════════════════════════════════════
                        ...
                        double adxVal, double flowScore, bool isForBuy)
{
   ...
   // 🩹 v14.14 [CLEANH-DISPLAY-GAP]: قبلاً از HMRuleState.cleanSince با
   // SQ_FormatDuration (تفاضل wall-clock، متورم بعد از آخر هفته/تعطیلات)
   // ساخته می‌شد. حالا از dirCleanH خام کندلی — همان مبنای تصمیم واقعی
   // Rule_Transition — تا متن نمایشی و آستانهٔ تصمیم همیشه هم‌راستا بمانند.
   string   cleanDurStr    = SQ_FormatDurationBars(dirCleanH, g_chartPeriodSeconds);
   string   dcSuffix        = SQ_DirtyCooldownSuffix(isForBuy);
   ...
   {
      if(dirCleanH >= 24)
         return StringFormat("🟢 Run | Clean=%s (RESUME)", cleanDurStr);
      return StringFormat("🔴 Close | Clean=%s%s%s", cleanDurStr, restartNote, dcSuffix);
   }
   if(dirLevel == 1)
   {
      if(dirCleanH >= 24)
         return StringFormat("🟢 Run | Clean=%s (RESUME)", cleanDurStr);
      return StringFormat("⛔ Stop | Clean=%s%s%s", cleanDurStr, restartNote, dcSuffix);
   }
   ...
   // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
   bool highAlert = (adxVal >= 35.0) && (flowScore < -7.0);
   ...
   if(dirCleanH >= 24)
      return StringFormat("🟢 Run — Spike=%s | CRISIS=%s | Clean=%s متوالی تمیز ",
                          spikeName, crisisName, cleanDurStr);

   // 🐛 v14.12 FIX-RULE-CLARITY: رفع تناقض «Spike تأیید شد» vs «Rule=Run»
   // وقتی Spike>=Warning ولی Crisis=Green، Rule=Run است و این درست است:
   //   _stopA نیاز به crisis>=1 (Yellow+) دارد → Green بلاک می‌کند
   //   _stopB نیاز به crisis==Orange/Red دارد → Green بلاک می‌کند
   // پس Rule=Run عمداً است — نه اشتباه! Crisis سبز یعنی بازار تحت کنترل است.
   // یادداشت توضیحی اضافه می‌شود تا با اخطار Spike تلگرام تناقض ظاهری نداشته باشد.
   if(spikeVal >= 1 && crisisVal == 0)
      return StringFormat("🟢 Run — Spike=%s | CRISIS=%s | Clean=%s%s%s"
                          " ℹ️(Rule فعال نمی‌شود: CRISIS سبز است — Stop نیاز به Crisis≥Yellow دارد)",
                          spikeName, crisisName, cleanDurStr, restartNote, dcSuffix);

   // 🐛 v13.44 FIX5: فرمت نمایشی تمیزتر — CrisisRedH حذف شد (اطلاعات اضافه)
   return StringFormat("🟢 Run — Spike=%s | CRISIS=%s | Clean=%s%s%s",
                       spikeName, crisisName,
                       cleanDurStr, restartNote, dcSuffix);
}
...
string StatusQuery_BuildReport()
{
   ...
   // 🐛 v14.18 [PORTABILITY-GATE-FIX]: قبلاً فقط GBPNZD قبول می‌شد
   // (StringFind(sym,"GBPNZD")) با اینکه بقیهٔ این تابع و کل موتور Rule
   // از v14.00 عمومی/بر مبنای sym بودند. حالا از IsRuleSymbol() — همان
   // تابع عمومی موجود که GBPNZD/EURGBP/AUDCAD را می‌شناسد — استفاده
   // می‌شود. صفر تغییر رفتار برای GBPNZD.
   // ── چک ساپورت ارز ──────────────────────────────────────────────────
   if(!IsRuleSymbol(sym))
   {
      return "📊 " + sym + " — HelpMe Status\n"
           ...
           + "HelpMe فقط برای GBPNZD، EURGBP، و AUDCAD طراحی شده.\n"
           + "─────────────────────\n"
           + "لطفاً چارت رو روی یکی از این سه ارز باز کن.";
   }
   ...
   // 🆕 v14.13 Patch 1/7 [BUG-CRISIS-HIDDEN-ALL]: Snapshot هر دو جهت این‌جا،
   // زودتر از همه‌جا ساخته می‌شود — مستقل از activeMode/hasPos. این باعث
   // می‌شود چه خط خلاصهٔ Crisis بالای گزارش، چه خطوط جزئیِ Buy/Sell پایین‌تر،
   // یک منبع واحد و همیشه‌معتبر داشته باشند؛ دیگر نیازی به «آیا پوزیشن یا
   // Virtual فعال است» برای داشتن یک عدد Crisis معنادار نیست.
   HMDirSnapshot sqSnapBuy, sqSnapSell;
   BuildDirectionSnapshot(_Symbol, true,  PERIOD_H1, 1, sqSnapBuy);
   BuildDirectionSnapshot(_Symbol, false, PERIOD_H1, 1, sqSnapSell);
   // Rule فعاله یعنی حداقل یک جهت STOP/CLOSE است — حتی بدون پوزیشن واقعی
   bool ruleActiveNoPos = (!activeMode) && (g_gbpnzdLevelBuy > 0 || g_gbpnzdLevelSell > 0);
   ...
   // ── Crisis ──────────────────────────────────────────────────────
   // 🆕 v14.13 Patch 7 [BUG-CRISIS-HIDDEN-ALL]: قبلاً این خط با
   // !activeMode همیشه ⚫ -- نشان می‌داد، حتی وقتی Rule واقعاً Stop/Close
   // بود (Direction=All بدون پوزیشن). حالا اگه Rule فعاله، بدترین Crisis
   // بین Buy/Sell (از همان Snapshot که تصمیم Rule را ساخته) نشان داده
   // می‌شود — با برچسب روشن که این "بدترین جهت" است، نه یک پوزیشن خاص.
   ...
   else if(ruleActiveNoPos && sqSnapBuy.valid && sqSnapSell.valid)
   {
      // رتبه‌بندی شدت واقعی: Green(0) < Yellow(1) < Orange(3) < Red(2)
      int rBuy  = (sqSnapBuy.crisis==2)?3  : (sqSnapBuy.crisis==3)?2  : (sqSnapBuy.crisis==1)?1  : 0;
      int rSell = (sqSnapSell.crisis==2)?3 : (sqSnapSell.crisis==3)?2 : (sqSnapSell.crisis==1)?1 : 0;
      int worstCrisis = (rBuy >= rSell) ? sqSnapBuy.crisis : sqSnapSell.crisis;
      int worstRC     = (rBuy >= rSell) ? sqSnapBuy.rc     : sqSnapSell.rc;
      string worstDir = (rBuy >= rSell) ? "Buy" : "Sell";
      crisisStr = SQ_Light(worstCrisis) + StringFormat("  (RC=%d, بدترین جهت=%s — بدون پوزیشن)", worstRC, worstDir);
   }
   else
      crisisStr = "⚫  --";
   ...
   // ── Hi Alert ────────────────────────────────────────────────────
   // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
   // 🆕 v14.13 Patch 7: مثل Crisis، وقتی Rule بدون پوزیشن فعاله از Snapshot می‌آید
   bool highAlert = activeMode &&
                    ((forBuy  && g_lastAdxVal >= 35.0 && g_lastFlowScore < -7.0) ||
                     (!forBuy && g_lastAdxVal >= 35.0 && g_lastFlowScore < -7.0));
   bool highAlertNoPos = ruleActiveNoPos && ((sqSnapBuy.valid && sqSnapBuy.highAlert) || (sqSnapSell.valid && sqSnapSell.highAlert));
   string hiAlertStr;
   if(highAlert || highAlertNoPos)
      hiAlertStr = "🔴  Active (ADX>=35 + Flow against)" + (highAlertNoPos && !activeMode ? " — بدون پوزیشن" : "");
   else if(activeMode || ruleActiveNoPos)
      hiAlertStr = "🟢  OK";
   else
      hiAlertStr = "⚫  --";
   ...
   {
      // 🆕 v14.13 Patch 11 [MKTPHASE-STALE-LOOK]: این مقدار روزانه محاسبه
      // می‌شود — بدون این برچسب، کاربر ممکن است آن را فریز‌شده تعبیر کند.
      string mpDateStr = (g_mktPhaseLastCalcTime > 0)
         ? TimeToString(g_mktPhaseLastCalcTime, TIME_DATE)
         : "نامشخص";
      mktPhaseStr = SQ_Light(g_lightMktPhase)
                    + StringFormat("  ratio=%.3f  (محاسبهٔ روزانه — آخرین: %s)", g_mktPhaseRatio, mpDateStr);
   }

   // ── Spike Detector — چهار خط جداگانه Buy و Sell 🆕 v13.24/v13.25 ─
   // 🆕 v14.13 Patch 1/7: قبلاً این‌جا دو مسیر جدا بود («بدون پوزیشن/Virtual»
   // با یک محاسبهٔ ساده‌شدهٔ fallback، و «پوزیشن واقعی» با خواندن globals
   // که ممکن بود کهنه باشند). حالا هر دو حذف شدند: BuildDirectionSnapshot
   // مستقل از وضعیت پوزیشن همیشه معتبر است، پس اینجا فقط یک‌بار (برای هر
   // دو جهت) صدا زده می‌شود. این خودِ ریشهٔ BUG-CRISIS-HIDDEN-ALL را هم حل
   // می‌کند: دیگر نیازی به «فعال بودن حالت» برای داشتن یک عدد Crisis
   // معتبر نیست.
   int    spikeSellVal, spikeBuyVal;
   int    crisisSellVal, crisisBuyVal;
   double adxSellVal, adxBuyVal;
   double flowSell, flowBuy;
   double spikeSellDisplayScore, spikeBuyDisplayScore;

   spikeBuyVal   = sqSnapBuy.valid  ? sqSnapBuy.spike   : -1;
   spikeSellVal  = sqSnapSell.valid ? sqSnapSell.spike  : -1;
   crisisBuyVal  = sqSnapBuy.valid  ? sqSnapBuy.crisis  : -1;
   crisisSellVal = sqSnapSell.valid ? sqSnapSell.crisis : -1;
   adxBuyVal     = sqSnapBuy.valid  ? sqSnapBuy.adx     : g_lastAdxVal;
   adxSellVal    = sqSnapSell.valid ? sqSnapSell.adx    : g_lastAdxVal;
   flowBuy       = sqSnapBuy.valid  ? sqSnapBuy.flow    : 0.0;
   flowSell      = sqSnapSell.valid ? sqSnapSell.flow   : 0.0;

   // 🆕 v14.09 P0 (حفظ‌شده): پنالتی نمایشی کوچک بر اساس Flow منفیِ جهت —
   // فقط روی عدد نمایش‌داده‌شده اثر دارد، منطق Rule/Crisis/rawPhase را
   // دست‌نمی‌زند (دقیقاً همان فرمول قبلی، فقط از snap.spikeScore/flow می‌آید).
   double sqBaseScoreBuy  = sqSnapBuy.valid  ? sqSnapBuy.spikeScore  : 0.0;
   double sqBaseScoreSell = sqSnapSell.valid ? sqSnapSell.spikeScore : 0.0;
   double flowPenaltyBuy  = MathMax(0.0, -flowBuy)  * 0.04;
   double flowPenaltySell = MathMax(0.0, -flowSell) * 0.04;
   spikeBuyDisplayScore   = sqBaseScoreBuy  * (1.0 + flowPenaltyBuy);
   spikeSellDisplayScore  = sqBaseScoreSell * (1.0 + flowPenaltySell);
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 5: Telegram Outbox — صف ارسال با retry+backoff
//
// قبل از این پچ: Alert_SendTelegram یک WebRequest تک‌باره بود، بدون
// مقدار برگشتی، بدون retry — اگه شکست می‌خورد، پاسخ «وضعیت» برای همیشه
// گم می‌شد (دقیقاً باگ گزارش‌شده: گاهی پاسخی دریافت نمی‌شد).
//
// حالا: StatusQuery_PollTelegram فقط enqueue می‌کند؛ ارسال واقعی این‌جا
// (از OnTimer، هر ۳۰ ثانیه توسط TG_Outbox_Process) با retry انجام
// می‌شود. برای پاسخ‌های Status، متن گزارش در لحظهٔ *ارسال* (نه enqueue)
// از StatusQuery_BuildReport تازه ساخته می‌شود — یعنی حتی اگه تلاش اول
// شکست بخورد و ۳۰ ثانیه بعد retry شود، کاربر همیشه آخرین وضعیت را
// می‌گیرد، نه یک گزارش کهنه.
//
// idempotency: کلید update_id+chat_id+command — همان کلیدی که
// StatusQuery_PollTelegram قبل از enqueue چک می‌کند تا یک پیام ورودی
// تلگرام دوبار در صف قرار نگیرد.
// ════════════════════════════════════════════════════════════════════
#define TG_OUTBOX_MAX   20
#define TG_KIND_TEXT    0
#define TG_KIND_STATUS  1

struct TGOutboxItem
{
   bool     used;
   int      kind;
   string   text;          // برای TG_KIND_TEXT — نادیده گرفته می‌شود برای TG_KIND_STATUS
   long     chatId;
   long     updateId;
   string   idemKey;
   datetime nextAttempt;
   int      attempts;
   bool     sendPhoto;      // فقط برای TG_KIND_STATUS: بعد از متن، عکس چارت هم بفرست
};
TGOutboxItem g_tgOutbox[TG_OUTBOX_MAX];
bool         g_tgOutboxInited = false;

void TG_Outbox_Init()
{
   if(g_tgOutboxInited) return;
   for(int i = 0; i < TG_OUTBOX_MAX; i++) g_tgOutbox[i].used = false;
   g_tgOutboxInited = true;
}

bool TG_Outbox_HasIdemKey(string key)
{
   if(key == "") return false;
   TG_Outbox_Init();
   for(int i = 0; i < TG_OUTBOX_MAX; i++)
      if(g_tgOutbox[i].used && g_tgOutbox[i].idemKey == key) return true;
   return false;
}

void TG_Outbox_Enqueue(int kind, string text, long chatId, long updateId, string idemKey, bool sendPhoto = false)
{
   TG_Outbox_Init();
   if(TG_Outbox_HasIdemKey(idemKey))
   {
      if(ShowDebugLogs) Print("TG_Outbox: idempotency skip key=", idemKey);
      return;
   }
   int slot = -1;
   for(int i = 0; i < TG_OUTBOX_MAX; i++)
      if(!g_tgOutbox[i].used) { slot = i; break; }
   if(slot < 0)
   {
      // صف پر (بسیار بعید — یعنی ۲۰ پیام هم‌زمان در حال retry هستند)
      // بهتره جدیدترین درخواست را از دست ندیم؛ قدیمی‌ترین را جایگزین می‌کنیم.
      slot = 0;
      Print("⚠️ v14.13 TG_Outbox پر شد (", TG_OUTBOX_MAX, ") — قدیمی‌ترین پیام صف جایگزین شد");
   }
   g_tgOutbox[slot].used        = true;
   g_tgOutbox[slot].kind         = kind;
   g_tgOutbox[slot].text         = text;
   g_tgOutbox[slot].chatId       = chatId;
   g_tgOutbox[slot].updateId     = updateId;
   g_tgOutbox[slot].idemKey      = idemKey;
   g_tgOutbox[slot].nextAttempt = TimeCurrent();
   g_tgOutbox[slot].attempts     = 0;
   g_tgOutbox[slot].sendPhoto    = sendPhoto;
}

// از OnTimer صدا زده می‌شود — نه فقط StatusQuery، بلکه هر پیامی که از
// طریق TG_Outbox_Enqueue وارد صف شده (Patch 5 محدود به Status نیست).
void TG_Outbox_Process()
{
   ...
   for(int i = 0; i < TG_OUTBOX_MAX; i++)
   {
      if(!g_tgOutbox[i].used) continue;
      if(nowT < g_tgOutbox[i].nextAttempt) continue;

      string body = (g_tgOutbox[i].kind == TG_KIND_STATUS) ? StatusQuery_BuildReport() : g_tgOutbox[i].text;
      bool ok = Alert_SendTelegramEx(body);

      if(ok)
      {
         if(g_tgOutbox[i].kind == TG_KIND_STATUS && g_tgOutbox[i].sendPhoto)
         {
         ...
         if(ShowDebugLogs) Print("TG_Outbox: ✅ sent key=", g_tgOutbox[i].idemKey, " attempts=", g_tgOutbox[i].attempts + 1);
         g_tgOutbox[i].used = false;
      }
      else
      {
         g_tgOutbox[i].attempts++;
         if(g_tgOutbox[i].attempts >= 5)
         {
            Print("❌ v14.13 TG_Outbox: تسلیم بعد از 5 تلاش — key=", g_tgOutbox[i].idemKey);
            g_tgOutbox[i].used = false;
         }
         else
         {
            // backoff نمایی با سقف ۳ دقیقه: ۵، ۱۵، ۴۵، ۱۳۵ ثانیه...
            int backoffSec = 5;
            for(int p = 1; p < g_tgOutbox[i].attempts; p++) backoffSec *= 3;
            backoffSec = (int)MathMin(180, backoffSec);
            g_tgOutbox[i].nextAttempt = nowT + backoffSec;
            if(ShowDebugLogs)
               Print("🔁 v14.13 TG_Outbox retry #", g_tgOutbox[i].attempts, " in ", backoffSec,
                     "s — key=", g_tgOutbox[i].idemKey);
         }
      }
   }
}

// ── آفست تلگرام پایدار در GlobalVariable (Patch 5) ────────────────────
// scoped به سمبل چون هر چارت/EA توکن یکسان ولی offset مستقل خودش را
// دنبال می‌کند (تا هر سه سمبل مستقل به «وضعیت» جواب بدهند — رفتار فعلی
// حفظ می‌شود؛ اگه offset مشترک بین سمبل‌ها می‌شد فقط یکی جواب می‌داد).
string TG_OffsetGVKey()
{
   return "HM_TG_Offset_" + CleanSymbol(_Symbol);
}

void TG_Offset_Load()
{
   string key = TG_OffsetGVKey();
   if(GlobalVariableCheck(key))
      g_lastTelegramUpdateId = (long)GlobalVariableGet(key);
}

void TG_Offset_Save(long updateId)
{
   GlobalVariableSet(TG_OffsetGVKey(), (double)updateId);
}
...
void StatusQuery_PollTelegram()
{
   ...
   while(true)
   {
      ...
      // 🆕 v14.13 Patch 5 [BUG-TG-OFFSET]: بلافاصله در GlobalVariable هم
      // ذخیره می‌شود — نه فقط در RAM. اگه EA همین‌جا وسط پردازش این
      // update کرش کند یا ری‌استارت شود، بعد از بازگشت این update دوباره
      // fetch نمی‌شود (از «پیام مسموم» که هر بار EA را گیر بیندازد جلوگیری
      // می‌کند). تحویل واقعی پاسخ به outbox صف‌بندی‌شده سپرده شده که
      // مستقل از این ack با retry+backoff کار می‌کند — نگاه کن به
      // TG_Outbox_Enqueue پایین‌تر.
      if(updateId > g_lastTelegramUpdateId)
      {
         g_lastTelegramUpdateId = updateId;
         TG_Offset_Save(g_lastTelegramUpdateId);
      }
      ...
      if(txtPos >= 0)
      {
         ...
         if(isStatus && chatOk)
         {
            // 🆕 v14.13 Patch 5: به‌جای static local + ارسال مستقیم، از outbox
            // با کلید idempotency (update_id+chat_id+command) استفاده می‌شود.
            // این هم از ارسال تکراری جلوگیری می‌کند (Bug: پاسخ ۱۸:۲۷ در ۱۸:۳۰
            // تکرار شد) و هم — چون گزارش در لحظهٔ واقعیِ ارسال از outbox تازه
            // ساخته می‌شود نه در لحظهٔ enqueue — دیگر پاسخ کهنه (CleanH قدیمی)
            // به کاربر نمی‌رسد، حتی اگه ارسال اول شکست بخورد و retry بعداً انجام شود.
            string idemKey = IntegerToString(updateId) + "_" + IntegerToString(chatId) + "_status";
            if(TG_Outbox_HasIdemKey(idemKey) || alreadyReplied)
               Print("StatusQuery: skip duplicate reply update_id=", updateId);
            else
            {
               alreadyReplied = true;
               TG_Outbox_Enqueue(TG_KIND_STATUS, "", chatId, updateId, idemKey, true);
               Print("StatusQuery: 📥 enqueued reply update_id=", updateId, "  chat=", chatId);
               // cooldown: بعد از enqueue، 30 ثانیه poll نرو تا پیام تکراری نیاد
         ...
         if(!isStatus && chatOk)
         {
            ...
            if(odReq != "")
            {
               string odIdemKey = IntegerToString(updateId) + "_" + IntegerToString(chatId) + "_od_" + odReq;
               if(TG_Outbox_HasIdemKey(odIdemKey) || alreadyReplied)
               {
               ...
               else
               {
                  ...
                  if(odReq == CleanSymbol(_Symbol))
                  {
                     ...
                     TG_Outbox_Enqueue(TG_KIND_STATUS, "", chatId, updateId, odIdemKey, true);
                  }
                  else
                  {
                     // سمبل دیگری‌ست — محاسبه on-demand (async، چند ثانیه طول می‌کشد)
                     // 🆕 v14.13: OnDemand_RequestSymbol خودش وقتی نتیجه آماده شد پیام
                     // را می‌سازد و مستقیم می‌فرستد (مسیر async جدا از outbox متنی —
                     // چون محاسبه‌اش چند ثانیه طول می‌کشد و نمی‌شود الان متن ساخت).
...
int OnInit()
{
   // 🆕 v14.18: یک‌بار محاسبه می‌شود — آیا بلوک موازی EURGBP (v14.17) باید
   // فعال باشد؟ فقط وقتی چارت خودش EURGBP نیست (وگرنه موتور عمومی
   // Alert_CheckGBPNZDRule از طریق _Symbol خودش EURGBP را مدیریت می‌کند).
   g_eurgbpParallelActive = (CleanSymbol(_Symbol) != "EURGBP");
   ...
   // 🆕 v14.13 Patch 5: بازیابی آفست تلگرام از GlobalVariable — اگه EA
   // ری‌استارت شده باشد، از همان‌جا که مانده بود ادامه می‌دهد (نه از صفر
   // که باعث fetch دوبارهٔ آپدیت‌های قدیمی می‌شد).
   TG_Offset_Load();

   if(EnableAllLogs) Print("🚀 HelpMe v14.15 - Initializing...");
   ...
   if(EnableAllLogs)
   {
      Print("✅ HelpMe v14.11 initialized! [BUG-HA1 FIXED + CALIBRATED GBPNZD]");
      Print("🤖 AI Features:");
   ...
   // 🆕 v14.17: handle های اختصاصی و دائمیِ EURGBP — کاملاً مجزا از
   // handle های بالا (که مخصوص _Symbol/GBPNZD هستند) و از on-demand
   // موقت (g_odHandle*) هم استفاده نمی‌کنند، تا موتور Rule پیوستهٔ
   // EURGBP هیچ‌وقت به وضعیت لحظه‌ایِ سیستم on-demand وابسته نباشد.
   // 🆕 v14.18: فقط وقتی چارت خودش EURGBP نباشد (g_eurgbpParallelActive) —
   // وگرنه موتور عمومی از طریق _Symbol خودش EURGBP را مدیریت می‌کند.
   if(g_eurgbpParallelActive)
   {
      g_eurgbpHandleEMA200_H1 = iMA  ("EURGBP", PERIOD_H1, 200, 0, MODE_EMA, PRICE_CLOSE);
      g_eurgbpHandleATR_H1    = iATR ("EURGBP", PERIOD_H1, ATRPeriod);
      g_eurgbpHandleADX_H4    = iADX ("EURGBP", PERIOD_H4, ADXPeriod);
      g_eurgbpHandleATR_D1    = iATR ("EURGBP", PERIOD_D1, ATRPeriod);
      if(g_eurgbpHandleEMA200_H1 == INVALID_HANDLE || g_eurgbpHandleATR_H1 == INVALID_HANDLE ||
         g_eurgbpHandleADX_H4    == INVALID_HANDLE || g_eurgbpHandleATR_D1 == INVALID_HANDLE)
         Print("⚠️ v14.17: EURGBP dedicated handles failed — EURGBP_InitReplay/Alert_CheckEURGBPRule will retry via OnTimer");
      else
         Print("✅ v14.17: EURGBP dedicated handles OK (H1/H4/D1)");
   }
   else
      Print("ℹ️ v14.18: چارت خودش EURGBP است — بلوک موازی EURGBP غیرفعال ماند (موتور عمومی مدیریت می‌کند)");
   ...
   // 🆕 v14.17: همون تحریک دانلود، برای EURGBP هم (تا handleهای اختصاصی آن سریع‌تر گرم شوند)
   // 🆕 v14.18: فقط اگه بلوک موازی EURGBP فعال باشه
   if(g_eurgbpParallelActive)
   {
      double closeBufE[];
      CopyClose("EURGBP", PERIOD_H1, 0, 300, closeBufE);
      CopyClose("EURGBP", PERIOD_H4, 0, 300, closeBufE);
      CopyClose("EURGBP", PERIOD_D1, 0, 300, closeBufE);
   }
   ...
   Print("📊 HelpMe v14.10 (Calibrated for GBPNZD/EURGBP/AUDCAD) — Chart TF: ",
   ...
   if(PERIOD_CURRENT >= PERIOD_D1)
      Print("⚠️ HelpMe v14.10: Chart TF=", EnumToString(PERIOD_CURRENT),
   ...
   // 🆕 v14.17: همان کار برای EURGBP — کاملاً مستقل، هیچ اثری روی بلوک
   // GBPNZD بالا ندارد. اگه handleهای اختصاصی EURGBP هنوز warm نشده
   // باشند، EURGBP_InitReplay خودش فوراً return می‌کند و OnTimer طبق
   // همان مکانیزم retry موجود (تا ۲۰ بار) دوباره تلاش خواهد کرد.
   // 🆕 v14.18: + شرط g_eurgbpParallelActive (رجوع به بالای OnInit)
   if(g_eurgbpParallelActive && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol)
      && !(bool)MQLInfoInteger(MQL_TESTER))
   {
      EURGBP_InitReplay();
      g_eurgbpNeedsLiveCheck = true;
   }

   // 🆕 v14.11: تأیید Inputs بعد از شروع — نتیجه در Experts Log قابل مشاهده است
   v14_11_SanityCheck();
   Print("▶️ HelpMe v14.11 | OnInit OK | Symbol=", _Symbol, " | TF=", _Period,
         " | SpikeWarn=", inp_GBPNZD_SpikeWarn, " | FlowRed=", inp_GBPNZD_FlowRed);
...
void OnTimer()
{
   ...
   // 🆕 v14.13 Patch 4: یادآوری STOP/CLOSE مستقل از کندل — هر ۳۰ ثانیه چک
   // می‌شود (فاصلهٔ یادآوری واقعی حداقل ۵ دقیقه‌ست، پس این کاملاً کافی
   // است و بار اضافه‌ای هم ندارد). رفع BUG-REMINDER-TRANSITION.
   Rule_CheckReminders();
   // 🆕 v14.17: یادآوری مستقل STOP/CLOSE برای EURGBP — همان الگو
   // 🆕 v14.18: + شرط g_eurgbpParallelActive
   if(g_eurgbpParallelActive) EURGBP_CheckReminders();

   // 🆕 v14.13 Patch 5: تخلیهٔ صف outbox تلگرام — retry با backoff برای
   // پیام‌هایی که WebRequest اولشان شکست خورده بود (رفع BUG-TG-OFFSET).
   TG_Outbox_Process();
   ...
   // 🆕 v14.17: همان retry مکانیزم برای EURGBP — کاملاً مستقل از بلوک بالا
   // 🆕 v14.18: + شرط g_eurgbpParallelActive
   if(g_eurgbpParallelActive && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol)
      && !(bool)MQLInfoInteger(MQL_TESTER)
      && !g_eurgbpReplayDone
      && g_eurgbpReplayRetry < 20)
   {
      g_eurgbpReplayRetry++;
      EURGBP_InitReplay();
      if(g_eurgbpReplayDone)
         Print("✅ v14.17 EURGBP_InitReplay succeeded on retry #", g_eurgbpReplayRetry);
   }
...
void OnDeinit(const int reason)
{
   ...
   // 🆕 v14.17: آزاد کردن handle های اختصاصی EURGBP
   if(g_eurgbpHandleEMA200_H1 != INVALID_HANDLE) { IndicatorRelease(g_eurgbpHandleEMA200_H1); g_eurgbpHandleEMA200_H1 = INVALID_HANDLE; }
   if(g_eurgbpHandleATR_H1    != INVALID_HANDLE) { IndicatorRelease(g_eurgbpHandleATR_H1);    g_eurgbpHandleATR_H1    = INVALID_HANDLE; }
   if(g_eurgbpHandleADX_H4    != INVALID_HANDLE) { IndicatorRelease(g_eurgbpHandleADX_H4);    g_eurgbpHandleADX_H4    = INVALID_HANDLE; }
   if(g_eurgbpHandleATR_D1    != INVALID_HANDLE) { IndicatorRelease(g_eurgbpHandleATR_D1);    g_eurgbpHandleATR_D1    = INVALID_HANDLE; }
...
void OnTick()
{
   ...
   // 🆕 v14.17: چک Rule مستقل EURGBP — درست بعد از تشخیص کندل جدید، مستقل
   // از منطق هج/تک‌جهته/بدون‌پوزیشن GBPNZD که پایین‌تر در این تابع اجرا
   // می‌شود. g_isNewChartBar به‌جای صدا زدن دوبارهٔ Chart_IsNewBarClosed()
   // خوانده می‌شود چون آن تابع stateful است و فقط یک‌بار true برمی‌گرداند.
   // 🆕 v14.18: + شرط g_eurgbpParallelActive (رجوع به بالای OnInit)
   if(g_isNewChartBar && g_eurgbpParallelActive) Alert_CheckEURGBPRule();
   if(isNewBar)
   {
...
// 🆕 v14.13 Patch 5: نسخهٔ bool-برگردان — برای outbox که باید بداند
// ارسال واقعاً موفق بود یا نه (تا retry کند). Alert_SendTelegram قدیمی
// همچنان وجود دارد (wrapper نازک) تا همهٔ فراخوان‌های موجود بدون تغییر کار کنند.
bool Alert_SendTelegramEx(const string message)
{
   if(!Alert_Telegram) return true;   // غیرفعال — «موفق در انجام ندادن»، نه شکست قابل retry
   // v11.3: اگه token پیش‌فرض یا خالی باشه → silent skip
   if(StringLen(Alert_TelegramToken) < 5 || StringLen(Alert_TelegramChatID) < 2
      || StringFind(Alert_TelegramToken, "97864663566") >= 0)
      return true;  // silent - پیش‌فرض یا خالیه — retry فایده‌ای ندارد
   ...
   bool ok = (res == 200);
   if(ShowDebugLogs) Print(ok ? "✅ Telegram OK" :
...
void Alert_Send(const string message)
{
   ...
   // 🆕 v14.13 Patch 5: تلاش فوری (بدون تأخیر در حالت موفق)؛ اگه WebRequest
   // شکست خورد، به‌جای گم‌شدن کامل (رفتار قبلی)، به outbox برای retry+backoff
   // می‌رود. این پیام‌های STOP/CLOSE حیاتی را هم پوشش می‌دهد، نه فقط پاسخ Status.
   if(!Alert_SendTelegramEx(message))
   {
      string idem = "alertsend_" + IntegerToString((long)TimeCurrent()) + "_" + IntegerToString(MathRand());
      TG_Outbox_Enqueue(TG_KIND_TEXT, message, (long)StringToInteger(Alert_TelegramChatID), 0, idem, false);
      if(ShowDebugLogs) Print("⚠️ v14.13 Alert_Send: ارسال فوری شکست خورد — به outbox اضافه شد (retry بعدی از OnTimer)");
   }
}
   ...
   string symOverride = "")   // 🆕 v14.00: برای FOREIGN on-demand — سمبلی غیر از _Symbol
{
   ...
   {
      // 🆕 v14.11: تمام آستانه‌ها از Inputs می‌خوانند — قابل تنظیم بدون کامپایل
      // GBPNZD: ADX به‌طور طبیعی بالاتره — آستانه‌های مخصوص این جفت
      // تغییرات v14.11:
      //   OrA_Flow: 5.5 → 6.0 (هماهنگ با FlowRed جدید -5.0)
      //   Y2_Flow:  4.0 → 5.0 (هماهنگ با FlowRed جدید -5.0)
      //   بقیه: بدون تغییر از v14.10
      th_RedA_ADX = inp_GBPNZD_RedA_ADX;  th_RedA_Flow = inp_GBPNZD_RedA_Flow;  // 28.0 / 6.0
      th_RedB_ADX = inp_GBPNZD_RedB_ADX;  th_RedB_Flow = inp_GBPNZD_RedB_Flow;  // 35.0 / 7.5
      th_OrA_ADX  = inp_GBPNZD_OrA_ADX;   th_OrA_Flow  = inp_GBPNZD_OrA_Flow;   // 26.0 / 6.0 ⬆️
      th_OrB_ADX  = inp_GBPNZD_OrB_ADX;   th_OrB_Ts    = inp_GBPNZD_OrB_Ts;     // 24.0 / 40.0
      th_OrC_ADX  = inp_GBPNZD_OrC_ADX;   th_OrC_Flow  = inp_GBPNZD_OrC_Flow;   // 28.0 / 6.5
      th_OrC_Ts   = inp_GBPNZD_OrC_Ts;                                            // 25.0
      th_OrD_ADX  = inp_GBPNZD_OrD_ADX;   th_OrD_Ts    = inp_GBPNZD_OrD_Ts;     // 30.0 / 40.0
      th_Y2_ADX   = inp_GBPNZD_Y2_ADX;    th_Y2_Flow   = inp_GBPNZD_Y2_Flow;    // 24.0 / 5.0 ⬆️
      th_Y3_Flow  = inp_GBPNZD_Y3_Flow;   th_Y3_Ts     = inp_GBPNZD_Y3_Ts;      // 6.0 / 30.0
      th_Y4_ADX   = inp_GBPNZD_Y4_ADX;    th_Y4_Ts     = inp_GBPNZD_Y4_Ts;      // 21.0 / 45.0
      th_Y5_ADX   = inp_GBPNZD_Y5_ADX;    th_Y5_Flow   = inp_GBPNZD_Y5_Flow;    // 31.0 / 6.5
   }
   else if(StringFind(checkSym, "EURGBP") >= 0)
   {
      // 🆕 v14.09 P1: کالیبراسیون اولیه EURGBP — حرکت آرام فرسایشی
      // max 24h = 0.98٪، P99 = 0.62٪. آستانه‌های قبلی برای «اسپایک ناگهانی»
      // طراحی شده بودند و «فرسایش چند روزه» را نمی‌دیدند (کال73: 80٪ زمان Run
      // در حالی که قیمت 1.2٪ افت کرده بود). آستانه‌ها به‌شدت کاهش یافتند.
      // 🆕 v14.17: تأیید کاربر روی اجماع ۱۳ تحلیل (گزارش v14.16 بخش ۲.۱) —
      // آستانه‌های ADX افزایش یافتند چون ADX>40 فقط در ۴٪ ساعات EURGBP رخ
      // می‌دهد (بخش ۳.۵ بریف کالیبراسیون). Flow/TrendScore بدون تغییر ماندند.
      th_RedA_ADX = 30.0;  th_RedA_Flow = 4.0;
      th_RedB_ADX = 35.0;  th_RedB_Flow = 7.0;    // بدون تغییر (شاخه فشار بالا)
      th_OrA_ADX  = 28.0;  th_OrA_Flow  = 3.5;
      th_OrB_ADX  = 26.0;  th_OrB_Ts    = 25.0;
      th_OrC_ADX  = 30.0;  th_OrC_Flow  = 6.0; th_OrC_Ts = 20.0;  // بدون تغییر
      th_OrD_ADX  = 32.0;  th_OrD_Ts    = 30.0;
      th_Y2_ADX   = 24.0;  th_Y2_Flow   = 2.5;
      th_Y3_Flow  = 5.5;   th_Y3_Ts     = 28.0;   // بدون تغییر
      th_Y4_ADX   = 23.0;  th_Y4_Ts     = 40.0;
      th_Y5_ADX   = 33.0;  th_Y5_Flow   = 6.0;
   }
   else if(StringFind(checkSym, "AUDCAD") >= 0)
   {
      // 🆕 v14.09 P2: کالیبراسیون AUDCAD — ADX ذاتاً بالاتر (تا 79 در بحران)
      // Red بالاتر (38 vs 32) تا هشدارهای کاذب نباشد
      // OrD پایین‌تر (35 vs 50) و Y2 پایین‌تر (22/3.0 vs 28/5.0) تا
      // روندهای کند صعودی زودتر تشخیص داده شوند
      th_RedA_ADX = 38.0;  th_RedA_Flow = 6.0;
      th_RedB_ADX = 40.0;  th_RedB_Flow = 8.5;    // بدون تغییر
      th_OrA_ADX  = 32.0;  th_OrA_Flow  = 5.0;
      th_OrB_ADX  = 28.0;  th_OrB_Ts    = 35.0;
      th_OrC_ADX  = 32.0;  th_OrC_Flow  = 7.5; th_OrC_Ts = 25.0;  // بدون تغییر
      th_OrD_ADX  = 34.0;  th_OrD_Ts    = 35.0;
      th_Y2_ADX   = 22.0;  th_Y2_Flow   = 3.0;
      th_Y3_Flow  = 7.0;   th_Y3_Ts     = 30.0;   // بدون تغییر
      th_Y4_ADX   = 25.0;  th_Y4_Ts     = 45.0;   // بدون تغییر
      th_Y5_ADX   = 35.0;  th_Y5_Flow   = 7.5;    // بدون تغییر
   }
   // fallback — برای سمبل‌های ناشناخته (مقادیر پایه اول تابع)
}
...
void Calc_MktPhase()
{
   ...
   if(firstRun)
   {
      ...
      if(cachedDay > 0 && cachedVal > 0 && (datetime)cachedDay == todayD1)
      {
         ...
         g_mktPhaseLastCalcTime = todayD1;   // 🆕 v14.13 Patch 11
   ...
   g_mktPhaseLastCalcTime = todayD1;   // 🆕 v14.13 Patch 11 [MKTPHASE-STALE-LOOK]
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.09 P1: Spike_GetThresholds — آستانه‌های Normal/Warning/Spike وابسته به سمبل
//
// دلیل: مقیاس SpikeScore برای هر سمبل متفاوت است.
//   GBPNZD: نوسان سریع → Warning=1.10, Spike=1.60
//   EURGBP: حرکت کند → هر چیز بالای 1.20 مستقیماً Spike (بدون مرحله Warning)
//           (اگر th_warn == th_spike باشد، Warning هرگز فعال نمی‌شود)
//   AUDCAD: مقیاس پیش‌فرض → Warning=1.00, Spike=1.50
//
// این تابع در Calc_SpikeDetector (لایو) و StatusQuery_BuildReport (تلگرام) استفاده می‌شود.
// ════════════════════════════════════════════════════════════════════
void Spike_GetThresholds(string sym, double &th_warn, double &th_spike)
{
   string s = CleanSymbol(sym);
   if(s == "GBPNZD")
   {
      // 🆕 v14.11: از Inputs می‌خواند — بدون کامپایل مجدد قابل تنظیم
      // P0: Warning 1.10→1.50 | Spike 1.60→1.90
      // ریشه: median=1.141، قدیمی زیر میانه بود → Infinite Stop Lock
      // نتیجه: dirty rate 19.3%→8.7% | E[Resume] 37روز→3.8روز
      th_warn  = inp_GBPNZD_SpikeWarn;   // default 1.50
      th_spike = inp_GBPNZD_SpikeSig;    // default 1.90
   }
   else if(s == "EURGBP")
   {
      // 🆕 v14.16: کالیبره شد — دیگر Placeholder نیست (اجماع ۱۳ تحلیل مستقل)
      // Warning == Spike عمداً حفظ شد: چون FLOW=Red به‌ندرت برای EURGBP رخ
      // می‌دهد، جدا کردن این دو آستانه یک «منطقهٔ مرده» ایجاد می‌کرد (نگاه
      // کنید v14.16_Changes.md برای جزئیات کامل)
      th_warn  = inp_EURGBP_SpikeWarn;   // v14.16: 1.20 → 1.60
      th_spike = inp_EURGBP_SpikeSig;    // v14.16: 1.20 → 1.60 (برابر با Warn)
   }
   else if(s == "AUDCAD")
   {
      // 🆕 v14.11: از Inputs می‌خواند (placeholder — کالیبراسیون مستقل بعداً)
      th_warn  = inp_AUDCAD_SpikeWarn;   // default 1.00
      th_spike = inp_AUDCAD_SpikeSig;    // default 1.50
   }
   else
   {
      // fallback برای سمبل‌های ناشناخته
      th_warn  = 1.00;
      th_spike = 1.50;
   }
}
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.11: Flow_GetThresholds — آستانه‌های Flow per-symbol از Inputs
//
// چرا: در v14.10 آستانه‌های Flow در TL_Update و LogHourlySnapshot هاردکد بودند
// (-4.0 برای همه جا). این باعث می‌شد تغییر آستانه نیاز به کامپایل مجدد داشته باشد
// و در شاخه‌های مختلف ناهماهنگی ایجاد می‌شد.
//
// GBPNZD: green=2.5, red=-5.0  (قبلاً: 2.0 / -4.0)
// AUDCAD:  green=3.0, red=-5.0  (بدون تغییر)
// EURGBP:  green=3.0, red=-5.0  (بدون تغییر)
// ════════════════════════════════════════════════════════════════════
void Flow_GetThresholds(string sym, double &greenTh, double &redTh)
{
   ...
   if(s == "GBPNZD")
   {
      greenTh = inp_GBPNZD_FlowGreen;   // default 2.50
      redTh   = inp_GBPNZD_FlowRed;     // default -5.00
   }
   else if(s == "AUDCAD")
   {
      greenTh = inp_AUDCAD_FlowGreen;   // default 3.0
      redTh   = inp_AUDCAD_FlowRed;     // default -5.0
   }
   else if(s == "EURGBP")
   {
      greenTh = inp_EURGBP_FlowGreen;   // default 3.0
      redTh   = inp_EURGBP_FlowRed;     // default -5.0
   }
   else
   {
      greenTh =  3.0;
...
void Calc_SpikeDetector()
{
   ...
   // 🆕 v14.09 BONUS: وزن‌دهی از Rule_SpikeWeightsForSymbol — single source of truth
   // قبلاً وزن‌ها اینجا هاردکد بودند و با Rule_SpikeWeightsForSymbol (Replay/بکتست)
   // ناهماهنگ می‌شدند. حالا لایو، بکتست و Replay همه از یک منبع می‌خوانند.
   ...
   // 🆕 v14.09 P1: rawPhase از آستانه‌های سمبل‌محور (Spike_GetThresholds)
   // اگه th_warn == th_spike (مثل EURGBP=1.20/1.20)، Warning هرگز فعال نمی‌شود
   double th_w, th_s;
   Spike_GetThresholds(_Symbol, th_w, th_s);
   int    rawPhase;
   string rawLabel;
   if(score < th_w)
   ...
   // 🆕 v14.11: th_spike از Spike_GetThresholds برای هماهنگی با آستانه‌های جدید
   double _csd_thW, _csd_thS; Spike_GetThresholds(_Symbol, _csd_thW, _csd_thS);
   bool flowYellowHigh  = (g_lightFLOW == 1 && score >= _csd_thS); // 🆕 v14.11: th_spike (قبلاً 2.0 هاردکد)
   ...
   {
      // 🐛 v14.12 FIX-SPIKE-LABEL: ناسازگاری برچسب Spike با Score رفع شد
      // قبلاً (v14.11): هر rawPhase>=1 بدون FLOW → dispPhase=0 (Normal!) حتی اگه score بالای th_spike بود
      // این باعث تناقض می‌شد: score=2.59 (بالای Spike آستانه=1.90) ولی لیبل Normal
      // در حالی که StatusQuery_BuildReport همان score را Warning نشون می‌داد (flowYellowSell=true)
      //
      // راه‌حل: rawPhase==2 (score > th_spike) → حداقل Warning نشون بده
      //          rawPhase==1 (score در [th_warn, th_spike]) → Normal بدون تغییر (واقعاً نویز است)
      // تأثیر: g_lightSpike از 0 (Normal) به 1 (Warning) تغییر می‌کند → g_gbnzdSpikeBuy/Sell=1
      //         → HourDirtyBuy/Sell=true → CleanH ریست می‌شود (رفتار درست‌تر)
      //         → StatusQuery و Calc_SpikeDetector دیگر متفاوت لیبل نمی‌دهند
      dispPhase = (rawPhase == 2) ? 1 : 0;
      dispLabel  = (rawPhase == 2) ? "Warning (Score≥Spike | بدون تأیید FLOW)" : "Normal (Vol↑)";
   }
   else
   {
      dispPhase = 0;
   ...
   if(Alert_OnSpike && dispPhase == 2)
   {
      ...
      {
         // 🐛 v14.12 FIX-ALERT-CLARITY: اصلاح پیام Spike برای رفع تناقض با Rule
         // قبلاً: «ورود جدید ممنوع» — اما Rule ممکن است «Run» باشد (وقتی Crisis=Green)
         // بعد: توضیح که این اخطار مستقل است و Rule STOP/CLOSE جداگانه اطلاع می‌دهد
         // Crisis=Green + Spike = Rule=Run (طراحی عمدی: Spike بدون بحران = تریدپذیر)
         // Crisis>=Yellow + Spike = Rule=STOP (اطلاع جداگانه از Alert_CheckGBPNZDRule)
         string aMsg = StringFormat(
            ...
            "پوزیشن باز رو زیر نظر بگیر\n"
            "⚠️ Rule STOP/CLOSE فقط با Crisis≥Yellow فعال می‌شود",
   ...
   // 🆕 v14.13 Patch 6: اگه یکی از دو جهت الان زیر Cooldown است (Spike
   // Warning دیده شده ولی هنوز streak به آستانه نرسیده)، در برچسب و
   // tooltip چراغ SPIKE مشخص می‌شود — کاربر می‌فهمد چرا هنوز STOP نشده.
   string cooldownNote = "";
   if(inp_SpikeCooldownBars > 0)
   {
      bool buyCooling  = (g_gbpnzdSpikeStreakBuy  > 0 && g_gbpnzdSpikeStreakBuy  < inp_SpikeCooldownBars);
      bool sellCooling = (g_gbpnzdSpikeStreakSell > 0 && g_gbpnzdSpikeStreakSell < inp_SpikeCooldownBars);
      if(buyCooling || sellCooling)
      {
         string cdBuyTxt  = buyCooling  ? StringFormat("Buy %d/%d",  g_gbpnzdSpikeStreakBuy,  inp_SpikeCooldownBars) : "";
         string cdSellTxt = sellCooling ? StringFormat("Sell %d/%d", g_gbpnzdSpikeStreakSell, inp_SpikeCooldownBars) : "";
         string sep = (buyCooling && sellCooling) ? " | " : "";
         cooldownNote = " ⏳Cooldown " + cdBuyTxt + sep + cdSellTxt;
         dispLabel += cooldownNote;
      }
   }
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 1: BuildDirectionSnapshot — Single Source of Truth
//
// یک عکسِ کامل و مستقل از وضعیت پوزیشن/دکمه برای یک جهت مشخص می‌سازد.
// فرمول‌ها عیناً از GBPNZD_Replay_CrisisAtBar / GBPNZD_Replay_SpikeAtBar
// (که سال‌ها در Replay/بکتست تست و کالیبره شده‌اند) کپی شده‌اند — هیچ
// فرمول جدیدی اختراع نشده، فقط محل محاسبه یکی شده است.
//
// قرارداد shift: از قرارداد استاندارد MQL پیروی می‌کند — shift=1 یعنی
// «آخرین کندل کاملاً بسته» (دقیقاً هم‌معنی با shift پیش‌فرض Spike_TFScore
// و با offset=1 که خود TL_Update برای RTM/TREND زنده به کار می‌برد).
// برای حفظ ۱۰۰٪ رفتار توابع قدیمی (که h1Shift+1 را به‌عنوان offset واقعی
// به کار می‌بردند)، اینجا h1ShiftLegacy=shift-1 محاسبه و دقیقاً همان
// فرمول‌های قدیمی روی آن اجرا می‌شود — یعنی BuildDirectionSnapshot(shift=1)
// معادل GBPNZD_Replay_CrisisAtBar(h1Shift=0) قدیمی است (نگاه کن به
// v14.13_Changes.md برای جدول نگاشت کامل و چرایی این انتخاب).
//
// 🔒 DESIGN LOCK حفظ‌شده از v14.07: RTM/TREND/STRUCT همیشه از H1/H4/D1
// چارت خود سمبل می‌آیند، صرف‌نظر از پارامتر tf — این تایم‌فریم‌ها از
// بکتست واقعی کالیبره شده‌اند. پارامتر tf برای مسیرهای آیندهٔ چندتایم‌فریمی
// و مستندسازی نگه داشته شده (طبق درخواست Patch 1) ولی امروز فقط برای
// انتخاب مسیر «سمبل خودِ چارت» در برابر «سمبل خارجی (FOREIGN)» اثر ندارد؛
// شیفت‌دهی همیشه بر مبنای کندل H1 است.
// ════════════════════════════════════════════════════════════════════
void BuildDirectionSnapshot_SpikeOwnChart(bool forBuy, int h1ShiftLegacy, HMDirSnapshot &out)
{
   int m15Shift = h1ShiftLegacy * 4;
   int m30Shift = h1ShiftLegacy * 2;
   int h4Shift  = h1ShiftLegacy / 4;
   int d1Shift  = h1ShiftLegacy / 24;
   ...
   double sM15 = 0.0, sM30 = 0.0, sH1 = 0.0, sH4 = 0.0, sD1 = 0.0;
   {
      ...
      int             shifts[5] = {m15Shift, m30Shift, h1ShiftLegacy, h4Shift, d1Shift};
      double          results[5];
      int atrPeriod = 14;
   ...
   double thW, thS;
   Spike_GetThresholds(_Symbol, thW, thS);
   int rawPhase = (score < thW) ? 0 : (score <= thS) ? 1 : 2;
   out.rawSpikePhase = rawPhase;

   int h4ShiftFlow = h1ShiftLegacy / 4;
   double fsBuy = 0.0, fsSell = 0.0;
   SisterEntry spArr[];
   int spN = Sister_GetTable(_Symbol, spArr);
   for(int si = 0; si < spN; si++)
   {
      string ssymFlow = FlowFindSym(spArr[si].sym);
      if(Bars(ssymFlow, PERIOD_H4) < h4ShiftFlow + 8) continue;
      double c1 = iClose(ssymFlow, PERIOD_H4, h4ShiftFlow + 1);
      double c6 = iClose(ssymFlow, PERIOD_H4, h4ShiftFlow + 6);
   ...
   double fgTh, frTh;
   Flow_GetThresholds(_Symbol, fgTh, frTh);

   if(forBuy)
   {
      bool flowRedBuy    = (fsBuy < frTh);
      bool flowYellowBuy = (fsBuy >= frTh && fsBuy < fgTh && score >= thS);
      if(rawPhase >= 1 && flowRedBuy)  out.spike = rawPhase;
      else if(flowYellowBuy)           out.spike = 1;
      else                             out.spike = 0;
   }
   else
   {
      bool flowRedSell    = (fsSell < frTh);
      bool flowYellowSell = (fsSell >= frTh && fsSell < fgTh && score >= thS);
      if(rawPhase >= 1 && flowRedSell) out.spike = rawPhase;
      else if(flowYellowSell)          out.spike = 1;
      else                             out.spike = 0;
   }
}

void BuildDirectionSnapshot(string symbol, bool forBuy, ENUM_TIMEFRAMES tf, int shift, HMDirSnapshot &out)
{
   // ── مقادیر پیش‌فرض ایمن ────────────────────────────────────────────
   out.valid         = false;
   out.symbol         = symbol;
   ...
   out.shift           = shift;
   out.evalBarTime    = 0;
   out.rtm             = 1; out.trend = 1; out.structD1 = 1; out.flowLight = 1;
   out.rc               = 0; out.crisis = -1;
   out.adx              = 0.0; out.flow = 0.0; out.trendScore = 0.0; out.highAlert = false;
   out.rawSpikePhase = -1; out.spike = -1; out.spikeScore = 0.0;

   if(shift < 1) shift = 1;
   int h1ShiftLegacy = shift - 1;   // معادل دقیق h1Shift در توابع Replay پیشین

   bool isOwnChart = (symbol == _Symbol);

   if(isOwnChart)
   {
      if(handleEMA200_H1_TL == INVALID_HANDLE || handleATR_H1_TL == INVALID_HANDLE ||
         handleADX_H4_TL == INVALID_HANDLE || handleATR_D1_TL == INVALID_HANDLE)
         return;   // handle ها هنوز گرم نشده — valid=false باقی می‌ماند

      out.evalBarTime = iTime(_Symbol, PERIOD_H1, h1ShiftLegacy + 1);

      // ── RTM ──────────────────────────────────────────────────────
      int newRTM = 1;
      if(BarsCalculated(handleEMA200_H1_TL) > 210 + h1ShiftLegacy &&
         BarsCalculated(handleATR_H1_TL)    > 20  + h1ShiftLegacy)
      {
         double ema[], atr[];
         ArraySetAsSeries(ema, true);
         ArraySetAsSeries(atr, true);
         bool ok = (CopyBuffer(handleEMA200_H1_TL, 0, h1ShiftLegacy + 1, 3, ema) == 3) &&
                   (CopyBuffer(handleATR_H1_TL,    0, h1ShiftLegacy + 1, 2, atr) == 2) &&
                   (atr[0] > 0);
         if(ok)
         {
            double h1c0 = iClose(_Symbol, PERIOD_H1, h1ShiftLegacy + 1);
            double h1c1 = iClose(_Symbol, PERIOD_H1, h1ShiftLegacy + 2);
      ...
      double adxVal    = 0.0;
      double erH4      = 0.0;
      if(BarsCalculated(handleADX_H4_TL) > 20 + h4Shift)
      {
      ...
      int newStruct = 1;
      {
         ...
         double curPrice = iClose(_Symbol, PERIOD_H1, h1ShiftLegacy + 1);
      ...
      double flowScore = 0.0;
      {
         ...
         for(int si = 0; si < sN; si++)
         {
            ...
            if(Bars(sSym, PERIOD_H4) < 8 + h4ShiftFlow) continue;
         ...
         flowScore = fs;
         double _fgTh, _frTh; Flow_GetThresholds(_Symbol, _fgTh, _frTh);
         if(fs > _fgTh)        newFLOW = 0;
         else if(fs >= _frTh)  newFLOW = 1;
         else                   newFLOW = 2;
      }
      out.flowLight = newFLOW;
      out.flow      = flowScore;
      ...
      double trendScoreEst = (g_csvScore != 0.0) ? g_csvScore : (erH4 * adxVal * 0.5);
      out.trendScore = trendScoreEst;
      double tsAgainst   = forBuy ? -trendScoreEst : trendScoreEst;
      double flowAgainst = -flowScore;

      int crisisLevel;
      if(rc < 0)
         crisisLevel = -1;
      else if((rc >= 3 && flowAgainst >= th_RedA_Flow && adxVal > th_RedA_ADX) ||
              (rc >= 2 && flowAgainst >= th_RedB_Flow && adxVal > th_RedB_ADX))
         crisisLevel = 2;
      else if((rc >= 2 && flowAgainst >= th_OrA_Flow && adxVal > th_OrA_ADX) ||
              (rc >= 3 && tsAgainst > th_OrB_Ts && adxVal > th_OrB_ADX) ||
              (rc >= 2 && flowAgainst >= th_OrC_Flow && adxVal > th_OrC_ADX && tsAgainst > th_OrC_Ts) ||
              (tsAgainst > th_OrD_Ts && adxVal > th_OrD_ADX))
         crisisLevel = 3;
         ...
         crisisLevel = 0;
      out.crisis = crisisLevel;

      // 🐛 v14.10 BUG-HA1 FIX (حفظ‌شده): علامت Sell دیگر معکوس نیست
      out.highAlert = (adxVal >= 35.0) && (flowScore < -7.0);

      // ── Spike (۵ تایم‌فریم، همان فرمول GBPNZD_Replay_SpikeAtBar) ──
      BuildDirectionSnapshot_SpikeOwnChart(forBuy, h1ShiftLegacy, out);

      out.valid = true;
   }
   else
   {
      // ── سمبل خارجی (FOREIGN On-Demand) — فقط وقتی handle های موقت
      // OnDemand برای همین سمبل گرم شده باشند (Patch 11: در غیر این
      // صورت valid=false می‌ماند و فراخوان باید Low-Confidence نشان دهد)
      if(CleanSymbol(symbol) == g_odCleanSymbol &&
         g_odHandleEMA200 != INVALID_HANDLE && g_odHandleATR_H1 != INVALID_HANDLE &&
         g_odHandleADX_H4 != INVALID_HANDLE && g_odHandleATR_D1 != INVALID_HANDLE)
      {
         double flowOut = 0.0, adxOut = 0.0;
         int    rcOut   = 0;
         int crisisLevel = Generic_ReplayCrisisAtBar(g_odBrokerSymbol,
                              g_odHandleEMA200, g_odHandleATR_H1, g_odHandleADX_H4, g_odHandleATR_D1,
                              h1ShiftLegacy, forBuy, flowOut, adxOut, rcOut);
         out.crisis     = crisisLevel;
         out.rc          = rcOut;
         out.flow        = flowOut;
         out.adx         = adxOut;
         out.highAlert  = (adxOut >= 35.0) && (flowOut < -7.0);
         out.evalBarTime = iTime(g_odBrokerSymbol, PERIOD_H1, h1ShiftLegacy + 1);
         // rtm/trend/structD1 مجزا برای سمبل خارجی در دسترس نیست —
         // Generic_ReplayCrisisAtBar فقط rc ترکیبی برمی‌گرداند؛ crisis/rc/flow/adx معتبرند.
         out.rtm = -1; out.trend = -1; out.structD1 = -1;

         int spBuyOut = -1, spSellOut = -1;
         int rawPh = Generic_ReplaySpikeAtBar(g_odBrokerSymbol, h1ShiftLegacy, spBuyOut, spSellOut);
         out.rawSpikePhase = rawPh;
         out.spike           = forBuy ? spBuyOut : spSellOut;
         out.valid            = (crisisLevel >= 0);
      }
   }
}
...
{
   // 🆕 v14.13 Patch 1: اکنون یک wrapper نازک روی BuildDirectionSnapshot است.
   // نگاشت: shift جدید = h1Shift قدیمی + 1 (تا h1ShiftLegacy داخل
   // BuildDirectionSnapshot دقیقاً همان h1Shift قدیمی بازسازی شود) —
   // یعنی رفتار این تابع برای همهٔ فراخوان‌های موجود صددرصد حفظ شده.
   HMDirSnapshot snap;
...
{
   // 🆕 v14.13 Patch 1: wrapper نازک روی BuildDirectionSnapshot — همان
   // نگاشت shift = h1Shift+1 مثل GBPNZD_Replay_CrisisAtBar (رفتار قدیمی حفظ شده).
   HMDirSnapshot snapBuy, snapSell;
   BuildDirectionSnapshot(_Symbol, true,  PERIOD_H1, h1Shift + 1, snapBuy);
   BuildDirectionSnapshot(_Symbol, false, PERIOD_H1, h1Shift + 1, snapSell);
   spikeBuyOut  = snapBuy.spike;
   spikeSellOut = snapSell.spike;
   if(!snapBuy.valid && !snapSell.valid) return -1;
   return snapBuy.valid ? snapBuy.rawSpikePhase : snapSell.rawSpikePhase;
}
                               ...
                               double &flowScoreOut, double &adxValOut, int &rcOut)
{
   ...
   double flowScore = 0.0;
   {
      ...
      flowScore = fs;
      // 🆕 v14.11: Flow threshold از Flow_GetThresholds (رفع BUG-FLOW-HARDCODE)
      { double _fgTh, _frTh; Flow_GetThresholds(sym, _fgTh, _frTh);
        if(fs > _fgTh)        newFLOW = 0;
        else if(fs >= _frTh)  newFLOW = 1;
        else                   newFLOW = 2; }
   }
...
int Generic_ReplaySpikeAtBar(string sym, int h1Shift, int &spikeBuyOut, int &spikeSellOut)
{
   ...
   // 🆕 v14.11: rawPhase از per-symbol thresholds
   double _gr_thW, _gr_thS; Spike_GetThresholds(sym, _gr_thW, _gr_thS);
   int rawPhase = (score < _gr_thW) ? 0 : (score <= _gr_thS) ? 1 : 2;
   ...
   // 🆕 v14.11: Flow gate از Flow_GetThresholds (رفع BUG-FLOW-HARDCODE)
   double _gr_fgTh, _gr_frTh; Flow_GetThresholds(sym, _gr_fgTh, _gr_frTh);
   {
      bool flowRedBuy     = (fsBuy < _gr_frTh);
      bool flowYellowBuy  = (fsBuy >= _gr_frTh && fsBuy < _gr_fgTh && score >= _gr_thS);
      if(rawPhase >= 1 && flowRedBuy)  spikeBuyOut = rawPhase;
      else if(flowYellowBuy)           spikeBuyOut = 1;
      else                             spikeBuyOut = 0;
   }
   {
      bool flowRedSell    = (fsSell < _gr_frTh);
      bool flowYellowSell = (fsSell >= _gr_frTh && fsSell < _gr_fgTh && score >= _gr_thS);
...
void OnDemand_RunAndSend()
{
   ...
   // 🆕 v14.13 Patch 11 [FOREIGN-INSUFFICIENT-DATA]: علاوه بر iBars، وضعیت
   // SERIES_SYNCHRONIZED هم چک می‌شود (ممکنه iBars>0 باشه ولی ترمینال هنوز
   // کامل sync نکرده باشه). به‌جای skip کامل در حالت مرزی، پرچم
   // odLowConfidence ست می‌شود و در متن پیام نهایی به کاربر گفته می‌شود —
   // بجای اینکه بی‌صدا هیچ پاسخی نیاد.
   bool odLowConfidence = false;
   {
      int _availBars = iBars(broker, PERIOD_CURRENT);
      bool _synced = SeriesInfoInteger(broker, PERIOD_CURRENT, SERIES_SYNCHRONIZED);
      if(!_synced)
      {
         Print("⚠️ FOREIGN ", broker, " SERIES_SYNCHRONIZED=false — Low Confidence");
         odLowConfidence = true;
      }
   ...
   for(int odShift = odBarsToScan; odShift >= 1; odShift--)
   {
      ...
      // 🐛 v14.10 BUG-HA1 FIX: علامت haSell معکوس بود (> 7.0 → < -7.0)
      // flowSell < 0 یعنی بازار به ضرر Sell می‌رود → این شرط درست HighAlert است
   ...
   // 🆕 v14.15 P2 [OD-TIME-ALIGN]: بررسی هم‌ترازی زمانی کندل آخر نماد خارجی
   // با نماد اصلی — اگه بروکر هنوز کندل نماد خارجی را آپدیت نکرده باشه،
   // گزارش روی دادهٔ کهنه ساخته می‌شه. فقط نمایشی — روی محاسبات اثر ندارد.
   bool odTimeMisaligned = false;
   {
      datetime mainLastBarTime = iTime(_Symbol, PERIOD_CURRENT, 1);
      datetime foreignLastBarTime = iTime(broker, PERIOD_CURRENT, 1);
      if(mainLastBarTime > 0 && foreignLastBarTime > 0 &&
         MathAbs(mainLastBarTime - foreignLastBarTime) > PeriodSeconds(PERIOD_CURRENT))
         odTimeMisaligned = true;
   }
   ...
   string hdr = "📟 وضعیت لحظه‌ای " + clean + " (" + broker + ")\n"
              ...
              + (odLowConfidence ? "⚠️ Low Confidence — دادهٔ این سمبل کامل sync/کافی نبود، نتیجه تقریبی است.\n" : "")
              + (odTimeMisaligned ? "⚠️ هشدار: زمان آخرین کندل " + clean + " با " + _Symbol + " هم‌تراز نیست — ممکن است داده‌ها قدیمی باشند.\n" : "")
              + "─────────────────────\n";
...
// 🆕 v14.18: یادداشت روشن‌سازی (بدون تغییر منطق) — با اینکه اسمش GBPNZD
// دارد، این تابع از v14.00 بر مبنای Rule_GVPrefix(_Symbol) کار می‌کند و
// وقتی این EA مستقیم روی چارت EURGBP یا AUDCAD باز شود هم صحیح و برای
// همان سیمبل چارت عمل می‌کند — اسم صرفاً تاریخی است (از زمانی که فقط
// GBPNZD پشتیبانی می‌شد)، نه یک محدودیت واقعی.
void GBPNZD_GV_Save()
{
   // 🩹 PATCH v13.51: کلید نسخه ساختار GV — برای تشخیص GV قدیمی (قبل از پچ)
   // از GV جدید که واقعاً Level=0 (GREEN) هست.
   // 🆕 v14.13 Patch 9 [MISSING-SCHEMA-VERSION]: قبلاً این کلید همیشه مقدار
   // ثابت 1400.0 داشت (هرگز بین v14.01 تا v14.12 بروز نشد) — یعنی عملاً
   // فقط "قبل/بعد از v13.51" را تشخیص می‌داد، نه نسخهٔ واقعی. حالا واقعاً
   // با هر نسخهٔ اسکیمای state تغییر می‌کند (الان 1413) و periodSeconds
   // هم ذخیره می‌شود — اگه هرکدام عوض شده باشد (آپدیت EA یا تغییر TF
   // چارت)، GBPNZD_GV_Load آن را نامعتبر می‌داند و Replay کامل اجرا
   // می‌شود، به‌جای استفاده از state ای که واحدهایش دیگر معتبر نیست.
   string pfx = Rule_GVPrefix(_Symbol);
   GlobalVariableSet(pfx+"Ver",       HM_SCHEMA_VERSION);
   GlobalVariableSet(pfx+"PerSec",    (double)g_chartPeriodSeconds);
   ...
   // 🆕 v14.13 Patch 2/4/8: spikeStreak/cleanSince/redSince/lastDirty*/lastReminder*
   GlobalVariableSet(pfx+"SpkStrBuy",  (double)g_gbpnzdSpikeStreakBuy);
   GlobalVariableSet(pfx+"SpkStrSell", (double)g_gbpnzdSpikeStreakSell);
   GlobalVariableSet(pfx+"ClSinceBuy", (double)g_gbpnzdCleanSinceBuy);
   GlobalVariableSet(pfx+"ClSinceSell",(double)g_gbpnzdCleanSinceSell);
   GlobalVariableSet(pfx+"RdSinceBuy", (double)g_gbpnzdRedSinceBuy);
   GlobalVariableSet(pfx+"RdSinceSell",(double)g_gbpnzdRedSinceSell);
   GlobalVariableSet(pfx+"DrtTBuy",    (double)g_gbpnzdLastDirtyTimeBuy);
   GlobalVariableSet(pfx+"DrtTSell",   (double)g_gbpnzdLastDirtyTimeSell);
   GlobalVariableSet(pfx+"DrtRBuy",    (double)g_gbpnzdLastDirtyReasonBuy);
   GlobalVariableSet(pfx+"DrtRSell",   (double)g_gbpnzdLastDirtyReasonSell);
   GlobalVariableSet(pfx+"RemTBuy",    (double)g_gbpnzdLastReminderBuy);
   GlobalVariableSet(pfx+"RemTSell",   (double)g_gbpnzdLastReminderSell);
   GlobalVariableSet(pfx+"RemLBuy",    (double)g_gbpnzdLastReminderLevelBuy);
   GlobalVariableSet(pfx+"RemLSell",   (double)g_gbpnzdLastReminderLevelSell);
   GlobalVariableSet(pfx+"Saved",   (double)TimeCurrent());
}

// بازگشت: true = لود موفق | false = GV موجود نبود یا stale بود
// 🆕 v14.18: یادداشت روشن‌سازی (بدون تغییر منطق) — مثل GBPNZD_GV_Save،
// این تابع هم بر مبنای Rule_GVPrefix(_Symbol) کار می‌کند؛ روی چارت
// EURGBP یا AUDCAD هم صحیح عمل می‌کند. اسم صرفاً تاریخی است.
bool GBPNZD_GV_Load()
{
   string pfx = Rule_GVPrefix(_Symbol);
   ...
   // 🆕 v14.13 Patch 9 [MISSING-SCHEMA-VERSION]: اگه نسخهٔ ذخیره‌شده قدیمی‌تر
   // از اسکیمای فعلی باشه (مثلاً از v14.12 و قبل‌تر که Ver همیشه 1400.0
   // بود)، یا periodSeconds با تایم‌فریم فعلی چارت فرق کند (کاربر TF چارت
   // را عوض کرده)، state ذخیره‌شده را نامعتبر بدان — واحدهای CleanH/
   // spikeStreak دیگر با شمارندهٔ فعلی قابل مقایسه نیستند. Replay کامل
   // با شمارنده‌های صفر شروع می‌شود که همیشه ایمن‌تر از استفادهٔ کورکورانه
   // از state با واحد اشتباه است.
   double savedVer = GlobalVariableGet(pfx+"Ver");
   if(savedVer < HM_SCHEMA_VERSION)
   {
      Print("ℹ️ v14.13 GBPNZD_GV_Load: نسخهٔ GV قدیمی (", savedVer, " < ", HM_SCHEMA_VERSION,
            ") — نادیده گرفته شد، Replay کامل اجرا می‌شود");
      return false;
   }
   if(GlobalVariableCheck(pfx+"PerSec"))
   {
      int savedPerSec = (int)GlobalVariableGet(pfx+"PerSec");
      if(savedPerSec != g_chartPeriodSeconds)
      {
         Print("ℹ️ v14.13 GBPNZD_GV_Load: periodSeconds تغییر کرده (ذخیره‌شده=", savedPerSec,
               " فعلی=", g_chartPeriodSeconds, ") — نادیده گرفته شد، Replay کامل اجرا می‌شود");
         return false;
      }
   }
   ...
   // 🆕 v14.13 Patch 2/4/8: بازیابی spikeStreak/cleanSince/redSince/lastDirty*/
   // lastReminder* — اگه کلیدها نبودند (GV از نسخهٔ قدیمی‌تر) مقدار پیش‌فرض
   // امن می‌گیرند؛ در عمل چون بالاتر savedVer<HM_SCHEMA_VERSION کل GV را رد
   // می‌کند، این حالت فقط برای ایمنی دفاعی اضافه اینجاست.
   g_gbpnzdSpikeStreakBuy      = GlobalVariableCheck(pfx+"SpkStrBuy")   ? (int)GlobalVariableGet(pfx+"SpkStrBuy")    : 0;
   g_gbpnzdSpikeStreakSell     = GlobalVariableCheck(pfx+"SpkStrSell")  ? (int)GlobalVariableGet(pfx+"SpkStrSell")   : 0;
   g_gbpnzdCleanSinceBuy       = GlobalVariableCheck(pfx+"ClSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"ClSinceBuy") : 0;
   g_gbpnzdCleanSinceSell      = GlobalVariableCheck(pfx+"ClSinceSell") ? (datetime)GlobalVariableGet(pfx+"ClSinceSell"): 0;
   g_gbpnzdRedSinceBuy         = GlobalVariableCheck(pfx+"RdSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"RdSinceBuy") : 0;
   g_gbpnzdRedSinceSell        = GlobalVariableCheck(pfx+"RdSinceSell") ? (datetime)GlobalVariableGet(pfx+"RdSinceSell"): 0;
   g_gbpnzdLastDirtyTimeBuy    = GlobalVariableCheck(pfx+"DrtTBuy")     ? (datetime)GlobalVariableGet(pfx+"DrtTBuy")    : 0;
   g_gbpnzdLastDirtyTimeSell   = GlobalVariableCheck(pfx+"DrtTSell")    ? (datetime)GlobalVariableGet(pfx+"DrtTSell")   : 0;
   g_gbpnzdLastDirtyReasonBuy  = GlobalVariableCheck(pfx+"DrtRBuy")     ? (int)GlobalVariableGet(pfx+"DrtRBuy")      : 0;
   g_gbpnzdLastDirtyReasonSell = GlobalVariableCheck(pfx+"DrtRSell")    ? (int)GlobalVariableGet(pfx+"DrtRSell")     : 0;
   g_gbpnzdLastReminderBuy      = GlobalVariableCheck(pfx+"RemTBuy")     ? (datetime)GlobalVariableGet(pfx+"RemTBuy")    : 0;
   g_gbpnzdLastReminderSell     = GlobalVariableCheck(pfx+"RemTSell")    ? (datetime)GlobalVariableGet(pfx+"RemTSell")   : 0;
   g_gbpnzdLastReminderLevelBuy = GlobalVariableCheck(pfx+"RemLBuy")     ? (int)GlobalVariableGet(pfx+"RemLBuy")      : 0;
   g_gbpnzdLastReminderLevelSell= GlobalVariableCheck(pfx+"RemLSell")    ? (int)GlobalVariableGet(pfx+"RemLSell")     : 0;
...
void Rule_SpikeWeightsForSymbol(string sym, double &wM15, double &wM30, double &wH1, double &wH4, double &wD1)
{
   ...
   {
      // 🆕 v14.11: از Inputs می‌خواند
      // P2: M15↓0.25→0.20 | M30↓0.20→0.15 | H1↑0.30→0.35 | H4↑0.15→0.20
      // اسپایک‌های واقعی در H1/H4 قابل‌مشاهده‌اند — M15/M30 نویز اضافه می‌کنند
      wM15 = inp_GBPNZD_wM15;   // default 0.20
      wM30 = inp_GBPNZD_wM30;   // default 0.15
      wH1  = inp_GBPNZD_wH1;    // default 0.35
      wH4  = inp_GBPNZD_wH4;    // default 0.20
      wD1  = inp_GBPNZD_wD1;    // default 0.10
   }
   else if(CleanSymbol(sym) == "EURGBP")
   {
      // 🆕 v14.11: از Inputs می‌خواند (placeholder — مقادیر v14.09 حفظ شدند)
      wM15 = inp_EURGBP_wM15;   // default 0.05
      wM30 = inp_EURGBP_wM30;   // default 0.08
      wH1  = inp_EURGBP_wH1;    // default 0.32
      wH4  = inp_EURGBP_wH4;    // default 0.35
      wD1  = inp_EURGBP_wD1;    // default 0.20
   }
   else
   {
      // AUDCAD از Inputs (placeholder) + پیش‌فرض برای سمبل‌های ناشناخته
      if(CleanSymbol(sym) == "AUDCAD")
      {
         wM15 = inp_AUDCAD_wM15;   // default 0.15
         wM30 = inp_AUDCAD_wM30;   // default 0.15
         wH1  = inp_AUDCAD_wH1;    // default 0.30
         wH4  = inp_AUDCAD_wH4;    // default 0.25
         wD1  = inp_AUDCAD_wD1;    // default 0.15
      }
      else
      {
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.11: SanityCheck — تأیید درستی لود Inputs بعد از شروع
// در OnInit فراخوانی می‌شود — اگه Print با ⚠️ دیدید مشکل دارد
// ════════════════════════════════════════════════════════════════════
void v14_11_SanityCheck()
{
   double warn, spike;
   Spike_GetThresholds("GBPNZD", warn, spike);
   if(MathAbs(warn - inp_GBPNZD_SpikeWarn) > 0.001 || MathAbs(spike - inp_GBPNZD_SpikeSig) > 0.001)
      Print("⚠️ v14.11 SANITY FAIL: Spike_GetThresholds GBPNZD mismatch warn=", warn, " spike=", spike);
   else
      Print("✅ v14.11 SANITY OK: Spike GBPNZD warn=", warn, " spike=", spike);

   double gTh, rTh;
   Flow_GetThresholds("GBPNZD", gTh, rTh);
   if(MathAbs(gTh - inp_GBPNZD_FlowGreen) > 0.001 || MathAbs(rTh - inp_GBPNZD_FlowRed) > 0.001)
      Print("⚠️ v14.11 SANITY FAIL: Flow_GetThresholds GBPNZD mismatch green=", gTh, " red=", rTh);
   else
      Print("✅ v14.11 SANITY OK: Flow GBPNZD green=", gTh, " red=", rTh);

   double wM15, wM30, wH1, wH4, wD1;
   Rule_SpikeWeightsForSymbol("GBPNZD", wM15, wM30, wH1, wH4, wD1);
   double wSum = wM15 + wM30 + wH1 + wH4 + wD1;
   if(MathAbs(wSum - 1.0) > 0.005)
      Print("⚠️ v14.11 SANITY FAIL: SpikeWeights GBPNZD sum=", wSum, " (expected 1.0)");
   else
      Print("✅ v14.11 SANITY OK: SpikeWeights GBPNZD H1=", wH1, " H4=", wH4, " sum=", wSum);
}
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 2: ApplyClosedBarToRule — منطق واحد Rule + Cooldown
//
// دقیقاً یک‌بار پیاده‌سازی می‌شود و در هر سه مسیر صدا زده می‌شود:
//   • Alert_CheckGBPNZDRule   (لایو، هر کندل بسته چارت)
//   • GBPNZD_InitReplay       (بازسازی state بعد از ری‌استارت EA)
//   • LogHourlySnapshot       (بکتست، هر «ساعت» شبیه‌سازی‌شده)
// پیش از این پچ، Cooldown فقط در Alert_CheckGBPNZDRule وجود داشت —
// یعنی بکتست هرگز Cooldown را نمی‌دید (BUG-COOLDOWN-BT).
//
// snap        : HMDirSnapshot تازه برای این جهت (shift=1 → آخرین کندل بسته)
// extraDirty/extraRed: accumulator تیک‌محورِ لایو (فقط لایو مقدار غیر-false
//               دارد؛ Replay/بکتست همیشه false می‌فرستند چون آنجا هر
//               فراخوانی خودش دقیقاً یک ساعت کامل است، نه انباشتِ چند تیک).
//               این OR صرفاً یک شبکهٔ ایمنی اضافه است: حتی اگه به هر
//               دلیلی accumulator قدیمی چیزی گرفته باشد که Snapshot از
//               دست داده، نتیجه باز هم درست (بدتر) می‌ماند.
// cooldownBars: inp_SpikeCooldownBars (0=غیرفعال)
// nowTime     : برای cleanSince/redSince/lastDirtyTime (Patch 8/6/7)
// ════════════════════════════════════════════════════════════════════
void ApplyClosedBarToRule(const HMDirSnapshot &snap, bool extraDirty, bool extraRed,
                           int cooldownBars, datetime nowTime, HMRuleState &state)
{
   if(!snap.valid) return;   // داده ناکافی — این کندل رد می‌شود، state دست‌نخورده می‌ماند

   bool crisisRed        = (snap.crisis == 2);
   bool spikeWarningOnly = (snap.spike == 1);
   bool spikeFull        = (snap.spike == 2);
   bool spikeAny         = (snap.spike >= 1);

   // 🐛 BUG-SPIKEONLY-DIRTY FIX: streak فقط با Spike واقعی (Warning/Full)
   // بالا/پایین می‌رود — هرگز با HighAlert یا Crisis قاطی نمی‌شود.
   if(spikeAny) state.spikeStreak++;
   else          state.spikeStreak = 0;

   // ── Cooldown فقط روی Spike Warning تنها اعمال می‌شود ──────────────
   // Full Spike و Crisis Red هرگز مشمول Cooldown نیستند — همیشه فوری.
   bool cooldownActive      = (cooldownBars > 0);
   bool spikeHeldByCooldown = spikeWarningOnly && cooldownActive && (state.spikeStreak < cooldownBars);
   bool spikeDirty          = spikeFull || (spikeWarningOnly && !spikeHeldByCooldown);

   bool hourRed       = crisisRed || extraRed;
   bool effectiveDirty = hourRed || spikeDirty || extraDirty;

   // ── DirtyReason (Patch 6/7) ───────────────────────────────────────
   if(effectiveDirty)
   {
      state.lastDirtyTime   = nowTime;
      state.lastDirtyReason = (hourRed && spikeDirty) ? HM_DIRTY_REASON_BOTH :
                               hourRed                  ? HM_DIRTY_REASON_CRISIS :
                                                            HM_DIRTY_REASON_SPIKE;
   }
   else
      state.lastDirtyReason = HM_DIRTY_REASON_NONE;

   // ── شمارنده‌های کندلی (نمایش/سازگاری با کد قدیمی) ──────────────────
   Rule_UpdateCounters(hourRed, effectiveDirty, state.crisisRedH, state.cleanH);

   // ── زمان واقعی — Patch 8 (جایگزین/مکمل شمارش خام کندل) ─────────────
   if(hourRed) { if(state.redSince == 0) state.redSince = nowTime; }
   else          state.redSince = 0;
   if(!effectiveDirty) { if(state.cleanSince == 0) state.cleanSince = nowTime; }
   else                  state.cleanSince = 0;

   // ── ماشین‌حالت سه‌سطحی GREEN→STOP→CLOSE (تابع مشترک قدیمی، دست‌نخورده) ──
   Rule_Transition(snap.crisis, snap.spike, snap.highAlert,
                    state.crisisRedH, state.cleanH, state.level, state.stopReason);
}

// ── کمکی: HMRuleState را از globalهای مسطح فعلی «hydrate» می‌کند ─────
void HMRuleState_LoadBuy(HMRuleState &st)
{
   st.level            = g_gbpnzdLevelBuy;
   st.stopReason        = g_gbpnzdStopReasonBuy;
   st.crisisRedH        = g_gbpnzdCrisisRedHBuy;
   st.cleanH            = g_gbpnzdCleanHBuy;
   st.cleanSince        = g_gbpnzdCleanSinceBuy;
   st.redSince          = g_gbpnzdRedSinceBuy;
   st.spikeStreak       = g_gbpnzdSpikeStreakBuy;
   st.lastReminderTime = g_gbpnzdLastReminderBuy;
   st.lastDirtyTime     = g_gbpnzdLastDirtyTimeBuy;
   st.lastDirtyReason   = g_gbpnzdLastDirtyReasonBuy;
}
void HMRuleState_SaveBuy(const HMRuleState &st)
{
   g_gbpnzdLevelBuy            = st.level;
   g_gbpnzdStopReasonBuy       = st.stopReason;
   g_gbpnzdCrisisRedHBuy       = st.crisisRedH;
   g_gbpnzdCleanHBuy           = st.cleanH;
   g_gbpnzdCleanSinceBuy       = st.cleanSince;
   g_gbpnzdRedSinceBuy         = st.redSince;
   g_gbpnzdSpikeStreakBuy      = st.spikeStreak;
   g_gbpnzdLastReminderBuy     = st.lastReminderTime;
   g_gbpnzdLastDirtyTimeBuy    = st.lastDirtyTime;
   g_gbpnzdLastDirtyReasonBuy  = st.lastDirtyReason;
}
void HMRuleState_LoadSell(HMRuleState &st)
{
   st.level            = g_gbpnzdLevelSell;
   st.stopReason        = g_gbpnzdStopReasonSell;
   st.crisisRedH        = g_gbpnzdCrisisRedHSell;
   st.cleanH            = g_gbpnzdCleanHSell;
   st.cleanSince        = g_gbpnzdCleanSinceSell;
   st.redSince          = g_gbpnzdRedSinceSell;
   st.spikeStreak       = g_gbpnzdSpikeStreakSell;
   st.lastReminderTime = g_gbpnzdLastReminderSell;
   st.lastDirtyTime     = g_gbpnzdLastDirtyTimeSell;
   st.lastDirtyReason   = g_gbpnzdLastDirtyReasonSell;
}
void HMRuleState_SaveSell(const HMRuleState &st)
{
   g_gbpnzdLevelSell           = st.level;
   g_gbpnzdStopReasonSell      = st.stopReason;
   g_gbpnzdCrisisRedHSell      = st.crisisRedH;
   g_gbpnzdCleanHSell          = st.cleanH;
   g_gbpnzdCleanSinceSell      = st.cleanSince;
   g_gbpnzdRedSinceSell        = st.redSince;
   g_gbpnzdSpikeStreakSell     = st.spikeStreak;
   g_gbpnzdLastReminderSell    = st.lastReminderTime;
   g_gbpnzdLastDirtyTimeSell   = st.lastDirtyTime;
   g_gbpnzdLastDirtyReasonSell = st.lastDirtyReason;
}

// 🆕 v14.18: یادداشت روشن‌سازی (بدون تغییر منطق) — این تابع هم از v14.00
// بر مبنای _Symbol کار می‌کند (IsRuleSymbol/Sister_GetTable/Rule_FlowForSymbol
// همگی generic هستند). روی چارت EURGBP یا AUDCAD هم صحیح replay می‌کند.
// اسم صرفاً تاریخی است، نه یک محدودیت واقعی.
void GBPNZD_InitReplay()
{
   ...
   for(int sh = _replayBarsToScan; sh >= 1; sh--)
   {
      ...
      //
      // 🆕 v14.13 Patch 2 [BUG-COOLDOWN-BT]: قبلاً این‌جا مستقیماً
      // Rule_UpdateCounters+Rule_Transition صدا زده می‌شد — بدون هیچ منطق
      // Cooldown (که فقط در Alert_CheckGBPNZDRule زنده وجود داشت). یعنی
      // بکتست/replay هرگز Cooldown نمی‌دید و رفتارش با لایو فرق می‌کرد.
      // حالا از همان ApplyClosedBarToRule مشترک استفاده می‌شود — Snapshot
      // سبک‌وزن از همان مقادیر crBuy/spikeBuy/rcBuy/haBuy که همین بالا با
      // GBPNZD_Replay_CrisisAtBar/SpikeAtBar (که خودشان اکنون به
      // BuildDirectionSnapshot تفویض می‌کنند) محاسبه شدند ساخته می‌شود —
      // هیچ فرمولی عوض نشده، فقط مسیر کنترل یکی شده.
      datetime replayEvalTime = iTime(_Symbol, PERIOD_H1, sh + 1);

      HMDirSnapshot snapReplayBuy;
      snapReplayBuy.valid = true; snapReplayBuy.crisis = crBuy; snapReplayBuy.spike = spikeBuy;
      snapReplayBuy.highAlert = haBuy; snapReplayBuy.evalBarTime = replayEvalTime;
      HMRuleState stReplayBuy;
      HMRuleState_LoadBuy(stReplayBuy);
      ApplyClosedBarToRule(snapReplayBuy, false, false, inp_SpikeCooldownBars, replayEvalTime, stReplayBuy);
      HMRuleState_SaveBuy(stReplayBuy);

      HMDirSnapshot snapReplaySell;
      snapReplaySell.valid = true; snapReplaySell.crisis = crSell; snapReplaySell.spike = spikeSell;
      snapReplaySell.highAlert = haSell; snapReplaySell.evalBarTime = replayEvalTime;
      HMRuleState stReplaySell;
      HMRuleState_LoadSell(stReplaySell);
      ApplyClosedBarToRule(snapReplaySell, false, false, inp_SpikeCooldownBars, replayEvalTime, stReplaySell);
      HMRuleState_SaveSell(stReplaySell);

      // 🩹 حفظ رفتار قدیمی: rHourRed/DirtyBuy/Sell باید مقدار «آخرین کندل
      // این حلقه» را نگه دارند — بعد از پایان loop برای seed کردن
      // accumulator زنده (g_gbpnzdHourRedBuy و...، خط‌های پایین‌تر همین
      // تابع) استفاده می‌شوند. حذف این خط‌ها یک رگرسیون بود که در بازبینی
      // پچ ۲ پیدا و همین‌جا اصلاح شد.
      rHourRedBuy    = (crBuy  == 2);
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v14.18: یادداشت روشن‌سازی (بدون تغییر منطق) — این تابع هسته‌ای، با
// اینکه اسمش GBPNZD دارد، از v14.00 کاملاً بر مبنای _Symbol کار می‌کند:
// روی هر چارتی (GBPNZD، EURGBP، یا AUDCAD) که این EA مستقیم باز شود،
// همان سیمبل چارت را به‌درستی مدیریت می‌کند. اسم صرفاً تاریخی است (از
// زمانی که فقط GBPNZD پشتیبانی می‌شد، v13). برای EURGBP وقتی چارت خودش
// GBPNZD است، بلوک موازی جدید (Alert_CheckEURGBPRule, v14.17) این کار
// را انجام می‌دهد.
void Alert_CheckGBPNZDRule()
{
   // ── پیش‌شرط‌ها (بدون تغییر) ──────────────────────────────────────────
   if(!Alert_OnGBPNZDRule)                return;
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!IsRuleSymbol(_Symbol))             return;

   // ── accumulator تیک‌محور (بدون تغییر) ─────────────────────────────
   // 🆕 v14.13: این‌ها دیگر تنها منبع تصمیم نیستند — فقط یک شبکهٔ ایمنیِ
   // OR اضافه به Snapshot تازه‌ای هستند که پایین‌تر ساخته می‌شود (رفع
   // BUG-STALE-INPUT بدون از دست دادن هیچ حساسیتی که accumulator قبلاً
   // به رویدادهای گذرای وسط کندل داشت — نگاه کن به v14.13_Changes.md).
   ...
   // اسنپ‌شات‌ها در سطح تابع تا بعد از بلوک هم برای پیام قابل دسترس باشند
   HMDirSnapshot snapBuy, snapSell;
   bool haveSnap = false;
   ...
   {
      // ════════════════════════════════════════════════════════════
      // 🆕 v14.13 Patch 1/2/3: منبع واحد — به‌جای خواندن g_gbnzdCrisisBuy/
      // Sell و g_gbnzdSpikeBuy/Sell (که بسته به شاخهٔ TL_Update که همین
      // الان اجرا شده، ممکن بود برای یک جهت به‌روز نشده باشند)، مستقیماً
      // BuildDirectionSnapshot برای هر دو جهت صدا زده می‌شود. این کار
      // مستقل از Direction=All/Buy/Sell و مستقل از وجود پوزیشن واقعی است
      // — دقیقاً رفع BUG-STALE-INPUT (مهم‌ترین باگ گزارش‌شده).
      // ════════════════════════════════════════════════════════════
      ...
      {
         HMRuleState stBuy;
         HMRuleState_LoadBuy(stBuy);
         ApplyClosedBarToRule(snapBuy, g_gbpnzdHourDirtyBuy, g_gbpnzdHourRedBuy,
                               inp_SpikeCooldownBars, snapBuy.evalBarTime, stBuy);
         HMRuleState_SaveBuy(stBuy);
      }
      else if(ShowDebugLogs)
         Print("⚠️ v14.13: BuildDirectionSnapshot(Buy) invalid این کندل — Rule Buy بدون تغییر ماند");

      if(snapSell.valid)
      {
         HMRuleState stSell;
         HMRuleState_LoadSell(stSell);
         ApplyClosedBarToRule(snapSell, g_gbpnzdHourDirtySell, g_gbpnzdHourRedSell,
                               inp_SpikeCooldownBars, snapSell.evalBarTime, stSell);
         HMRuleState_SaveSell(stSell);
      }
      else if(ShowDebugLogs)
         Print("⚠️ v14.13: BuildDirectionSnapshot(Sell) invalid این کندل — Rule Sell بدون تغییر ماند");

      if(ShowDebugLogs)
         Print("🔧 v14.13 Rule Buy: streak=", g_gbpnzdSpikeStreakBuy, "/", inp_SpikeCooldownBars,
               " level=", g_gbpnzdLevelBuy, " cleanH=", g_gbpnzdCleanHBuy,
               " | Sell: streak=", g_gbpnzdSpikeStreakSell, "/", inp_SpikeCooldownBars,
               " level=", g_gbpnzdLevelSell, " cleanH=", g_gbpnzdCleanHSell);
   ...
   {
      // شروع کندل جدید: seed accumulator با وضعیت اولین تیک کندل جدید (بدون تغییر)
      g_gbpnzdHourRedBuy    = (g_gbnzdCrisisBuy  == 2);
      g_gbpnzdHourDirtyBuy  = (g_gbnzdCrisisBuy  == 2) || (g_gbnzdSpikeBuy  >= 1);
      g_gbpnzdHourRedSell   = (g_gbnzdCrisisSell == 2);
      g_gbpnzdHourDirtySell = (g_gbnzdCrisisSell == 2) || (g_gbnzdSpikeSell >= 1);
   ...
   // اگه هیچ تغییری در سطح نبود → خروج.
   // 🆕 v14.13 Patch 4: یادآوری تکراری دیگر این‌جا نیست — Rule_CheckReminders()
   // از OnTimer (هر ثانیه، مستقل از کندل) این کار را انجام می‌دهد؛ رفع
   // BUG-REMINDER-TRANSITION (یادآوری‌ای که فقط سر کندل گذار چک می‌شد).
   if(g_gbpnzdLevelBuy == oldLevelBuy && g_gbpnzdLevelSell == oldLevelSell) return;

   int newLevelBuy   = g_gbpnzdLevelBuy;
   int newLevelSell  = g_gbpnzdLevelSell;
   int newReasonBuy  = g_gbpnzdStopReasonBuy;
   int newReasonSell = g_gbpnzdStopReasonSell;

   // worst-case مشترک (سازگاری با کد قدیمی که از این globals می‌خواند)
   g_gbpnzdLevel      = MathMax(g_gbpnzdLevelBuy, g_gbpnzdLevelSell);
   g_gbpnzdStopReason = (g_gbpnzdLevelBuy >= g_gbpnzdLevelSell) ? g_gbpnzdStopReasonBuy : g_gbpnzdStopReasonSell;
   ...
   // 🆕 v14.13 Patch 11 [BUG-TELEMETRY-EVALID]: این‌جا دیگر از globals پراکندهٔ
   // g_gbnzdCrisisBuy/Sell و g_gbnzdAdxBuy/Sell خوانده نمی‌شود — همه از همان
   // snapBuy/snapSell که خودِ تصمیم Rule را ساختند می‌آیند. اگه به هر دلیلی
   // این کندل Snapshot معتبر نداشت، N/A چاپ می‌شود (نه یک عدد گمراه‌کننده
   // مثل ADX=0 از یک evalId قدیمی‌تر).
   int    crBuyDisp   = haveSnap && snapBuy.valid  ? snapBuy.crisis  : -1;
   int    crSellDisp  = haveSnap && snapSell.valid ? snapSell.crisis : -1;
   int    spBuyDisp   = haveSnap && snapBuy.valid  ? snapBuy.spike   : -1;
   int    spSellDisp  = haveSnap && snapSell.valid ? snapSell.spike  : -1;
   double adxBuyDisp  = haveSnap && snapBuy.valid  ? snapBuy.adx     : 0.0;
   double adxSellDisp = haveSnap && snapSell.valid ? snapSell.adx    : 0.0;
   double flowBuyDisp = haveSnap && snapBuy.valid  ? snapBuy.flow    : 0.0;
   double flowSellDisp= haveSnap && snapSell.valid ? snapSell.flow   : 0.0;
   bool   adxDataOk    = haveSnap && snapBuy.valid && snapSell.valid;

   string timeStr   = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);
   string crBuyTxt  = (crBuyDisp==2)?"🔴 Red":(crBuyDisp==3)?"🟠 Orange":(crBuyDisp==1)?"🟡 Yellow":(crBuyDisp==0)?"🟢 Green":"⚪ N/A";
   string crSellTxt = (crSellDisp==2)?"🔴 Red":(crSellDisp==3)?"🟠 Orange":(crSellDisp==1)?"🟡 Yellow":(crSellDisp==0)?"🟢 Green":"⚪ N/A";
   string spBuyTxt  = (spBuyDisp==2)?"🔴 Spike":(spBuyDisp==1)?"⚡ Warning":(spBuyDisp==0)?"🟢 Normal":"⚪ N/A";
   string spSellTxt = (spSellDisp==2)?"🔴 Spike":(spSellDisp==1)?"⚡ Warning":(spSellDisp==0)?"🟢 Normal":"⚪ N/A";
   string detailLine= "Buy:  CRISIS=" + crBuyTxt  + " | Spike=" + spBuyTxt  + "\n"
                    + "Sell: CRISIS=" + crSellTxt + " | Spike=" + spSellTxt;
   // 🆕 Patch 11: اگه یکی از دو Snapshot نامعتبر بود، ADX/Flow را N/A چاپ کن
   // به‌جای عدد گمراه‌کنندهٔ 0.0 (رفع BUG-TELEMETRY-EVALID دقیقاً همین حالت بود)
   string adxFlowLine = adxDataOk
      ? StringFormat("ADX: %.0f | Flow: %.1f",
           MathMax(adxBuyDisp, adxSellDisp),
           (MathAbs(flowBuyDisp) > MathAbs(flowSellDisp) ? flowBuyDisp : flowSellDisp))
      : "ADX: N/A | Flow: N/A (دادهٔ این کندل ناقص بود)";
   ...
   if(newLevelBuy > oldLevelBuy || newLevelSell > oldLevelSell)
   {
      ...
      // 🆕 v14.15 P1 [SMART-CLOSE-TEXT]: بدون پوزیشن واقعی روی این نماد،
      // دستور «ببند» گمراه‌کننده است — HelpMe خودش معامله باز/بسته نمی‌کند.
      bool noRealPositions = !HasRealPositionsOnSymbol(_Symbol);
      ...
      if(newLevel == 1)
      {
         ...
         // 🆕 v14.15 P1: بدون پوزیشن واقعی، دستورات مدیریت/هج بی‌معنی‌اند —
         // فقط یک خط هشدار روشن اضافه می‌شود، بقیه متن دست‌نخورده می‌ماند.
         if(noRealPositions)
            msg += "\n⚠️ پوزیشن بازی نداری — فقط معامله جدید باز نکن";
      }
      else if(newLevel == 2)
      {
         ...
         // 🆕 v14.15 P1: بدون پوزیشن واقعی، «ببند» گمراه‌کننده است —
         // به‌جایش فقط هشدار عدم ورود جدید داده می‌شود.
         string closeActionLine = noRealPositions
            ? "بازار فوق‌العاده خطرناکه — هیچ معامله جدیدی باز نکن\nXmoon رو خاموش کن"
         ...
         msg = "🔴 CLOSE — ببند الان\n"
             ...
             + "━━━━━━━━━━━━━━━━\n"
             + detailLine + "\n"
             + adxFlowLine + StringFormat(" | قرمز %dh", dirRedH) + "\n"
             + "━━━━━━━━━━━━━━━━\n"
   ...
   // 🆕 v14.13 Patch 4: هنگام transition، lastReminderTime هر جهتی که تازه
   // STOP/CLOSE شده را «الان» ست کن — تا Rule_CheckReminders (در OnTimer)
   // بلافاصله بعد از همین پیام گذار یک یادآوری تکراری اضافه نفرستد؛
   // شمارش فاصلهٔ یادآوری از همین لحظه شروع می‌شود (طبق درخواست Patch 4:
   // "هنگام transition فقط پیام گذار ارسال شود و lastReminderTime تنظیم گردد").
   if(newLevelBuy > oldLevelBuy && newLevelBuy > 0)
   { g_gbpnzdLastReminderBuy = TimeCurrent(); g_gbpnzdLastReminderLevelBuy = newLevelBuy; }
   if(newLevelSell > oldLevelSell && newLevelSell > 0)
   { g_gbpnzdLastReminderSell = TimeCurrent(); g_gbpnzdLastReminderLevelSell = newLevelSell; }
   if(newLevelBuy == 0)  { g_gbpnzdLastReminderBuy  = 0; g_gbpnzdLastReminderLevelBuy  = 0; }
   if(newLevelSell == 0) { g_gbpnzdLastReminderSell = 0; g_gbpnzdLastReminderLevelSell = 0; }

   // 🆕 v14.13 Patch 4: بلوک یادآوری تکراری قدیمی از این‌جا حذف شد —
   // اکنون Rule_CheckReminders() این کار را از OnTimer انجام می‌دهد،
   // کاملاً مستقل از بسته‌شدن کندل (رفع BUG-REMINDER-TRANSITION).
}


// ════════════════════════════════════════════════════════════════════
// 🆕 v14.13 Patch 4: Rule_CheckReminders — یادآوری کاملاً مستقل از کندل
//
// از OnTimer صدا زده می‌شود (هر ثانیه) — نه از Alert_CheckGBPNZDRule.
// رفع BUG-REMINDER-TRANSITION: قبلاً این چک فقط وقتی Alert_CheckGBPNZDRule
// اجرا می‌شد (یعنی فقط سر بستن کندل) انجام می‌شد؛ اگه کندل بعدی دیر
// می‌بست، یادآوری هم دیر می‌رسید (دقیقاً رفتاری که علی در ۱۴:۳۰→۱۸:۳۰
// مشاهده کرد). حالا صرفاً بر اساس گذر زمان واقعی چک می‌شود؛ هر جهت و هر
// سطح یک lastReminderTime مستقل دارد.
// ════════════════════════════════════════════════════════════════════
void Rule_CheckReminderForDirection(bool forBuy, int level, int cleanH,
                                     datetime &lastReminderTime, int &lastReminderLevel,
                                     datetime nowR)
{
   if(level <= 0)
   {
   ...
   // 🆕 v13.50 RULE-04 (حفظ‌شده): CLOSE هر Alert_CLOSERepeatMinutes، STOP هر
   // Alert_GBPNZDRule_RepeatMinutes — با clamp به حداقل ۵ دقیقه *و* حداقل
   // یک کندل چارت (درخواست صریح Patch 4: "clamp به حداقل g_chartPeriodSeconds")
   int repMinFloor = (int)MathMax(5, g_chartPeriodSeconds / 60);
   int repMin = (int)MathMax(repMinFloor, (level == 2) ? Alert_CLOSERepeatMinutes : Alert_GBPNZDRule_RepeatMinutes);

   if(lastReminderTime != 0 && (nowR - lastReminderTime) < repMin * 60) return;

   lastReminderTime  = nowR;
   lastReminderLevel = level;

   // 🆕 v14.15 P1 [SMART-CLOSE-TEXT]: بدون پوزیشن واقعی روی این نماد، «الان
   // ببند» گمراه‌کننده است — همان فیلتر HasRealPositionsOnSymbol که در
   // Alert_CheckGBPNZDRule برای پیام‌های گذار استفاده شد.
   bool noRealPositions = !HasRealPositionsOnSymbol(_Symbol);

   string lbl    = (level == 2) ? "🔴 CLOSE هنوز فعاله" : "⛔ STOP هنوز فعاله";
   string dirLbl = forBuy ? "Buy" : "Sell";
   string tailMsg = (level == 2)
      ? (noRealPositions ? "بازار هنوز پرخطره — پوزیشن جدید باز نکن" : "هنوز نبستی؟ الان ببند.")
      : (noRealPositions ? "بازار همچنان پرخطره — هنوز پوزیشن جدید باز نکن." : "هنوز پوزیشن جدید باز نکن.");
   string remMsg = lbl + " — " + dirLbl + "\n"
      + _Symbol + " | " + TimeToString(nowR, TIME_DATE|TIME_MINUTES) + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + dirLbl + ": " + ((level==2)?"🔴 CLOSE":"⛔ STOP") + " | CleanH=" + IntegerToString(cleanH) + "h\n"
      + "━━━━━━━━━━━━━━━━\n"
      + tailMsg;
   Alert_Send(remMsg);
}

void Rule_CheckReminders()
{
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!Alert_OnGBPNZDRule)                return;
   if(!Alert_GBPNZDRule_Repeat)            return;
   if(!IsRuleSymbol(_Symbol))              return;

   datetime nowR = TimeCurrent();
   Rule_CheckReminderForDirection(true,  g_gbpnzdLevelBuy,  g_gbpnzdCleanHBuy,
                                   g_gbpnzdLastReminderBuy, g_gbpnzdLastReminderLevelBuy, nowR);
   Rule_CheckReminderForDirection(false, g_gbpnzdLevelSell, g_gbpnzdCleanHSell,
                                   g_gbpnzdLastReminderSell, g_gbpnzdLastReminderLevelSell, nowR);
}


// ════════════════════════════════════════════════════════════════════
// 🆕 v14.17: معماری Stateful مستقل EURGBP — بلوک کاملاً جدید
//
// همهٔ توابع این بلوک جدید هستند؛ هیچ‌کدام هیچ تابع GBPNZD (از جمله
// همهٔ توابعی که در بریف اصلی «لیست ممنوعه» نامیده شدند) را ویرایش
// نمی‌کنند یا صدا نمی‌زنند به‌جز موتور کاملاً generic مشترک:
// ApplyClosedBarToRule، Rule_UpdateCounters (داخل آن)، Rule_Transition
// (داخل آن)، Rule_GVPrefix، IsRuleSymbol، HasRealPositionsOnSymbol،
// Alert_Send، Generic_ReplayCrisisAtBar، Generic_ReplaySpikeAtBar —
// همگی از قبل کاملاً symbol-generic بودند (تأیید شده با خواندن مستقیم
// بدنهٔ هرکدام) و صدا زدنشان با آرگومان "EURGBP" هیچ کد GBPNZD را
// لمس نمی‌کند.
//
// دو ساده‌سازی عمدی نسبت به GBPNZD (مستند در v14.17_Changes.md بخش ۳):
//  ۱) بدون فایل snapshot ساعتی (GBPNZD_SaveHourStateFile معادل) — یک
//     محافظ اختیاری لبهٔ کور «بحران وسط کندلِ نیمه‌کاره + ری‌استارت
//     دقیقاً همان لحظه» که در کد اصلی هم «اختیاری» علامت خورده بود.
//  ۲) ارزیابی EURGBP فقط سر بستن کندل H1 انجام می‌شود (بدون accumulator
//     تیک‌محورِ درون‌کندلی مثل GBPNZD) — چون هیچ مسیر tick-by-tick
//     موجودی برای EURGBP وجود نداشت که این accumulator را به‌روز کند؛
//     ساختنش نیازمند فراخوانی سنگین Generic_Replay* هر تیک بود.
//     Full Spike و Crisis Red همچنان بدون Cooldown فوری dirty می‌شوند —
//     این ساده‌سازی فقط حساسیت به یک اسپایک گذرا که کاملاً وسط کندل رخ
//     داده و قبل از بسته‌شدن برگشته را کمی کم می‌کند.
// ════════════════════════════════════════════════════════════════════

void EURGBP_HMRuleState_LoadBuy(HMRuleState &st)
{
   st.level             = g_eurgbpLevelBuy;
   st.stopReason        = g_eurgbpStopReasonBuy;
   st.crisisRedH        = g_eurgbpCrisisRedHBuy;
   st.cleanH            = g_eurgbpCleanHBuy;
   st.cleanSince        = g_eurgbpCleanSinceBuy;
   st.redSince          = g_eurgbpRedSinceBuy;
   st.spikeStreak       = g_eurgbpSpikeStreakBuy;
   st.lastReminderTime  = g_eurgbpLastReminderBuy;
   st.lastDirtyTime     = g_eurgbpLastDirtyTimeBuy;
   st.lastDirtyReason   = g_eurgbpLastDirtyReasonBuy;
}
void EURGBP_HMRuleState_SaveBuy(const HMRuleState &st)
{
   g_eurgbpLevelBuy            = st.level;
   g_eurgbpStopReasonBuy       = st.stopReason;
   g_eurgbpCrisisRedHBuy       = st.crisisRedH;
   g_eurgbpCleanHBuy           = st.cleanH;
   g_eurgbpCleanSinceBuy       = st.cleanSince;
   g_eurgbpRedSinceBuy         = st.redSince;
   g_eurgbpSpikeStreakBuy      = st.spikeStreak;
   g_eurgbpLastReminderBuy     = st.lastReminderTime;
   g_eurgbpLastDirtyTimeBuy    = st.lastDirtyTime;
   g_eurgbpLastDirtyReasonBuy  = st.lastDirtyReason;
}
void EURGBP_HMRuleState_LoadSell(HMRuleState &st)
{
   st.level             = g_eurgbpLevelSell;
   st.stopReason        = g_eurgbpStopReasonSell;
   st.crisisRedH        = g_eurgbpCrisisRedHSell;
   st.cleanH            = g_eurgbpCleanHSell;
   st.cleanSince        = g_eurgbpCleanSinceSell;
   st.redSince          = g_eurgbpRedSinceSell;
   st.spikeStreak       = g_eurgbpSpikeStreakSell;
   st.lastReminderTime  = g_eurgbpLastReminderSell;
   st.lastDirtyTime     = g_eurgbpLastDirtyTimeSell;
   st.lastDirtyReason   = g_eurgbpLastDirtyReasonSell;
}
void EURGBP_HMRuleState_SaveSell(const HMRuleState &st)
{
   g_eurgbpLevelSell           = st.level;
   g_eurgbpStopReasonSell      = st.stopReason;
   g_eurgbpCrisisRedHSell      = st.crisisRedH;
   g_eurgbpCleanHSell          = st.cleanH;
   g_eurgbpCleanSinceSell      = st.cleanSince;
   g_eurgbpRedSinceSell        = st.redSince;
   g_eurgbpSpikeStreakSell     = st.spikeStreak;
   g_eurgbpLastReminderSell    = st.lastReminderTime;
   g_eurgbpLastDirtyTimeSell   = st.lastDirtyTime;
   g_eurgbpLastDirtyReasonSell = st.lastDirtyReason;
}

// پیشوند GV: Rule_GVPrefix("EURGBP") == "HM_EURGBP_" (طبق CleanSymbol —
// همان تابع generic موجود، صدا زده شده با آرگومان جدید؛ صفر خط تغییر
// در خود Rule_GVPrefix).
void EURGBP_GV_Save()
{
   string pfx = Rule_GVPrefix("EURGBP");
   GlobalVariableSet(pfx+"Ver",        HM_EURGBP_SCHEMA_VERSION);
   GlobalVariableSet(pfx+"PerSec",     (double)g_chartPeriodSeconds);
   GlobalVariableSet(pfx+"LvBuy",      (double)g_eurgbpLevelBuy);
   GlobalVariableSet(pfx+"LvSell",     (double)g_eurgbpLevelSell);
   GlobalVariableSet(pfx+"ClHBuy",     (double)g_eurgbpCleanHBuy);
   GlobalVariableSet(pfx+"ClHSell",    (double)g_eurgbpCleanHSell);
   GlobalVariableSet(pfx+"RdHBuy",     (double)g_eurgbpCrisisRedHBuy);
   GlobalVariableSet(pfx+"RdHSell",    (double)g_eurgbpCrisisRedHSell);
   GlobalVariableSet(pfx+"LastH",      (double)g_eurgbpLastH);
   GlobalVariableSet(pfx+"SRBuy",      (double)g_eurgbpStopReasonBuy);
   GlobalVariableSet(pfx+"SRSell",     (double)g_eurgbpStopReasonSell);
   GlobalVariableSet(pfx+"PrCrBuy",    (double)g_eurgbpPrevCrisisBuy);
   GlobalVariableSet(pfx+"PrCrSell",   (double)g_eurgbpPrevCrisisSell);
   GlobalVariableSet(pfx+"PrSpBuy",    (double)g_eurgbpPrevSpikeBuy);
   GlobalVariableSet(pfx+"PrSpSell",   (double)g_eurgbpPrevSpikeSell);
   GlobalVariableSet(pfx+"CrBuy",      (double)g_eurgbpCrisisBuy);
   GlobalVariableSet(pfx+"CrSell",     (double)g_eurgbpCrisisSell);
   GlobalVariableSet(pfx+"SpBuy",      (double)g_eurgbpSpikeBuy);
   GlobalVariableSet(pfx+"SpSell",     (double)g_eurgbpSpikeSell);
   GlobalVariableSet(pfx+"HRedBuy",    (double)(g_eurgbpHourRedBuy    ? 1 : 0));
   GlobalVariableSet(pfx+"HDirtyBuy",  (double)(g_eurgbpHourDirtyBuy  ? 1 : 0));
   GlobalVariableSet(pfx+"HRedSell",   (double)(g_eurgbpHourRedSell   ? 1 : 0));
   GlobalVariableSet(pfx+"HDirtySell", (double)(g_eurgbpHourDirtySell ? 1 : 0));
   GlobalVariableSet(pfx+"SpkStrBuy",  (double)g_eurgbpSpikeStreakBuy);
   GlobalVariableSet(pfx+"SpkStrSell", (double)g_eurgbpSpikeStreakSell);
   GlobalVariableSet(pfx+"ClSinceBuy", (double)g_eurgbpCleanSinceBuy);
   GlobalVariableSet(pfx+"ClSinceSell",(double)g_eurgbpCleanSinceSell);
   GlobalVariableSet(pfx+"RdSinceBuy", (double)g_eurgbpRedSinceBuy);
   GlobalVariableSet(pfx+"RdSinceSell",(double)g_eurgbpRedSinceSell);
   GlobalVariableSet(pfx+"DrtTBuy",    (double)g_eurgbpLastDirtyTimeBuy);
   GlobalVariableSet(pfx+"DrtTSell",   (double)g_eurgbpLastDirtyTimeSell);
   GlobalVariableSet(pfx+"DrtRBuy",    (double)g_eurgbpLastDirtyReasonBuy);
   GlobalVariableSet(pfx+"DrtRSell",   (double)g_eurgbpLastDirtyReasonSell);
   GlobalVariableSet(pfx+"RemTBuy",    (double)g_eurgbpLastReminderBuy);
   GlobalVariableSet(pfx+"RemTSell",   (double)g_eurgbpLastReminderSell);
   GlobalVariableSet(pfx+"RemLBuy",    (double)g_eurgbpLastReminderLevelBuy);
   GlobalVariableSet(pfx+"RemLSell",   (double)g_eurgbpLastReminderLevelSell);
   GlobalVariableSet(pfx+"Saved",      (double)TimeCurrent());
}

// بازگشت: true = لود موفق | false = GV موجود نبود یا stale بود — همان
// قرارداد GBPNZD_GV_Load، با schema مستقل HM_EURGBP_SCHEMA_VERSION.
bool EURGBP_GV_Load()
{
   string pfx = Rule_GVPrefix("EURGBP");
   if(!GlobalVariableCheck(pfx+"Saved")) return false;
   ...
   double savedVer = GlobalVariableGet(pfx+"Ver");
   if(savedVer < HM_EURGBP_SCHEMA_VERSION)
   {
      Print("ℹ️ v14.17 EURGBP_GV_Load: نسخهٔ GV قدیمی (", savedVer, " < ", HM_EURGBP_SCHEMA_VERSION,
            ") — نادیده گرفته شد، Replay کامل اجرا می‌شود");
      return false;
   }
   if(GlobalVariableCheck(pfx+"PerSec"))
   {
      ...
      if(savedPerSec != g_chartPeriodSeconds)
      {
         Print("ℹ️ v14.17 EURGBP_GV_Load: periodSeconds تغییر کرده — نادیده گرفته شد، Replay کامل اجرا می‌شود");
         return false;
      }
   }

   datetime savedAt = (datetime)GlobalVariableGet(pfx+"Saved");
   if(TimeCurrent() - savedAt > 72 * 3600) return false;   // stale

   g_eurgbpLevelBuy        = (int)GlobalVariableGet(pfx+"LvBuy");
   g_eurgbpLevelSell       = (int)GlobalVariableGet(pfx+"LvSell");
   g_eurgbpCleanHBuy       = (int)GlobalVariableGet(pfx+"ClHBuy");
   g_eurgbpCleanHSell      = (int)GlobalVariableGet(pfx+"ClHSell");
   g_eurgbpCrisisRedHBuy   = (int)GlobalVariableGet(pfx+"RdHBuy");
   g_eurgbpCrisisRedHSell  = (int)GlobalVariableGet(pfx+"RdHSell");
   g_eurgbpLastH           = (datetime)GlobalVariableGet(pfx+"LastH");
   g_eurgbpStopReasonBuy   = (int)GlobalVariableGet(pfx+"SRBuy");
   g_eurgbpStopReasonSell  = (int)GlobalVariableGet(pfx+"SRSell");
   g_eurgbpCrisisBuy       = GlobalVariableCheck(pfx+"CrBuy")  ? (int)GlobalVariableGet(pfx+"CrBuy")  : -1;
   g_eurgbpCrisisSell      = GlobalVariableCheck(pfx+"CrSell") ? (int)GlobalVariableGet(pfx+"CrSell") : -1;
   g_eurgbpSpikeBuy        = GlobalVariableCheck(pfx+"SpBuy")  ? (int)GlobalVariableGet(pfx+"SpBuy")  : -1;
   g_eurgbpSpikeSell       = GlobalVariableCheck(pfx+"SpSell") ? (int)GlobalVariableGet(pfx+"SpSell") : -1;
   g_eurgbpHourRedBuy      = GlobalVariableCheck(pfx+"HRedBuy")   ? (GlobalVariableGet(pfx+"HRedBuy")   != 0.0) : (g_eurgbpCrisisBuy  == 2);
   g_eurgbpHourDirtyBuy    = GlobalVariableCheck(pfx+"HDirtyBuy") ? (GlobalVariableGet(pfx+"HDirtyBuy") != 0.0) : (g_eurgbpCrisisBuy  == 2 || g_eurgbpSpikeBuy  >= 1);
   g_eurgbpHourRedSell     = GlobalVariableCheck(pfx+"HRedSell")  ? (GlobalVariableGet(pfx+"HRedSell")  != 0.0) : (g_eurgbpCrisisSell == 2);
   g_eurgbpHourDirtySell   = GlobalVariableCheck(pfx+"HDirtySell")? (GlobalVariableGet(pfx+"HDirtySell") != 0.0): (g_eurgbpCrisisSell == 2 || g_eurgbpSpikeSell >= 1);
   g_eurgbpSpikeStreakBuy      = GlobalVariableCheck(pfx+"SpkStrBuy")   ? (int)GlobalVariableGet(pfx+"SpkStrBuy")    : 0;
   g_eurgbpSpikeStreakSell     = GlobalVariableCheck(pfx+"SpkStrSell")  ? (int)GlobalVariableGet(pfx+"SpkStrSell")   : 0;
   g_eurgbpCleanSinceBuy       = GlobalVariableCheck(pfx+"ClSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"ClSinceBuy") : 0;
   g_eurgbpCleanSinceSell      = GlobalVariableCheck(pfx+"ClSinceSell") ? (datetime)GlobalVariableGet(pfx+"ClSinceSell"): 0;
   g_eurgbpRedSinceBuy         = GlobalVariableCheck(pfx+"RdSinceBuy")  ? (datetime)GlobalVariableGet(pfx+"RdSinceBuy") : 0;
   g_eurgbpRedSinceSell        = GlobalVariableCheck(pfx+"RdSinceSell") ? (datetime)GlobalVariableGet(pfx+"RdSinceSell"): 0;
   g_eurgbpLastDirtyTimeBuy    = GlobalVariableCheck(pfx+"DrtTBuy")     ? (datetime)GlobalVariableGet(pfx+"DrtTBuy")    : 0;
   g_eurgbpLastDirtyTimeSell   = GlobalVariableCheck(pfx+"DrtTSell")    ? (datetime)GlobalVariableGet(pfx+"DrtTSell")   : 0;
   g_eurgbpLastDirtyReasonBuy  = GlobalVariableCheck(pfx+"DrtRBuy")     ? (int)GlobalVariableGet(pfx+"DrtRBuy")      : 0;
   g_eurgbpLastDirtyReasonSell = GlobalVariableCheck(pfx+"DrtRSell")    ? (int)GlobalVariableGet(pfx+"DrtRSell")     : 0;
   g_eurgbpLastReminderBuy      = GlobalVariableCheck(pfx+"RemTBuy")  ? (datetime)GlobalVariableGet(pfx+"RemTBuy")  : 0;
   g_eurgbpLastReminderSell     = GlobalVariableCheck(pfx+"RemTSell") ? (datetime)GlobalVariableGet(pfx+"RemTSell") : 0;
   g_eurgbpLastReminderLevelBuy = GlobalVariableCheck(pfx+"RemLBuy")  ? (int)GlobalVariableGet(pfx+"RemLBuy")      : 0;
   g_eurgbpLastReminderLevelSell= GlobalVariableCheck(pfx+"RemLSell") ? (int)GlobalVariableGet(pfx+"RemLSell")     : 0;
   return true;
}

// EURGBP_InitReplay — معادل GBPNZD_InitReplay، با دو ساده‌سازی مستند در
// سربرگ این بلوک (بدون hour-state-file). Generic_ReplayCrisisAtBar/
// Generic_ReplaySpikeAtBar (کاملاً generic، صفر تغییر) با handle های
// اختصاصی EURGBP صدا زده می‌شوند — نه BuildDirectionSnapshot مستقیم
// (دلیل فنی کامل در v14.17_Changes.md بخش ۲).
void EURGBP_InitReplay()
{
   if(!IsRuleSymbol(_Symbol)) return;
   if((bool)MQLInfoInteger(MQL_TESTER)) return;

   if(g_eurgbpHandleEMA200_H1 == INVALID_HANDLE ||
      g_eurgbpHandleATR_H1    == INVALID_HANDLE ||
      g_eurgbpHandleADX_H4    == INVALID_HANDLE ||
      g_eurgbpHandleATR_D1    == INVALID_HANDLE) return;

   if(BarsCalculated(g_eurgbpHandleEMA200_H1) < 290) return;
   if(BarsCalculated(g_eurgbpHandleADX_H4)    < 25)  return;

   bool gvLoaded = EURGBP_GV_Load();
   if(gvLoaded)
   {
      g_eurgbpReplayDone = true;
      Print(StringFormat(
         "✅ v14.17 EURGBP state loaded from GlobalVariable (versioned) — Buy=%s CleanH=%d | Sell=%s CleanH=%d",
         g_eurgbpLevelBuy  == 0 ? "GREEN" : g_eurgbpLevelBuy  == 1 ? "STOP" : "CLOSE", g_eurgbpCleanHBuy,
         g_eurgbpLevelSell == 0 ? "GREEN" : g_eurgbpLevelSell == 1 ? "STOP" : "CLOSE", g_eurgbpCleanHSell));
      return;
   }

   // ریست قبل از replay کامل
   g_eurgbpCrisisRedHBuy = 0; g_eurgbpCrisisRedHSell = 0;
   g_eurgbpCleanHBuy     = 0; g_eurgbpCleanHSell     = 0;
   g_eurgbpLevelBuy      = 0; g_eurgbpLevelSell      = 0;
   g_eurgbpStopReasonBuy = -1; g_eurgbpStopReasonSell = -1;

   int _replayPeriodSec  = (g_chartPeriodSeconds > 0) ? g_chartPeriodSeconds : 3600;
   int _replayBarsToScan = (int)MathMax(1, 259200 / _replayPeriodSec);   // ۷۲ ساعت — همان پنجرهٔ GBPNZD
   ...
   for(int sh = _replayBarsToScan; sh >= 1; sh--)
   {
      double flowBuyOut = 0.0, adxBuyOut = 0.0;  int rcBuyOut = 0;
      double flowSellOut= 0.0, adxSellOut= 0.0;  int rcSellOut= 0;
      int crBuy  = Generic_ReplayCrisisAtBar("EURGBP", g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              sh, true,  flowBuyOut,  adxBuyOut,  rcBuyOut);
      int crSell = Generic_ReplayCrisisAtBar("EURGBP", g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              sh, false, flowSellOut, adxSellOut, rcSellOut);

      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar("EURGBP", sh, spikeBuyOut, spikeSellOut);

      bool haBuy  = (adxBuyOut  >= 35.0) && (flowBuyOut  < -7.0);
      bool haSell = (adxSellOut >= 35.0) && (flowSellOut < -7.0);

      datetime replayEvalTime = iTime("EURGBP", PERIOD_H1, sh + 1);

      HMDirSnapshot snapReplayBuy;
      snapReplayBuy.valid = true; snapReplayBuy.crisis = crBuy; snapReplayBuy.spike = spikeBuyOut;
      snapReplayBuy.highAlert = haBuy; snapReplayBuy.evalBarTime = replayEvalTime;
      snapReplayBuy.adx = adxBuyOut; snapReplayBuy.flow = flowBuyOut;
      HMRuleState stReplayBuy;
      EURGBP_HMRuleState_LoadBuy(stReplayBuy);
      ApplyClosedBarToRule(snapReplayBuy, false, false, inp_SpikeCooldownBars, replayEvalTime, stReplayBuy);
      EURGBP_HMRuleState_SaveBuy(stReplayBuy);

      HMDirSnapshot snapReplaySell;
      snapReplaySell.valid = true; snapReplaySell.crisis = crSell; snapReplaySell.spike = spikeSellOut;
      snapReplaySell.highAlert = haSell; snapReplaySell.evalBarTime = replayEvalTime;
      snapReplaySell.adx = adxSellOut; snapReplaySell.flow = flowSellOut;
      HMRuleState stReplaySell;
      EURGBP_HMRuleState_LoadSell(stReplaySell);
      ApplyClosedBarToRule(snapReplaySell, false, false, inp_SpikeCooldownBars, replayEvalTime, stReplaySell);
      EURGBP_HMRuleState_SaveSell(stReplaySell);

      rHourRedBuy    = (crBuy  == 2);
      rHourDirtyBuy  = (crBuy  == 2) || (spikeBuyOut  >= 1);
      rHourRedSell   = (crSell == 2);
      rHourDirtySell = (crSell == 2) || (spikeSellOut >= 1);

      g_eurgbpCrisisBuy = crBuy; g_eurgbpCrisisSell = crSell;
      g_eurgbpSpikeBuy  = spikeBuyOut; g_eurgbpSpikeSell = spikeSellOut;
   }

   g_eurgbpHourRedBuy    = rHourRedBuy;
   g_eurgbpHourDirtyBuy  = rHourDirtyBuy;
   g_eurgbpHourRedSell   = rHourRedSell;
   g_eurgbpHourDirtySell = rHourDirtySell;

   g_eurgbpLastH = iTime("EURGBP", PERIOD_CURRENT, 1);

   g_eurgbpPrevCrisisBuy  = g_eurgbpCrisisBuy;
   g_eurgbpPrevSpikeBuy   = g_eurgbpSpikeBuy;
   g_eurgbpPrevCrisisSell = g_eurgbpCrisisSell;
   g_eurgbpPrevSpikeSell  = g_eurgbpSpikeSell;

   g_eurgbpReplayDone = true;
   EURGBP_GV_Save();

   Print(StringFormat(
      "✅ v14.17 EURGBP_InitReplay done (full Replay) — Buy=%s CleanH=%d | Sell=%s CleanH=%d",
      g_eurgbpLevelBuy  == 0 ? "GREEN" : g_eurgbpLevelBuy  == 1 ? "STOP" : "CLOSE", g_eurgbpCleanHBuy,
      g_eurgbpLevelSell == 0 ? "GREEN" : g_eurgbpLevelSell == 1 ? "STOP" : "CLOSE", g_eurgbpCleanHSell));
}

void EURGBP_CheckReminderForDirection(bool forBuy, int level, int cleanH,
                                       datetime &lastReminderTime, int &lastReminderLevel,
                                       datetime nowR)
{
   if(level <= 0)
   ...
   bool noRealPositions = !HasRealPositionsOnSymbol("EURGBP");
   ...
   string tailMsg = (level == 2)
      ? (noRealPositions ? "بازار هنوز پرخطره — پوزیشن جدید باز نکن" : "هنوز نبستی؟ الان ببند.")
      : (noRealPositions ? "بازار همچنان پرخطره — هنوز پوزیشن جدید باز نکن." : "هنوز پوزیشن جدید باز نکن.");
   string remMsg = lbl + " — " + dirLbl + " (EURGBP)\n"
      + "EURGBP | " + TimeToString(nowR, TIME_DATE|TIME_MINUTES) + "\n"
      + "━━━━━━━━━━━━━━━━\n"
      + dirLbl + ": " + ((level==2)?"🔴 CLOSE":"⛔ STOP") + " | CleanH=" + IntegerToString(cleanH) + "h\n"
      + "━━━━━━━━━━━━━━━━\n"
      + tailMsg;
...
void EURGBP_CheckReminders()
{
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!Alert_OnGBPNZDRule)                return;   // سوییچ مشترک — طبق تصمیم مستند در v14.17_Changes.md
   if(!Alert_GBPNZDRule_Repeat)            return;
   if(!IsRuleSymbol(_Symbol))              return;

   datetime nowR = TimeCurrent();
   EURGBP_CheckReminderForDirection(true,  g_eurgbpLevelBuy,  g_eurgbpCleanHBuy,
                                     g_eurgbpLastReminderBuy, g_eurgbpLastReminderLevelBuy, nowR);
   EURGBP_CheckReminderForDirection(false, g_eurgbpLevelSell, g_eurgbpCleanHSell,
                                     g_eurgbpLastReminderSell, g_eurgbpLastReminderLevelSell, nowR);
}

// Alert_CheckEURGBPRule — معادل Alert_CheckGBPNZDRule برای EURGBP.
// ارزیابی فقط سر بستن کندل H1 چارت انجام می‌شود (طبق ساده‌سازی #۲ در
// سربرگ این بلوک) — extraDirty/extraRed همیشه false، صرفاً بر مبنای
// Snapshot تازهٔ shift=1 که ApplyClosedBarToRule خودش با Full
// Spike/Crisis Red بدون هیچ Cooldown فوری dirty می‌کند.
void Alert_CheckEURGBPRule()
{
   if(!Alert_OnGBPNZDRule)                return;   // سوییچ مشترک
   if((bool)MQLInfoInteger(MQL_TESTER))   return;
   if(!IsRuleSymbol(_Symbol))             return;

   datetime lastQuoteTimeE = (datetime)SymbolInfoInteger("EURGBP", SYMBOL_TIME);
   int  _weekendStaleThresholdE = (int)MathMax(2 * 3600, 3 * g_chartPeriodSeconds);
   bool eurgbpMarketStale = (lastQuoteTimeE > 0) && ((TimeCurrent() - lastQuoteTimeE) > _weekendStaleThresholdE);

   bool barBoundaryReachedE = (g_eurgbpLastH > 0) &&
                              (g_lastBarCloseTime > g_eurgbpLastH) &&
                              !eurgbpMarketStale;

   int oldLevelBuy  = g_eurgbpLevelBuy;
   int oldLevelSell = g_eurgbpLevelSell;

   HMDirSnapshot snapBuy, snapSell;
   bool haveSnap = false;

   if(barBoundaryReachedE)
   {
      double flowBuyOut = 0.0, adxBuyOut = 0.0;  int rcBuyOut = 0;
      double flowSellOut= 0.0, adxSellOut= 0.0;  int rcSellOut= 0;
      int crBuy  = Generic_ReplayCrisisAtBar("EURGBP", g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              1, true,  flowBuyOut,  adxBuyOut,  rcBuyOut);
      int crSell = Generic_ReplayCrisisAtBar("EURGBP", g_eurgbpHandleEMA200_H1, g_eurgbpHandleATR_H1,
                                              g_eurgbpHandleADX_H4, g_eurgbpHandleATR_D1,
                                              1, false, flowSellOut, adxSellOut, rcSellOut);
      int spikeBuyOut = 0, spikeSellOut = 0;
      Generic_ReplaySpikeAtBar("EURGBP", 1, spikeBuyOut, spikeSellOut);

      datetime evalTimeE = iTime("EURGBP", PERIOD_H1, 2);   // آخرین کندل کاملاً بسته (shift=1 در Generic_Replay* یعنی +1 نسبت به shift جاری؛ سازگار با iTime(sh+1) در InitReplay)

      snapBuy.valid = true;  snapBuy.crisis = crBuy;  snapBuy.spike = spikeBuyOut;
      snapBuy.highAlert  = (adxBuyOut  >= 35.0) && (flowBuyOut  < -7.0);
      snapBuy.evalBarTime = evalTimeE; snapBuy.adx = adxBuyOut; snapBuy.flow = flowBuyOut;

      snapSell.valid = true; snapSell.crisis = crSell; snapSell.spike = spikeSellOut;
      snapSell.highAlert = (adxSellOut >= 35.0) && (flowSellOut < -7.0);
      snapSell.evalBarTime = evalTimeE; snapSell.adx = adxSellOut; snapSell.flow = flowSellOut;
      haveSnap = true;

      HMRuleState stBuy;
      EURGBP_HMRuleState_LoadBuy(stBuy);
      ApplyClosedBarToRule(snapBuy, false, false, inp_SpikeCooldownBars, snapBuy.evalBarTime, stBuy);
      EURGBP_HMRuleState_SaveBuy(stBuy);

      HMRuleState stSell;
      EURGBP_HMRuleState_LoadSell(stSell);
      ApplyClosedBarToRule(snapSell, false, false, inp_SpikeCooldownBars, snapSell.evalBarTime, stSell);
      EURGBP_HMRuleState_SaveSell(stSell);

      g_eurgbpCrisisBuy = crBuy; g_eurgbpCrisisSell = crSell;
      g_eurgbpSpikeBuy  = spikeBuyOut; g_eurgbpSpikeSell = spikeSellOut;
      g_eurgbpLastH = g_lastBarCloseTime;
      EURGBP_GV_Save();
   }

   if(g_eurgbpLevelBuy == oldLevelBuy && g_eurgbpLevelSell == oldLevelSell) return;

   int newLevelBuy   = g_eurgbpLevelBuy;
   int newLevelSell  = g_eurgbpLevelSell;

   bool noRealPositionsE = !HasRealPositionsOnSymbol("EURGBP");
   string timeStrE = TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES);

   int    crBuyDisp   = haveSnap ? snapBuy.crisis  : -1;
   int    crSellDisp  = haveSnap ? snapSell.crisis : -1;
   int    spBuyDisp   = haveSnap ? snapBuy.spike   : -1;
   int    spSellDisp  = haveSnap ? snapSell.spike  : -1;
   string crBuyTxt  = (crBuyDisp==2)?"🔴 Red":(crBuyDisp==3)?"🟠 Orange":(crBuyDisp==1)?"🟡 Yellow":(crBuyDisp==0)?"🟢 Green":"⚪ N/A";
   string crSellTxt = (crSellDisp==2)?"🔴 Red":(crSellDisp==3)?"🟠 Orange":(crSellDisp==1)?"🟡 Yellow":(crSellDisp==0)?"🟢 Green":"⚪ N/A";
   string spBuyTxt  = (spBuyDisp==2)?"🔴 Spike":(spBuyDisp==1)?"⚡ Warning":(spBuyDisp==0)?"🟢 Normal":"⚪ N/A";
   string spSellTxt = (spSellDisp==2)?"🔴 Spike":(spSellDisp==1)?"⚡ Warning":(spSellDisp==0)?"🟢 Normal":"⚪ N/A";
   string detailLineE = "Buy:  CRISIS=" + crBuyTxt  + " | Spike=" + spBuyTxt  + "\n"
                       + "Sell: CRISIS=" + crSellTxt + " | Spike=" + spSellTxt;
   ...
   if(newLevelBuy > oldLevelBuy || newLevelSell > oldLevelSell)
   {
      ...
      if(newLevel == 1)
      {
         msg = "⛔ STOP — ورود ممنوع (EURGBP)\n"
             + "EURGBP | " + timeStrE + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + detailLineE + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + "هیچ معامله جدیدی روی EURGBP باز نکن\n"
             + "معاملات فعلی رو مدیریت کن";
         if(noRealPositionsE)
            msg += "\n⚠️ پوزیشن بازی روی EURGBP نداری — فقط معامله جدید باز نکن";
      }
      else if(newLevel == 2)
      {
         int dirRedHE = (newLevelBuy == 2) ? g_eurgbpCrisisRedHBuy : g_eurgbpCrisisRedHSell;
         string closeActionLineE = noRealPositionsE
            ? "بازار EURGBP فوق‌العاده خطرناکه — هیچ معامله جدیدی باز نکن"
            : "با هر ضرری پوزیشن‌های EURGBP رو ببند";
         msg = "🔴 CLOSE — ببند الان (EURGBP)\n"
             + "EURGBP | " + timeStrE + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + detailLineE + StringFormat(" | قرمز %dh", dirRedHE) + "\n"
             + "━━━━━━━━━━━━━━━━\n"
             + closeActionLineE;
      }
   }
   else if(newLevelBuy == 0 && newLevelSell == 0 && (oldLevelBuy > 0 || oldLevelSell > 0))
   {
      msg = "✅ RESUME — وضعیت EURGBP آروم شد\n"
          + "EURGBP | " + timeStrE + "\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "Crisis=🟢  Spike=🟢  (Buy و Sell)\n"
          + "━━━━━━━━━━━━━━━━\n"
          + "میتونی دوباره با Xmoon روی EURGBP معامله کنی";
   }
   ...
   if(newLevelBuy > oldLevelBuy && newLevelBuy > 0)
   { g_eurgbpLastReminderBuy = TimeCurrent(); g_eurgbpLastReminderLevelBuy = newLevelBuy; }
   if(newLevelSell > oldLevelSell && newLevelSell > 0)
   { g_eurgbpLastReminderSell = TimeCurrent(); g_eurgbpLastReminderLevelSell = newLevelSell; }
   if(newLevelBuy == 0)  { g_eurgbpLastReminderBuy  = 0; g_eurgbpLastReminderLevelBuy  = 0; }
   if(newLevelSell == 0) { g_eurgbpLastReminderSell = 0; g_eurgbpLastReminderLevelSell = 0; }
}
...
void TL_Update()
{
   ...
   else  // g_dirMode == 0 → All (حالت پیش‌فرض)
   {
      ...
      if(isHedge)
      {
         ...
         if(runRuleBlock && !((bool)MQLInfoInteger(MQL_TESTER)) && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol))
         {
            ...
            // 🆕 v14.11: rawPhase و Flow gate از توابع per-symbol
            double _hg_fgTh, _hg_frTh; Flow_GetThresholds(_Symbol, _hg_fgTh, _hg_frTh);
            double _hg_thW,  _hg_thS;  Spike_GetThresholds(_Symbol, _hg_thW, _hg_thS);
            int _rpBuy  = (_scBuyH  < _hg_thW) ? 0 : (_scBuyH  <= _hg_thS) ? 1 : 2;
            int _rpSell = (_scSellH < _hg_thW) ? 0 : (_scSellH <= _hg_thS) ? 1 : 2;
            bool _frBuy  = (_fsBuyH  < _hg_frTh);
            bool _frSell = (_fsSellH < _hg_frTh);
            bool _fyBuy  = (_scBuyH  >= _hg_thS && _fsBuyH  < _hg_fgTh && _fsBuyH  >= _hg_frTh);
            bool _fySell = (_scSellH >= _hg_thS && _fsSellH < _hg_fgTh && _fsSellH >= _hg_frTh);
   ...
   if(!hasPos)
   {
      ...
      if(!(bool)MQLInfoInteger(MQL_TESTER) && Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol))
      {
         ...
         // 🆕 v14.11: rawPhase و Flow gate از توابع per-symbol (رفع BUG-FLOW-HARDCODE)
         double _ls_fgTh, _ls_frTh; Flow_GetThresholds(_Symbol, _ls_fgTh, _ls_frTh);
         double _ls_thW,  _ls_thS;  Spike_GetThresholds(_Symbol, _ls_thW, _ls_thS);
         int _rawPhaseSell    = (_scoreSell < _ls_thW) ? 0 : (_scoreSell <= _ls_thS) ? 1 : 2;
         int _rawPhaseBuy     = (_scoreBuy  < _ls_thW) ? 0 : (_scoreBuy  <= _ls_thS) ? 1 : 2;

         bool _flowRedSell    = (_fsSell < _ls_frTh);
         bool _flowRedBuy     = (_fsBuy  < _ls_frTh);
         bool _flowYellowSell = (_scoreSell >= _ls_thS && _fsSell < _ls_fgTh && _fsSell >= _ls_frTh);
         bool _flowYellowBuy  = (_scoreBuy  >= _ls_thS && _fsBuy  < _ls_fgTh && _fsBuy  >= _ls_frTh);
...

   {
      ...
      else
      {
         ...
         // 🆕 v14.11: از Flow_GetThresholds() می‌خواند (رفع BUG-FLOW-HARDCODE)
         // GBPNZD: green=2.5, red=-5.0 (قبلاً 2.0/-4.0) | AUDCAD/EURGBP: بدون تغییر
         Flow_GetThresholds(baseSym, flowGreenTh, flowRedTh);
   ...
   if((bool)MQLInfoInteger(MQL_TESTER))
   {
      ...
      // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
      bool _btHighAlert = (bt_adxVal >= 35.0) && (bt_flowScore < -7.0);
      LogHourlySnapshot(
         newRTM, newTrend, newStruct, newFLOW, newZOMBIE,
   ...
   // شرط Sell: ADX >= 35 AND Flow < -7.0   (🐛 v14.10 BUG-HA1 FIX: قبلاً > +7.0 بود — اشتباه)
   // flowSell < 0 یعنی بازار به ضرر Sell — همان تعریف برای Buy
   ...
   {
      // 🐛 v14.10 BUG-HA1 FIX: علامت Sell معکوس بود (> 7.0 → < -7.0)
      bool _highAlertNow = (g_lastAdxVal >= 35.0) && (g_lastFlowScore < -7.0);
      ...
      if(forBuy)
      {
         ...
         if(Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol))
         {
            ...
            // 🆕 v14.11: از توابع per-symbol
            double _os_fgTh, _os_frTh; Flow_GetThresholds(_Symbol, _os_fgTh, _os_frTh);
            double _os_thW,  _os_thS;  Spike_GetThresholds(_Symbol, _os_thW, _os_thS);
            int    _rpOpp     = (_baseOpp < _os_thW) ? 0 : (_baseOpp <= _os_thS) ? 1 : 2;
            bool   _frOpp     = (_fsSellOpp < _os_frTh);
            bool   _fyOpp     = (_baseOpp >= _os_thS && _fsSellOpp < _os_fgTh && _fsSellOpp >= _os_frTh);
      ...
      else
      {
         ...
         if(Alert_OnGBPNZDRule && IsRuleSymbol(_Symbol))
         {
            ...
            double _baseOpp   = (g_lightSpike >= 0 && g_spikeScore > 0.0) ? g_spikeScore : 0.0;
            // 🆕 v14.11: از توابع per-symbol
            double _ob_fgTh, _ob_frTh; Flow_GetThresholds(_Symbol, _ob_fgTh, _ob_frTh);
            double _ob_thW,  _ob_thS;  Spike_GetThresholds(_Symbol, _ob_thW, _ob_thS);
            int    _rpOpp     = (_baseOpp < _ob_thW) ? 0 : (_baseOpp <= _ob_thS) ? 1 : 2;
            bool   _frOpp     = (_fsBuyOpp < _ob_frTh);
            bool   _fyOpp     = (_baseOpp >= _ob_thS && _fsBuyOpp < _ob_fgTh && _fsBuyOpp >= _ob_frTh);
   ...
   if(IsRuleSymbol(_Symbol))
   {
      ...
      if(isBuySnap)
      {
         ...
         // راه‌حل: SpikeSell را با Flow جهت‌دار برعکس محاسبه کن
         {
            ...
            // 🆕 v14.11: از توابع per-symbol
            double _bs_fgTh, _bs_frTh; Flow_GetThresholds(_Symbol, _bs_fgTh, _bs_frTh);
            double _bs_thW,  _bs_thS;  Spike_GetThresholds(_Symbol, _bs_thW, _bs_thS);
            int    _btRp     = (_btBase < _bs_thW) ? 0 : (_btBase <= _bs_thS) ? 1 : 2;
            bool   _btFr     = (_btFsSell < _bs_frTh);
            bool   _btFy     = (_btBase >= _bs_thS && _btFsSell < _bs_fgTh && _btFsSell >= _bs_frTh);
      ...
      else
      {
         ...
         // 🐛 v13.47 FIX4: جهت مقابل (Buy) در بکتست بیات نماند
         {
            ...
            double _btBase   = (g_lightSpike >= 0 && g_spikeScore > 0.0) ? g_spikeScore : 0.0;
            // 🆕 v14.11: از توابع per-symbol
            double _bb_fgTh, _bb_frTh; Flow_GetThresholds(_Symbol, _bb_fgTh, _bb_frTh);
            double _bb_thW,  _bb_thS;  Spike_GetThresholds(_Symbol, _bb_thW, _bb_thS);
            int    _btRp     = (_btBase < _bb_thW) ? 0 : (_btBase <= _bb_thS) ? 1 : 2;
            bool   _btFr     = (_btFsBuy < _bb_frTh);
            bool   _btFy     = (_btBase >= _bb_thS && _btFsBuy < _bb_fgTh && _btFsBuy >= _bb_frTh);
      ...
      if(canUpdate)
      {
         ...
         // 🐛 v14.10 BUG-HA1 FIX: علامت _haSell معکوس بود (> 7.0 → < -7.0)
         bool _haBuy  = (g_gbnzdAdxBuy  >= 35.0) && (g_gbnzdFlowBuy  < -7.0);
         bool _haSell = (g_gbnzdAdxSell >= 35.0) && (g_gbnzdFlowSell < -7.0);

         // 🆕 v14.13 Patch 2 [BUG-COOLDOWN-BT]: قبلاً این‌جا مستقیماً
         // Rule_UpdateCounters+Rule_Transition با محاسبهٔ دستیِ _bDirty/_sDirty
         // صدا زده می‌شد که نه Cooldown داشت و نه Full-Spike/Crisis-Red را از
         // Warning-Spike جدا می‌کرد (دقیقاً BUG-SPIKEONLY-DIRTY). حالا از همان
         // ApplyClosedBarToRule مشترک لایو/replay استفاده می‌شود — یعنی بکتست
         // اکنون *دقیقاً* همان رفتار Cooldown را می‌بیند که لایو می‌بیند.
         datetime btEvalTime = (now > 0) ? now : TimeCurrent();

         HMDirSnapshot snapBtBuy;
         snapBtBuy.valid = true; snapBtBuy.crisis = g_gbnzdCrisisBuy; snapBtBuy.spike = g_gbnzdSpikeBuy;
         snapBtBuy.highAlert = _haBuy; snapBtBuy.evalBarTime = btEvalTime;
         HMRuleState stBtBuy;
         HMRuleState_LoadBuy(stBtBuy);
         ApplyClosedBarToRule(snapBtBuy, false, false, inp_SpikeCooldownBars, btEvalTime, stBtBuy);
         HMRuleState_SaveBuy(stBtBuy);

         HMDirSnapshot snapBtSell;
         snapBtSell.valid = true; snapBtSell.crisis = g_gbnzdCrisisSell; snapBtSell.spike = g_gbnzdSpikeSell;
         snapBtSell.highAlert = _haSell; snapBtSell.evalBarTime = btEvalTime;
         HMRuleState stBtSell;
         HMRuleState_LoadSell(stBtSell);
         ApplyClosedBarToRule(snapBtSell, false, false, inp_SpikeCooldownBars, btEvalTime, stBtSell);
         HMRuleState_SaveSell(stBtSell);
   ...
   if(g_hourlyCount > 0)
   {
      ...
      if(hh != INVALID_HANDLE)
      {
         string hhdr =
            ...
            "Regime,Session,Spread_Points," + sym + "_Rule\n";
...
void CreateDashboard()
{
   ...
   bgH += lineH + btnH + 2*secGap;    // 🆕 v14.15 P3/P4: Last Update label + Force Reset Rule button
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      CreateLabel(dashboardPrefix+"TitleLbl", tLblX, yPos+(int)(btnH*0.15),
                  "HelpMe v14.15", clrNavy, dashFontSmall);
   ...
   // ─── LAST UPDATE + FORCE RESET RULE — 🆕 v14.15 P3/P4 ────────────
   // بخش Direction بالا از dirY محلی استفاده می‌کند (yPos بیرونی را جلو
   // نمی‌برد)؛ همان افزایش لحاظ‌شده در محاسبهٔ bgH بالا ("direction title +
   // buttons") این‌جا هم اعمال می‌شود تا این بخش دقیقاً زیر دکمه‌های
   // Direction قرار بگیرد.
   {
      yPos += lineH + btnH + secGap;   // ارتفاع بخش Direction (title + دکمه‌ها)

      // P3: لیبل زمان آخرین کندل ارزیابی‌شده — مقدار واقعی در UpdateDashboard ست می‌شود
      CreateLabel(dashboardPrefix+"LastUpdateLabel", labelLx, yPos,
                  "Last Update: Initializing…", clrDimGray, dashFontSmall);
      ObjectSetString(0,dashboardPrefix+"LastUpdateLabel",OBJPROP_TOOLTIP,
         "زمان آخرین کندل بسته‌شده‌ای که Rule/چراغ‌ها بر اساس آن ارزیابی شدند\nاگه این زمان خیلی عقب بمونه، احتمالاً EA/داده منجمد شده");
      yPos += lineH + secGap;

      // P4: دکمه ریست اجباری ماشین Rule — جدا از ResetBtn «R» (که فقط
      // تنظیمات نمایشی را ریست می‌کند). متن OrangeRed روی DarkSlateGray:
      // هم‌رنگ دکمه‌های ابزار دیگر ولی با تأکید بصری برای احتیاط کاربر.
      int rrBtnW = (int)MathRound(aBtnW * 2.0);
      int rrX    = ROW_X(rrBtnW);
      CreateButton(dashboardPrefix+"ForceResetRuleBtn", rrX, yPos, rrBtnW, btnH,
                   "Force Reset Rule", clrOrangeRed, clrDarkSlateGray);
      ObjectSetString(0,dashboardPrefix+"ForceResetRuleBtn",OBJPROP_TOOLTIP,
         "ریست کامل حافظه Rule (Stop/Close/CleanH) و بازسازی از ۷۲ ساعت گذشته");
      yPos += btnH + secGap;
   }
...
void UpdateDashboard()
{
   ...
   // 🆕 v14.15 P3 [LAST-UPDATE-LABEL]: زمان آخرین کندل بسته‌شده که
   // Chart_IsNewBarClosed در g_lastBarCloseTime ثبت می‌کند — صرفاً نمایشی.
   {
      string lastUpdateText = (g_lastBarCloseTime > 0)
         ? "Last Update: " + TimeToString(g_lastBarCloseTime, TIME_DATE|TIME_MINUTES)
         : "Last Update: Initializing…";
      UpdateLabel(dashboardPrefix + "LastUpdateLabel", lastUpdateText, clrDimGray);
   }
                  ...
                  const string &sparam)
{
   ...
   SigMsg_Clear();
      ...
      // ─── Force Reset Rule Button — 🆕 v14.15 P4 [FORCE-RESET-RULE] ────
      // برخلاف ResetBtn «R» بالا (فقط تنظیمات نمایشی)، این دکمه حافظهٔ
      // کامل ماشین Rule GBPNZD (هر دو جهت) را پاک و از ۷۲ ساعت گذشته
      // بازسازی می‌کند — برای زمانی که Rule به هر دلیلی قفل کرده.
      if(sparam == dashboardPrefix + "ForceResetRuleBtn")
      {
         ObjectSetInteger(0, sparam, OBJPROP_STATE, false);
         g_processingChartEvent = false;   // مثل ResetBtn: مطمئن شو ری‌درا داخلی قفل نمی‌کند

         // ① پاک کردن همهٔ کلیدهای GlobalVariable حافظهٔ Rule (هر دو جهت) —
         // دقیقاً همان کلیدهایی که GBPNZD_GV_Save می‌نویسد (خط‌به‌خط سینک،
         // شامل کلیدهای v14.13 که پچ اولیه ذکر نکرده بود: PerSec/SpkStr*/
         // ClSince*/RdSince*/DrtT*/DrtR*/RemT*/RemL*).
         string pfx = Rule_GVPrefix(_Symbol);
         GlobalVariableDel(pfx + "Ver");        GlobalVariableDel(pfx + "PerSec");
         GlobalVariableDel(pfx + "LvBuy");      GlobalVariableDel(pfx + "LvSell");
         GlobalVariableDel(pfx + "ClHBuy");     GlobalVariableDel(pfx + "ClHSell");
         GlobalVariableDel(pfx + "RdHBuy");     GlobalVariableDel(pfx + "RdHSell");
         GlobalVariableDel(pfx + "LastH");
         GlobalVariableDel(pfx + "SRBuy");      GlobalVariableDel(pfx + "SRSell");
         GlobalVariableDel(pfx + "PrCrBuy");    GlobalVariableDel(pfx + "PrCrSell");
         GlobalVariableDel(pfx + "PrSpBuy");    GlobalVariableDel(pfx + "PrSpSell");
         GlobalVariableDel(pfx + "CrBuy");      GlobalVariableDel(pfx + "CrSell");
         GlobalVariableDel(pfx + "SpBuy");      GlobalVariableDel(pfx + "SpSell");
         GlobalVariableDel(pfx + "HRedBuy");    GlobalVariableDel(pfx + "HDirtyBuy");
         GlobalVariableDel(pfx + "HRedSell");   GlobalVariableDel(pfx + "HDirtySell");
         GlobalVariableDel(pfx + "SpkStrBuy");  GlobalVariableDel(pfx + "SpkStrSell");
         GlobalVariableDel(pfx + "ClSinceBuy"); GlobalVariableDel(pfx + "ClSinceSell");
         GlobalVariableDel(pfx + "RdSinceBuy"); GlobalVariableDel(pfx + "RdSinceSell");
         GlobalVariableDel(pfx + "DrtTBuy");    GlobalVariableDel(pfx + "DrtTSell");
         GlobalVariableDel(pfx + "DrtRBuy");    GlobalVariableDel(pfx + "DrtRSell");
         GlobalVariableDel(pfx + "RemTBuy");    GlobalVariableDel(pfx + "RemTSell");
         GlobalVariableDel(pfx + "RemLBuy");    GlobalVariableDel(pfx + "RemLSell");
         GlobalVariableDel(pfx + "Saved");

         // ② بازگرداندن globalهای RAM به پیش‌فرض. شامل فیلدهای پشتیبان
         // HMRuleState (spikeStreak/cleanSince/redSince/lastDirty*) که
         // HMRuleState_LoadBuy/LoadSell در همان اولین کندل GBPNZD_InitReplay
         // از این globalهای مسطح hydrate می‌کنند — بدون این ریست، ۷۲ ساعت
         // Replay روی streak/since کهنه سوار می‌شد (نه یک ریست واقعاً کامل).
         g_gbpnzdLevelBuy       = 0;     g_gbpnzdLevelSell       = 0;
         g_gbpnzdCleanHBuy      = 0;     g_gbpnzdCleanHSell      = 0;
         g_gbpnzdCrisisRedHBuy  = 0;     g_gbpnzdCrisisRedHSell  = 0;
         g_gbpnzdLastH          = 0;
         g_gbpnzdStopReasonBuy  = -1;    g_gbpnzdStopReasonSell  = -1;
         g_gbpnzdHourRedBuy     = false; g_gbpnzdHourDirtyBuy    = false;
         g_gbpnzdHourRedSell    = false; g_gbpnzdHourDirtySell   = false;
         g_gbpnzdReplayDone     = false;
         g_gbpnzdNeedsLiveCheck = false;
         g_spikeCooldownBuyCounter  = 0; g_spikeCooldownSellCounter = 0;
         g_gbnzdCrisisBuy  = -1;  g_gbnzdCrisisSell = -1;
         g_gbnzdSpikeBuy   = -1;  g_gbnzdSpikeSell  = -1;
         g_gbpnzdCleanSinceBuy      = 0; g_gbpnzdCleanSinceSell      = 0;
         g_gbpnzdRedSinceBuy        = 0; g_gbpnzdRedSinceSell        = 0;
         g_gbpnzdSpikeStreakBuy     = 0; g_gbpnzdSpikeStreakSell     = 0;
         g_gbpnzdLastDirtyTimeBuy   = 0; g_gbpnzdLastDirtyTimeSell   = 0;
         g_gbpnzdLastDirtyReasonBuy = 0; g_gbpnzdLastDirtyReasonSell = 0;
         g_gbpnzdLastReminderBuy      = 0; g_gbpnzdLastReminderSell      = 0;
         g_gbpnzdLastReminderLevelBuy = 0; g_gbpnzdLastReminderLevelSell = 0;
         // worst-case مشترک (سازگاری با کد قدیمی‌ای که این globalها را می‌خواند)
         g_gbpnzdLevel      = 0;  g_gbpnzdStopReason = -1;
         g_gbpnzdCrisisRedH = 0;  g_gbpnzdCleanH     = 0;

         // ③ Replay کامل از ۷۲ ساعت گذشته — دقیقاً همان مسیری که در
         // OnInit/ری‌استارت EA اجرا می‌شود (کاملاً ایمن، بدون معامله)
         GBPNZD_InitReplay();

         // ④ داشبورد و پیام تلگرام
         g_forceUpdateDashboard = true;
         UpdateDashboard();
         Alert_SendTelegram("🔄 حافظه Rule به صورت دستی ریست و بازسازی ۷۲ ساعته انجام شد.");
