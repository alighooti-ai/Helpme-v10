//+------------------------------------------------------------------+
//|                                                HelpMe_V10.3.2.mq5 |
//|                 Combined EA - Ali Naderi v10.3.2 (EA)             |
//+------------------------------------------------------------------+
#property copyright "HelpMe v10.3.2 - Ali Naderi"
#property version   "10.32"
// ═══════════════════════════════════════════════════════════════════
...
input double BacktestZombieRef   = 0.0;   // v10.1: ZOMBIE entry reference price (0=auto=D1 channel center)
// ── v10.3.1: Zone Line Visualizer ────────────────────────────────
input bool   ShowZoneLines       = false; // v10.3.1: رسم خطوط مرز طبقات Zone روی چارت | Draw Zone boundary lines on chart
input int    ZoneLineCount       = 8;     // v10.3.1: تعداد طبقه در هر طرف (پیش‌فرض ±8) | Number of zone lines each side
...
int    handleATR_D1_200   = INVALID_HANDLE; // v10.3: ATR(D1,200) برای ZoneWidth محاسبه Zone Counter
...
// ╔═════════════════════════════════════════════════════════════════╗
// 🆕 v10.3: ZOMBIE → Zone Counter (بازنویسی کامل)
// ╔═════════════════════════════════════════════════════════════════╗
// ZOMBIE دیگر چراغ رنگی نیست — یک عدد صحیح است که نشان می‌دهد
// قیمت چند «منطقه» از نقطه ورود فاصله گرفته است.
// Zone = 0 : قیمت هنوز در محدوده ورود است (طبیعی)
// Zone = -1: یک منطقه در ضرر رفته
// Zone = -2: دو منطقه در ضرر (توجه!)
// Zone < -3: شرایط بحرانی — Xmoon خیلی صبر کرده
// ─────────────────────────────────────────────────────────────────
// g_lightZOMBIE دیگر state رنگی ندارد; همیشه = 0 (برای RC محاسبه نمی‌شود)
int    g_lightZOMBIE         = -1;   // نگه داشته شد برای سازگاری با کدهای قدیمی
double g_zombieEntryPrice    = 0.0;  // قیمت ورود (از پوزیشن باز یا BacktestZombieRef)
double g_zombieZoneWidthPips = 0.0;  // عرض یک منطقه (pip) — در لحظه ورود محاسبه و ثابت
int    g_zombieCurrentZone   = 0;    // شماره منطقه فعلی (0=نزدیک ورود, -1,-2,... = ضرر)
...
struct HourlySnapshot
{
   ...
   int      stStruct;
   int      stFLOW;
   int      stZOMBIE;   // v10.3: همیشه 0 (Zone شماره است نه رنگ)
   int      stCRISIS;
   double   rtmX;
   string   rtmArrow;
   double   adxVal;
   ...
   double   flowScore;
   double   trendScore;
   int      zombieZone;       // v10.3: شماره منطقه (0,-1,-2,...)
   int      zombieWidthPips;  // v10.3: عرض منطقه (pip)
   string   d1Status;
   string   regime;
   string   session;
   int      spreadPt;
...
struct BtLightEvent
{
   ...
   // ── v10.3: ZOMBIE → Zone Counter ─────────────────────────────
   int      zombieZone;        // شماره منطقه (0,-1,-2,...)
   int      zombieWidthPips;   // عرض منطقه (pip)
};
...
// v10.3.1: DrawZoneLines — رسم خطوط مرز طبقات Zone روی چارت
// ─────────────────────────────────────────────────────────────────
// وقتی BacktestZombieRef>0 و ShowZoneLines=true:
//   خطوط افقی در فواصل ZoneWidth از Entry رسم می‌شوند
//   Zone 0 (Entry) = خط سفید ضخیم
//   Zones مثبت (ضرر) = خط قرمز‌رنگ با برچسب Zone+N
//   Zones منفی (سود) = خط سبزرنگ با برچسب Zone-N
//
// v10.3.2 FIX: پارامتر forBuy اضافه شد تا جهت خطوط درست باشد
//   Buy : level = Entry - (i × Width)  → i مثبت = پایین‌تر = ضرر  ✓
//   Sell: level = Entry + (i × Width)  → i مثبت = بالاتر = ضرر    ✓
//
// هر بار که ZoneWidth تغییر کند (ورود جدید) خطوط بازسازی می‌شوند
// ════════════════════════════════════════════════════════════════
void DeleteZoneLines()
{
   // پاک کردن همه خطوط قبلی
   int total = ObjectsTotal(0, 0, OBJ_HLINE);
   for(int i = total - 1; i >= 0; i--)
   {
      string nm = ObjectName(0, i, 0, OBJ_HLINE);
      if(StringFind(nm, "ZombieLine_") == 0)
         ObjectDelete(0, nm);
   }
   // برچسب‌های Zone هم پاک شوند
   int totalTxt = ObjectsTotal(0, 0, OBJ_TEXT);
   for(int i = totalTxt - 1; i >= 0; i--)
   {
...
void DrawZoneLines(double entryPrice, double zoneWidthPrice, double pipSz, int maxZones, bool forBuy = true)
{
   if(entryPrice <= 0.0 || zoneWidthPrice <= 0.0) return;
   DeleteZoneLines();

   int zoneWPipsLocal = (int)MathRound(zoneWidthPrice / pipSz);

   for(int i = -maxZones; i <= maxZones; i++)
   {
      // v10.3.2 FIX: جهت خطوط بر اساس نوع پوزیشن
      // Buy : i مثبت = پایین‌تر از Entry = ضرر → level = Entry - i×Width
      // Sell: i مثبت = بالاتر از Entry  = ضرر → level = Entry + i×Width
      double level = forBuy ? (entryPrice - (i * zoneWidthPrice))
                             : (entryPrice + (i * zoneWidthPrice));
      string name  = "ZombieLine_" + IntegerToString(i);
      string lname = "ZoneLabel_"  + IntegerToString(i);

      // رنگ و ضخامت بر اساس Zone
      color  lineCol;
      int    lineWidth;
      ...
      if(i == 0)
      {
         ...
         labelTxt  = "ENTRY (Zone 0)";
      }
      else if(i > 0)
      {
         // ضرر: برای Buy پایین‌تر، برای Sell بالاتر از Entry
         int intensity = MathMin(i, 5);
         lineCol   = (color)((0xFF << 16) | ((0xFF - intensity*40) << 8) | (0xFF - intensity*40));
         // از زرد به قرمز
         if(i == 1)       lineCol = clrGold;
         else if(i == 2)  lineCol = clrOrange;
         else if(i == 3)  lineCol = clrOrangeRed;
         else             lineCol = clrRed;
         lineWidth = 1;
         lineStyle = STYLE_DASH;
         // v10.3.2: برچسب متناسب با جهت — Buy پایین، Sell بالا
         labelTxt = forBuy
            ? StringFormat("Zone +%d  (-%d pip | LOSS)", i, i * zoneWPipsLocal)
            : StringFormat("Zone +%d  (+%d pip | LOSS)", i, i * zoneWPipsLocal);
      }
      else
      {
         // سود: برای Buy بالاتر، برای Sell پایین‌تر از Entry
         if(i == -1)      lineCol = clrMediumSeaGreen;
         else if(i == -2) lineCol = clrLimeGreen;
         else             lineCol = clrSpringGreen;
         lineWidth = 1;
         lineStyle = STYLE_DOT;
         // v10.3.2: برچسب متناسب با جهت
         labelTxt = forBuy
            ? StringFormat("Zone %d  (+%d pip | PROFIT)", i, MathAbs(i) * zoneWPipsLocal)
            : StringFormat("Zone %d  (-%d pip | PROFIT)", i, MathAbs(i) * zoneWPipsLocal);
      }
   ...
   if(ShowDebugLogs)
      Print("📐 ZoneLines v10.3.2: Entry=", DoubleToString(entryPrice,_Digits),
            " | Width=", zoneWPipsLocal, "pip | ±", maxZones, " lines | Dir=",
            forBuy ? "BUY" : "SELL");
}
...
int OnInit()
{
   ...
   // v10.3: ATR(D1,200) برای ZOMBIE Zone Width — باید دوره ۲۰۰ کندل D1 باشد
   handleATR_D1_200   = iATR (_Symbol, PERIOD_D1, 200);
...
void OnDeinit(const int reason)
{
   ...
   if(handleATR_D1_200   != INVALID_HANDLE) { IndicatorRelease(handleATR_D1_200);   handleATR_D1_200   = INVALID_HANDLE; } // v10.3
   // حذف اشیاء چراغ‌ها
...
void TL_Update()
{
   ...
   if(hasPos)
   {
      ...
      if(oldestPx > 0.0)
      {
         // v10.3: اگه پوزیشن جدیدی باز شد (ورود تغییر کرد)، ZoneWidth را ریست کن
         // تا در ابتدای معامله جدید، ZoneWidth از ATR تازه محاسبه شود
         if(MathAbs(oldestPx - g_zombieEntryPrice) > _Point * 10)
         {
            g_zombieZoneWidthPips = 0.0;   // ریست → محاسبه مجدد در بلوک ZOMBIE
            if(ShowZoneLines) DeleteZoneLines(); // v10.3.1: خطوط قدیمی پاک شود
         }
         g_zombieEntryPrice = oldestPx;
      }
   }
   // 🆕 v10.1: اگر BacktestZombieRef دستی تنظیم شده، override کن
   if(BacktestZombieRef > 0.0)
   {
      if(MathAbs(BacktestZombieRef - g_zombieEntryPrice) > _Point * 10)
         g_zombieZoneWidthPips = 0.0;   // v10.3: ZoneWidth ریست شود تا بر اساس تاریخ شروع محاسبه شود
      g_zombieEntryPrice = BacktestZombieRef;
   }
   ...
   if(!hasPos)
   {
      if(g_lightRTM != -1 || g_lightTrend != -1 || g_lightStruct != -1 || g_lightFLOW != -1 || g_lightGOLDEN != -1)
      {
         ...
         // v10.3: ریست ZOMBIE Zone state
         g_zombieEntryPrice    = 0.0;
         g_zombieZoneWidthPips = 0.0;   // ZoneWidth ریست می‌شود تا در معامله بعدی دوباره محاسبه شود
   ...
   // ════════════════════════════════════════════════════════════════
   // 🆕 v10.3: ZOMBIE → Zone Counter (بازنویسی کامل، بدون رنگ)
   // ─────────────────────────────────────────────────────────────────
   // ZOMBIE دیگر چراغ نیست. یک Zone Counter است:
   //   ZONE: 0  → قیمت هنوز در محدوده ورود است
   //   ZONE: -1 → یک منطقه در ضرر
   //   ZONE: -2 → دو منطقه در ضرر (معامله دارد سنگین می‌شود)
   //   ZONE: -3 → سه منطقه (Xmoon باید خیلی صبر کند)
   //
   // ZoneWidth = ATR(D1,200) × 2.0 — یک‌بار محاسبه، تا بسته شدن ثابت
   // ─────────────────────────────────────────────────────────────────

   int    newZOMBIE   = 0;   // همیشه 0 — از RC حذف شد (v10.3)
   string zombieTxt   = "● ZONE: --     ";
   string zombieTip   = "ZONE: اطلاعات در حال بارگذاری...";
   int    zoneNumber  = 0;
   int    zoneWPips   = 0;

   if(handleATR_D1_200 != INVALID_HANDLE && BarsCalculated(handleATR_D1_200) > 205)
   {
      double atrD1Buf[];
      ArraySetAsSeries(atrD1Buf, true);
      // ATR(D1,200): برای ZoneWidth از 200 کندل D1 استفاده می‌کنیم
      // یک کندل بیشتر کپی می‌کنیم تا آخرین کندل بسته شده داشته باشیم
      if(CopyBuffer(handleATR_D1_200, 0, 1, 1, atrD1Buf) == 1 && atrD1Buf[0] > 0)
      {
         double atrD1   = atrD1Buf[0];
         // v10.3.2: استفاده از تابع GetPipSize به جای محاسبه inline
         double pipSz   = GetPipSize(_Symbol);

         // ── محاسبه ZoneWidth ────────────────────────────────────────
         // اگه ZoneWidth از قبل محاسبه نشده (اول بکتست یا اول معامله)، محاسبه کن
         // در live: وقتی g_zombieEntryPrice جدید set می‌شود، ZoneWidth هم ریست می‌شود
         if(g_zombieZoneWidthPips <= 0.0)
         {
            // ATR(D1,200) × Factor(2.0) = عرض یک منطقه به پیپ
            g_zombieZoneWidthPips = (atrD1 / pipSz) * 2.0;
            if(g_zombieZoneWidthPips < 20) g_zombieZoneWidthPips = 20; // حداقل ایمن
            if(ShowDebugLogs)
               Print("📐 ZOMBIE v10.3: ZoneWidth=", DoubleToString(g_zombieZoneWidthPips,0),
                     "pip | ATR_D1=", DoubleToString(atrD1/pipSz,0), "pip | x2.0");

            // v10.3.1: اگه ShowZoneLines روشن است، خطوط مرز طبقات رسم کن
            if(ShowZoneLines && g_zombieEntryPrice > 0.0)
            {
               double zwPrice = g_zombieZoneWidthPips * pipSz;
               // v10.3.2 FIX: forBuy پاس داده می‌شود تا جهت خطوط درست باشد
               DrawZoneLines(g_zombieEntryPrice, zwPrice, pipSz, ZoneLineCount, forBuy);
            }
         }

         double zoneWidthPrice = g_zombieZoneWidthPips * pipSz;
         zoneWPips = (int)MathRound(g_zombieZoneWidthPips);

         // ── مرجع قیمتی ───────────────────────────────────────────────
         double refPrice = g_zombieEntryPrice;

         if(refPrice <= 0.0)
         {
            // fallback: مرکز کانال D1 ۲۰ روزه (بکتست بدون ZombieRef)
            double hiF[], loF[];
            ArraySetAsSeries(hiF, true);
            ArraySetAsSeries(loF, true);
            if(CopyHigh(_Symbol, PERIOD_D1, 1, 20, hiF) == 20 &&
               CopyLow (_Symbol, PERIOD_D1, 1, 20, loF) == 20)
            {
               double hh = hiF[0], ll = loF[0];
               for(int _j=1; _j<20; _j++){ hh=MathMax(hh,hiF[_j]); ll=MathMin(ll,loF[_j]); }
               refPrice = (hh + ll) * 0.5;
               g_zombieEntryPrice = refPrice;
            }
         }

         if(refPrice > 0.0 && zoneWidthPrice > 0.0)
         {
            // ── قیمت فعلی ────────────────────────────────────────────
            double currH1 = iClose(_Symbol, PERIOD_H1, 1);
            if(currH1 <= 0.0) currH1 = iClose(_Symbol, PERIOD_D1, 1);

            // ── محاسبه Zone Number ────────────────────────────────────
            // v10.3.1 FIX: فرمول ساده و بدون اگر اضافه
            //
            // Buy:
            //   diff > 0  → قیمت پایین‌تر از ورود = ضرر     → zone مثبت (1,2,3,...)
            //   diff < 0  → قیمت بالاتر از ورود  = سود      → zone منفی (-1,-2,...)
            //   diff = 0  → دقیقاً روی ورود                  → zone = 0
            //
            // Sell:
            //   diff > 0  → قیمت بالاتر از ورود = ضرر      → zone مثبت (1,2,3,...)
            //   diff < 0  → قیمت پایین‌تر از ورود = سود    → zone منفی (-1,-2,...)
            //
            // MathFloor خودش علامت را حفظ می‌کند:
            //   floor( +2.7 ) = +2   |   floor( -0.3 ) = -1   |   floor( 0 ) = 0
            //
            double diff = forBuy ? (refPrice - currH1) : (currH1 - refPrice);
            zoneNumber = (int)MathFloor(diff / zoneWidthPrice);

            g_zombieCurrentZone = zoneNumber;

            // ── متن نمایش ────────────────────────────────────────────
            zombieTxt = StringFormat("● ZONE:%+d (W:%dp)", zoneNumber, zoneWPips);

            // ── Tooltip ───────────────────────────────────────────────
            double dispPips = diff / pipSz;

            // وضعیت بر اساس zone — zone مثبت = ضرر، منفی = سود
            string zoneDesc;
            if     (zoneNumber <= -1)  zoneDesc = "✅ قیمت به نفع ماست — در سود";
            else if(zoneNumber == 0)   zoneDesc = "✅ قیمت در محدوده ورود — طبیعی";
            else if(zoneNumber == 1)   zoneDesc = "⚠️ یک طبقه در ضرر";
            else if(zoneNumber == 2)   zoneDesc = "⚠️⚠️ دو طبقه — معامله سنگین‌تر می‌شود";
            else if(zoneNumber == 3)   zoneDesc = "🚨 سه طبقه — Xmoon باید صبر کند";
            else                       zoneDesc = StringFormat("🚨🚨 %d طبقه — بحرانی! بررسی فوری", zoneNumber);

            zombieTip = StringFormat(
               "ZONE v10.3.1 — شمارنده منطقه قیمتی\n"
               "─────────────────────────────────\n"
               "مرجع ورود : %s\n"
               "قیمت فعلی : %s\n"
               "جابجایی   : %+.0f pip\n"
               "─────────────────────────────────\n"
               "عرض منطقه : %d pip  (ATR_D1×2.0)\n"
               "شماره منطقه: %+d\n"
               "─────────────────────────────────\n"
               "ZONE < 0 → قیمت به سود ما حرکت کرده\n"
               "ZONE = 0 → قیمت در محدوده ورود\n"
               "ZONE = 1 → یک طبقه در ضرر\n"
               "ZONE = 2 → دو طبقه (توجه!)\n"
               "ZONE ≥ 3 → بحرانی — بررسی فوری\n"
               "─────────────────────────────────\n"
               "وضعیت: %s\n"
               "※ ZOMBIE دیگر رنگی ندارد — تصمیم با شماست",
               DoubleToString(refPrice, _Digits),
               DoubleToString(currH1,  _Digits),
               dispPips,
               zoneWPips,
               zoneNumber,
               zoneDesc);
         ...
         {
            zombieTxt = "● ZONE: init..";
            zombieTip = "مرجع قیمتی هنوز تنظیم نشده";
            zoneNumber = 0;
         }
      }
      else
      {
         zombieTxt = "● ZONE: init..";
         zombieTip = "ATR_D1(200) در حال بارگذاری...";
      }
   }
   else
   {
      zombieTxt = "● ZONE: init..";
      zombieTip = "handle ATR_D1(200) آماده نیست (نیاز به ۲۰۵+ کندل D1)";
   }

   // v10.3: bt_zombieScore و bt_zombieMidShift دیگر استفاده نمی‌شوند
   // به جای آن‌ها: zoneNumber و zoneWPips برای CSV
   double bt_zombieScore    = (double)zoneNumber;   // سازگاری امضای BT_TrackLightChanges
   double bt_zombieMidShift = (double)zoneWPips;    // سازگاری امضای BT_TrackLightChanges
   ...
   // v10.3: ZOMBIE دیگر رنگ ندارد — state=0 (سبز/خاکستری) همیشه
   // zombieTxt حاوی "ZONE: -2 (W:187p)" است که توسط بلوک Zone محاسبه شده
   TL_SetLight("ZOMBIE", 0, zombieTxt, zombieTip);
   ...
   string direction)
{
   ...
   g_hourlyLog[_hi].stZOMBIE       = 0;   // v10.3: همیشه 0 (Zone Counter)
   ...
   g_hourlyLog[_hi].zombieWidthPips= (int)zombieMidShiftOrWidth; // v10.3: Width pips
   ...
   string direction)
{
   ...
   int newSt[5]; newSt[0]=nRTM;  newSt[1]=nTrend;  newSt[2]=nStruct;  newSt[3]=nFLOW;  newSt[4]=nZOMBIE;
   int oldSt[5]; oldSt[0]=oRTM;  oldSt[1]=oTrend;  oldSt[2]=oStruct;  oldSt[3]=oFLOW;  oldSt[4]=oZOMBIE;

   // v10.3: ZOMBIE از RC حذف شد — max RC = 4
   int redCount = (nRTM==2?1:0)+(nTrend==2?1:0)+(nStruct==2?1:0)+(nFLOW==2?1:0);
   ...
   // v10.3: فقط چراغ‌های 0-3 (RTM,TREND,STRUCT,FLOW) چراغ رنگی دارند
   // چراغ 4 (ZOMBIE) دیگر رنگی ندارد → از حلقه حذف
   for(int li = 0; li < 4; li++)
   {
      // تازه قرمز شد
      if(newSt[li] == 2 && oldSt[li] != 2)
      {
         BtLightEvent ev;
         ev.startTime      = now;
         ev.endTime        = 0;
         ev.redCountAtStart= redCount;
         ev.direction      = direction;
         ev.stRTM    = nRTM;    ev.stTrend  = nTrend;
         ev.stStruct = nStruct; ev.stFLOW   = nFLOW;
         // v10.3: zombieZone و zombieWidthPips جایگزین zombieScore/zombieMidShift
         ev.zombieZone     = (int)zombieScoreOrZone;
         ev.zombieWidthPips= (int)zombieMidShiftOrWidth;
         ev.rtmX     = rtmX;   ev.rtmArrow= rtmArrow; ev.rtmCT = rtmCT;
         ev.adxVal   = adxVal;  ev.erH4    = erH4;     ev.erLabel = erLabel;
         ev.d1Status = d1Status; ev.d1Pips = d1Pips;
         ev.flowScore= flowScore;
         ev.trendScore  = g_csvScore;
         ev.regime      = g_csvRegime;
         ev.session     = sess;
         ev.spreadPt    = g_csvSpread;
         // v10.2: وضعیت CRISIS لحظه‌ای با Orange جدید
         ev.crisisState = (g_crisisState == 2) ? "Red" :
                          (g_crisisState == 3) ? "Orange" :
                          (g_crisisState == 1) ? "Yellow" : "Green";
         // 🔧 v7.2 FIX: ChartBG
         {
            bool   bg_isBuy  = (direction == "BUY");
            double bg_ts     = g_csvScore;
            bool   bg_isBull = (bg_ts > 0);
            double bg_abs    = MathAbs(bg_ts);
            bool   bg_align  = (bg_isBull && bg_isBuy) || (!bg_isBull && !bg_isBuy);
            if(bg_abs < 10)
               ev.chartBG = "Neutral";
            else if(bg_align)
               ev.chartBG = "Safe";
         ...
         ArrayResize(g_btEvents, g_btEventCount + 1);
         g_btEvents[g_btEventCount++] = ev;
      }
      // از قرمز خارج شد → endTime ثبت کن
      else if(newSt[li] != 2 && oldSt[li] == 2)
      {
         for(int k = g_btEventCount - 1; k >= 0; k--)
         {
            if(g_btEvents[k].stRTM   == (li==0?2:oRTM)   &&
               g_btEvents[k].stTrend == (li==1?2:oTrend)  &&
               g_btEvents[k].stStruct== (li==2?2:oStruct) &&
               g_btEvents[k].stFLOW  == (li==3?2:oFLOW)  &&
               g_btEvents[k].endTime == 0)
            {
               g_btEvents[k].endTime = now;
               break;
            }
         }
      }
...
double OnTester()
{
   // بستن رویدادهای باز (که تا پایان بکتست قرمز ماندند)
   datetime lastTime = TimeCurrent();
   for(int k = 0; k < g_btEventCount; k++)
      if(g_btEvents[k].endTime == 0)
         g_btEvents[k].endTime = lastTime;

   if(g_btEventCount == 0)
   {
      Print("✅ HelpMe Backtest CSV: هیچ رویداد قرمزی ثبت نشد (چراغ‌ها همیشه سبز/زرد بودند)");
      return 0.0;
   }

   // مرتب‌سازی نزولی بر اساس redCountAtStart (4 قرمز اول)
   BtLightEvent sorted[];
   ArrayResize(sorted, g_btEventCount);
   for(int i = 0; i < g_btEventCount; i++) sorted[i] = g_btEvents[i];
   for(int i = 0; i < g_btEventCount - 1; i++)
      for(int j = i+1; j < g_btEventCount; j++)
         if(sorted[j].redCountAtStart > sorted[i].redCountAtStart)
         { BtLightEvent t = sorted[i]; sorted[i] = sorted[j]; sorted[j] = t; }

   // ── نام فایل ──────────────────────────────────────────────────
   string sym = _Symbol;
   StringReplace(sym, ".", "");
   MqlDateTime dt0, dt1;
   TimeToStruct(sorted[g_btEventCount-1].startTime, dt0);
   TimeToStruct(sorted[0].startTime, dt1);
   string fname = StringFormat("HelpMe_BT_%s_%04d%02d%02d_%04d%02d%02d.csv",
      sym, dt0.year, dt0.mon, dt0.day, dt1.year, dt1.mon, dt1.day);

   int h = FileOpen(fname, FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON);
   if(h == INVALID_HANDLE)
   {
      Print("❌ CSV open failed: err=", GetLastError(), " | file=", fname);
      return 0.0;
   }

   // ── هدر CSV (English) ──────────────────────────────────────────
   // RedCount   : 4=All RED (flee!) | 3=Critical | 2=Warning | 1=Watch  → Sort here first
   // StartTime  : When first light turned red in this event
   // EndTime    : When lights returned to green/yellow (OPEN = still active)
   // Duration_H : Duration in hours (float, easier to read)
   // Direction  : BUY or SELL (position direction in backtest)
   // RTM|ADX|D1|FLOW : Light color  Red | Yellow | Green
   // RTM_Distance_xATR: Distance from EMA200 in ATR multiples — larger = more dangerous
   // RTM_Arrow  : Up=moving away (bad) | Down=returning (good) | Flat=stable
   // RTM_Type   : Counter-Trend (v7.2: now triggers RED when not returning) | With-Trend
   // ADX_Val    : ADX value on H4  (>32=strong trend=danger | <20=ranging=safe)
   // EfficiencyRatio_H4      : Trend efficiency ratio H4 (0-1)
   // EfficiencyRatio_Category: Ranging<0.20 | Moderate=0.20-0.35 | Trending>0.35
   // Flow_Score : Sister-pairs flow  (>3=safe | >-0.5=mixed | <-0.5=tsunami)
   // D1_Status  : OK=intact | Edge=near level | Break=broken (danger)
   // D1_StructurePips: Distance in pips from key D1 swing level (+ = safe side)
   // Trend_Score: HelpMe overall score -100 to +100
   // ChartBG    : SAFE=position aligned with trend | ALARM=against trend | NEUTRAL | NoPos
   // Regime     : Trending | Ranging | Volatile | Quiet
   // Session    : Tokyo | London | New York | Tokyo-London | London-NewYork | Off-Hours
   // Spread_Points: Spread in points at event time
   // TsAgainst  : v9.0 — TrendScore علیه پوزیشن (Long:-TS | Short:+TS) بالاتر=خطرناک‌تر
   // CrisisState: v10.2 — Green=امن | Yellow=احتیاط | Orange=هشدار زودهنگام | Red=فرار فوری!
   //   🟢 Green:  هیچ‌کدام از شروط
   //   🟡 Yellow: Y1-Y5 (قبلی)
   //   🟠 Orange: OA(RC≥2+Flow≤-6.5+ADX>30) | OB(RC≥3+TsAg>40+ADX>28) | OC
   //   🔴 Red-A:  RC≥3 AND Flow≤-7.0 AND ADX>32
   //   🔴 Red-B:  RC≥2 AND Flow≤-8.5 AND ADX>40
   // Zombie_Zone: v10.3 — شماره منطقه (0=نزدیک ورود, -1,-2,... = ضرر)
   // Zombie_WidthPips: v10.3 — عرض هر منطقه (pip) = ATR(D1,200)×2.0
   string hdr =
      "RedCount,"
      "StartTime,"
      "EndTime,"
   ...
   for(int i = 0; i < g_btEventCount; i++)
   {
      BtLightEvent ev = sorted[i];
      double durH = (ev.endTime > ev.startTime)
                    ? (double)(ev.endTime - ev.startTime) / 3600.0
                    : 0.0;
      // مقادیر کامل به جای مخفف
      string arrowFull   = (ev.rtmArrow == "UP") ? "Up"    :
                           (ev.rtmArrow == "DN") ? "Down"  : "Flat";
      string rtmTypeFull = ev.rtmCT ? "Counter-Trend" : "With-Trend";
      string erCatFull   = (ev.erLabel == "TRD") ? "Trending"  :
                           (ev.erLabel == "mod") ? "Moderate"  :
                           (ev.erLabel == "rng") ? "Ranging"   : "Unknown";
      string d1StatFull  = (ev.d1Status == "BREAK") ? "Break" :
                           (ev.d1Status == "EDGE")  ? "Edge"  :
                           (ev.d1Status == "OK")    ? "OK"    : ev.d1Status;
      string regimeFull  = (ev.regime == "?") ? "Unknown" : ev.regime;

      // v10.2: CrisisState — منطق کامل با Orange
      bool isBuy       = (ev.direction == "BUY");
      double tsAgainst = isBuy ? -ev.trendScore : ev.trendScore;
      int    rcS       = ev.redCountAtStart;
      double flS       = ev.flowScore;
      double adS       = ev.adxVal;

      bool isRedA_csv   = (rcS >= 3 && flS <= -7.0  && adS > 32.0);
      bool isRedB_csv   = (rcS >= 2 && flS <= -8.5  && adS > 40.0);
      bool isOrangeA_csv= (rcS >= 2 && flS <= -6.5  && adS > 30.0);
      bool isOrangeB_csv= (rcS >= 3 && tsAgainst > 40.0 && adS > 28.0);
      bool isOrangeC_csv= (rcS >= 2 && flS <= -7.5  && adS > 32.0 && tsAgainst > 25.0);
      bool isYellow_csv = (rcS >= 2)
                       || (flS <= -5.0 && adS > 28.0)
                       || (rcS >= 2 && flS <= -7.0 && tsAgainst > 30.0)
                       || (rcS >= 2 && tsAgainst > 45.0 && adS > 25.0)
                       || (rcS >= 2 && flS <= -7.5 && adS > 35.0);

      string crisis = (isRedA_csv || isRedB_csv)               ? "Red" :
                      (isOrangeA_csv||isOrangeB_csv||isOrangeC_csv) ? "Orange" :
                      isYellow_csv                              ? "Yellow" : "Green";

      string line = StringFormat(
         "%d,%s,%s,%.1f,%s,%s,%s,%s,%s,%.2f,%s,%s,%.1f,%.3f,%s,%.2f,%s,%.0f,%.1f,%s,%s,%s,%d,%.1f,%s,%d,%d\n",
         ev.redCountAtStart,
         TimeToString(ev.startTime, TIME_DATE|TIME_MINUTES),
         (ev.endTime > 0 ? TimeToString(ev.endTime, TIME_DATE|TIME_MINUTES) : "OPEN"),
         durH,
         ev.direction,
         BT_StateStr(ev.stRTM),
         BT_StateStr(ev.stTrend),
         BT_StateStr(ev.stStruct),
         BT_StateStr(ev.stFLOW),
         ev.rtmX,
         arrowFull,
         rtmTypeFull,
         ev.adxVal,
         ...
         tsAgainst,
         crisis,
         ev.zombieZone,        // v10.3: Zone Number
         ev.zombieWidthPips    // v10.3: Width Pips
      );
   ...
   if(g_hourlyCount > 0)
   {
      ...
      if(hh != INVALID_HANDLE)
      {
         string hhdr =
            ...
            "RTM,ADX,D1,FLOW,ZOMBIE_Zone,CRISIS,"
            "RTM_Dist_xATR,RTM_Arrow,"
            "ADX_Val,EffRatio_H4,Flow_Score,Trend_Score,"
            "D1_Status,Zombie_Zone,Zombie_WidthPips,"
         ...
         for(int _i = 0; _i < g_hourlyCount; _i++)
         {
            ...
            string _line = StringFormat(
               "%s,%s,%s,%s,%s,%s,%d,%s,%.2f,%s,%.1f,%.3f,%+.2f,%.1f,%s,%d,%d,%s,%s,%d\n",
               ...
               g_hourlyLog[_i].zombieZone,     // v10.3: Zone number as integer
               _cris,
               g_hourlyLog[_i].rtmX,
               ...
               g_hourlyLog[_i].zombieZone,     // v10.3
               g_hourlyLog[_i].zombieWidthPips, // v10.3
   ...
   // ── مسیر صحیح Common\Files ────────────────────────────────────
   string commonPath = TerminalInfoString(TERMINAL_COMMONDATA_PATH);
   string fullPath   = commonPath + "\\Files\\" + fname;

   Print("✅ HelpMe Backtest CSV: ", g_btEventCount, " رویداد قرمز ذخیره شد");
   Print("📂 مسیر فایل (کامل): ", fullPath);
   Print("   یا در File Explorer آدرس زیر را Paste کنید:");
   Print("   %APPDATA%\\MetaQuotes\\Terminal\\Common\\Files\\", fname);
   Print("   ─────────────── آمار ───────────────");
   int c4=0,c3=0,c2=0,c1=0;
   for(int k=0;k<g_btEventCount;k++)
   {
      int rc=g_btEvents[k].redCountAtStart;
      if(rc==4)c4++; else if(rc==3)c3++; else if(rc==2)c2++; else c1++;
   }
   Print("   🔴×4 همه قرمز (فرار فوری!): ", c4, " بار");
   Print("   🔴×3 بحرانی:                ", c3, " بار");
   Print("   🔴×2 هشدار:                  ", c2, " بار");
   Print("   🔴×1 توجه:                   ", c1, " بار");
   return 0.0;
}
