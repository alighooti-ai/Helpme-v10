//+------------------------------------------------------------------+
//|                                                 HelpMe_V11.2.mq5  |
//|                 Combined EA - Ali Naderi v11.2  (EA)              |
//+------------------------------------------------------------------+
#property copyright "HelpMe v11.2 - Ali Naderi"
#property version   "11.20"
// ═══════════════════════════════════════════════════════════════════
// v11.2 CHANGELOG
// ─────────────────────────────────────────────────────────────────
// 🔔 NEW: سیستم اعلان (Notification System)
//    دو کانال: MetaTrader Mobile Push + Telegram Bot
...
//
//    منطق:
//      • هر چراغ فقط یک بار اعلان می‌دهد (state قدیم ≠ قرمز → state جدید = قرمز)
//      • Cooldown مانع spam می‌شود
//      • Telegram از WebRequest استفاده می‌کند (باید مجاز باشد)
// ─────────────────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════
// v11.1 CHANGELOG
// ─────────────────────────────────────────────────────────────────
...
//    ADX >= 40 AND Flow < -7 (Buy) یا Flow > +7 (Sell)
...
// ─── Zone Lines ────────────────────────────────────────────────
input bool   ShowZoneLines        = true; // رسم خطوط مرز طبقات روی چارت | Draw Zone Lines
input int    ZoneLineCount        = 12;   // تعداد طبقه در هر طرف از منطقه 0 | Lines Each Side
input int    ZoneBorderYellowPips = 10;   // فاصله از مرز برای چراغ زرد (پیپ) | Yellow Border Pips
// ─── فیلتر تأیید H1 برای ZOMBIE ────────────────────────────────
input int    ZombieH1ConfirmBars  = 2;   // تعداد کندل ساعت متوالی برای تأیید طبقه جدید |  H1 Confirm BarsGoodGood

//------------------------------------------------------------------------
// ─── سیستم اعلان‌ها | Notification System v11.2 ──────────────────
//------------------------------------------------------------------------
input group "════════ 🔔 اعلان‌ها | Notifications v11.2 ════════"
input bool   Alert_MT5Push           = true;   // 📱 ارسال Push Notification به MT5 موبایل | MT5 Mobile Push
input bool   Alert_Telegram          = true;   // 📲 ارسال پیام به ربات تلگرام | Telegram Bot Alert
input string Alert_TelegramToken     = "";     // 🔑 توکن ربات تلگرام (از BotFather) | Bot Token
input string Alert_TelegramChatID    = "";     // 💬 شناسه چت یا کانال (با - شروع می‌شود برای کانال) | Chat/Channel ID
// ─── کدام چراغ‌ها اعلان بدهند ───────────────────────────────────
input bool   Alert_OnCrisis          = true;   // 🚨 اعلان CRISIS قرمز/نارنجی | Alert on CRISIS Red/Orange
input bool   Alert_OnZombie          = true;   // 🧟 اعلان ZOMBIE قرمز | Alert on ZOMBIE Red
input bool   Alert_OnHighAlert       = true;   // ⚠️ اعلان HIGH ALERT فعال | Alert on HIGH ALERT
input bool   Alert_OnFlow            = true;   // 🌊 اعلان FLOW قرمز | Alert on FLOW Red
input bool   Alert_OnADX             = true;   // 📈 اعلان ADX قرمز | Alert on ADX Red
input bool   Alert_OnRTM             = true;   // 📉 اعلان RTM قرمز | Alert on RTM Red
input bool   Alert_OnStruct          = true;   // 🏗️ اعلان STRUCT قرمز | Alert on STRUCT Red
// ─── کنترل تکرار اعلان ───────────────────────────────────────────
input int    Alert_CooldownMinutes   = 60;     // ⏱️ حداقل فاصله بین دو اعلان مشابه (دقیقه) | Cooldown Minutes
...
SymbolZoneCfg g_zoneTable[] =
{
   ...
   { "AUDCAD",  0.84000,  0.86000,   200,       true  },
   // برای افزودن ارز: base_high = base_low + widthPips×0.0001
   // مثال (عرض ۲۰۰ پیپ): { "AUDNZD",  1.05000,  1.07000,  200,  true  },
};
...
// ─── بررسی Cooldown — آیا اعلان مجاز است؟ ──────────────────────
bool Alert_CooldownOK(datetime &lastTime)
{
...
// ─── ارسال پیام به تلگرام از طریق WebRequest ────────────────────
void Alert_SendTelegram(const string message)
{
   if(!Alert_Telegram) return;
   if(StringLen(Alert_TelegramToken) < 10 || StringLen(Alert_TelegramChatID) < 2)
   {
      if(ShowDebugLogs)
         Print("⚠️ Telegram: توکن یا ChatID تنظیم نشده");
      return;
   }

   string url = "https://api.telegram.org/bot" + Alert_TelegramToken
              + "/sendMessage?chat_id=" + Alert_TelegramChatID
              + "&text=" + message
              + "&parse_mode=HTML";

   // حذف کاراکترهای مشکل‌ساز از URL
   StringReplace(url, " ", "%20");

   char   post[];
   char   result[];
   string headers;
   string userAgent = "Mozilla/5.0 HelpMe EA v11.2";

   ResetLastError();
   int res = WebRequest("GET", url, userAgent, NULL, 5000, post, 0, result, headers);
   int err = GetLastError();

   if(res == 200 && err == 0)
   {
      if(ShowDebugLogs) Print("✅ Telegram: پیام ارسال شد");
   }
   else
   {
      if(ShowDebugLogs)
         Print("❌ Telegram WebRequest خطا: HTTP=", res, " Err=", err,
               " — مطمئن شوید URL تلگرام در WebRequest مجاز است");
   }
}
...
// این تابع بعد از هر آپدیت TL_Update فراخوانی می‌شود
void Alert_CheckAndSend(bool forBuy, bool highAlertActive)
{
   ...
   // ─── CRISIS ──────────────────────────────────────────────────
   if(Alert_OnCrisis && g_crisisState != g_prevAlertCrisis)
   {
      bool wasRed    = (g_prevAlertCrisis == 2);
      bool isRed     = (g_crisisState == 2);
      bool isOrange  = (g_crisisState == 3);

      if((isRed || isOrange) && !wasRed && Alert_CooldownOK(g_lastAlertTime_Crisis))
      {
         string msg = head;
         if(isRed)
            msg += "🔴 CRISIS: CLOSE NOW!\nهمین الان معامله را ببند!";
         else
            msg += "🟠 CRISIS: EARLY WARNING\nشرایط در آستانه بحران است - آماده باش";
   ...
   // ─── HIGH ALERT ──────────────────────────────────────────────
   if(Alert_OnHighAlert && highAlertActive != g_prevAlertHighAlt)
   {
      if(highAlertActive && Alert_CooldownOK(g_lastAlertTime_HighAlt))
      {
         string msg = head
            + "⚠️ HIGH ALERT فعال شد!\n"
            + StringFormat("ADX=%.1f (≥40) | Flow=%.2f", g_lastAdxVal, g_lastFlowScore)
            + "\nقبل از CRISIS قرمز خروج را در نظر بگیر";
         Alert_Send(msg);
      }
   ...
   // ─── ZOMBIE ──────────────────────────────────────────────────
   if(Alert_OnZombie && g_lightZOMBIE != g_prevAlertZOMBIE)
   {
      if(g_lightZOMBIE == 2 && g_prevAlertZOMBIE != 2
         && Alert_CooldownOK(g_lastAlertTime_ZOMBIE))
      {
         string msg = head
            + "🧟 ZOMBIE قرمز شد!\n"
            + "طبقه قیمتی عوض شده - معامله در خطر زامبی شدن است";
         Alert_Send(msg);
      }
   ...
   // ─── FLOW ────────────────────────────────────────────────────
   if(Alert_OnFlow && g_lightFLOW != g_prevAlertFLOW)
   {
      if(g_lightFLOW == 2 && g_prevAlertFLOW != 2
         && Alert_CooldownOK(g_lastAlertTime_FLOW))
      {
         string msg = head
            + "🌊 FLOW قرمز شد!\n"
            + StringFormat("امتیاز جریان: %.2f - سونامی جریان ارز!", g_lastFlowScore);
         Alert_Send(msg);
      }
   ...
   // ─── ADX (TREND) ─────────────────────────────────────────────
   if(Alert_OnADX && g_lightTrend != g_prevAlertTrend)
   {
      if(g_lightTrend == 2 && g_prevAlertTrend != 2
         && Alert_CooldownOK(g_lastAlertTime_Trend))
      {
         string msg = head
            + "📈 ADX قرمز شد!\n"
            + StringFormat("ADX=%.1f - روند قوی خلاف پوزیشن", g_lastAdxVal);
         Alert_Send(msg);
      }
   ...
   // ─── RTM ─────────────────────────────────────────────────────
   if(Alert_OnRTM && g_lightRTM != g_prevAlertRTM)
   {
      if(g_lightRTM == 2 && g_prevAlertRTM != 2
         && Alert_CooldownOK(g_lastAlertTime_RTM))
      {
         string msg = head
            + "📉 RTM قرمز شد!\n"
            + "قیمت از میانگین دور می‌شود - هشدار بازگشت به میانگین";
         Alert_Send(msg);
      }
   ...
   // ─── STRUCT ──────────────────────────────────────────────────
   if(Alert_OnStruct && g_lightStruct != g_prevAlertStruct)
   {
      if(g_lightStruct == 2 && g_prevAlertStruct != 2
         && Alert_CooldownOK(g_lastAlertTime_Struct))
      {
         string msg = head
            + "🏗️ STRUCT قرمز شد!\n"
            + "ساختار روزانه شکسته شد - ریسک ادامه روند بالاست";
         Alert_Send(msg);
      }
...
void TL_Update()
{
   ...
   else
   {
      ...
      if(ShowZoneLines)
      {
         ...
         if(needRebuild)
         {
            ...
            DrawAbsoluteZoneLines(basePx, wPx, pipSz, ZoneLineCount, g_zombieEntryZone);
   ...
   // شرط Buy : ADX >= 40 AND Flow < -7.0
   // شرط Sell: ADX >= 40 AND Flow > +7.0
   // فعال می‌شود قبل از اینکه CRISIS قرمز بشود
   // ══════════════════════════════════════════════════════════════
   {
      bool highAlert = false;
      if(forBuy  && g_lastAdxVal >= 40.0 && g_lastFlowScore < -7.0) highAlert = true;
      if(!forBuy && g_lastAdxVal >= 40.0 && g_lastFlowScore >  7.0) highAlert = true;
      ...
      else
      {
         TL_SetLight("HIGHALT", 0, "● ALERT: OK    ",
            "HIGH ALERT: ADX < 40 یا Flow در محدوده مجاز");
      }
   }
   ...
   {
      bool _highAlertNow = (g_lastAdxVal >= 40.0) &&
...
void CreateDashboard()
{
   ...
   CreateLabel(dashboardPrefix + "TL_ZOMBIE",lx, yPos, "● ZONE: --     ",    clrDimGray, DashboardFontSize); yPos += lineHeight;
   // v11.0: HIGH ALERT — پیش‌هشدار ADX+Flow
   CreateLabel(dashboardPrefix + "TL_HIGHALT",lx, yPos,"● ALERT: --    ",    clrDimGray, DashboardFontSize); yPos += lineHeight + sectionGap;
                  ...
                  const string &sparam)
{
      ...
      if(ShowZoneLines)
      {
         ...
         else if(ObjectFind(0, "ZombieLine_0") < 0 && Zone_GetCfg(_Symbol, _zc_cc))
         {
            ...
            DrawAbsoluteZoneLines(_zc_cc.base_low, (double)_zc_cc.widthPips * _pip_cc,
                                  _pip_cc, ZoneLineCount, g_zombieEntryZone);
         }
      }
   }
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if(StringFind(sparam, dashboardPrefix + "DirBtn_") >= 0)
      {
         ...
         // v11.1 FIX: mutex را قبل از TL_Update آزاد کن
         // مشکل قبلی: TL_Update() درون خودش ChartRedraw دارد که event دوباره fire می‌کند
         // چون g_processingChartEvent=true بود، هیچ کلیکی ثبت نمی‌شد → "هنگ" ظاهری
         g_processingChartEvent = false;
         TL_Update();
         ChartRedraw(0);
         return;
      ...
      if(sparam == dashboardPrefix + "ResetBtn")
      {
         ...
         // v11.0: ریست دکمه‌های Buy/All/Sell هم به All برمیگرده
         g_dirMode = 0;
         ...
         if(EnableAllLogs) Print("↺ RESET: mutex cleared, DirMode → All, forcing recalculation");
         // محاسبه مجدد کامل
         g_ratesTotal = Bars(_Symbol, PERIOD_CURRENT);
         ForceRecalculation();
         g_processingChartEvent = false;
      ...
      if(StringFind(sparam, dashboardPrefix + "AIBtn_") >= 0)
      {
         ...
         else if(clicked == "SR")
         {
            ...
            // v11.1 FIX: mutex را تا پایان DrawSRLevels نگه‌دار (نه قبل از آن)
            // باگ قبلی: g_processingChartEvent=false → re-entrant event → localEnableSR دوباره toggle
            // → S/R اصلاً رسم نمی‌شد و نیاز به Reset بود
            ChartRedraw(0);   // فقط برای آپدیت بصری رنگ دکمه
            ...
            ChartRedraw(0);
            g_processingChartEvent = false;   // ← آخرین خط: بعد از تمام کار
            return;
         }
      }
   }
