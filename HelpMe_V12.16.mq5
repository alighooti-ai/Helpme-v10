//+------------------------------------------------------------------+
//|                                                 HelpMe_V12.15.mq5 |
//|                     Combined EA - Ali Naderi v12.15 (EA)          |
//+------------------------------------------------------------------+
#property copyright "HelpMe v12.16 - Ali Naderi"
#property version   "12.16"
...
string StatusQuery_BuildReport()
{
   ...
   msg += "Steps:          " + stepsStr        + "\n";
   msg += "Open Time:  " + openTimeStr     + "\n";
   msg += "─────────────────────\n";
   msg += "Trend:       " + trendStr        + "\n";
   msg += "Crisis:       " + crisisStr       + "\n";
   msg += "Hi Alert:    " + hiAlertStr      + "\n";
   msg += "Zombie:    " + zombieStr       + "\n";
      ...
      if(txtPos >= 0)
      {
         ...
         if(isStatus && chatOk)
         {
            ...
            else
            {
               ...
               Alert_SendTelegramPhoto(_Symbol + " | Score:" + DoubleToString(g_csvScore, 0));
...
void OnTimer()
{
   ...
   // FIX v12.15f: poll اینجاست (هر 5 ثانیه) نه هر 1 ثانیه
   // WebRequest بلاکینگ است — هر 5 ثانیه قابل قبول‌تر است
   // EventSetTimer داخل OnTimer صدا زده نمیشه چون تایمر رو kill میکنه
   s_tick30++;
   if(s_tick30 < 10) return;
   s_tick30 = 0;
...
void OnTick()
{
   ...
   // FIX v12.15d: poll در OnTick هم صدا زده میشه
   // دلیل: WebRequest بلاکینگ است — بعد از اجرا MT5 تایمر رو reset میکنه
   // و OnTimer دیگه fire نمیشه. OnTick همیشه اجرا میشه و throttle داخلی
   // (g_lastStatusPollTime) مانع اجرای بیشتر از یک‌بار در pollSec ثانیه میشه
   if(StatusQuery_Enable && Alert_Telegram)
      StatusQuery_PollTelegram();
...
void Alert_SendTelegramPhoto(const string caption)
{
   ...
   // screenshot
   string fname = "hm_chart.png";
   if(!ChartScreenShot(0, fname, 1280, 720, ALIGN_LEFT))
   {
      Print("❌ ChartScreenShot failed err=", GetLastError());
      return;
   }
   ...
   if(fsize == 0) { FileClose(fh); Print("❌ Screenshot empty"); return; }
   uchar imgData[];
   ...
   // multipart boundary — بدون dash اضافه در ابتدا
   ...
   // part 2: caption (فارسی را ساده نگه داریم)
   string cap = StringSubstr(caption, 0, 200); // تلگرام max 1024 — کوتاه نگه داریم
   string p2 = "--" + bnd + CRLF
                       ...
                       double trendScore = 0.0, bool forBuy = true)
{
   ...
   // FIX v12.09: Flow علیه پوزیشن — برای Buy: Flow منفی بد است | برای Sell: Flow مثبت بد است
   // flowAgainst همیشه مثبت = بد، منفی = خوب (هم‌جهت با پوزیشن)
   // مثال Buy:  Flow=-9 → flowAgainst=+9 (خیلی بد)
   // مثال Sell: Flow=+9 → flowAgainst=+9 (خیلی بد)
   double flowAgainst = forBuy ? -flowScore : flowScore;
   ...
   // ══════════════════════════════════════════════════════════════
      ...
      (rc >= 3 && flowAgainst >= 7.0 && adxVal > 32.0)
      ||
      // الگوی B (فشار شدید — جدید v9.0):
      (rc >= 2 && flowAgainst >= 8.5 && adxVal > 40.0)
   )
   {
      g_crisisState = 2;
      ...
      if(rc >= 3 && flowAgainst >= 7.0 && adxVal > 32.0)
   ...
   // ══════════════════════════════════════════════════════════════
      ...
      (rc >= 2 && flowAgainst >= 6.5 && adxVal > 30.0)
      ||
      ...
      (rc >= 2 && flowAgainst >= 7.5 && adxVal > 32.0 && tsAgainst > 25.0)
   )
   {
      ...
      string whyOrange = "";
      if(rc >= 2 && flowAgainst >= 6.5 && adxVal > 30.0)
      ...
      if(rc >= 2 && flowAgainst >= 7.5 && adxVal > 32.0 && tsAgainst > 25.0)
   ...
   // ══════════════════════════════════════════════════════════════
      ...
      (rc >= 2 && flowAgainst >= 7.5 && adxVal > 35.0)
   )
   {
      g_crisisState = 1;
      ...
      if(flowAgainst >= 5.0 && adxVal > 28.0)
         whyYellow += StringFormat("Y2: FlowAgainst=%.1f + ADX=%.1f\n", flowAgainst, adxVal);
      if(rc >= 2 && flowAgainst >= 7.0 && tsAgainst > 30.0)
      ...
      if(rc >= 2 && flowAgainst >= 7.5 && adxVal > 35.0)
...
string TL_FriendlyName(string id)
{
   ...
   if(id == "KILLER") return "Killer";   // 🆕 v12.14
   return id;
}
...
void TL_Update()
{
         ...
         if(!(bool)MQLInfoInteger(MQL_TESTER) && Alert_OnKiller)
         {
            ...
            if(_kState == 2 && Alert_CooldownOK(g_lastAlertTime_KILLER, _kEdge))
            {
               ...
               Alert_Send(StringFormat(
                  ...
                  "پله فعلی: %d از ۵\n"
                  "Zone ضرر: %d طبقه\n"
                  "Trend_Score: %+.1f (روند %s)\n"
                  "H1 Regime: Trending\n"
                  "─────────────────────\n"
                  "هج بگیر یا با ضرر ببند",
                  _Symbol, _kDir, _kStep, _kDelta, g_csvScore, _kDng));
            }
            if(_kState == 1 && g_prevAlertKILLER < 1)
            {
               ...
               Alert_Send(StringFormat(
                  "HelpMe [%s %s]\n"
                  "⚠️ KILLER زرد - هشدار اولیه\n"
                  "پله: %d | Zone ضرر: %d | Score: %+.1f (%s)\n"
                  "اگه پله ۴ فعال شد KILLER قرمز میشه",
                  _Symbol, _kDir, _kStep, _kDelta, g_csvScore, _kDng));
            }
   ...
   // ── v10.2: CRISIS — محاسبه لحظه‌ای با Orange state جدید ─────────
   {
      ...
      // FIX v12.09: flowAgainst برای Buy و Sell هر دو صحیح کار می‌کند
      double _fa = forBuy ? -g_lastFlowScore : g_lastFlowScore;  // flowAgainst (مثبت = بد)
      bool isRedA   = (rc >= 3 && _fa >= 7.0 && g_lastAdxVal > 32.0);
      bool isRedB   = (rc >= 2 && _fa >= 8.5 && g_lastAdxVal > 40.0);
      bool isOrangeA= (rc >= 2 && _fa >= 6.5 && g_lastAdxVal > 30.0);
      bool isOrangeB= (rc >= 3 && tsAgainst > 40.0 && g_lastAdxVal > 28.0);
      bool isOrangeC= (rc >= 2 && _fa >= 7.5 && g_lastAdxVal > 32.0 && tsAgainst > 25.0);
      bool isYellow = (rc >= 2)
                   || (_fa >= 5.0 && g_lastAdxVal > 28.0)
                   || (rc >= 2 && _fa >= 7.0 && tsAgainst > 30.0)
                   || (rc >= 2 && tsAgainst > 45.0 && g_lastAdxVal > 25.0)
                   || (rc >= 2 && _fa >= 7.5 && g_lastAdxVal > 35.0);
      ...
      if(hh != INVALID_HANDLE)
      {
         ...
         for(int _i = 0; _i < g_hourlyCount; _i++)
         {
            ...
            string _line = StringFormat(
               "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%.1f,%.3f,%+.2f,%.1f,%s,%d,%s,%s,%d\n",
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if(StringFind(sparam, dashboardPrefix + "DirBtn_") >= 0)
      {
         ...
         // mutex آزاد + CSL_Execute برای رنگ فوری چارت
         // TL_Update حذف شد: CSL_Execute خودش داخلش TL_Update رو صدا میزنه
         // قبلاً دو بار TL_Update اجرا میشد = دو بار FlowEvaluate H4 = کندی Buy↔Sell
         g_processingChartEvent = false;
         SaveButtonStates();
         CSL_Execute();
         ChartRedraw(0);
