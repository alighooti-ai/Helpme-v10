...
input int      DashboardXOffset  = 100; // ↔️ فاصله افقی داشبورد (پیکسل) | Horizontal Offset
...
//+------------------------------------------------------------------+
//| v12: CREATE SIGNAL LABEL (High/Mid/Low) + tooltip با همه عوامل  |
//+------------------------------------------------------------------+
                       ...
                       double adxV, double fastMAV, double slowMAV)
{
   ...
   double closeOffset = (barTime > 0 && handleATR != INVALID_HANDLE) ?
                        (g_cacheSize > 1 && ArraySize(g_atr) > 1 ? g_atr[1] * 0.15 : 0) : 0;
   double lblPrice    = forBuy ? price - closeOffset : price + closeOffset;

   ObjectCreate(0, name, OBJ_TEXT, 0, barTime, lblPrice);
   ObjectSetString (0, name, OBJPROP_TEXT,      "? " + level);   // ? High / ? Mid / ? Low
   ...
   // Part A — on the label object: فقط Score (محدودیت کاراکتر متاتریدر)
   string tipA = StringFormat("Score: %d", score);

   // Part B — on the arrow object (secondary filters)
   string tipB = StringFormat(
      "%s | Score: %d | [hover arrow for filters A-Z]\n"
      "─ Extra filters ─\n"
      "Candle Pattern : %s\n"
      "Ichimoku       : %s\n"
      "Price Gap(FVG) : %s\n"
      ...
      "Market Regime  : %s",
      dir, score,
      prTxt, icTxt, fvTxt, swTxt,
      rtTxt, mtTxt, vlTxt, rgTxt
   );
                       ...
                       double trendScore = 0.0, bool forBuy = true)
{
   ...
   // متن ساده: «ایمن» / «خطر» با tooltip کامل
   {
      ...
      {
         crTxt   = "● بحران (Crisis): خطر 🔴";
         crColor = clrRed;
      }
      else if(g_crisisState == 1)
      {
         crTxt   = "● بحران (Crisis): هشدار 🟡";
         crColor = clrGold;
      }
      else
      {
         crTxt   = "● بحران (Crisis): ایمن ✅";
         crColor = clrLime;
      }
...
void TL_Create()
{
   ...
   if(ObjectFind(0, crNm) >= 0)
   {
      ObjectSetString (0, crNm, OBJPROP_TEXT,  "● بحران (Crisis): --");
...
void CreateDashboard()
{
   ...
   int padX         = (int)MathMax(8, MathRound(8*dpiScale));
   ...
   int innerW  = (int)MathMax(presetW, MathMax(filterW, MathMax(ai4W, dirW)));
   ...
   // پس چپ‌ترین دکمه: آن‌که XDISTANCE + XSIZE = max → دکمه AI با XDISTANCE = lx + innerW - aBtnW
   // و close button با XDISTANCE + titleBtnW = lx + innerW
   int maxContentXDist = MathMax(lx + innerW - aBtnW,
                                 lx + innerW - titleBtnW);
   int bgXDist = maxContentXDist + padX;            // padX فضای خالی سمت چپ پنل
   int bgXSize = (maxContentXDist - lx) + aBtnW + 2*padX;  // کل عرض پنل
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      CreateButton(dashboardPrefix+"ResetBtn",
                   closeX, yPos, titleBtnW, btnH, "↺", clrWhite, clrDarkSlateGray);
   ...
   if(ShowAIButtons)
   {
      ...
      // row 1: Support/Resist | Candle Pattern | Trend Filter (Market State removed from row1, added to row2)
      {
         string ns[] = {"SR",            "Price",          "MTF",           "Regime"};
         string ls[] = {"Support/Resist","Candle Pattern", "Trend Filter",  "Market State"};
         bool   as[] = {localEnableSR, localEnablePriceAction, localEnableMTF, localEnableMarketRegime};
         string ts[] = {
         ...
         for(int i=0;i<4;i++){
            ...
            CreateButton(nm,rx+i*(aBtnW+gap),yPos,aBtnW,btnH,ls[i],clrWhite,as[i]?clrDarkGreen:clrMaroon);
      ...
      {
         string ns[] = {"LiqSwp",    "FVG",          "Vol",      "Ichi"};
         string ls[] = {"Stop Hunt", "Price Gap",    "Volume",   "Ichimoku"};
         bool   as[] = {localEnableLiqSwp, localEnableFVG, localEnableSmartVol, localEnableIchimoku};
         string ts[] = {
      ...
      // row 3: Mean Return | Local EMA | Global EMA | Liquid Level
      {
         bool maAny = g_maM15Active||g_maM30Active||g_maH1Active||g_maH4Active||g_maD1Active;
         int  rx    = ROW_X(ai4W);
         CreateButton(dashboardPrefix+"AIBtn_RTM",    rx,               yPos, aBtnW, btnH,
            "Mean Return", clrWhite, localEnableRTM?clrDarkGreen:clrMaroon);
         ObjectSetString(0,dashboardPrefix+"AIBtn_RTM",OBJPROP_TOOLTIP,
            "اعمال فیلترهای بیشتر برای بهتر کردن سیگنال‌ها\nبررسی فاصله‌ی قیمت از میانگین دویست کندل → هرچه دورتر، احتمال برگشت بیشتر");
         CreateButton(dashboardPrefix+"MABtn_MAPLUS", rx+aBtnW+gap,     yPos, aBtnW, btnH,
         ...
         CreateButton(dashboardPrefix+"MABtn_FEMA",   rx+2*(aBtnW+gap), yPos, aBtnW, btnH,
         ...
         CreateButton(dashboardPrefix+"LiqLevelFilterBtn", rx+3*(aBtnW+gap), yPos, aBtnW, btnH,
            "Liquid Level", clrWhite, clrDarkSlateGray);
   ...
   // ─── MARKET ─────────────────────────────────────────────────
   {
      ...
      yPos += lineH;
      // Profit
      CreateLabel(dashboardPrefix+"InfoPL",       labelLx, yPos, "Profit: معامله باز ندارید", clrGray, dashFont);
   ...
   // ─── ALERTS (چراغ‌ها) ────────────────────────────────────────
   {
      ...
      CreateLabel(dashboardPrefix+"TL_RTM",    labelLx,    yPos, "● Reversion: --",        clrDimGray, dashFont);
      ...
      CreateLabel(dashboardPrefix+"TL_TREND",  labelMidX,  yPos, "● Trend Strength: --",   clrDimGray, dashFont);
      ...
      CreateLabel(dashboardPrefix+"TL_STRUCT", labelLx,    yPos, "● Daily Struct: --",     clrDimGray, dashFont);
      ObjectSetString(0,dashboardPrefix+"TL_STRUCT",OBJPROP_TOOLTIP,
         "ساختار روزانه\nکف/سقف ده کندل اخیر یک روزه شکسته شد");
      CreateLabel(dashboardPrefix+"TL_FLOW",   labelMidX,  yPos, "● Currency Flow: --",    clrDimGray, dashFont);
      ...
      CreateLabel(dashboardPrefix+"TL_ZOMBIE", labelLx,    yPos, "● Price Zone: --",       clrDimGray, dashFont);
      ObjectSetString(0,dashboardPrefix+"TL_ZOMBIE",OBJPROP_TOOLTIP,
         "منطقه قیمت\nمثلاً: دو طبقه از ورود دور شده‌اید");
      CreateLabel(dashboardPrefix+"TL_HIGHALT",labelMidX,  yPos, "● Hi Alert: --",         clrDimGray, dashFont);
      ...
      CreateLabel(dashboardPrefix+"TL_CRISIS", labelLx,    yPos, "● Crisis: --",            clrDimGray, dashFont);
   ...
   // ─── LIQUID LEVEL BUTTON ─────────────────────────────────────
   {
      int lqW = (int)(panelW * 0.82);
      // در corner 1: دکمه از ROW_X(lqW) شروع و lqW به راست می‌ره
      int lqX = ROW_X(lqW);
      CreateButton(dashboardPrefix+"LiquidBtn",   lqX, yPos, lqW, lqBtnH,
                   "Liquid Level", clrBlack, clrGold);
      ObjectSetString(0,dashboardPrefix+"LiquidBtn",OBJPROP_TOOLTIP,
         "محاسبه و رسم خط لیکوئید شدن حساب\nفاصله‌ی قیمت تا صفر شدن موجودی را نشان می‌دهد");
      // label زیر دکمه: از labelLx شروع (چپ بصری پنل)
      CreateLabel(dashboardPrefix+"LiquidStatus", labelLx, yPos+lqBtnH+2,
                   "غیرفعال - برای فعال‌سازی کلیک کن", clrDimGray, dashFontSmall);
   ...
   {
      int dirY = yPos + lqBtnH + lineH + secGap;
      // عنوان Direction
      CreateLabel(dashboardPrefix+"DirTitle", labelLx, dirY - lineH, "Direction:", clrCyan, dashFont);
      int rx   = ROW_X(dirW);
...
void UpdateDashboard()
{
   ...
   // ── Chart Trend (جایگزین StatusLabel خارجی + InfoPosition) ────
   {
      ...
      if(buy_cnt == 0 && sell_cnt == 0)
      {
         ...
         plLine = "Profit: معامله باز ندارید";
         plColor = clrDimGray;
      }
      else if(isHedge)
      {
   ...
   if(marketClosed)
   {
      ...
      UpdateLabel(dashboardPrefix + "SpreadStatus", "Spread: بازار بسته", clrDimGray);
   }
   else
   {
      color sessionColor;
      ...
      if(nowT - s_lastSpread >= 10 || s_lastSpread == 0)
      {
         ...
         if(sp == 0)
            UpdateLabel(dashboardPrefix + "SpreadStatus", "Spread: صفر", clrDimGray);
         else
         {
            color spC = (sp <= 3) ? clrLime : (sp <= 12) ? clrGold : clrRed;
            string spTip = (sp <= 3) ? "اسپرد عالی ✅" : (sp <= 12) ? "اسپرد قابل قبول ⚠️" : "اسپرد بالا ❌";
            UpdateLabel(dashboardPrefix + "SpreadStatus",
                        StringFormat("Spread: %d pt | %s", sp, spTip), spC);
         }
      }
   }
                  ...
                  const string &sparam)
{
   ...
   {
      // ─── Signal Label ? click → MessageBox با جزئیات کامل ─────────
      if(StringFind(sparam, HELPME_ARROW_PREFIX) >= 0 &&
         (StringFind(sparam, "BLbl_") >= 0 || StringFind(sparam, "SLbl_") >= 0))
      {
         ObjectSetInteger(0, sparam, OBJPROP_STATE, false);
         // بررسی اینکه آیا این label یک ? دارد
         string _lblTxt = ObjectGetString(0, sparam, OBJPROP_TEXT);
         if(StringFind(_lblTxt, "? ") == 0)
         {
            // تشخیص جهت و time از نام
            bool _forBuy = (StringFind(sparam, "BLbl_") >= 0);
            string _level = StringSubstr(_lblTxt, 2); // حذف "? "
            // پیدا کردن arrow متناظر برای tip کامل
            string _levelTag = (_level == "High") ? "H" : (_level == "Mid") ? "M" : "L";
            string _arrowTag = (_forBuy ? "B" : "S") + _levelTag;
            // استخراج timestamp از نام label
            string _timePart = StringSubstr(sparam, StringLen(HELPME_ARROW_PREFIX) + 5); // بعد از "BLbl_" یا "SLbl_"
            string _arrowName = HELPME_ARROW_PREFIX + _arrowTag + "_" + _timePart;
            string _tipB = ObjectGetString(0, _arrowName, OBJPROP_TOOLTIP);
            string _tipA = ObjectGetString(0, sparam, OBJPROP_TOOLTIP);
            // ساخت متن MessageBox
            string _msgTitle = (_forBuy ? "سیگنال خرید" : "سیگنال فروش") + " | " + _level;
            string _msgBody =
               "─────────────────────────────────────────\n"
               "MA Cross            : " + (StringFind(_tipB,"MA Cross") >= 0 ? StringSubstr(_tipB, StringFind(_tipB,"MA Cross")+12, 20) : "--") + "\n"
               "RSI                        : --\n"
               "ADX                      : --\n"
               "BB Filter              : --\n"
               "ATR Intensity    : --\n"
               "Wick Filter          : --\n"
               "─────────────────────────────────────────\n"
               "Candle Pattern (الگوی کندلی) :  --\n"
               "Ichimoku (ایچیموکو) :   --\n"
               "Price Gap (شکاف قیمتی) :   --\n"
               "Stop Hunt  (نقدینگی) :   --\n"
               "Mean Return (بازگشت به میانگین)  :  --\n"
               "Trend Filter (همجهتی با روند) : --\n"
               "Volume (حجم هوشمند)  :  --\n"
               "Market State (رژیم بازار)  :  --\n"
               "RSI (واگرایی مخفی)  : --\n"
               "Fractal (تأیید فرکتالی) : --\n"
               "─────────────────────────────────────────\n"
               + _tipA;
            // نمایش MessageBox
