//+------------------------------------------------------------------+
//|                                                 HelpMe_V12.08.mq5 |
//|                     Combined EA - Ali Naderi v12.08 (EA)          |
//+------------------------------------------------------------------+
#property copyright "HelpMe v12.08 - Ali Naderi"
#property version   "12.08"
...
void OnTimer()
{
   ...
   // ── هر 5 ثانیه (FIX v12.08: از ۳۰ به ۵ کاهش یافت) ────────────
   s_tick30++;
   if(s_tick30 < 5) return;
   s_tick30 = 0;

   // v11.1 FIX: ریست خودکار mutex‌ها هر ۵ ثانیه
   // FIX v12.08: از ۳۰ ثانیه به ۵ ثانیه کاهش یافت — کاربر دیگر لازم نیست ۳۰ ثانیه صبر کند
   // تا بعد از هر کرش/کلیک تند دکمه‌ها دوباره جواب دهند
   g_recalcBusy           = false;
   g_processingChartEvent = false;
   ...
   else
   {
      ...
      // FIX v12.08: هر 60 ثانیه (۱۲ چرخه ۵ثانیه‌ای) سشن آپدیت می‌شود
...
void CreateDashboard()
{
   ...
   // ─── TITLE BAR ─────────────────────────────────────────────────
   {
      ...
      // FIX v12.08: از MathMax برای جلوگیری از منفی شدن در DPI بالا استفاده می‌شود
      int pwrOffX = MathMax(lx, lx + (int)MathRound(-90*dpiScale));  // کمی فاصله از لبه راست BG
   ...
   if(ShowPresetButtons)
   {
      ...
      int rx = ROW_X(presetW)-(int)MathRound(43*dpiScale);
      for(int i=0;i<5;i++)
      {
   ...
   if(ShowAIButtons)
   {
      ...
      // row 1: Support/Resist | Candle Pattern | Trend Filter
      {
         ...
         int rx = ROW_X(ai3rowW) +(int)MathRound(18*dpiScale);
         for(int i=0;i<3;i++){
   ...
   // ─── DIRECTION — DPI-safe, panel-anchored ────────────────────
   {
      ...
      dirY += lineH;
      int rx   = ROW_X(dirW)-(int)MathRound(20*dpiScale);
                  ...
                  const string &sparam)
{
   ...
   SigMsg_Clear();
      ...
      if(sparam == dashboardPrefix + "ResetBtn")
      {
         ...
         if(EnableAllLogs) Print("↺ FULL RESET v12.08: all buttons → default, mutexes + chart color cleared");
