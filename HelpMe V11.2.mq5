...
input bool   Alert_Eitaa             = false;  // 🟢 ارسال پیام به ربات ایتا | Eitaa Bot Alert (ایران)
input string Alert_EitaaToken        = "";     // 🔑 توکن ربات ایتا (از BotFather ایتا) | Eitaa Bot Token
input string Alert_EitaaChatID       = "";     // 💬 شناسه چت ایتا | Eitaa Chat ID
...
input bool   Alert_OnThreeLights     = true;   // 🔴🔴🔴 اعلان وقتی ۳ چراغ از ۴ (ADX/RTM/STRUCT/FLOW) همزمان قرمز | 3 of 4 Red
// ─── کنترل تکرار اعلان ───────────────────────────────────────────
...
input int    Alert_CooldownMinutes   = 60;     // ⏱️ حداقل فاصله بین دو اعلان در حالت پایدار قرمز (دقیقه) | Cooldown Minutes
...
void Alert_SendTelegram(const string message)
{
   ...
   if(StringLen(Alert_TelegramToken) < 10 || StringLen(Alert_TelegramChatID) < 2)
      { if(ShowDebugLogs) Print("⚠️ Telegram: توکن یا ChatID تنظیم نشده"); return; }
   string enc = message;
   StringReplace(enc, " ", "%20"); StringReplace(enc, "\n", "%0A");
   StringReplace(enc, "!", "%21"); StringReplace(enc, "#", "%23");
   string url = "https://api.telegram.org/bot" + Alert_TelegramToken
              + "/sendMessage?chat_id=" + Alert_TelegramChatID + "&text=" + enc;
...
void Alert_SendEitaa(const string message)
{
   ...
   if(StringLen(Alert_EitaaToken) < 10 || StringLen(Alert_EitaaChatID) < 2)
      { if(ShowDebugLogs) Print("⚠️ Eitaa: توکن یا ChatID تنظیم نشده"); return; }
   string enc = message;
   StringReplace(enc, " ", "%20"); StringReplace(enc, "\n", "%0A");
...
void Alert_CheckAndSend(bool forBuy, bool highAlertActive)
{
   ...
   if(Alert_OnCrisis)
   {
      bool nowAlert  = (g_crisisState == 2 || g_crisisState == 3);
      bool prevAlert = (g_prevAlertCrisis == 2 || g_prevAlertCrisis == 3);
      bool isNewEdge = nowAlert && !prevAlert;
      if(nowAlert && Alert_CooldownOK(g_lastAlertTime_Crisis, isNewEdge))
      {
         string msg = head;
         if(g_crisisState == 2) msg += "CRISIS: CLOSE NOW!\nهمین الان معامله را ببند!";
         else msg += "CRISIS: EARLY WARNING\nشرایط در آستانه بحران - آماده باش";
   ...
   if(Alert_OnHighAlert)
   {
      ...
      {
         string msg = head + "HIGH ALERT!\n"
            + StringFormat("ADX=%.1f | Flow=%.2f\n", g_lastAdxVal, g_lastFlowScore)
            + "قبل از CRISIS قرمز خروج را بررسی کن";
         Alert_Send(msg);
      }
   ...
   if(Alert_OnZombie)
   {
      ...
      {
         Alert_Send(head + "ZOMBIE قرمز!\nطبقه قیمتی عوض شده - معامله در خطر");
      }
   ...
   if(Alert_OnFlow)
   {
      ...
      {
         Alert_Send(head + "FLOW قرمز!\n"
            + StringFormat("Flow=%.2f - سونامی جریان ارز!", g_lastFlowScore));
      }
   ...
   if(Alert_OnRTM)
   {
      ...
      {
         Alert_Send(head + "RTM قرمز!\nقیمت از میانگین دور می‌شود");
      }
...
void ForceRecalculation()
{
   ...
   if(g_recalcBusy)
   {
      ...
      if(TimeCurrent() - g_recalcBusySince > 30)
      {
                  ...
                  const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if(sparam == dashboardPrefix + "ResetBtn")
      {
         ...
         // ③ AI filter buttons → default از Input ها
         localEnablePriceAction  = UseAIFilter_PriceAction;
         localEnableMTF          = UseAIFilter_MTF;
         localEnableMarketRegime = UseAIFilter_Regime;
         localEnableSmartVol     = UseAIFilter_VolumeProfile;
         localEnableIchimoku     = UseAIFilter_Ichimoku;
         localEnableFVG          = UseAIFilter_FVG;
         localEnableLiqSwp       = UseAIFilter_LiquiditySweep;
         localEnableRTM          = UseAIFilter_RTM;
         ...
         if(EnableAllLogs) Print("↺ FULL RESET v11.2: all buttons → default, mutexes cleared");
...
void SmartLoadNews()
{
   ...
   // v11.2 FIX: اگر در این session یک‌بار تلاش کردیم و شکست خوردیم، دیگه retry نکن
   // GlobalVariable "HelpMe_NewsFailedAt" زمان آخرین شکست رو نگه می‌داره
   // فقط هر 15 دقیقه یک‌بار مجدداً تلاش کن (نه هر تیک)
   {
      datetime _newsFailedAt = (datetime)GlobalVariableGet("HelpMe_NewsFailedAt_" + _Symbol);
      datetime _now = TimeCurrent();
      bool _skipDownload = (_newsFailedAt > 0 && _now - _newsFailedAt < 900); // 15 min cooldown
      if(_skipDownload)
      {
         if(ShowDebugLogs) Print("ℹ️ News: last download failed recently, skipping retry");
         goto news_fallback;
      }
   }
