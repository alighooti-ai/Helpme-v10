//+------------------------------------------------------------------+
//|                                                HelpMe_V10.3.1.mq5 |
//|                 Combined EA - Ali Naderi v10.3.1 (EA)             |
//+------------------------------------------------------------------+
#property copyright "HelpMe v10.3.1 - Ali Naderi"
#property version   "10.31"
// ═══════════════════════════════════════════════════════════════════
...
void DrawZoneLines(double entryPrice, double zoneWidthPrice, double pipSz, int maxZones)
{
   ...
   {
      // Buy: i مثبت = پایین‌تر از Entry = ضرر
      // خط در: Entry - (i × ZoneWidth)
      double level = entryPrice - (i * zoneWidthPrice);
      ...
      else if(i > 0)
      {
         ...
         labelTxt  = StringFormat("Zone +%d  (-%d pip از ورود)", i, i * zoneWPipsLocal);
      }
      else
      {
         // سود (برای Buy: قیمت بالاتر از ورود)
         if(i == -1)      lineCol = clrMediumSeaGreen;
         else if(i == -2) lineCol = clrLimeGreen;
         else             lineCol = clrSpringGreen;
         lineWidth = 1;
         lineStyle = STYLE_DOT;
         labelTxt  = StringFormat("Zone %d  (+%d pip از ورود)", i, MathAbs(i) * zoneWPipsLocal);
      }
   ...
   if(ShowDebugLogs)
      Print("📐 ZoneLines: Entry=", DoubleToString(entryPrice,_Digits),
            " | Width=", zoneWPipsLocal, "pip | ±", maxZones, " lines drawn");
}
...
void TL_Update()
{
   ...
   if(handleATR_D1_200 != INVALID_HANDLE && BarsCalculated(handleATR_D1_200) > 205)
   {
      ...
      if(CopyBuffer(handleATR_D1_200, 0, 1, 1, atrD1Buf) == 1 && atrD1Buf[0] > 0)
      {
         ...
         double pipSz   = _Point * (((_Digits==3||_Digits==5) ? 10 : 1));
         ...
         if(g_zombieZoneWidthPips <= 0.0)
         {
            ...
            if(ShowZoneLines && g_zombieEntryPrice > 0.0)
            {
               ...
               DrawZoneLines(g_zombieEntryPrice, zwPrice, pipSz, ZoneLineCount);
            }
         }
