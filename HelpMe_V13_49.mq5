//+------------------------------------------------------------------+
//|                                                 HelpMe_V13.45.mq5 |
//|                     Combined EA - Ali Naderi v13.45 (EA)          |
//+------------------------------------------------------------------+
#property copyright "HelpMe v13.49 - Ali Naderi"
#property version   "13.49"
// ════════════════════════════════════════════════════════════════════
...
input int    Alert_GBPNZDRule_RepeatMinutes = 120;   // ⏱️ فاصله یادآوری (دقیقه، حداقل ۵) | Reminder interval minutes
// ─── EMA Cross Alert ─────────────────────────────────────────────
...
string StatusQuery_BuildReport()
{
   ...
   if(!activeMode || hasVirtual)
   {
      ...
      Calc_SpikeDetector();  // 🐛 v13.34 FIX1: g_spikeScore را به‌روز کن قبل از هر snapshot
      double fsBuy  = (g_gbnzdFlowBuy  != 0.0) ? g_gbnzdFlowBuy  : FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), true);
      double fsSell = (g_gbnzdFlowSell != 0.0) ? g_gbnzdFlowSell : FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), false);
      flowBuy  = fsBuy;
      flowSell = fsSell;
...
int OnInit()
{
   ...
   if(EnableAllLogs)
   {
      Print("✅ HelpMe v13.22 initialized!");
      Print("🤖 AI Features:");
   ...
   if(Alert_OnGBPNZDRule && StringFind(_Symbol, "GBPNZD") >= 0
...
void OnTimer()
{
   ...
   // 🆕 v13.14: Spike Detector — هر بار M15 جدید (throttle داخل تابع)
   Calc_SpikeDetector();
...
double FlowEvaluate(SisterEntry &mat[], int sz, bool forBuy)
{
   ...
   // 🆕 v13.49 P1 FIX: renormalize — اگه بعضی خواهرها resolve نشدن ولی حداقل
   // نیمی از وزن کل واقعاً مشارکت کرده، امتیاز را نسبت به وزن کل مقیاس کن تا
   // غیبت موقت چند خواهر باعث کوچک‌شدن/فریز مصنوعی Flow_Score نشود.
   // اگه کمتر از نیمی از وزن resolve شده، اعتماد کافی نیست → renormalize نکن
   // (چون تقویت بیش‌ازحد امتیاز یک/دو خواهر باقی‌مانده می‌تونه noisy باشه).
   if(usedWeight > 0.0 && usedWeight < totalWeight && usedWeight >= totalWeight * 0.5)
   {
      double renormFactor = totalWeight / usedWeight;
      if(ShowDebugLogs && renormFactor > 1.05)
         Print("ℹ️ v13.49 FlowEvaluate renormalize: usedWeight=", usedWeight,
...
double Spike_TFScore(ENUM_TIMEFRAMES tf, int atrPeriod = 14)
{
   ...
   if(CopyHigh(_Symbol, tf, 0, need, high)   < need) return 0.0;
   if(CopyLow (_Symbol, tf, 0, need, low)    < need) return 0.0;
   if(CopyOpen(_Symbol, tf, 0, need, openP)  < need) return 0.0;
   if(CopyClose(_Symbol, tf, 0, need, closeP)< need) return 0.0;

   // ATR ساده روی (atrPeriod) کندل قبلی (بدون کندل جاری ایندکس 0)
   double atrSum = 0.0;
   ...
   // Range کندل جاری (در حال شکل‌گیری) نسبت به ATR
   double curRange = high[0] - low[0];
   double rangeScore = curRange / atr;

   // Body Ratio: بدنه کندل جاری نسبت به بدنه کندل قبلی
...
{
   // 🆕 v13.16 FIX: throttle قبلی (مبتنی بر iTime M15) در بکتست H1
   // فقط روی کندل‌هایی که دقیقاً مضرب ۱۵ دقیقه‌اند فعال می‌شد —
   // در عمل فقط ساعت 23:00 محاسبه می‌شد. حالا throttle بر اساس
   // TimeCurrent است: هر ۶۰ ثانیه یک‌بار کافیه (M15 در حال شکل‌گیریه).
   static datetime s_lastCalc = 0;
   datetime now = TimeCurrent();
   bool firstRun = (g_lightSpike == -1);
   if(!firstRun && (now - s_lastCalc) < 60) return;
   s_lastCalc = now;

   double sM15 = Spike_TFScore(PERIOD_M15, 14);
   double sM30 = Spike_TFScore(PERIOD_M30, 14);
   double sH1  = Spike_TFScore(PERIOD_H1,  14);
   double sH4  = Spike_TFScore(PERIOD_H4,  14);
   double sD1  = Spike_TFScore(PERIOD_D1,  14);
...
int GBPNZD_Replay_CrisisAtBar(int h1Shift, bool forBuy, int &rcOut)
{
   ...
   double flowScore = 0.0;
   {
      ...
      {
         double fs = 0.0;
         for(int si = 0; si < ArraySize(SISTER_GBPNZD); si++)
         {
            string sym = FlowFindSym(SISTER_GBPNZD[si].sym);
            ...
            int    effSign   = forBuy ? SISTER_GBPNZD[si].signForBuy : -SISTER_GBPNZD[si].signForBuy;
            double rawSignal = effSign * dir;
            double mult      = (rawSignal < 0.0) ? SISTER_GBPNZD[si].negMult : 1.0;
            fs += rawSignal * SISTER_GBPNZD[si].weight * mult;
         }
         flowScore = fs;
         if(fs > 2.0)        newFLOW = 0;
...
int GBPNZD_Replay_SpikeAtBar(int h1Shift, int &spikeBuyOut, int &spikeSellOut)
{
   ...
   // وزن‌دهی GBPNZD
   double score = sM15*0.25 + sM30*0.20 + sH1*0.30 + sH4*0.15 + sD1*0.10;
   ...
   for(int si = 0; si < ArraySize(SISTER_GBPNZD); si++)
   {
      ...
      if(dir == 0.0) continue;
      // Buy
      double rawB  = SISTER_GBPNZD[si].signForBuy * dir;
      double multB = (rawB < 0.0) ? SISTER_GBPNZD[si].negMult : 1.0;
      fsBuy  += rawB * SISTER_GBPNZD[si].weight * multB;
      // Sell (جهت معکوس)
      double rawS  = -SISTER_GBPNZD[si].signForBuy * dir;
      double multS = (rawS < 0.0) ? SISTER_GBPNZD[si].negMult : 1.0;
      fsSell += rawS * SISTER_GBPNZD[si].weight * multS;
   }
...
// ════════════════════════════════════════════════════════════════════
// 🆕 v13.38: GBPNZD Rule State — ذخیره/بازیابی در GlobalVariable
//
...
// ════════════════════════════════════════════════════════════════════
#define HM_GBNZD_HOURSTATE_FILE "HelpMe_GBNZD_HourStateSnapshot.dat"

void GBPNZD_SaveHourStateFile()
{
   if((bool)MQLInfoInteger(MQL_TESTER)) return;
   datetime thisHour = (datetime)(((long)TimeCurrent()) / 3600 * 3600);
   int fh = FileOpen(HM_GBNZD_HOURSTATE_FILE, FILE_WRITE | FILE_TXT | FILE_COMMON);
   if(fh == INVALID_HANDLE) return;
   FileWrite(fh, (long)thisHour,
...
bool GBPNZD_LoadHourStateFile(datetime &fileHour, bool &redBuy, bool &dirtyBuy, bool &redSell, bool &dirtySell)
{
   ...
   fileHour = 0;
   if(!FileIsExist(HM_GBNZD_HOURSTATE_FILE, FILE_COMMON)) return false;
   int fh = FileOpen(HM_GBNZD_HOURSTATE_FILE, FILE_READ | FILE_TXT | FILE_COMMON);
...
void GBPNZD_GV_Save()
{
   GlobalVariableSet("HM_GBNZD_LvBuy",   (double)g_gbpnzdLevelBuy);
   GlobalVariableSet("HM_GBNZD_LvSell",  (double)g_gbpnzdLevelSell);
   GlobalVariableSet("HM_GBNZD_ClHBuy",  (double)g_gbpnzdCleanHBuy);
   GlobalVariableSet("HM_GBNZD_ClHSell", (double)g_gbpnzdCleanHSell);
   GlobalVariableSet("HM_GBNZD_RdHBuy",  (double)g_gbpnzdCrisisRedHBuy);
   GlobalVariableSet("HM_GBNZD_RdHSell", (double)g_gbpnzdCrisisRedHSell);
   GlobalVariableSet("HM_GBNZD_LastH",   (double)g_gbpnzdLastH);
   GlobalVariableSet("HM_GBNZD_SRBuy",   (double)g_gbpnzdStopReasonBuy);
   GlobalVariableSet("HM_GBNZD_SRSell",  (double)g_gbpnzdStopReasonSell);
   ...
   GlobalVariableSet("HM_GBNZD_PrCrBuy",  (double)g_gbpnzdPrevCrisisBuy);
   GlobalVariableSet("HM_GBNZD_PrCrSell", (double)g_gbpnzdPrevCrisisSell);
   GlobalVariableSet("HM_GBNZD_PrSpBuy",  (double)g_gbpnzdPrevSpikeBuy);
   GlobalVariableSet("HM_GBNZD_PrSpSell", (double)g_gbpnzdPrevSpikeSell);
   GlobalVariableSet("HM_GBNZD_CrBuy",    (double)g_gbnzdCrisisBuy);
   GlobalVariableSet("HM_GBNZD_CrSell",   (double)g_gbnzdCrisisSell);
   GlobalVariableSet("HM_GBNZD_SpBuy",    (double)g_gbnzdSpikeBuy);
   GlobalVariableSet("HM_GBNZD_SpSell",   (double)g_gbnzdSpikeSell);
   // 🆕 v13.48 FIX1: accumulator ذخیره
   GlobalVariableSet("HM_GBNZD_HRedBuy",   (double)(g_gbpnzdHourRedBuy   ? 1 : 0));
   GlobalVariableSet("HM_GBNZD_HDirtyBuy", (double)(g_gbpnzdHourDirtyBuy ? 1 : 0));
   GlobalVariableSet("HM_GBNZD_HRedSell",  (double)(g_gbpnzdHourRedSell  ? 1 : 0));
   GlobalVariableSet("HM_GBNZD_HDirtySell",(double)(g_gbpnzdHourDirtySell? 1 : 0));
   GlobalVariableSet("HM_GBNZD_Saved",   (double)TimeCurrent());
}

// بازگشت: true = لود موفق | false = GV موجود نبود یا stale بود
bool GBPNZD_GV_Load()
{
   if(!GlobalVariableCheck("HM_GBNZD_Saved")) return false;

   datetime savedAt = (datetime)GlobalVariableGet("HM_GBNZD_Saved");
   // اگه بیشتر از 72 ساعت گذشته → stale، از Replay استفاده کن
   if(TimeCurrent() - savedAt > 72 * 3600) return false;

   g_gbpnzdLevelBuy        = (int)GlobalVariableGet("HM_GBNZD_LvBuy");
   g_gbpnzdLevelSell       = (int)GlobalVariableGet("HM_GBNZD_LvSell");
   g_gbpnzdCleanHBuy       = (int)GlobalVariableGet("HM_GBNZD_ClHBuy");
   g_gbpnzdCleanHSell      = (int)GlobalVariableGet("HM_GBNZD_ClHSell");
   g_gbpnzdCrisisRedHBuy   = (int)GlobalVariableGet("HM_GBNZD_RdHBuy");
   g_gbpnzdCrisisRedHSell  = (int)GlobalVariableGet("HM_GBNZD_RdHSell");
   g_gbpnzdLastH           = (datetime)GlobalVariableGet("HM_GBNZD_LastH");
   g_gbpnzdStopReasonBuy   = (int)GlobalVariableGet("HM_GBNZD_SRBuy");
   g_gbpnzdStopReasonSell  = (int)GlobalVariableGet("HM_GBNZD_SRSell");
   // 🆕 v13.48 FIX1: accumulator بازیابی (جایگزین PrevCrisis/Spike)
   g_gbnzdCrisisBuy        = GlobalVariableCheck("HM_GBNZD_CrBuy")    ? (int)GlobalVariableGet("HM_GBNZD_CrBuy")    : -1;
   g_gbnzdCrisisSell       = GlobalVariableCheck("HM_GBNZD_CrSell")   ? (int)GlobalVariableGet("HM_GBNZD_CrSell")   : -1;
   g_gbnzdSpikeBuy         = GlobalVariableCheck("HM_GBNZD_SpBuy")    ? (int)GlobalVariableGet("HM_GBNZD_SpBuy")    : -1;
   g_gbnzdSpikeSell        = GlobalVariableCheck("HM_GBNZD_SpSell")   ? (int)GlobalVariableGet("HM_GBNZD_SpSell")   : -1;
   g_gbpnzdHourRedBuy    = GlobalVariableCheck("HM_GBNZD_HRedBuy")   ? (GlobalVariableGet("HM_GBNZD_HRedBuy")   != 0.0) : (g_gbnzdCrisisBuy  == 2);
   g_gbpnzdHourDirtyBuy  = GlobalVariableCheck("HM_GBNZD_HDirtyBuy") ? (GlobalVariableGet("HM_GBNZD_HDirtyBuy") != 0.0) : (g_gbnzdCrisisBuy  == 2 || g_gbnzdSpikeBuy  >= 1);
   g_gbpnzdHourRedSell   = GlobalVariableCheck("HM_GBNZD_HRedSell")  ? (GlobalVariableGet("HM_GBNZD_HRedSell")  != 0.0) : (g_gbnzdCrisisSell == 2);
   g_gbpnzdHourDirtySell = GlobalVariableCheck("HM_GBNZD_HDirtySell")? (GlobalVariableGet("HM_GBNZD_HDirtySell") != 0.0): (g_gbnzdCrisisSell == 2 || g_gbnzdSpikeSell >= 1);

   // ── gap-fill با accumulator ──────────────────────────────────────
   {
      datetime nowH = (datetime)(((long)TimeCurrent()) / 3600 * 3600);
      datetime h    = g_gbpnzdLastH + 3600;
      int maxFill   = 72;
      while(h <= nowH && maxFill > 0)
      {
      ...
      Print(StringFormat(
         "✅ v13.48 GV_Load gap-fill: Buy CleanH=%d RedH=%d | Sell CleanH=%d RedH=%d",
         g_gbpnzdCleanHBuy, g_gbpnzdCrisisRedHBuy,
...
{
   if(StringFind(_Symbol, "GBPNZD") < 0) return;
   if((bool)MQLInfoInteger(MQL_TESTER)) return;
   ...
   if(gvLoaded && (g_gbpnzdLevelBuy > 0 || g_gbpnzdLevelSell > 0))
   {
      // GV معتبر با state واقعی — از Replay بپرهیز
      ...
      Print(StringFormat(
         "✅ v13.41 GBPNZD state loaded from GlobalVariable — Buy=%s CleanH=%d | Sell=%s CleanH=%d",
   ...
   // GV موجود نبود یا هر دو Level=0 بودن → Replay اجرا میشه
   // (اگه GV از نسخه قدیمی بود و Level=0 ذخیره کرده بود، Replay state درست میسازه)
   ...
   // ── loop از کهنه‌ترین به جدیدترین (shift=72 → shift=1) ──────
   // 🐛 v13.30 FIX4: پنجره Replay از 24h به 72h افزایش یافت.
   // قبلاً با shift=24، Replay از صفر شروع می‌کرد و اگه CLOSE episode
   // بیشتر از 24h پیش شروع شده بود، state اولیه غلط بود.
   // مثال: بکتست نشون میده از 22 Jun 16:00 تا 23 Jun 22:00 CLOSE بوده (30h).
   // با shift=24 → Replay از 23 Jun ~22:00 شروع میشه → counter از صفر →
   // Crisis ساعت‌های بعد Green/Yellow → CleanH رشد → RESUME اشتباه.
   // با shift=72 → Replay از ~21 Jun 22:00 شروع میشه → CLOSE از shift~44
   // درست ساخته میشه → CleanH از صفر (بعد از CLOSE) → RESUME درست.
   // shift=1 = آخرین کندل H1 بسته‌شده
   // 🆕 v13.48 FIX1: accumulator در Replay — مثل Alert_CheckGBPNZDRule
   // هر ساعت accumulator از ساعت جاری OR می‌شه، در مرز ساعت counter آپدیت
   ...
   for(int sh = 72; sh >= 1; sh--)
   {
      ...
      for(int si = 0; si < ArraySize(SISTER_GBPNZD); si++)
      {
         ...
         double rawB = SISTER_GBPNZD[si].signForBuy  * dir;
         double rawS = -SISTER_GBPNZD[si].signForBuy * dir;
         fsBuyHist  += rawB * SISTER_GBPNZD[si].weight * ((rawB < 0) ? SISTER_GBPNZD[si].negMult : 1.0);
         fsSellHist += rawS * SISTER_GBPNZD[si].weight * ((rawS < 0) ? SISTER_GBPNZD[si].negMult : 1.0);
      }
      ...
      if(rHourRedBuy)     g_gbpnzdCrisisRedHBuy++;  else g_gbpnzdCrisisRedHBuy = 0;
      if(!rHourDirtyBuy)  g_gbpnzdCleanHBuy++;      else g_gbpnzdCleanHBuy = 0;
      // seed برای ساعت بعد
      rHourRedBuy   = (crBuy == 2);
      rHourDirtyBuy = (crBuy == 2) || (spikeBuy >= 1);
      // 🐛 v13.49 P0 FIX: خط «else g_gbpnzdCleanHBuy = 0؛» یتیم (بدون if منطبق) از اینجا حذف شد.
      // این خط باقی‌مانده بازنویسی accumulator در v13.48 FIX1 بود و از نظر گرامر C/MQL5
      // نامعتبر بود (Compile Error). منطق درست همان دو خط if/else بالاست.

      {
         bool _stopA  = (spikeBuy == 2) && (crBuy >= 1);
         bool _stopB  = (spikeBuy == 1) && (crBuy == 2 || crBuy == 3);
         bool _closeA = (crBuy == 2) && haBuy;
         bool _closeB = (g_gbpnzdCrisisRedHBuy >= 6);
         bool _resume = (g_gbpnzdCleanHBuy >= 24);
         if(g_gbpnzdLevelBuy == 0)
         {
            if(_closeA || _closeB)    { g_gbpnzdLevelBuy = 2; g_gbpnzdStopReasonBuy = _closeA ? 10 : 11; }
            else if(_stopA || _stopB) { g_gbpnzdLevelBuy = 1; g_gbpnzdStopReasonBuy = _stopA  ?  0 :  1; }
         }
         else if(g_gbpnzdLevelBuy == 1)
         {
            if(_closeA || _closeB)    { g_gbpnzdLevelBuy = 2; g_gbpnzdStopReasonBuy = _closeA ? 10 : 11; }
            else if(_resume)          { g_gbpnzdLevelBuy = 0; g_gbpnzdStopReasonBuy = -1; }
         }
         else if(g_gbpnzdLevelBuy == 2)
         {
            if(_resume)               { g_gbpnzdLevelBuy = 0; g_gbpnzdStopReasonBuy = -1; }
         }
      }
      ...
      if(rHourRedSell)     g_gbpnzdCrisisRedHSell++;  else g_gbpnzdCrisisRedHSell = 0;
      if(!rHourDirtySell)  g_gbpnzdCleanHSell++;      else g_gbpnzdCleanHSell = 0;
      rHourRedSell   = (crSell == 2);
      rHourDirtySell = (crSell == 2) || (spikeSell >= 1);
      // 🐛 v13.49 P0 FIX: همان else یتیم که در بلوک BUY حذف شد، اینجا هم حذف شد.

      {
         bool _stopA  = (spikeSell == 2) && (crSell >= 1);
         bool _stopB  = (spikeSell == 1) && (crSell == 2 || crSell == 3);
         bool _closeA = (crSell == 2) && haSell;
         bool _closeB = (g_gbpnzdCrisisRedHSell >= 6);
         bool _resume = (g_gbpnzdCleanHSell >= 24);
         if(g_gbpnzdLevelSell == 0)
         {
            if(_closeA || _closeB)    { g_gbpnzdLevelSell = 2; g_gbpnzdStopReasonSell = _closeA ? 10 : 11; }
            else if(_stopA || _stopB) { g_gbpnzdLevelSell = 1; g_gbpnzdStopReasonSell = _stopA  ?  0 :  1; }
         }
         else if(g_gbpnzdLevelSell == 1)
         {
            if(_closeA || _closeB)    { g_gbpnzdLevelSell = 2; g_gbpnzdStopReasonSell = _closeA ? 10 : 11; }
            else if(_resume)          { g_gbpnzdLevelSell = 0; g_gbpnzdStopReasonSell = -1; }
         }
         else if(g_gbpnzdLevelSell == 2)
         {
            if(_resume)               { g_gbpnzdLevelSell = 0; g_gbpnzdStopReasonSell = -1; }
         }
      }
   ...
   // اگه Crisis وسط ساعت در حال شکل‌گیری (که هنوز به کندل بسته تبدیل نشده)
   // رخ داده باشه، فایل snapshot (که با ری‌استارت کامل MT5 هم پاک نمی‌شه)
   // اون رو داره. اگه فایل مربوط به همین ساعت جاری باشه، اطلاعاتش رو OR کن
   // تا کوری موقت Replay بعد از ری‌استارت کامل تا حد امکان جبران بشه.
   {
      datetime curFormingHour = (datetime)(((long)TimeCurrent()) / 3600 * 3600);
      datetime fHour; bool fRedBuy, fDirtyBuy, fRedSell, fDirtySell;
      if(GBPNZD_LoadHourStateFile(fHour, fRedBuy, fDirtyBuy, fRedSell, fDirtySell) && fHour == curFormingHour)
      {
         ...
         if(ShowDebugLogs)
            Print("ℹ️ v13.49 P3: hour-state snapshot فایلی برای ساعت جاری اعمال شد.");
      }
   }

   // ── g_gbpnzdLastH به آخرین ساعت پردازش‌شده تنظیم بشه ──────
   g_gbpnzdLastH = (datetime)(((long)iTime(_Symbol, PERIOD_H1, 1)) / 3600 * 3600);
...
void Alert_CheckGBPNZDRule()
{
   ...
   // 🆕 v13.49 P2 FIX: HourRed/HourDirty در آخر هفته بدون tick واقعی
   // مشکل: بلوک زیر با هر فراخوانی (OnTick/OnTimer) از مقدار *فعلی* accumulator
   // می‌خونه، نه از یک تیک تازه. آخر هفته که قیمت جدید نمی‌آد، این مقدار همان
   // آخرین وضعیت جمعه است — اگه جمعه Clean بسته شده باشه، هر ساعت (با OnTimer
   // هر 30 ثانیه) دوباره به‌عنوان "این ساعت Clean" ثبت می‌شه و تا 48 ساعت
   // CleanH مصنوعی در طول تعطیلی اضافه می‌کنه. این می‌تونه باعث RESUME زودتر
   // از حد لازم دوشنبه صبح بشه — دقیقاً همون لحظه‌ای که gap قیمتی خطرناک‌ترینه.
   // راه‌حل: قبل از عبور از مرز ساعت، آخرین quote واقعی سیمبل (SYMBOL_TIME) رو
   // چک کن. اگه بیش از 2 ساعت قدیمی باشه (بازار بی‌داده)، نه شمارنده رو ++ کن
   // نه ریست — فقط skip کن و g_gbpnzdLastH رو هم آپدیت نکن تا با بازگشت دیتای
   // واقعی (دوشنبه)، همین بلوک یک‌بار با وضعیت واقعیِ لحظه بازگشایی پردازش بشه.
   datetime lastQuoteTime  = (datetime)SymbolInfoInteger(_Symbol, SYMBOL_TIME);
   bool     gbpnzdMarketStale = (lastQuoteTime > 0) && ((TimeCurrent() - lastQuoteTime) > 7200);

   datetime thisHour = (datetime)(((long)TimeCurrent()) / 3600 * 3600);
   if(g_gbpnzdLastH > 0 && thisHour > g_gbpnzdLastH && !gbpnzdMarketStale)
   {
      if(g_gbpnzdHourRedBuy)     g_gbpnzdCrisisRedHBuy++;  else g_gbpnzdCrisisRedHBuy = 0;
      if(!g_gbpnzdHourDirtyBuy)  g_gbpnzdCleanHBuy++;      else g_gbpnzdCleanHBuy = 0;
      if(g_gbpnzdHourRedSell)    g_gbpnzdCrisisRedHSell++; else g_gbpnzdCrisisRedHSell = 0;
   ...
   if(thisHour > g_gbpnzdLastH && !gbpnzdMarketStale)
   {
      // شروع ساعت جدید: seed accumulator با وضعیت لحظه اول
      g_gbpnzdHourRedBuy    = (g_gbnzdCrisisBuy  == 2);
      g_gbpnzdHourDirtyBuy  = (g_gbnzdCrisisBuy  == 2) || (g_gbnzdSpikeBuy  >= 1);
      g_gbpnzdHourRedSell   = (g_gbnzdCrisisSell == 2);
      g_gbpnzdHourDirtySell = (g_gbnzdCrisisSell == 2) || (g_gbnzdSpikeSell >= 1);
      g_gbpnzdLastH = thisHour;
      // 🆕 v13.38: بعد از آپدیت ساعتی، state رو در GV ذخیره کن
      GBPNZD_GV_Save();
   }
   else if(gbpnzdMarketStale && ShowDebugLogs && thisHour > g_gbpnzdLastH)
   {
      Print("ℹ️ v13.49 P2: بازار بی‌داده (آخر هفته) — مرز ساعت skip شد. lastQuote=",
   ...
   {
      bool _stopA  = (g_gbnzdSpikeBuy == 2) && (g_gbnzdCrisisBuy >= 1);
      bool _stopB  = (g_gbnzdSpikeBuy == 1) && (g_gbnzdCrisisBuy == 2 || g_gbnzdCrisisBuy == 3);
      bool _closeA = (g_gbnzdCrisisBuy == 2) && haBuy;
      bool _closeB = (g_gbpnzdCrisisRedHBuy >= 6);
      bool _resume = (g_gbpnzdCleanHBuy >= 24);
      if(oldLevelBuy == 0)
      {
         if(_closeA || _closeB)    { newLevelBuy = 2; newReasonBuy = _closeA ? 10 : 11; }
         else if(_stopA || _stopB) { newLevelBuy = 1; newReasonBuy = _stopA  ?  0 :  1; }
      }
      else if(oldLevelBuy == 1)
      {
         if(_closeA || _closeB)    { newLevelBuy = 2; newReasonBuy = _closeA ? 10 : 11; }
         else if(_resume)          { newLevelBuy = 0; newReasonBuy = -1; }
      }
      else if(oldLevelBuy == 2)
      {
         if(_resume)               { newLevelBuy = 0; newReasonBuy = -1; }
      }
   }
   ...
   {
      bool _stopA  = (g_gbnzdSpikeSell == 2) && (g_gbnzdCrisisSell >= 1);
      bool _stopB  = (g_gbnzdSpikeSell == 1) && (g_gbnzdCrisisSell == 2 || g_gbnzdCrisisSell == 3);
      bool _closeA = (g_gbnzdCrisisSell == 2) && haSell;
      bool _closeB = (g_gbpnzdCrisisRedHSell >= 6);
      bool _resume = (g_gbpnzdCleanHSell >= 24);
      if(oldLevelSell == 0)
      {
         if(_closeA || _closeB)    { newLevelSell = 2; newReasonSell = _closeA ? 10 : 11; }
         else if(_stopA || _stopB) { newLevelSell = 1; newReasonSell = _stopA  ?  0 :  1; }
      }
      else if(oldLevelSell == 1)
      {
         if(_closeA || _closeB)    { newLevelSell = 2; newReasonSell = _closeA ? 10 : 11; }
         else if(_resume)          { newLevelSell = 0; newReasonSell = -1; }
      }
      else if(oldLevelSell == 2)
      {
         if(_resume)               { newLevelSell = 0; newReasonSell = -1; }
      }
   }
   ...
   if(!(bool)MQLInfoInteger(MQL_TESTER) && Alert_GBPNZDRule_Repeat)
   {
      ...
      if(worstLevel > 0)
      {
         int repMin = MathMax(5, Alert_GBPNZDRule_RepeatMinutes);
...
void TL_Update()
{
   ...
   else  // g_dirMode == 0 → All (حالت پیش‌فرض)
   {
      ...
      if(isHedge)
      {
         ...
         // v13.14: Spike هم در هج نشون میده (مستقل از پوزیشن)
         Calc_SpikeDetector();
         UpdateCrisisLight(-1, 0.0, 0.0, 0.0, true);
         ...
         if(!((bool)MQLInfoInteger(MQL_TESTER)) && Alert_OnGBPNZDRule && StringFind(_Symbol,"GBPNZD")>=0)
         {
            // Spike جهت‌دار در هج
            ...
            double _fsBuyH  = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), true);
            double _fsSellH = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), false);
   ...
   if(!hasPos)
   {
      ...
      // v13.14: Spike هم حتی بدون پوزیشن نشون میده
      Calc_SpikeDetector();
      UpdateCrisisLight(-1, 0.0, 0.0, 0.0, true);
      ...
      if(!(bool)MQLInfoInteger(MQL_TESTER) && Alert_OnGBPNZDRule && StringFind(_Symbol,"GBPNZD")>=0)
      {
         ...
         double _fsBuy  = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), true);
         double _fsSell = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), false);
         ...
         Alert_CheckGBPNZDRule();   // CleanH در no-position هم باید هر ساعت پیشرفت کنه

         // 🐛 v13.39 FIX2: snapshot فعلی را برای PrevCrisis/Spike ذخیره کن (هر ۳۰ثانیه)
         // قبلاً: PrevCrisis فقط در start-of-hour داخل Alert_CheckGBPNZDRule آپدیت می‌شد.
         // اگه Crisis در وسط ساعت Orange می‌شد (مثل ساعت 16:12)، ساعت بعد counter
         // از مقدار اول ساعت (که هنوز Green بود) می‌خواند — نه از Orange بحرانی.
         // حالا هر ۳۰ ثانیه آخرین وضعیت در Prev ذخیره می‌شود تا ساعت بعد درست بخواند.
         g_gbpnzdPrevCrisisBuy  = g_gbnzdCrisisBuy;
         g_gbpnzdPrevSpikeBuy   = g_gbnzdSpikeBuy;
         g_gbpnzdPrevCrisisSell = g_gbnzdCrisisSell;
         g_gbpnzdPrevSpikeSell  = g_gbnzdSpikeSell;
   ...
   // 🆕 v13.14: Spike Detector — نمایش آخرین مقدار محاسبه‌شده
   Calc_SpikeDetector();
   ...
   if(!(bool)MQLInfoInteger(MQL_TESTER))  // در بکتست اعلان نده
   {
      ...
      if(forBuy)
      {
         ...
         // 🐛 v13.40 FIX-B: PrevCrisis را هم فوری sync کن
         g_gbpnzdPrevCrisisBuy = g_gbnzdCrisisBuy;
         g_gbpnzdPrevSpikeBuy  = g_gbnzdSpikeBuy;
         ...
         if(Alert_OnGBPNZDRule && StringFind(_Symbol,"GBPNZD") >= 0)
         {
            double _fsSellOpp = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), false);
      ...
      else
      {
         ...
         if(Alert_OnGBPNZDRule && StringFind(_Symbol,"GBPNZD") >= 0)
         {
            double _fsBuyOpp  = FlowEvaluate(SISTER_GBPNZD, ArraySize(SISTER_GBPNZD), true);
            double _baseOpp   = (g_lightSpike >= 0 && g_spikeScore > 0.0) ? g_spikeScore : 0.0;
            int    _rpOpp     = (_baseOpp < 1.00) ? 0 : (_baseOpp <= 1.50) ? 1 : 2;
      ...
      Alert_CheckGBPNZDRule();   // 🆕 v13.21: قانون ترید ۳سطحی GBPNZD
   }
   ...
   if(StringFind(_Symbol, "GBPNZD") >= 0)
   {
      ...
      if(canUpdate)
      {
         ...
         // BUY
         {
            ...
            // 🐛 v13.43 FIX: wasClean در بکتست (LogHourlySnapshot) — Orange بدون Spike ریست نمی‌کند
            // نکته: پچ v2 این بلوک را BT_CheckGBPNZDRule نامیده بود؛ آن تابع در کد وجود ندارد —
            // منطق بکتست واقعاً همینجا، داخل LogHourlySnapshot، پیاده‌سازی شده.
            bool _bClean = (g_gbnzdCrisisBuy != 2)                          // نه Crisis=Red
                         && (g_gbnzdSpikeBuy  <= 0 || g_gbnzdSpikeBuy  == -1); // نه Spike>=Warning
            if(_bRed)   g_gbpnzdCrisisRedHBuy++;
            else        g_gbpnzdCrisisRedHBuy = 0;
            if(_bClean) g_gbpnzdCleanHBuy++;
            else        g_gbpnzdCleanHBuy = 0;

            bool _stopA  = (g_gbnzdSpikeBuy == 2) && (g_gbnzdCrisisBuy >= 1);
            bool _stopB  = (g_gbnzdSpikeBuy == 1) && (g_gbnzdCrisisBuy == 2 || g_gbnzdCrisisBuy == 3);
            bool _closeA = (g_gbnzdCrisisBuy == 2) && _haBuy;
            bool _closeB = (g_gbpnzdCrisisRedHBuy >= 6);
            bool _resume = (g_gbpnzdCleanHBuy >= 24);
            if(g_gbpnzdLevelBuy == 0)
            {
               if(_closeA || _closeB)    g_gbpnzdLevelBuy = 2;
               else if(_stopA || _stopB) g_gbpnzdLevelBuy = 1;
            }
            else if(g_gbpnzdLevelBuy == 1)
            {
               if(_closeA || _closeB)    g_gbpnzdLevelBuy = 2;
               else if(_resume)          g_gbpnzdLevelBuy = 0;
            }
            else if(g_gbpnzdLevelBuy == 2)
            {
               if(_resume)               g_gbpnzdLevelBuy = 0;
            }
         }
         ...
         // SELL
         {
            ...
            // 🐛 v13.43 FIX: همان منطق wasClean برای SELL
            bool _sClean = (g_gbnzdCrisisSell != 2)                          // نه Crisis=Red
                         && (g_gbnzdSpikeSell  <= 0 || g_gbnzdSpikeSell  == -1); // نه Spike>=Warning
            if(_sRed)   g_gbpnzdCrisisRedHSell++;
            else        g_gbpnzdCrisisRedHSell = 0;
            if(_sClean) g_gbpnzdCleanHSell++;
            else        g_gbpnzdCleanHSell = 0;

            bool _stopA  = (g_gbnzdSpikeSell == 2) && (g_gbnzdCrisisSell >= 1);
            bool _stopB  = (g_gbnzdSpikeSell == 1) && (g_gbnzdCrisisSell == 2 || g_gbnzdCrisisSell == 3);
            bool _closeA = (g_gbnzdCrisisSell == 2) && _haSell;
            bool _closeB = (g_gbpnzdCrisisRedHSell >= 6);
            bool _resume = (g_gbpnzdCleanHSell >= 24);
            if(g_gbpnzdLevelSell == 0)
            {
               if(_closeA || _closeB)    g_gbpnzdLevelSell = 2;
               else if(_stopA || _stopB) g_gbpnzdLevelSell = 1;
            }
            else if(g_gbpnzdLevelSell == 1)
            {
               if(_closeA || _closeB)    g_gbpnzdLevelSell = 2;
               else if(_resume)          g_gbpnzdLevelSell = 0;
            }
            else if(g_gbpnzdLevelSell == 2)
            {
               if(_resume)               g_gbpnzdLevelSell = 0;
            }
         }
